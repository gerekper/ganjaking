	...
	if( $( '.page-layouts' ).length ) {
		$( 'body' ).on( 'click', '.layout-box .layout-part', function() {
			...
			$.ajax({
				...
				failure: function() {
					alert('Opening failed. Please refresh and try again.');
				}
			});
			...
			$.ajax({
				...
				failure: function() {
					alert('Removing failed. Please refresh and try again.');
				}
			});
		...
		$('body').on('change', '.porto-builder-cond-wrap .condition-type', function(e) {
			var val = $(this).val();
				$cond = $(this).closest('.porto-builder-condition');
			if ('single' == val) {
		...
		var init_live_search = function($obj) {
			...
			$obj.each(function() {
				var $this = $(this),
					...
					serviceUrl = ajaxurl + '?action=porto_page_layouts_search_posts&_nonce=' + porto_page_layouts.nonce;
