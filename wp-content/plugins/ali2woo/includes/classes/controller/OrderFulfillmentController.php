...
class OrderFulfillmentController extends AbstractController
{
    ...
    public function __construct()
    {
        ...
        add_filter('a2w_fill_additional_shipping_fields', array($this, 'fill_additional_shipping_fields'), 10, 3);
        add_action('a2w_update_custom_shipping_field', array($this, 'update_custom_shipping_field'), 10, 3);
    ...
    public function assets()
    {
        ...
        wp_enqueue_script('a2w-ali-orderfulfill-js', A2W()->plugin_url() . '/assets/js/orderfulfill.js', array('a2w-admin-script'), A2W()->version, true);
    ...
    {
        $bulk_actions['a2w_order_place_bulk'] = __("Place on AliExpress", 'ali2woo');
        $bulk_actions['a2w_order_sync_bulk'] = __("Sync with AliExpress", 'ali2woo');
    ...
    public function ajax_load_fulfillment_orders_html()
    {
        ...
        if (empty($account->get_purchase_code())) {
            ...
            foreach ($orders_data as $order_data) {
                ...
                $additional_shipping_fields = apply_filters('a2w_fill_additional_shipping_fields', self::$additional_shipping_fields, $order);
    ...
    public function ajax_save_order_shipping_info() {
        ...
        if(!isset($_POST['order_id'])) {
            ...
            $additional_shipping_fields = apply_filters('a2w_fill_additional_shipping_fields', self::$additional_shipping_fields, $order, $shiping_to_country);
            ...
            if ( ! empty( $shipping_fields ) ) {
                foreach ( $shipping_fields as $key => $field ) {
                    ...
                    if (!empty($field['custom'])) {
                        do_action('a2w_update_custom_shipping_field', $key, $field, $order);
                    } else {
    ...
    public function ajax_sync_order_info()
    {
        if(empty($_POST['order_id'])){
             ...
             */
            $wc_api = A2W()->getDI()->get('Ali2Woo\Woocommerce');
