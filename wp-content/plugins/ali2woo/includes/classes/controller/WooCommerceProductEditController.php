...
class WooCommerceProductEditController extends AbstractController {
    ...
    {
        $importPageScreenId = A2W()->plugin_slug . '_page_a2w_import';
        ...
        if ($current_screen->in_admin() && $checkPage) {
            wp_enqueue_script('a2w-admin-script',
                A2W()->plugin_url() . '/assets/js/admin_script.js',
                array('jquery'),
                A2W()->version
            );
    ...
    function ajax_edit_image_url() {
        ...
        if (empty($_POST['url'])) {
            ...
            $result = ResultBuilder::buildOk(array('url'=>A2W()->plugin_url() . '/includes/cdn.php?url=' . $url.'&_sign='.a2w_sign_request(array('url'=>$url))));
        }
        echo json_encode($result);
        wp_die();
    }
