    ...
    class SettingPageController extends AbstractAdminPage
    {
        ...
        private function common_handle()
        {
            if (isset($_POST['setting_form'])) {
                ...
                set_setting('item_purchase_code', isset($_POST['a2w_item_purchase_code']) ? wp_unslash($_POST['a2w_item_purchase_code']) : '');
                ...
                set_setting('default_product_type', isset($_POST['a2w_default_product_type']) ? wp_unslash($_POST['a2w_default_product_type']) : 'simple');
                set_setting('default_product_status', isset($_POST['a2w_default_product_status']) ? wp_unslash($_POST['a2w_default_product_status']) : 'publish');

                set_setting('delivered_order_status', isset($_POST['a2w_delivered_order_status']) ? wp_unslash($_POST['a2w_delivered_order_status']) : '');

                set_setting('tracking_code_order_status', isset($_POST['a2w_tracking_code_order_status']) ? wp_unslash($_POST['a2w_tracking_code_order_status']) : '');

                set_setting('placed_order_status', isset($_POST['a2w_placed_order_status']) ? wp_unslash($_POST['a2w_placed_order_status']) : '');

                set_setting('currency_conversion_factor', isset($_POST['a2w_currency_conversion_factor']) ? wp_unslash($_POST['a2w_currency_conversion_factor']) : '1');
                set_setting('import_product_images_limit', isset($_POST['a2w_import_product_images_limit']) && intval($_POST['a2w_import_product_images_limit']) ? intval($_POST['a2w_import_product_images_limit']) : '');
                set_setting('import_extended_attribute', isset($_POST['a2w_import_extended_attribute']) ? 1 : 0);

                set_setting('background_import', isset($_POST['a2w_background_import']) ? 1 : 0);
                set_setting('allow_product_duplication', isset($_POST['a2w_allow_product_duplication']) ? 1 : 0);
                set_setting('convert_attr_case', isset($_POST['a2w_convert_attr_case']) ? wp_unslash($_POST['a2w_convert_attr_case']) : 'original');

                set_setting('remove_ship_from', isset($_POST['a2w_remove_ship_from']) ? 1 : 0);
                set_setting('default_ship_from', isset($_POST['a2w_default_ship_from']) ? wp_unslash($_POST['a2w_default_ship_from']) : 'CN');

                set_setting('use_external_image_urls', isset($_POST['a2w_use_external_image_urls']));
                set_setting('not_import_attributes', isset($_POST['a2w_not_import_attributes']));
                set_setting('not_import_description', isset($_POST['a2w_not_import_description']));
                set_setting('not_import_description_images', isset($_POST['a2w_not_import_description_images']));

                set_setting('use_random_stock', isset($_POST['a2w_use_random_stock']));
                ...
                set_setting('auto_update', isset($_POST['a2w_auto_update']));
                set_setting('on_not_available_product', isset($_POST['a2w_on_not_available_product']) ? wp_unslash($_POST['a2w_on_not_available_product']) : 'trash');
                set_setting('on_not_available_variation', isset($_POST['a2w_on_not_available_variation']) ? wp_unslash($_POST['a2w_on_not_available_variation']) : 'trash');
                set_setting('on_new_variation_appearance', isset($_POST['a2w_on_new_variation_appearance']) ? wp_unslash($_POST['a2w_on_new_variation_appearance']) : 'add');
                set_setting('on_price_changes', isset($_POST['a2w_on_price_changes']) ? wp_unslash($_POST['a2w_on_price_changes']) : 'update');
                set_setting('on_stock_changes', isset($_POST['a2w_on_stock_changes']) ? wp_unslash($_POST['a2w_on_stock_changes']) : 'update');
                set_setting('untrash_product', isset($_POST['a2w_untrash_product']));
                set_setting('email_alerts', isset($_POST['a2w_email_alerts']));
                set_setting('email_alerts_email', isset($_POST['a2w_email_alerts_email']) ? wp_unslash($_POST['a2w_email_alerts_email']) : '');

                set_setting('fulfillment_prefship', isset($_POST['a2w_fulfillment_prefship']) ? wp_unslash($_POST['a2w_fulfillment_prefship']) : 'ePacket');
                set_setting('fulfillment_phone_code', isset($_POST['a2w_fulfillment_phone_code']) ? wp_unslash($_POST['a2w_fulfillment_phone_code']) : '');
                set_setting('fulfillment_phone_number', isset($_POST['a2w_fulfillment_phone_number']) ? wp_unslash($_POST['a2w_fulfillment_phone_number']) : '');
                set_setting('fulfillment_custom_note', isset($_POST['a2w_fulfillment_custom_note']) ? wp_unslash($_POST['a2w_fulfillment_custom_note']) : '');
                set_setting('fulfillment_cpf_meta_key', isset($_POST['a2w_fulfillment_cpf_meta_key']) ? wp_unslash($_POST['a2w_fulfillment_cpf_meta_key']) : '');
                set_setting('fulfillment_rut_meta_key', isset($_POST['a2w_fulfillment_rut_meta_key']) ? wp_unslash($_POST['a2w_fulfillment_rut_meta_key']) : '');

                set_setting('order_translitirate', isset($_POST['a2w_order_translitirate']));
                set_setting('order_third_name', isset($_POST['a2w_order_third_name']));

                settings()->commit();
                settings()->auto_commit(true);
                
                Update::clean_autoupdate_cache();
        ...
        private function buildUpgradeTariffUrl(): string
        {
            ...
            if (!a2w_check_defined('A2W_HIDE_KEY_FIELDS') && $purchaseCode){
                $urlComponents[] = 'purchase_code=' . esc_attr($purchaseCode);
            }

            $urlComponents[] = 'utm_source=full&utm_medium=full&utm_campaign=' . A2W()->plugin_slug;
        ...
        private function reviews()
        {
            if (isset($_POST['setting_form'])) {
                settings()->auto_commit(false);
                set_setting('load_review', isset($_POST['a2w_load_review']));
                set_setting('review_status', isset($_POST['a2w_review_status']));
                set_setting('review_translated', isset($_POST['a2w_review_translated']));
                set_setting('review_avatar_import', isset($_POST['a2w_review_avatar_import']));

                set_setting('review_schedule_load_period', 'a2w_15_mins');
                ...
                //update more field
                set_setting('review_load_attributes', isset($_POST['a2w_review_load_attributes']));

                set_setting('review_show_image_list', isset($_POST['a2w_review_show_image_list']));
                ...
                if (isset($_POST['a2w_review_show_image_list'])) {
                    ...
                    if ($a2w_review_thumb_width > 0) {
                        set_setting('review_thumb_width', $a2w_review_thumb_width);
                    } else {
                        set_setting('review_thumb_width', 30);
                    }
                }

                set_setting('review_skip_empty', isset($_POST['a2w_review_skip_empty']));
                set_setting('review_skip_keywords', isset($_POST['a2w_review_skip_keywords']) ? trim(wp_unslash($_POST['a2w_review_skip_keywords'])) : '');

                if (!isset($_POST['a2w_review_country']) || !is_array($_POST['a2w_review_country'])) {
                    set_setting('review_country', array());
                } else {
                    set_setting('review_country', $_POST['a2w_review_country']);
                }

                set_setting('moderation_reviews', isset($_POST['a2w_moderation_reviews']));
        ...
        private function shipping()
        {
            if (isset($_POST['setting_form'])) {
                ...
                set_setting('aliship_frontend', isset($_POST['a2w_aliship_frontend']));
                set_setting('default_shipping_class', !empty($_POST['a2w_default_shipping_class']) ? $_POST['a2w_default_shipping_class'] : false);
                ...
                if (isset($_POST['a2w_aliship_frontend'])) {
                    ...
                    set_setting('aliship_selection_type', isset($_POST['a2w_aliship_selection_type']) ? wp_unslash($_POST['a2w_aliship_selection_type']) : 'popup');

                    set_setting('aliship_shipping_type', isset($_POST['a2w_aliship_shipping_type']) ? wp_unslash($_POST['a2w_aliship_shipping_type']) : 'new');
                    ...
                    set_setting('aliship_shipping_label', isset($_POST['a2w_aliship_shipping_label']) ? wp_unslash($_POST['a2w_aliship_shipping_label']) : 'Shipping');
                    set_setting('aliship_free_shipping_label', isset($_POST['a2w_aliship_free_shipping_label']) ? wp_unslash($_POST['a2w_aliship_free_shipping_label']) : 'Free Shipping');

                    set_setting('aliship_product_enable', isset($_POST['a2w_aliship_product_enable']));

                    if (isset($_POST['a2w_aliship_product_enable'])) {
                        set_setting('aliship_product_position', isset($_POST['a2w_aliship_product_position']) ? wp_unslash($_POST['a2w_aliship_product_position']) : 'after_cart');
                    ...
                    set_setting('aliship_not_available_remove', isset($_POST['a2w_aliship_not_available_remove']));
        ...
        {
            if (isset($_POST['setting_form'])) {
                set_setting('write_info_log', isset($_POST['a2w_write_info_log']));
            }
