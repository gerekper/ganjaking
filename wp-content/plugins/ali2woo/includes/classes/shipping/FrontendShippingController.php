...
/**
 * Description of FrontendShippingController
 *
 * @author MA_GROUP
...
class FrontendShippingController extends AbstractController
{
        ...
        public function __construct()
    {
            ...
            if (get_setting('aliship_frontend') && Woocommerce::is_woocommerce_installed()) {

                $this->shipping_display_type = get_setting('aliship_selection_type');
        ...
        public function init() {
            $this->woocommerce_model = A2W()->getDI()->get('Ali2Woo\Woocommerce');
            ...
            if (get_setting('aliship_product_enable')) {
                ...
                if (get_setting('aliship_product_position') === 'before_cart') {
        ...
        public function assets()
    {
            ...
            if (is_cart() || is_checkout() || (is_product() && get_setting('aliship_product_enable'))) {
        ...
        public function render_shipping_on_product_page()
    {
            ...
            if ($external_id) {
                ...
                $default_shipping_method = get_setting('fulfillment_prefship');

                $countries = array_merge(array('' => __('Select a Country', 'ali2woo')), Shipping::get_countries());
                ...
                if ($this->shipping_display_type == "popup") {
                    ...
                    if ($default_shipping_method) {
                        ...
                        if (get_setting('aliship_not_available_remove')) {

                            //if this product page and option is enabled we use a separate not available message
                            $this->model_put("shipping_info", str_replace('{country}', $country_label, get_setting('aliship_product_not_available_message')));
                        } else {

                            $this->model_put("shipping_info", Shipping::get_not_available_shipping_message($country_label));
                    ...
                    if (!$default_shipping_method) {
                        $default_shipping_method = Shipping::get_fake_method_id();
                    }
    ...
    public function woocommerce_get_item_data($item_data, $cart_item)
    {
        ...
        if ($external_id) {
            ...
            if (isset($cart_item['a2w_to_country']) && $cart_item['a2w_to_country'] !== $default_shipping_to_country) {
                ...
                $cart_item['a2w_shipping_method'] = Shipping::get_fake_method_id();
            }
            ...
            $default_shipping_method ? $default_shipping_method : Shipping::get_fake_method_id();
        ...
        if (!empty($cart->removed_cart_contents)) {
            ...
            foreach ($cart->removed_cart_contents as $cart_item_key => $cart_item) {
                ...
                if (isset($cart_item['a2w_shipping_method']) && isset($cart_item['a2w_remove_no_shipping_item'])) {
                    ...
                    $result = Utils::get_product_shipping_info($_product, $cart_item['quantity'], $current_country, false);
                    ...
                    if ($result) {
                        ...
                        if (!empty($shipping_methods)) {
                            ...
                            foreach ($shipping_methods as $method) {
                                ...
                                $local_method = Shipping::get_local_method_by_company($method['company']);
            ...
            foreach ($cart->cart_contents as $cart_item_key => $cart_item) {
                ...
                if (!isset($checked_cart_keys[$cart_item_key]) && isset($cart_item['a2w_shipping_method']) && isset($cart_item['a2w_to_country'])) {
                    ...
                    if ($cart_item['a2w_to_country'] !== $current_country) {
                        ...
                        if ($cart_item['a2w_shipping_method'] === Shipping::get_fake_method_id() && $remove_cart_item) {
    ...
    public function woocommerce_checkout_create_order_line_item($item, $cart_item_key, $values, $order)
    {
        if (isset($values['a2w_shipping_method'])) {
            $cost_added = false;
            switch (get_setting('aliship_shipping_type', 'new')) {
                case 'new':
            ...
            if ($values['a2w_shipping_method'] === Shipping::get_fake_method_id()) {
                if (!get_setting('aliship_not_available_remove')) {
                    $shipping_info['shipping_cost'] = get_setting('aliship_not_available_cost');
                    $shipping_info['delivery_time'] = get_setting('aliship_not_available_time_min') . '-' . get_setting('aliship_not_available_time_max');
                    $shipping_info['company'] = '';
                    $shipping_info['service_name'] = '';
                }
            }

            $item->update_meta_data(Shipping::get_order_item_shipping_meta_key(), json_encode(array(
    ...
    private function get_default_shipping_country()
    {
        ...
        if (is_null($country) || empty($country)) {
            $country = get_setting('aliship_shipto');
        }
    ...
    private function remove_cart_item($cart_item_key, $cart_item, $country)
    {
        if ($cart_item) {
            if (WC()->cart->remove_cart_item($cart_item_key)) {
                ...
                WC()->cart->removed_cart_contents[$cart_item_key]['a2w_shipping_method'] = Shipping::get_fake_method_id();
    ...
    private function get_shipping_data_for_render($product, $quantity, $default_shipping_to_country, $default_shipping_method)
    {
        ...
        $result = Utils::get_product_shipping_info($product, $quantity, $default_shipping_to_country, false);
        ...
        if ($result) {
            ...
            if (!empty($shipping_methods)) {
                ...
                foreach ($shipping_methods as $method) {
                    ...
                    $normalized_method = Shipping::get_normalized($method, $default_shipping_to_country, "select", $product);
