...
if (!class_exists('A2W_Woocommerce')) {
    ...
    class A2W_Woocommerce {
        ...
        public function add_product($product, $params = array()) {
            ...
            if($step === false || $step === 'init'){
                ...
                if(isset($product['shipping_from_country'])) {
                    $shipping_meta->save_country_to($product['shipping_from_country'], false);
                }
        ...
        public function get_fulfilled_orders_data($only_not_delivered=true) {
           ...
           // $result = $wpdb->get_results("SELECT pm1.meta_value as ext_order_id, pm1.post_id as order_id FROM {$wpdb->postmeta} as pm1 LEFT JOIN {$wpdb->postmeta} as pm2 ON (pm2.meta_key = '" . A2W_Constants::old_order_tracking_code() . "' and pm1.post_id=pm2.post_id) WHERE pm1.meta_key = '" . A2W_Constants::old_order_external_order_id() . "' AND pm2.post_id is null AND pm1.meta_value <> ''");
           // return $result;
        ...
        public function save_tracking_code($order_id, $ext_id, $tracking_codes, $carrier_name, $carrier_url, $tracking_status) {
            ...
            if (!$order) {
                ...
                foreach (  $order_items as $item_id => $item ) {
                    ...
                    if ($external_order_id) {
                        ...
                        if ($external_order_id === intval($ext_id)){
            ...
            /*    $curr_tracking_codes = get_post_meta($order_id, '_a2w_tracking_code');
               ...
               /* $order_external_id_array = get_post_meta( $order->get_id(), '_a2w_external_order_id' );
                ...
                if (is_array($order_external_id_array) && is_array($order_tracking_codes) 
                    && count($order_external_id_array) == count($order_tracking_codes)){
                    
                    //update order status automatically, when ali2woo get all tracking codes only.
        ...
        public function get_sorted_products_ids($sort_type, $ids_count, $compare = false) {
            ...
            if ($compare) {
                ...
                if(!empty($operation) && !empty($value)){
                    $query = $wpdb->prepare("SELECT p.ID FROM $wpdb->posts p INNER JOIN $wpdb->postmeta pm1 ON (pm1.post_id=p.ID AND pm1.meta_key='_a2w_external_id') LEFT JOIN $wpdb->postmeta pm2 ON (pm2.post_id=p.ID AND pm2.meta_key=%s) LEFT JOIN $wpdb->postmeta pm3 ON (pm3.post_id=p.ID AND pm3.meta_key='_a2w_autoremove') WHERE p.post_type = 'product' AND pm1.meta_value != '' AND (p.post_status != 'trash' || NOT pm3.meta_key IS NULL) AND (pm2.meta_value is null OR pm2.meta_value $operation %s) ORDER BY pm2.meta_value LIMIT %d", $sort_type, $value, $ids_count);
                }
            }else{
                $query = $wpdb->prepare("SELECT p.ID FROM $wpdb->posts p INNER JOIN $wpdb->postmeta pm1 ON (pm1.post_id=p.ID AND pm1.meta_key='_a2w_external_id') LEFT JOIN $wpdb->postmeta pm2 ON (pm2.post_id=p.ID AND pm2.meta_key=%s) LEFT JOIN $wpdb->postmeta pm3 ON (pm3.post_id=p.ID AND pm3.meta_key='_a2w_autoremove') WHERE p.post_type = 'product' AND pm1.meta_value != '' AND (p.post_status != 'trash' || NOT pm3.meta_key IS NULL) ORDER BY pm2.meta_value LIMIT %d", $sort_type, $ids_count);
            }
