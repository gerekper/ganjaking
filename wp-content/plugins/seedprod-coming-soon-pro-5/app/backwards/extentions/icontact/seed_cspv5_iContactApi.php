...
class seed_cspv5_iContactApi {
	...
	**/
	public function setAccountId( $iAccountId = null ) {
		...
		if ( ! empty( $iAccountId ) ) {
			...
			$this->iAccountId = (int) $iAccountId;
		} else {
			// Check to see if the
			...
			if ( empty( $this->iAccountId ) ) {
				...
				if ( $aAccounts = $this->makeCall( '/a/', 'get', null, 'accounts' ) ) {
					...
					if ( intval( $aAccount->enabled ) === 1 ) {
						...
						// set the Account ID
						$this->iAccountId = (int) $aAccount->accountId;
					} else {
	...
	**/
	public function setClientFolderId( $iClientFolderId = null ) {
		...
		if ( ! empty( $iClientFolderId ) ) {
			...
			$this->iClientFolderId = (int) $iClientFolderId;
			...
			if ( $aClients = $this->makeCall( $sResource, 'get', null, 'clientfolders' ) ) {
				if ( empty( $aClients ) ) {
					...
					$aClient = $aClients[0];
					// Set the Client Folder ID
					$this->iClientFolderId = (int) $aClient->clientFolderId;
				}
			}
		}
		// Inevitably return instance
	...
	**/
	public function setLimit( $iLimit ) {
		...
		$this->aSearchParameters['limit'] = (int) $iLimit;
		// Return instance
		return $this;
	}
	...
	**/
	public function setOffset( $iOffset ) {
		...
		$this->aSearchParameters['offset'] = (int) $iOffset;
		// Return instance
		return $this;
	}
}
