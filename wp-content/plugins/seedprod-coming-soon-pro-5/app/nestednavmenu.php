...
function seedprod_pro_get_nested_navmenu() {
	if ( check_ajax_referer( 'seedprod_nonce' ) ) {
		...
		$navmenu_name = isset( $_POST['navmenu_name'] ) ? sanitize_text_field( wp_unslash( $_POST['navmenu_name'] ) ) : '';
		...
		if ( true == $walker_divider ) {
			...
			$args = array(
				...
				'walker'          => new SeedProd_Menu_Walker( $divider ),
			);
 ...
 */
class SeedProd_Menu_Walker extends Walker_Nav_Menu {
	/**
	 * Separators value.
	 *
	 * @var string
...
function seedprod_pro_wordpress_menuwidget( $atts ) {
	...
	if ( true == $walker_divider ) {
		$args = array(
			...
			'container_class' => 'nav-menu-bar',
			'menu_class'      => 'seedprod-menu-list',
			'walker'          => new SeedProd_Menu_Walker( $navmenu_seperator ),
		);
	} else {
		$args = array(
			'menu'            => $navmenu_name,
