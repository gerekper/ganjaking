...
final class Post extends Sorting\Factory implements Sorting\TableStorageFactory {
	...
	protected function create_default( Column $column ) {
		...
		$model = ( new MetaRelatedPostFactory() )->create( $column->get_meta_type(), $setting->get_value(), $column->get_meta_key() );

		if ( ! $model ) {
			$model = ( new MetaFormatFactory() )->create( $column->get_meta_type(), $column->get_meta_key(), new FormatValue\SettingFormatter( $setting ) );
		}
