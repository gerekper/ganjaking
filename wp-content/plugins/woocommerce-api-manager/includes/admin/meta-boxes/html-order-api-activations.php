		...
		foreach ( $resources as $resource ) {
			...
			if ( ! $is_expired ) {
				if ( $resource->api_key == $resource->master_api_key ) {
					$api_key_type = __( 'Master API Key', 'woocommerce-api-manager' );
				} elseif ( $resource->api_key == $resource->product_order_api_key ) {
					$api_key_type = __( 'Product Order API Key', 'woocommerce-api-manager' );
				} else {
					$api_key_type = __( 'Associated API Key', 'woocommerce-api-manager' );
				}
				?>
                <tr<?php if ( $i % 2 == 0 )
					echo ' class="alternate"' ?>>
                    <td><?php esc_html_e( $api_key_type ); ?></td>
