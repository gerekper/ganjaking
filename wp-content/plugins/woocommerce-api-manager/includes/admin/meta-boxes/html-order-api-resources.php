        ...
        <div class="wc-metabox closed">
            <h3 class="fixed">
                <div style="padding: 1em; border-radius: 1em;" <?php if ( $i % 2 == 0 )
                    ...
                    <strong><?php printf( __( 'Product ID: %s | Product Title: %s | Activations: %s out of %s | Current Version: %s | Expires: %s', 'woocommerce-api-manager' ), esc_attr( $resource->product_id ), esc_html( $resource->product_title ), esc_attr( $resource->activations_total ), esc_attr( $resource->activations_purchased_total ), ! empty( $version ) ? esc_attr( $version ) : esc_attr( '' ), esc_html( $expires ) ); ?></strong>
                </div>
            </h3>
