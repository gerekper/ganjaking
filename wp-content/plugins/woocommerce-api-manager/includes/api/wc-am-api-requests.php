	...
	private function api_doc_tab_choices( $product_id ) {
		...
		if ( $description == 'yes' ) {
			...
			if ( $api_description !== false ) {
				...
				$get_description_section   = $this->get_page_content( get_post( WC_AM_PRODUCT_DATA_STORE()->get_meta( $product_id, '_api_description' ) ) );
		...
		if ( $installation == 'yes' ) {
			...
			if ( $api_installation !== false ) {
				...
				$get_installation_section   = $this->get_page_content( get_post( WC_AM_PRODUCT_DATA_STORE()->get_meta( $product_id, '_api_installation' ) ) );
		...
		if ( $faq == 'yes' ) {
			...
			if ( $api_faq !== false ) {
				...
				$get_faq_section   = $this->get_page_content( get_post( WC_AM_PRODUCT_DATA_STORE()->get_meta( $product_id, '_api_faq' ) ) );
				$sections[ 'faq' ] = $get_faq_section;

				if ( WCAM()->get_db_cache() ) {
					WC_AM_SMART_CACHE()->set_or_get_cache( $trans_faq_name, $get_faq_section, WCAM()->get_api_cache_expires() * MINUTE_IN_SECONDS );
				}
			}
		}
		...
		if ( $screenshots == 'yes' ) {
			...
			if ( $api_screenshots !== false ) {
				...
				$get_screenshots_section   = $this->get_page_content( get_post( WC_AM_PRODUCT_DATA_STORE()->get_meta( $product_id, '_api_screenshots' ) ) );
				$sections[ 'screenshots' ] = $get_screenshots_section;

				if ( WCAM()->get_db_cache() ) {
					WC_AM_SMART_CACHE()->set_or_get_cache( $trans_screenshots_name, $get_screenshots_section, WCAM()->get_api_cache_expires() * MINUTE_IN_SECONDS );
				}
			}
		}
		...
		if ( $api_changelog !== false ) {
			...
			$get_changelog_section   = $this->get_page_content( get_post( WC_AM_PRODUCT_DATA_STORE()->get_meta( $product_id, '_api_changelog' ) ) );
