	...
	class WC_SC_Settings {
		...
		public function get_settings() {
			...
			$sc_settings = array(
				...
				array(
					...
					'desc'          => sprintf( __( 'Allow selling %s at discounted price', 'woocommerce-smart-coupons' ), strtolower( $plural ) ) . ' <a href="https://docs.woocommerce.com/document/smart-coupons/how-to-sell-gift-card-at-less-price/" target="_blank"><small>' . __( '[Read More]', 'woocommerce-smart-coupons' ) . '</small></a><span class="woocommerce-help-tip" data-tip="' . esc_attr( sprintf( __( 'When selling %1$s, if Regular and Sale price is found for the product, then coupon will be created with product\'s Regular Price but customer will pay product\'s Sale price. This setting will also make sure if any discount coupon is applied on the %2$s while purchasing, then customer will get %3$s in their picked price', 'woocommerce-smart-coupons' ), strtolower( $singular ), strtolower( $singular ), strtolower( $singular ) ) ) . '"></span>',
		...
		public function coupon_design_html( $design = '' ) {
			...
			$args = array(
				...
				'coupon_expiry'      => gmdate( 'd-M-Y', strtotime( 'Dec 31' ) ),
