...
(function ($) {
	...
	$(
		function () {
			...
			function get_conversation(conversation_id, currentSub) {
				var wrap = $( "#support-wrap-conversation" );
				$.ajax(
					{
						type: "GET",
						...
						success: function (data) {
							...
							if (null !== data && "" === data.error) {
								html += '<ul id="support-conversation-thread" style="margin:0 auto;">';
								$.each(
									data.threads,
									function (key, thread) {
										html += "<li>";
			...
			$( "#support-conversations-table .conversation-view" ).on(
				"click",
				function (e) {
					e.preventDefault();
					$( "#support-wrap-conversation" ).html( "" );
			...
			$( "body" ).on(
				...
				function (e) {
					...
					$( ".my-account-conversation-file-1" ).imageUploader( { maxFiles: 10, extensions: [".jpg", ".jpeg", ".png", ".pdf", ".JPG", ".JPEG", ".PNG", ".PDF"], mimes: ["image/jpeg", "image/png", "image/gif", "application/pdf"] } );
					var c = $( this ).data( "subject" ),
					a = $( this ).data( "conversation-id" );
					get_conversation( a, c );
					$( "#support-conversation-wrap-reply" ).fadeOut(
						500,
						function () {
							$( "#support-conversation-thread-head-reply" ).html( woocommerce_help_scout_myaccount_params.reply_to + " " + c ), $( "#support-conversation-reply .uploaded" ).html( "" );
							$( "#support-conversation-reply .image-uploader" ).removeClass( "has-files" );
							$( "#reply_conversation_id" ).val( a ), $( "#reply_user_id" ).val( woocommerce_help_scout_myaccount_params.user_id ), $( "#reply_submit_btn" ).val( woocommerce_help_scout_myaccount_params.send ), $( this ).show();
