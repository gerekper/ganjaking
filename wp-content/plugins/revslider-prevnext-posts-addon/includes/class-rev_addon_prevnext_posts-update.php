...
class RevAddOnPrevNextPostsUpdate {
	private $plugin_url		 = 'https://codecanyon.net/item/slider-revolution-responsive-wordpress-plugin/2751380';
	private $remote_url_info = 'addons/revslider-prevnext-posts-addon/revslider-prevnext-posts-addon.php';
	private $plugin_slug	 = 'revslider-prevnext-posts-addon';
	private $plugin_path	 = 'revslider-prevnext-posts-addon/revslider-prevnext-posts-addon.php';
	private $version;
	private $plugins;
	private $option;
	...
	protected function _check_updates($force_check = false){
		...
		$last_check = get_option('rev_prevnext_posts-update-check');

		if($last_check == false){ //first time called
			$last_check = time();
			update_option('rev_prevnext_posts-update-check', $last_check);
		}
		...
		if(time() - $last_check > 60 * 60 * 24 * 30 || $force_check == true){
			...
			if(isset($data->basic)){
				update_option('rev_prevnext_posts-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
				
				update_option('rev_prevnext_posts-latest-version', $data->full->version);
			}
		}
	...
	public function _retrieve_update_info(){
		...
		$rattr = array(
			...
			'version' => urlencode(REV_ADDON_PREVNEXT_POSTS_VERSION)
		);
