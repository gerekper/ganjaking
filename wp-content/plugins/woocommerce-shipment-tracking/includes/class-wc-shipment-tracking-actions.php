...
class WC_Shipment_Tracking_Actions {
	...
	public function meta_box( $post_or_order_object ) {
		...
		woocommerce_wp_text_input(
			array(
				...
				'value'       => date_i18n( __( 'Y-m-d', 'woocommerce-shipment-tracking' ), time() ),
			)
		);
	 ...
	 * @param int    $original_order_id Order ID when created the first time.
	 * @param int    $renewal_order_id  Order ID after renewal.
	 */
