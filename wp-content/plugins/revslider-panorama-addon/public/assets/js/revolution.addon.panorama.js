...
;(function() {
	...
	window.RsAddonPanorama = function(_$, slider) {
		...
		if(!slider.find('rs-slide[data-panorama]').length) return;
	...
	function Panorama(slider) {
		...
		this.slider = slider;	
		this.slides = [];
		this.panos = slider.find('rs-slide[data-panorama]').each(function(i) {
			
			if(i === 0) this.setAttribute('data-firstanim', 't:notransition;s:d;sl:d;');
			this.setAttribute('data-anim', 'ei:d;eo:d;s:d;r:0;t:fade;sl:d;');
	...
	Panorama.prototype = {
		...
		onReady: function(e, data) {
			...
			slide = $.data(slide[0], 'rsaddonpanorama');
		...
		afterSwap: function(e, data) {
			...
			// 'active-revslide' class no longer available by default
			$('.rs-addon-pano-active').removeClass('rs-addon-pano-active');
			slide.addClass('rs-addon-pano-active');
			
			this.currentSlide = $.data(slide[0], 'rsaddonpanorama');
		...
		layerAction: function(e, a, b, data) {
			
			var action = data.action;
			if(action && action.search('panorama') !== -1 && this.currentSlide) {
				
				var perc = data.hasOwnProperty('percentage') ? data.percentage * 0.05 : false;
	...
	function PanoramaSlide(slide, slider, width, height) {
		...
		this.mobilelock = options.mobilelock;
		this.controls = options.interaction.controls;
		this.lockVertical = options.interaction.lockVertical;
		...
		this.autoplay = isTrue(options.autoplay.enable);
		this.zoom = isTrue(options.zoom.enable);
		this.smoothZoom = isTrue(options.zoom.smooth);
		
		var direction = options.autoplay.direction === 'forward' ? 1 : -1;
		this.autoplaySpeed = parseInt(options.autoplay.speed, 10) * 0.001;
		this.speed = this.autoplaySpeed * direction;
		
		this.throwSpeed = parseInt(options.interaction.speed, 10) * 0.001;
		this.zoomMax = 2 - (parseInt(options.zoom.max, 10) * 0.01);
		this.zoomMin = 2 - (parseInt(options.zoom.min, 10) * 0.01);
		this.fov = parseInt(options.camera.fov, 10);
		...
		this.camera = new THREE.PerspectiveCamera(this.fov, width / height, 1, parseInt(options.camera.far, 10));
		...
		this.canvas = $(this.renderer.domElement).addClass('rsaddon-panorama').appendTo(slide.find('rs-sbg-wrap'));
		
		slide.on('mouseleave', this.onLeave.bind(this)).find('.rs-waction').each(setActions, [slider]);
	...
	PanoramaSlide.prototype = {
		...
		render: function() {
			...
			this.y = !this.mobilelock && this.lockVertical!==true ? Math.max(-85, Math.min(85, this.y)) : 0;
	...
	function setActions(slider) {
		
		var $this = jQuery(this),
			actions = $this.attr('data-actions');
			
		if(!actions || $this.hasClass('rspanoaction')) return;
		...
		while(i--) {
			...
			action = actions[i].split('a:');
			if(action.length < 2) continue;
			
			action = action[1].split(';')[0];
			if(action.search('panorama') !== -1) $this.addClass('rspanoaction').on('mousedown click mousemove touchmove', stopBubble);
