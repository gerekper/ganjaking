...
class GP_Pay_Per_Word extends GWPerk {
	...
	function field_settings_ui() {
		...
		<li class="<?php echo $this->slug; ?>_setting gp-field-setting field_setting" style="display:none;">
			...
			<div id="<?php echo $this->key( 'settings' ); ?>" class="gp-child-settings">

				<div class="gp-row">
					<label for="<?php echo $this->slug; ?>_word_field" class="section_label">
				...
				<div class="gp-row">
					<label for="<?php echo $this->slug; ?>_price_per_word" class="section_label">
				...
				<div id="<?php echo $this->key( 'base_price_settings' ); ?>" class="gp-child-settings"
					 style="display:none;">

					<div class="gp-row gp-group">
						<label for="<?php echo $this->slug; ?>_base_price" class="section_label">
					...
					<div class="gp-row gp-group">
						<label for="<?php echo $this->slug; ?>_base_word_count" class="section_label">
		...
		<script type="text/javascript">
			...
			jQuery( function ( $ ) {
				...
				$( document ).bind( 'gform_load_field_settings', function ( event, field ) {
					...
					jQuery( '#<?php echo $this->slug; ?>_price_per_word' ).val( gformFormatMoney( field['<?php echo $this->slug; ?>_price_per_word'], true ) );
					...
					if( field['<?php echo $this->slug; ?>_enable_base_price'] ) {
						jQuery( '#<?php echo $this->slug; ?>_base_price' ).val( gformFormatMoney( field['<?php echo $this->slug; ?>_base_price'], true ) );
	...
	public function validate( $validation_result ) {
		...
		foreach ( $form['fields'] as &$field ) {
			...
			} elseif ( ! empty( $quantity ) && ( ! is_numeric( $quantity ) || intval( $quantity ) != floatval( $quantity ) ) ) {
				$field['failed_validation']  = true;
	...
	public function calculate_price( $price_field, $form ) {
		...
		if ( $word_field->useRichTextEditor ) {
			...
			$words      = RGFormsModel::get_field_value( $word_field );
			$word_count = count( array_filter( preg_split( '/[ \n\r]+/', trim( $words ) ) ) );
		}
		 ...
		 * @param string                  $words       The string of words to be counted.
		 * @param \GF_Field_SingleProduct $price_field The Single Product field that has been configured to charge per word.
		 * @param \GF_Field               $word_field  The field for which the number of words will be counted.
		 * @param array                   $form        The current form object.
		 *
		 * @since 1.1.4
		 */
		$word_count = apply_filters( 'gpppw_word_count', $word_count, $words, $price_field, $word_field, $form );
