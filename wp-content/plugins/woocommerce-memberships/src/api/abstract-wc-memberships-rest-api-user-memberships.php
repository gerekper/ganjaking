...
class User_Memberships extends Controller
{
    ...
    protected function prepare_items_query_args($request)
    {
        ...
        $query_args = [
            ...
            'order'               => 'DESC', // may be overridden later, see below
            'post_status'		  => 'any', // maybe overriden later, see below
        ];
    ...
    /**
     * Gets a list of matched users for the search query.
     *
     * @internal
     *
     * @since 1.23.0
     ...
     */
    public function get_users_for_search($search_string = '')
    {
        global $wpdb;
    ...
    public function get_formatted_item_data($user_membership, $request = null)
    {
        ...
        if ($user_membership instanceof \WC_Memberships_User_Membership) {
            ...
            $includes = ! empty($request['_includes']) ? $request['_includes'] : '';
            ...
            // if request has _includes=customer_data paramater
            if (
                wc_memberships()->is_plugin_active('woocommerce.php')
                && Framework\SV_WC_Helper::str_exists($includes, 'customer_data')
            ) {
                if ($customer = new \WC_Customer($data['customer_id'])) {
