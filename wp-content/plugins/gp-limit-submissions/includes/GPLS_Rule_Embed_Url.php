...
class GPLS_Rule_Embed_Url extends GPLS_Rule {
	...
	public function context() {
		...
		if ( $this->type != "all" ) {
			...
			if ( $this->type == "full" ) {
				...
				$test_url = get_permalink( $this->post_id );
			}
			if ( $this->match_base_url() ) {

				// match base URL
				if ( strpos( GFFormsModel::get_current_page_url(), $test_url ) === false ) {
					return false;
				}
			} else {

				// match exact URL
				if ( GFFormsModel::get_current_page_url() != $test_url ) {
					return false;
				}
			}
		}
	...
	public function query() {
		...
		if ( $this->type == "all" ) {
			...
			return $wpdb->prepare( 'e.source_url = %s', GFFormsModel::get_current_page_url() );
		}
		if ( $this->type == "full" ) {
			// match full url provided
			if ( $this->match_base_url() ) {
				// like matching to prevent user appending the url to avoid limit
				return $wpdb->prepare( 'e.source_url LIKE %s', $this->url . '%' );
			} else {
				return $wpdb->prepare( 'e.source_url = %s', $this->url );
			}
		}
		if ( $this->type == "post_id" ) {
			...
			if ( $this->match_base_url() ) {
				return $wpdb->prepare( 'e.source_url LIKE %s', get_permalink( $this->post_id ) . '%' );
			} else {
				return $wpdb->prepare( 'e.source_url = %s', get_permalink( $this->post_id ) );
			}
		}
	}
