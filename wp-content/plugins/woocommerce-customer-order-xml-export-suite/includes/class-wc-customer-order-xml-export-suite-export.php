...
/**
 * Customer/Order XML Export Suite Export class
 *
 * Represents a single export.
 *
 * @since 2.4.0
 */
class WC_Customer_Order_XML_Export_Suite_Export {


	/** @var stdClass object containing the job attributes */
	protected $job;

	/** @var \WC_Customer_Order_XML_Export_Suite_Data_Store data store instance */
	protected $data_store;

	/** @var \WC_Customer_Order_XML_Export_Suite_Generator generator instance */
	protected $generator;
	...
	public function __construct( $args ) {
		...
		if ( is_string( $args ) && '' !== $args ) {
			...
			$data_store = WC_Customer_Order_XML_Export_Suite_Data_Store_Factory::create( $args['storage_method'] );
		...
		if ( ! $job ) {
			throw new SV_WC_Plugin_Exception( __( 'Unable to find or create export job', 'woocommerce-customer-order-xml-export-suite' ) );
		}
	 ...
	 * @since 2.4.0
	 *
	 * @return \WC_Customer_Order_XML_Export_Suite_Data_Store|null data store instance
	 */
	public function get_data_store() {
		...
		return $storage_method ? $this->data_store = WC_Customer_Order_XML_Export_Suite_Data_Store_Factory::create( $storage_method ) : null;
	}
	...
	public function get_generator() {
		...
		return $this->generator = new WC_Customer_Order_XML_Export_Suite_Generator( $this->get_type() );
	}
	...
	public function get_temporary_file_path() {
		...
		$file_path = wc_customer_order_xml_export_suite()->get_export_handler_instance()->create_temp_file( $filename, $file_stream );
