...
/**
 * Customer/Order XML Export Suite Database Data Store
 *
 * Persists exports to the database.
 *
 * @since 2.4.0
 */
class WC_Customer_Order_XML_Export_Suite_Data_Store_Database extends WC_Customer_Order_XML_Export_Suite_Data_Store {


	/** @var string custom database stream wrapper protocol */
	const STREAM_WRAPPER_PROTOCOL = 'wc-xml-export-suite-database';
	...
	public function __construct() {
		...
		require_once wc_customer_order_xml_export_suite()->get_plugin_path() . '/includes/class-wc-customer-order-xml-export-suite-lifecycle.php';

		// create the database table if it doesn't exist
		if ( ! WC_Customer_Order_XML_Export_Suite_Lifecycle::validate_table() ) {
			WC_Customer_Order_XML_Export_Suite_Lifecycle::create_tables();
		}
		...
		if ( ! in_array( self::STREAM_WRAPPER_PROTOCOL, stream_get_wrappers(), true ) ) {
			...
			stream_wrapper_register( self::STREAM_WRAPPER_PROTOCOL, 'WC_Customer_Order_XML_Export_Suite_Database_Stream_Wrapper' );
		}
	}
	...
	public static function get_table_name() {
		...
		return $wpdb->prefix . 'woocommerce_exported_xml_items';
	}
...
CREATE TABLE {$table_name} (
  ...
  KEY export_id (export_id ASC)
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::store_item()
	 *
	 * @since 2.4.0
	 *
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::get_file_size()
	 *
	 * @since 2.4.0
	 *
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::get_output()
	 *
	 * @since 2.4.0
	 *
	 * @param \WC_Customer_Order_XML_Export_Suite_Export $export the export object
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::stream_output()
	 *
	 * @since 2.4.0
	 *
	...
	public function stream_output( $export, $resource ) {
		...
		$export_iterator = new WC_Customer_Order_XML_Export_Suite_Database_Stream_Iterator( $export );
	...
	private function inefficient_stream_output( $export, $resource ) {
		...
		if ( ! is_resource( $wpdb->dbh ) || ! function_exists( 'mysql_query' ) || ! 'mysql link' === get_resource_type( $wpdb->dbh ) ) {
			...
			throw new SV_WC_Plugin_Exception( __( 'Unable to locate a legacy mysql connection in this WordPress install', 'woocommerce-customer-order-xml-export-suite' ) );
		}
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::get_file_stream()
	 *
	 * @since 2.4.0
	 *
	 * @param \WC_Customer_Order_XML_Export_Suite_Export $export the export object
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::delete_export()
	 *
	 * @since 2.4.0
	 *
	 * @param \WC_Customer_Order_XML_Export_Suite_Export $export the export object
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::prepare_content_for_storage()
	 *
	 * @since 2.4.0
	 *
	 ...
	 *
	 * @see WC_Customer_Order_XML_Export_Suite_Data_Store::process_content_from_storage()
	 *
	 * @since 2.4.0
	 *
