 ...
 * @since 4.4.0-dev.1
 */
class WC_Customer_Order_XML_Export_Suite_Batch_Export_Handler extends SV_WP_Job_Batch_Handler {
	...
	protected function process_job_status( $job ) {
		...
		if ( 'failed' === $job->status ) {
			...
			$error = $this->get_job_handler()->get_export_status_message( 'failed' );
		...
		if ( 'completed' === $job->status ) {
			...
			if ( 'failed' === $job->transfer_status ) {
				...
				$error = $this->get_job_handler()->get_export_status_message( 'transfer-failed' );
