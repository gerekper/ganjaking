...
class WC_Gateway_Redsys extends WC_Payment_Gateway {
		...
		$this->form_fields = array(
			...
			*/
			...
			'sendemailsdscard'     => array(
				'title'       => __( 'Send emails Ds_CardNumber problem', 'woocommerce-redsys' ),
				'type'        => 'checkbox',
	...
	function receipt_page( $order ) {
		...
		if ( isset( $_GET['threeDSServerTransID'] ) ) {
			...
			if ( 'yes' === $this->debug ) {
				...
				$this->log->add( 'redsys', '$user_id: ' . $user_id );
				$this->log->add( 'redsys', '$browserAcceptHeader: ' . implode( '', WCRed()->clean_data( $browserAcceptHeader ) ) );
				$this->log->add( 'redsys', '$browserColorDepth: ' . implode( '', $browserColorDepth ) );
				$this->log->add( 'redsys', '$browserLanguage: ' . implode( '', WCRed()->clean_data( $browserLanguage ) ) );
				$this->log->add( 'redsys', '$browserScreenHeight: ' . implode( '', WCRed()->clean_data( $browserScreenHeight ) ) );
				$this->log->add( 'redsys', '$browserScreenWidth: ' . implode( '', WCRed()->clean_data( $browserScreenWidth ) ) );
				$this->log->add( 'redsys', '$browserTZ: ' . implode( '', $browserTZ ) );
				$this->log->add( 'redsys', '$java_enabled: ' . implode( '', WCRed()->clean_data( $java_enabled ) ) );
				$this->log->add( 'redsys', '$protocolVersion: ' . $protocolVersion );
				$this->log->add( 'redsys', '$merchant_cof: ' . $merchant_cof );
				$this->log->add( 'redsys', '$merchant_type: ' . $merchant_type );
				$this->log->add( 'redsys', '$excep_sca: ' . $excep_sca );
	...
	function check_ipn_request_is_valid() {
		...
		if ( isset( $_GET['threeDSMethodURL'] ) ) {
			...
			if ( ! empty( $threeDSServerTransID ) && ! empty( $three_ds_method_url ) ) {
		...
		if ( $merchant_identifier && $merchant_txnid ) {
			...
			if ( isset( $responsews->trataPeticionReturn ) ) {
				...
				if ( isset( $xml_retorno->OPERACION->Ds_Merchant_Identifier ) ) {
					...
					$number         = (string) $xml_retorno->OPERACION->Ds_CardNumber;
				} else {
					$identifier = false;
				}
			}
	...
	function successful_request( $posted ) {
		...
		if ( 'yes' === $is_add_method ) {
			...
			$dscardnumbercompl = $mi_obj->getParameter( 'Ds_CardNumber' );
		...
		if ( 'yes' === $this->debug ) {
			...
			$this->log->add( 'redsys', '$order2: ' . $order2 );
			$this->log->add( 'redsys', 'Ds_TransactionType: ' . $dstransactiontype );
			$this->log->add( 'redsys', 'Ds_CardNumber: ' . $dscardnumbercomp );
		...
		if ( (int) $response <= 99 ) {
			...
			if ( ( ( WCRed()->order_contains_subscription( $order->get_id() ) && 'yes' !== $this->subsusetokensdisable ) ) || ( ( 'yes' === $this->usetokens ) && ( ! empty( $dsmerchantidenti ) ) && ( '0' === $dsmechandata || '1' === $dsmechandata ) ) ) {
				if ( WCRed()->order_contains_subscription( $order->get_id() ) ) {
					if ( WCRed()->order_contains_subscription( $order->get_id() ) ) {
						...
						if ( WCRed()->order_contains_subscription( $order->get_id() ) && 'yes' !== $this->subsusetokensdisable && 'yes' === $this->psd2 ) {
							if ( 'yes' === $this->debug ) {
						...
						} elseif ( WCRed()->order_contains_subscription( $order->get_id() ) && 'yes' !== $this->subsusetokensdisable ) {
							if ( 'yes' === $this->debug ) {
			...
			if ( ! empty( $dscode ) ) {
				update_post_meta( $order->get_id(), '_order_fuc_redsys', $dscode );
				if ( 'yes' === $this->debug ) {
	...
	function ask_for_refund( $order_id, $transaction_id, $amount ) {
		...
		$user_id        = $order->get_user_id();
		$secretsha256   = $this->get_redsys_sha256( $user_id );
		$commerce_fuc   = get_post_meta( $order_id, '_order_fuc_redsys', true );
	...
	function check_user_show_payment_method( $userid = false ) {
		...
		if ( 'yes' === $this->debug ) {
			$this->log->add( 'redsys', '$test_mode: ' . $test_mode );
			$this->log->add( 'redsys', '/****************************/' );
			$this->log->add( 'redsys', '$selections ' . print_r( $selections, true ) );
			$this->log->add( 'redsys', '/****************************/' );
			$this->log->add( 'redsys', ' ' );
		}
		...
		if ( 'yes' !== $test_mode ) {
			if ( 'yes' === $this->debug ) {
				$this->log->add( 'redsys', ' ' );
				$this->log->add( 'redsys', '/****************************/' );
				$this->log->add( 'redsys', '$test_mode different to yes showing Gateway' );
				$this->log->add( 'redsys', '/****************************/' );
				$this->log->add( 'redsys', ' ' );
			}
			return true;
		...
		if ( $selections[0] !== '' || empty( $selections ) ) {
			if ( 'yes' === $this->debug ) {
				$this->log->add( 'redsys', ' ' );
				$this->log->add( 'redsys', '/****************************/' );
				$this->log->add( 'redsys', '$selections NOT empty' );
				$this->log->add( 'redsys', '/****************************/' );
				$this->log->add( 'redsys', ' ' );
			}
			if ( ! $userid ) {
				if ( 'yes' === $this->debug ) {
					$this->log->add( 'redsys', ' ' );
					$this->log->add( 'redsys', '/****************************/' );
					$this->log->add( 'redsys', 'Not loged In hiding gateway' );
					$this->log->add( 'redsys', '/****************************/' );
					$this->log->add( 'redsys', ' ' );
				}
				return false;
			...
			foreach ( $selections as $user_id ) {
				if ( 'yes' === $this->debug ) {
					$this->log->add( 'redsys', '$user_id: ' . $user_id );
					$this->log->add( 'redsys', '$userid: ' . $userid );
					$this->log->add( 'redsys', ' ' );
				}
				if ( (int) $user_id === (int) $userid ) {
					...
					if ( 'yes' === $this->debug ) {
						$this->log->add( 'redsys', '/****************************/' );
						$this->log->add( 'redsys', ' $user_id === $userid, Showing gateway' );
						$this->log->add( 'redsys', '/****************************/' );
						$this->log->add( 'redsys', ' ' );
					}
					return true;
			...
			if ( 'yes' === $this->debug ) {
				$this->log->add( 'redsys', ' ' );
				$this->log->add( 'redsys', '/*********************************/' );
				$this->log->add( 'redsys', '$selections Empty, showing gateway' );
				$this->log->add( 'redsys', '/*********************************/' );
				$this->log->add( 'redsys', ' ' );
			}
			return true;
	...
	function show_payment_method( $available_gateways ) {
		...
		if ( ! is_admin() ) {
			if ( 'yes' === $this->debug ) {
				$this->log->add( 'redsys', ' ' );
				$this->log->add( 'redsys', '/****************************/' );
				$this->log->add( 'redsys', '   Is NOT admin ' );
				$this->log->add( 'redsys', '/****************************/' );
				$this->log->add( 'redsys', ' ' );
			}
			if ( is_user_logged_in() ) {
				if ( 'yes' === $this->debug ) {
					$this->log->add( 'redsys', ' ' );
					$this->log->add( 'redsys', '/****************************/' );
					$this->log->add( 'redsys', '   Is user logget in ' );
					$this->log->add( 'redsys', '/****************************/' );
					$this->log->add( 'redsys', ' ' );
				}
				$user_id = get_current_user_id();
				...
				if ( 'yes' === $this->debug ) {
					if ( $show ) {
						$this->log->add( 'redsys', ' ' );
						$this->log->add( 'redsys', '/****************************/' );
						$this->log->add( 'redsys', '   SHOW Gateway' );
						$this->log->add( 'redsys', '/****************************/' );
						$this->log->add( 'redsys', ' ' );
					} else {
						$this->log->add( 'redsys', ' ' );
						$this->log->add( 'redsys', '/****************************/' );
						$this->log->add( 'redsys', '   DONT SHOW Gateway' );
						$this->log->add( 'redsys', '/****************************/' );
						$this->log->add( 'redsys', ' ' );
					}
					$this->log->add( 'redsys', ' ' );
