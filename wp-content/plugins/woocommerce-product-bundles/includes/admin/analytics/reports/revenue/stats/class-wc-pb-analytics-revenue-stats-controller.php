...
class WC_PB_Analytics_Revenue_Stats_REST_Controller extends WC_REST_Reports_Controller {
	...
	public function get_items( $request ) {
		...
		$query = new WC_PB_Analytics_Revenue_Stats_Query( $query_args );
		try {
	...
	public function prepare_item_for_response( $report, $request ) {
		 ...
		 */
		return apply_filters( 'woocommerce_rest_prepare_report_bundles_stats', $response, $report, $request );
	}
	...
	public function get_item_schema() {
		$data_values = array(
			...
			'bundled_items_sold'   => array(
				'title'       => __( 'Bundled Items Sold', 'woocommerce' ),
				'description' => __( 'Number of items sold.', 'woocommerce' ),
				'type'        => 'integer',
				'context'     => array( 'view', 'edit' ),
				'readonly'    => true,
	...
	public function set_default_report_data( $results ) {
		if ( empty( $results ) ) {
			...
			$results->totals->bundled_items_sold = 0;
	...
	public function get_collection_params() {
		...
		$params['interval']   = array(
			'description'       => __( 'Time interval to use for buckets in the returned data.', 'woocommerce-product-bundles' ),
			'type'              => 'string',
			'default'           => 'week',
			'enum'              => array(
				'hour',
		...
		$params['segmentby']  = array(
			'description'       => __( 'Segment the response by additional constraint.', 'woocommerce-product-bundles' ),
			'type'              => 'string',
			'enum'              => array(
				'product',
				'category',
		...
		$params['fields']     = array(
			'description'       => __( 'Limit stats fields to the specified items.', 'woocommerce-product-bundles' ),
			'type'              => 'array',
