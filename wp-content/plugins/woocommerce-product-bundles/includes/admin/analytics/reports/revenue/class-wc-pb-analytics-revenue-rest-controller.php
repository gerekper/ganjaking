 ...
 * @version 6.9.0
 */
class WC_PB_Analytics_Revenue_REST_Controller extends WC_REST_Reports_Controller implements ExportableInterface {
	...
	public function get_items( $request ) {
		...
		$reports       = new WC_PB_Analytics_Revenue_Query( $args );
		...
		$schema = array(
			...
			'properties' => array(
				'product_id'    => array(
					...
					'description' => __( 'Product ID.', 'woocommerce-product-bundles' ),
				),
				'items_sold'    => array(
					'type'        => 'integer',
					'readonly'    => true,
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Number of items sold.', 'woocommerce-product-bundles' ),
				),
				'bundled_items_sold'    => array(
					'type'        => 'integer',
					'readonly'    => true,
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Number of bundled items sold.', 'woocommerce-product-bundles' ),
				),
				'net_revenue'   => array(
					'type'        => 'number',
					'readonly'    => true,
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Total Net Sales of all items sold.', 'woocommerce-product-bundles' ),
				),
				'orders_count'  => array(
					'type'        => 'integer',
					'readonly'    => true,
					'context'     => array( 'view', 'edit' ),
					'description' => __( 'Number of orders product appeared in.', 'woocommerce-product-bundles' ),
				),
				'extended_info' => array(
					'name'             => array(
						'type'        => 'string',
						'readonly'    => true,
						'context'     => array( 'view', 'edit' ),
						'description' => __( 'Product name.', 'woocommerce-product-bundles' ),
					),
					'price'            => array(
						'type'        => 'number',
						'readonly'    => true,
						'context'     => array( 'view', 'edit' ),
						'description' => __( 'Product price.', 'woocommerce-product-bundles' ),
					),
					'image'            => array(
						'type'        => 'string',
						'readonly'    => true,
						'context'     => array( 'view', 'edit' ),
						'description' => __( 'Product image.', 'woocommerce-product-bundles' ),
					),
					'permalink'        => array(
						'type'        => 'string',
						'readonly'    => true,
						'context'     => array( 'view', 'edit' ),
						'description' => __( 'Product link.', 'woocommerce-product-bundles' ),
					),
					'stock_status'     => array(
						'type'        => 'string',
						'readonly'    => true,
						'context'     => array( 'view', 'edit' ),
						'description' => __( 'Product inventory status.', 'woocommerce-product-bundles' ),
					),
					'sku'              => array(
						'type'        => 'string',
						'readonly'    => true,
						'context'     => array( 'view', 'edit' ),
						'description' => __( 'Product SKU.', 'woocommerce-product-bundles' ),
					),
				),
			),
		);
	...
	public function get_collection_params() {
		...
		$params[ 'after' ]      = array(
			'description'       => __( 'Limit response to resources published after a given ISO8601 compliant date.', 'woocommerce-product-bundles' ),
			'type'              => 'string',
			'format'            => 'date-time',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params[ 'before' ]     = array(
			'description'       => __( 'Limit response to resources published before a given ISO8601 compliant date.', 'woocommerce-product-bundles' ),
			'type'              => 'string',
			'format'            => 'date-time',
			'validate_callback' => 'rest_validate_request_arg',
		);
		...
		$params[ 'match' ]      = array(
			'description'       => __( 'Indicates whether all the conditions should be true for the resulting set, or if any one of them is sufficient. Match affects the following parameters: status_is, status_is_not, product_includes, product_excludes, coupon_includes, coupon_excludes, customer, categories', 'woocommerce-product-bundles' ),
			'type'              => 'string',
			'default'           => 'all',
			'enum'              => array(
				'all',
	...
	public function get_export_columns() {
		$export_columns = array(
			'product_name'       => __( 'Product Title', 'woocommerce-product-bundles' ),
			'sku'                => __( 'SKU', 'woocommerce-product-bundles' ),
			'items_sold'         => __( 'Items Sold', 'woocommerce-product-bundles' ),
			'bundled_items_sold' => __( 'Bundled Items Sold', 'woocommerce-product-bundles' ),
			'net_revenue'        => __( 'N. Revenue', 'woocommerce-product-bundles' ),
			'orders_count'       => __( 'Orders', 'woocommerce-product-bundles' ),
		);
		...
		/**
		 * `woocommerce_report_bundles_export_columns` filter.
		 *
		 * Filter to add or remove column names from the products report for
		 * export.
		 */
		return apply_filters(
			'woocommerce_report_bundles_export_columns',
			$export_columns
		);
	}
	...
	public function prepare_item_for_export( $item ) {
		$export_item = array(
			...
			'bundled_items_sold' => $item[ 'bundled_items_sold' ],
		...
		/**
		 * `woocommerce_report_bundles_prepare_export_item` filter.
		 *
		 ...
		 */
		return apply_filters(
			'woocommerce_report_bundles_prepare_export_item',
			$export_item,
			$item
		);
	}
