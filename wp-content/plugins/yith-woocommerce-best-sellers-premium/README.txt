== Changelog ==

= 1.1.17 - Released on 29 April 2020 =

* New: support for WooCommerce 4.1
* Update: language files
* Update: plugin framework
* Tweak: improved style in combination with YITH Proteo theme
* Dev: added yith_wcbsl_is_doing_query function

= 1.1.16 - Released on 28 February 2020 =

* New: support for WordPress 5.4
* New: support for WooCommerce 4.0
* New: support for YITH Proteo theme
* Update: language files
* Update: plugin framework
* Tweak: improved style

= 1.1.15 - Released on 23 December 2019 =

* New: support for WooCommerce 3.9
* Update: plugin framework
* Update: language files

= 1.1.14 - Released on 13 November 2019 =

* Update: plugin framework

= 1.1.13 - Released on 30 October 2019 =

* Update: plugin framework

= 1.1.12 - Released on 29 October 2019 =

* New: support for WordPress 5.3
* New: support for WooCommerce 3.8
* New: panel style
* Update: plugin framework

= 1.1.11 - Released on 5 August 2019 =

* New: support to WooCommerce 3.7
* New: enable/disable animation on best-sellers slider
* New: best-sellers slider by category
* Update: plugin framework
* Tweak: improved best-sellers slider

= 1.1.10 - Released on 29 May 2019 =

* Update: plugin framework

= 1.1.9 - Released on 17 April 2019 =

* New: support to WooCommerce 3.6
* Update: plugin framework
* Update: language files
* Fix: widget template overriding

= 1.1.8 - Released on 8 February 2019 =

* New: support to WooCommerce 3.5.4
* Update: plugin framework
* Update: language files

= 1.1.7 - Released on 11 December 2018 =
* New:  support to WordPress 5.0
* Update: plugin framework
* Update: language files

= 1.1.6 - Released on 23 October 2018 =

* Fix: WPML integration
* Update: Plugin Framework

= 1.1.5 - Released on 17 October 2018 =

* New: support to WooCommerce 3.5.x
* New: possibility to display Best Seller Seal only for TOP best sellers
* Update: Italian language
* Update: Plugin Framework

= 1.1.4 - Released on 4 June 2018 =

* New: added '2 Days' and '3 Days' in bestseller update time option
* New: support to WooCommerce 3.4.x
* New: support to WordPress 4.9.6
* Update: Italian language
* Update: Spanish language
* Update: Dutch language
* Update: plugin framework
* Tweak: improved style

= 1.1.3 - Released on 31 January 2018 =

* New: support to WooCommerce 3.3
* New: Dutch language
* Update: Plugin Framework

= 1.1.2 - Released on 9 January 2018 =

* Fix: WPML issues
* Update: Plugin Framework 3
* Dev: added yith_wcbs_remove_best_seller filter
* Dev: added yith_wcbs_get_best_sellers filter

= 1.1.1 - Released on 11 October 2017 =

* New: support to Support to WooCommerce 3.2.0 RC2
* Fix: bestseller image issue

= 1.1.0 - Released on 28 March 2017 =

* New: support to WooCommerce 3.0-RC2
* New: Italian language
* New: Spanish language
* Update: language files
* Fix: show bestseller limit when saving settings in admin
* Fix: issue with PHP 5.4

= 1.0.8 - Released on 21 November 2016 =

* New: choose the number of best sellers shown
* New: display the "Best Seller" badge only for products in general Top list

= Version  1.0.7 - Released: 03 Aug 2016 =

* New: Spanish language

= Version  1.0.6 - Released: 16 Feb 2016 =

* Tweak: fixed minor bugs

= Version  1.0.5 - Released: 31 Dec 2015 =

* New: support to WooCommerce 2.5 BETA 3

= Version  1.0.4 - Released: 17 Nov 2015 =

* New: compatibility with YITH Themes

= Version  1.0.3 - Released: 29 Oct 2015 =

* New: Italian Language
* Fix: minor bug

= Version  1.0.2 - Released: 30 Sep 2015 =

* New: Possibility to choose the number of bestsellers visible in widget
* Tweak: improved transient use

= Version  1.0.1 - Released: 15 Sep 2015 =

* Fix: minor bug

= Version  1.0.0 - Released: 09 Sep 2016=

* Initial release