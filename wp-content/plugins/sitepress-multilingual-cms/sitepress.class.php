	...
	function copy_custom_fields( $post_id_from, $post_id_to ) {
		...
		if ( $custom_fields_from && $custom_fields_to ) {
			...
			wpml_collect( $this->get_custom_fields_translation_settings( WPML_TRANSLATE_CUSTOM_FIELD ) )
				...
				->map( function ( $meta_key ) use ( $post_id_from, $post_id_to ) {
					$this->sync_custom_field( $post_id_from, $post_id_to, $meta_key );
				} );
		}
	...
	function get_ls_languages( $template_args = array() ) {
		...
		foreach ( $w_active_languages as $k => $lang ) {
			...
			if ( is_singular()
				 ...
				 || ( ! empty( $this->wp_query->queried_object_id ) && ( isset( $this->wp_query->query['paged'] ) || isset( $this->wp_query->query['page'] ) ) && $this->wp_query->queried_object_id == get_option( 'page_for_posts' ) )
			) {
