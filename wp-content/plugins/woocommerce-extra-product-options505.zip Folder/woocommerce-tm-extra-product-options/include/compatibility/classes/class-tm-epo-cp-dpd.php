...
final class THEMECOMPLETE_EPO_CP_DPD {
	...
	public function add_compatibility() {
		...
		if ( THEMECOMPLETE_EPO()->tm_epo_dpd_enable == "no" ) {
			...
			if ( version_compare( RP_WCDPD_VERSION, '2.3.4', '>=' ) ) {
				...
				add_filter( 'woocommerce_get_cart_subtotal', array( $this, 'woocommerce_get_cart_subtotal_99999' ), 99999, 2 );
	...
	public function epo_can_show_order_price( $can = TURE, $item_meta = array() ) {
		...
		if ( THEMECOMPLETE_EPO()->tm_epo_dpd_show_option_prices_in_order == "no" && empty( $item_meta[ $this->order_flag ] ) && empty( $item_meta[ '_' . $this->order_flag ] ) ) {
			$can = FALSE;
		}
