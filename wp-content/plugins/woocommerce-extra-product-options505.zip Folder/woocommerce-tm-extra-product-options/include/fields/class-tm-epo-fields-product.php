...
class THEMECOMPLETE_EPO_FIELDS_product extends THEMECOMPLETE_EPO_FIELDS {
	...
	public function display_field( $element = array(), $args = array() ) {
		...
		foreach ( $product_id_array["posts"] as $key => $product_id ) {
			...
			if ( $selected_value == - 1 ) {
				if ( ( THEMECOMPLETE_EPO()->is_quick_view() || ( empty( $this->post_data ) || ( isset( $this->post_data['action'] ) && $this->post_data['action'] === 'wc_epo_get_associated_product_html' ) ) || THEMECOMPLETE_EPO()->tm_epo_global_reset_options_after_add == "yes" ) && isset( $is_default_value ) ) {
					if ( $is_default_value ) {
						$selected = TRUE;
					}
				}
			...
			if ( ! empty( $product ) && is_object( $product ) ) {
				...
				if ( $type === "variable" ) {
					if ( $selected && is_callable( array( $product, 'get_variation_attributes' ) ) ) {
