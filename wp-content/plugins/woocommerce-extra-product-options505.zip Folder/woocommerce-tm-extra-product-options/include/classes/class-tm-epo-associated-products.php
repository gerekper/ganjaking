...
class THEMECOMPLETE_EPO_Associated_Products {
	...
	public function __construct() {
		...
		add_action( 'wc_epo_associated_product_display', array( $this, 'wc_epo_associated_product_display' ), 10, 2 );
	 ...
	 * @since 5.0
	 */
	public function wc_epo_associated_product_display( $product_id, $uniqid ) {

		if ( $product_id ) {
			?>
            <div class="tc-extra-product-options-inline">
			<?php
			...
			THEMECOMPLETE_EPO_DISPLAY()->tm_epo_fields( $product_id, $uniqid );
			THEMECOMPLETE_EPO_DISPLAY()->tm_epo_totals( $product_id, $uniqid );
	...
	public function woocommerce_add_to_cart( $cart_item_key = "", $product_id = "", $quantity = "", $variation_id = "", $variation = "", $cart_item_data = "" ) {
		...
		if ( THEMECOMPLETE_EPO()->cart_edit_key ) {
			...
			$new_data = WC()->cart->cart_contents[ $cart_item_key ];
			$keys     = array(
				"tmhasepo",
				"tmcartepo",
				"tmcartfee",
			...
			if ( isset( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ]['associated_products'] ) && is_array( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ]['associated_products'] ) ) {
				foreach ( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ]['associated_products'] as $key ) {
					unset( WC()->cart->cart_contents[ $key ] );
				}
			}

			// Copy new product data to edited product
			foreach ( $keys as $key ) {
				if ( isset( $new_data[ $key ] ) ) {
					WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ][ $key ] = $new_data[ $key ];
				}
			}

			if ( isset( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ]['associated_products'] ) && is_array( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ]['associated_products'] ) ) {

				// Set the parent of the new associated products to the edited product
				$associated_products = array();
				foreach ( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ]['associated_products'] as $key ) {
					if ( isset( WC()->cart->cart_contents[ $key ] ) ) {
						WC()->cart->cart_contents[ $key ]['associated_parent'] = THEMECOMPLETE_EPO()->cart_edit_key;
						$associated_products[ $key ]                           = WC()->cart->cart_contents[ $key ];
					}
				}

				// Reposition new associated product to be below the edited product
				$start_position = array_search( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ], array_keys( WC()->cart->cart_contents ) );
