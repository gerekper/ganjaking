					...
					if ( is_array( $thisattributes ) ) {
						foreach ( $thisattributes as $thisattribute ) {
							...
							if ( in_array( $type, array( "price", "value", "quantity" ) ) ) {
								switch ( $type ) {
									case 'price':
										$val += $this->calculate_price( $post_data, $thiselement, $thiskey, $thisattribute, $per_product_pricing, $cpf_product_price, $variation_id, $price_default_value, $currency, $current_currency, $_price_per_currencies );
										break;
									case 'value':
	...
	public function tm_check_field_match( $rule = FALSE, $sections = FALSE, $form_prefix = "" ) {
		...
		if ( isset( $this->visible_elements[ $array_prefix ][ $element_uniqueid ] ) ) {
			...
			if ( in_array( $element_uniqueid, $this->current_element_to_check ) ) {
				...
				//return FALSE;
			} else if ( ! $this->is_visible_do( $sections[ $section_id ]['elements'][ $element_id ], $sections[ $section_id ], $sections, $form_prefix ) ) {
				return FALSE;
			}
		}
											...
											if ( THEMECOMPLETE_EPO_WPML()->is_active() ) {
												...
												if ( $woocommerce_wpml && isset( $original_product_id ) && isset( $wpml_is_original_product ) ) {
													...
													foreach ( $woocommerce_wpml_currencies as $currency => $currency_data ) {
														...
														$_current_currency_price      = isset( $thisbuilder[ $current_element . '_price' . $mt_prefix ][ $current_counter ] ) ? $thisbuilder[ $current_element . '_price' . $mt_prefix ][ $current_counter ] : '';
														$_current_currency_sale_price = isset( $thisbuilder[ $current_element . '_sale_price' . $mt_prefix ][ $current_counter ] ) ? $thisbuilder[ $current_element . '_sale_price' . $mt_prefix ][ $current_counter ] : '';
												...
												if ( THEMECOMPLETE_EPO_WPML()->is_active() ) {
													...
													if ( $woocommerce_wpml && isset( $original_product_id ) && isset( $wpml_is_original_product ) ) {
														...
														foreach ( $woocommerce_wpml_currencies as $currency => $currency_data ) {
															...
															$_current_currency_price      = isset( $thisbuilder[ 'multiple_' . $current_element . '_options_price' ][ $current_counter ] )
																? $thisbuilder[ 'multiple_' . $current_element . '_options_price' ][ $current_counter ]
																: '';
															$_current_currency_sale_price = isset( $thisbuilder[ 'multiple_' . $current_element . '_options_sale_price' ][ $current_counter ] )
