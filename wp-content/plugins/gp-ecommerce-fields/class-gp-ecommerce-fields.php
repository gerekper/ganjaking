...
class GP_Ecommerce_Fields extends GWPerk {

	public $version                   = GP_ECOMMERCE_FIELDS_VERSION;
	public $min_gravity_perks_version = '1.2.18';
	public $min_gravity_forms_version = '2.1.3';
	...
	public function init() {
		...
		add_action( 'init', array( $this, 'post_init_cleanup' ), 11 );
		...
		$this->add_tooltip( 'tax_amount',         sprintf( '<h6>%s</h6> %s', __( 'Tax Amount', 'gp-ecommerce-fields' ), __( 'Specify the percentage amount that should be added to the form total as tax.', 'gp-ecommerce-fields' ) ) );
		$this->add_tooltip( 'discount_amount',    sprintf( '<h6>%s</h6> %s', __( 'Discount Amount', 'gp-ecommerce-fields' ), __( 'Specify the amount that should be discounted from the form total.', 'gp-ecommerce-fields' ) ) );
		$this->add_tooltip( 'ecommerce_products', sprintf( '<h6>%s</h6> %s', __( 'Applicable Products', 'gp-ecommerce-fields' ), __( 'Specify which Product fields should be included when calculating the value of this field.', 'gp-ecommerce-fields' ) ) );
	...
	public function field_settings_ui() {
		...
		<li class="ecommerce-amount-setting field_setting" >
			...
			<span class="inline-label discount-label ecommerce-label" style="opacity:0.5;">
                <?php printf( __( 'Supports 10&#37; or %s', 'gp-ecommerce-fields' ), GFCommon::to_money( '10' ) ); ?>
            </span>
	...
	public function register_scripts() {
		...
		// NOTE! gp-ecommerce-fields.js is registered/enqueued via enqueue_form_scripts() due to dynamic dependencies based on form fields.
		$this->register_script( $this->key( 'asm-select' ), $this->get_base_url() . "/js/jquery.asmselect{$min}.js", array( 'jquery' ), $this->version, false );
	...
	public function enqueue_admin_scripts() {
		...
		if( GFCommon::is_form_editor() ) {
			wp_enqueue_script( $this->key( 'asm-select' ) );
		}
	...
	public function enqueue_form_scripts( $form ) {
	    ...
	    if ( ! $this->has_ecommerce_field( $form ) && ! $this->has_ecommerce_merge_tag( $form ) ) {
	        return;
        }

		$deps = array( 'jquery', 'gform_conditional_logic' );
		if ( class_exists( 'GF_Coupon' ) && $this->has_field_types( $form, 'coupon' ) ) {
			$deps[] = 'gform_coupon_script';
		}

		$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG || isset( $_GET['gform_debug'] ) ? '' : '.min';

		wp_enqueue_script( 'gp-ecommerce-fields', $this->get_base_url() . "/js/gp-ecommerce-fields{$min}.js", $deps, $this->version );
	...
	public function add_ecommerce_fields_to_order( $order, $form, $entry ) {
		...
		$this->_order['shipping'] = array( 'name' => '', 'price' => 0 );
	...
	public function get_order_summary_markup( $order, $form, $entry, $order_summary = false, $labels = false, $is_inline = null ) {
	...
	public function get_full_order_summary_markup_html( $order, $form, $entry, $is_inline = null ) {
        ...
        <tr style="<?php $this->style( 'tr.order-label' ); ?>">
            <td colspan="2" class="entry-view-field-name" ><?php echo $labels['order']; ?></td>
        </tr>
        <tr class="lastrow">
            <td class="" colspan="2">
    ...
    public function get_styles( $is_inline = false ) {
	    ...
	    if( $this->_styles === null ) {
		    ...
		    if( $is_inline ) {
			    $this->_styles = array_merge_recursive( array(
	...
	public function get_default_order_summary_items( $order, $form, $entry ) {
		...
		if( $this->has_coupon( $order, $form, $entry ) ) {
			$coupons = gf_coupons()->get_submitted_coupon_codes( $form, $entry );
			foreach( $order['products'] as $key => $product ) {
				if( in_array( (string) $key, $coupons ) ) {
					$items['coupons'][] = array(
						'name'  => sprintf( '
	...
	public function add_merge_tags() {
		...
		<script type="text/javascript">
			...
			jQuery( document ).ready( function( $ ) {
				...
				<?php foreach( $this->merge_tags as $tag ): if( $tag['isCalculable'] ): ?>
		...
		foreach( $form['fields'] as $field ) {
			...
			if( $entry['id'] ) {
				...
				if( empty( $current_fields ) ) {
					...
					} else if( version_compare( $this->get_gravityforms_db_version(), '2.4', '<' ) ) {
	    	...
	    	foreach( explode( ',', $modifiers ) as $modifier ) {
	    		switch( $modifier ) {
				    case 'value':
						...
						$replace = $this->get_order_summary_markup( $this->get_order( $form, $entry, $use_choice_text ), $form, $entry );
				    	break;
			    }
		    }

		    if( ! $replace ) {
	    		$replace = $this->get_order_summary_markup( $this->get_order( $form, $entry ), $form, $entry );
		    }
	...
	public function replace_formula_merge_tags( $formula, $field, $form, $entry, $format = 'number' ) {
		...
		foreach( $order['products'] as $product ) {
			if( rgar( $product, 'isDiscount' ) ) {
				$discounts_total += $product['price'];
			} else if( rgar( $product, 'isCoupon' ) ) {
				$coupons_total += $product['price'];
			}
		}
	...
	public function get_products_total( $order, $products, $include_discounts, $form, $entry ) {
		...
		foreach( $order['products'] as $field_id => $product ) {
			if( in_array( $field_id, $products ) ) {
				...
				$product_total = $this->_get_product_total( array(
	...
	public function remove_field_from_all_fields( $field_value, $merge_tag, $options, $field, $field_label ) {
		...
		if( $merge_tag == 'all_fields' && in_array( $field->get_input_type(), array( 'tax', 'discount', 'subtotal' ) ) ) {
			$field_value = false;
		}
	...
	public function hide_field_from_entry_detail( $content, $field ) {
		if( $field->is_entry_detail() && in_array( $field->get_input_type(), array( 'tax', 'discount', 'subtotal' ) ) ) {
			$content = '';
		}
		return $content;
	}
	...
	public function is_valid_coupon( $key, $form, $entry ) {
		...
		return in_array( (string) $key, $coupons );
	}
    ...
    public function is_ecommerce_field( $field ) {
	    return in_array( $field->get_input_type(), $this->get_ecommerce_field_types() );
    }
...
function gp_ecommerce_fields() {
	return GP_Ecommerce_Fields::get_instance( null );
}