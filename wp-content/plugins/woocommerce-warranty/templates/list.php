...
class Warranty_List_Table extends WP_List_Table {
	...
	public function column_products( $item ) {
		...
		if ( empty( $products ) && ! empty( $item['product_name'] ) ) {
			return $item['product_name'] . '<br/>';
		}
		...
		foreach ( $products as $product ) {
			...
			$out  .= '<a href="' . esc_url( admin_url( 'post.php?post=' . $product['product_id'] . '&action=edit' ) ) . '">' . $title . '</a> &times; ' . $product['quantity'] . '<br/>';
		}
