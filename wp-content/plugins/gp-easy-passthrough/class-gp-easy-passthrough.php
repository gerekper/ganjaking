...
class GP_Easy_Passthrough extends GP_Feed_Plugin {
	...
	*/
	public function init() {
		...
		add_filter( 'gform_entry_post_save', [ $this, 'filter_gform_entry_post_save' ], 9, 1 );
				...
				'fields' => array(
					array(
						...
						'tooltip'  => sprintf(
							...
							esc_html__( 'Select which form you want to populate this form form.', 'gp-easy-passthrough' )
						),
					),
					array(
						'name'       => 'fieldMap',
	...
	public function get_field_map() {
		...
		*/
		foreach ( $source_form['fields'] as $field ) {
			...
			} else if ( ! in_array( $input_type, array( 'fileupload' ) ) ) {
			...
			switch ( $field->get_input_type() ) {
				...
				default:
					...
					if ( is_array( $inputs ) ) {
						...
						// Get field value.
						$value = is_array( rgar( $field_values, $field->id ) ) ? implode( ',', rgar( $field_values, $field->id ) ) : rgar( $field_values, $field->id );
