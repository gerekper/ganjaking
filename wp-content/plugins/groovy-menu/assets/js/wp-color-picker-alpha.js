      ...
      el.iris({
        ...
        change: function(event, ui) {
          if (self.options.alpha) {
            ...
            self.toggler.find('span.color-alpha').addClass('color-alpha--left');
          } else {
      ...
      if (this.options.alpha) {
        ...
        if (target.hasClass('wp-picker-open')) {
          ...
          if (self._color._alpha < 1) {
            controls.strip.attr('style', controls.strip.attr('style').replace(/rgba\(([0-9]+,)(\s+)?([0-9]+,)(\s+)?([0-9]+)(,(\s+)?[0-9\.]+)\)/g, 'rgb($1$3$5)')); // eslint-disable-line no-useless-escape
