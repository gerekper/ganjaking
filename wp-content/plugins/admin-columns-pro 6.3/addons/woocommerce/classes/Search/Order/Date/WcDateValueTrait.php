	...
	public function get_wc_formatted_date_comparison_value( $operator, Value $value ): string {
		...
		switch ( $operator ) {
			...
			case Operators::BETWEEN:
				$value_range = is_array( $value->get_value() ) && count( $value->get_value() ) === 2 ? $value->get_value() : [ 0, 0 ];
			...
			case Operators::WITHIN_DAYS:
				...
				return $date->format( 'U' ) . '...' . $to_date->modify( sprintf( '+%s days', $value->get_value() ) )->format( 'U' );
