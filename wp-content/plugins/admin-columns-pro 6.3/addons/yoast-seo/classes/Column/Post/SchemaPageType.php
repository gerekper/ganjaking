...
class SchemaPageType extends AC\Column\Meta implements ACP\Search\Searchable, ACP\Editing\Editable, ACP\Sorting\Sortable, ACP\ConditionalFormat\Formattable, ACP\Export\Exportable, ACP\Filtering\Filterable {
	...
	public function search(): ACP\Search\Comparison {
		return new ACP\Search\Comparison\Meta\Select( $this->get_meta_key(), AC\MetaType::POST, $this->page_types );
	}

	public function sorting(): ACP\Sorting\AbstractModel {
		$page_types = $this->page_types;
		natcasesort( $page_types );

		return ( new ACP\Sorting\Model\MetaMappingFactory )->create( AC\MetaType::POST, $this->get_meta_key(), array_keys( $page_types ) );
	}
	...
	public function filtering(): ACP\Filtering\Model {
		return new ACP\Filtering\Model\Meta\FormattedOptions( $this, $this->page_types );
	}
