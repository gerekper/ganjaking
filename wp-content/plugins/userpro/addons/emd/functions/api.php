...
class userpro_ed_api {
	...
	function field_value($field, $user_id){
		...
		if(userpro_get_option('date_to_age') == 1 && strtotime($value)){
			return $this->date_to_age($value);
		}
		return $value;
	}
			...
			foreach( $array as $custom_field => $data ) {
				if ( strstr($custom_field, 'emd_') ) {
					...
					<div class="emd-filter">
						...
						<?php } ?>
						<?php if($data['type'] == 'age' && userpro_get_option('date_to_age') == 1){
							?>
							<div class="userpro-input">
								<input type="text" name="emd-<?php echo $custom_field; ?>" id="emd-<?php echo $custom_field; ?>" placeholder="<?php echo __('Example for age filter: 19-25', 'userpro'); ?>" value="<?php echo $this->try_text_value( $custom_field ); ?>" />
							</div>
							<div class="userpro-clear"></div>
						<?php }?>
					</div>
