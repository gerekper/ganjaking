...
function elm_before_global_assets_filter($parentObj, $login_query) {
	...
	if (!is_admin()) {
		...
		if (isset($wp_query->query_vars['elmntr_wrapper_js']) && $wp_query->query_vars['elmntr_wrapper_js'] && $parentObj->is_permalink()) {
			...
			$filepath = ABSPATH . '/' . $wp_query->query_vars['elmntr_wrapper_js'];
			if (file_exists(ABSPATH . '/' . $wp_query->query_vars['elmntr_wrapper_js'])) {
				$is_js = true;
				$filepath = ABSPATH . '/' . $wp_query->query_vars['elmntr_wrapper_js'];
			}
		}
		if (isset($wp_query->query_vars['elmntr_wrapper_css']) && $wp_query->query_vars['elmntr_wrapper_css'] && $parentObj->is_permalink()) {
			...
			if (file_exists(ABSPATH . '/' . $wp_query->query_vars['elmntr_wrapper_css'])) {
				$is_css = true;
				$filepath = ABSPATH . '/' . $wp_query->query_vars['elmntr_wrapper_css'];
			}
		}
...
function elementor_add_rewrite_rules($rules) {
	...
	if (!isset($_GET['elementor-preview'])) {
		...
		$elmntr_rules = PHP_EOL . "#BEGIN - HMWP - Elementor Rules" . PHP_EOL
			...
			. "RewriteRule ^{$path}/(.*) index.php?elmntr_wrapper_js=wp-content/plugins/elementor/assets/$1" . $trust_key . "" . PHP_EOL
			...
			. "RewriteRule ^{$path}/(.*)/etr-(.*).css index.php?elmntr_wrapper_css=wp-content/plugins/elementor/assets/$1/elementor-$2\.css" . $trust_key . PHP_EOL
