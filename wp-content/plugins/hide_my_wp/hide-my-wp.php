...
";
	...
	public static function load_ip_countries_db_table() {
		...
		$allow_clm = $wpdb->get_results("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE `table_name`= '{$hmwp_blocked_ips_table}' AND `column_name`= 'allow'");
		if (!$allow_clm) {
			$wpdb->query("ALTER TABLE `{$hmwp_blocked_ips_table}` ADD `allow` TINYINT(1) NULL DEFAULT '0' AFTER `ip`;");
		}
		$source_clm = $wpdb->get_results("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE `table_name`= '{$hmwp_blocked_ips_table}' AND `column_name`= 'source'");
		if (!$source_clm) {
