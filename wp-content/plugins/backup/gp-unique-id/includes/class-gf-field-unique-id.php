...
class GF_Field_Unique_ID extends GF_Field {
    ...
    public function init() {
	    ...
	    add_action( 'gform_pre_render', array( $this, 'set_field_visibility' ) );
...
//        add_filter( 'gform_pre_render',                 array( $this, 'enable_dynamic_population' ) );
	...
	public function field_default_properties_js() {
        ...
        <script type="text/javascript">
			...
			function SetDefaultValues_<?php echo $this->type; ?>( field ) {
				...
				field['<?php echo gp_unique_id()->key( 'type' ); ?>'] = 'alphanumeric';
				return field;
			}
	...
	public function field_settings_ui() {
        ...
        <li class="uid_setting gwp_field_setting field_setting" xmlns="http://www.w3.org/1999/html">
            ...
            <div>
                <label for="<?php echo gp_unique_id()->key( 'type' ); ?>">
                    <?php _e( 'Type', 'gp-unique-id' ); ?>
                    <?php gform_tooltip( gp_unique_id()->key( 'type' ) ); ?>
                </label>
                <select name="<?php echo gp_unique_id()->key( 'type' ); ?>" id="<?php echo gp_unique_id()->key( 'type' ); ?>"
                        onchange="SetFieldProperty( '<?php echo gp_unique_id()->key( 'type' ); ?>', this.value ); gpui.toggleByType( this.value );">
    ...
    public function advanced_field_settings_ui() {
        ...
        <li class="uid_setting gwp_field_setting field_setting">
            ...
            <div style="padding-bottom:12px;">
                <label for="<?php echo gp_unique_id()->key( 'starting_number' ); ?>">
                    <?php _e( 'Starting Number', 'gp-unique-id' ); ?>
                    <?php gform_tooltip( gp_unique_id()->key( 'starting_number' ) ); ?>
                </label>
                <input type="number" name="<?php echo gp_unique_id()->key( 'starting_number' ); ?>" id="<?php echo gp_unique_id()->key( 'starting_number' ); ?>"
                       onkeyup="SetFieldProperty( '<?php echo gp_unique_id()->key( 'starting_number' ); ?>', this.value );"
                       onchange="SetFieldProperty( '<?php echo gp_unique_id()->key( 'starting_number' ); ?>', this.value );"
                       style="width:75px;" />

                <a href="#" style="margin-left:10px;" onclick="gpui.resetStartingNumber( this )"><?php _e( 'reset', 'gp-unique-id' ); ?></a>
                <?php gform_tooltip( gp_unique_id()->key( 'reset' ) ); ?>

            </div>

            <div style="padding-bottom:12px;">
                <label for="<?php echo gp_unique_id()->key( 'length' ); ?>">
                    <?php _e( 'Length', 'gp-unique-id' ); ?>
                    <?php gform_tooltip( gp_unique_id()->key( 'length' ) ); ?>
                </label>
                <input type="number" name="<?php echo gp_unique_id()->key( 'length' ); ?>" id="<?php echo gp_unique_id()->key( 'length' ); ?>"
            ...
            <div style="padding-bottom:12px;">
                <label for="<?php echo gp_unique_id()->key( 'prefix' ); ?>">
                    <?php _e( 'Prefix', 'gp-unique-id' ); ?>
                    <?php gform_tooltip( gp_unique_id()->key( 'prefix' ) ); ?>
                </label>
                <input type="text" class="merge-tag-support mt-position-right" name="<?php echo gp_unique_id()->key( 'prefix' ); ?>" id="<?php echo gp_unique_id()->key( 'prefix' ); ?>"
                        onkeyup="SetFieldProperty( '<?php echo gp_unique_id()->key( 'prefix' ); ?>', this.value );"
                        onchange="SetFieldProperty( '<?php echo gp_unique_id()->key( 'prefix' ); ?>', this.value );"
                        oninput="SetFieldProperty( '<?php echo gp_unique_id()->key( 'prefix' ); ?>', this.value );"
                        style="width:300px;" />
            </div>

            <div>
                <label for="<?php echo gp_unique_id()->key( 'suffix' ); ?>">
                    <?php _e( 'Suffix', 'gp-unique-id' ); ?>
                    <?php gform_tooltip( gp_unique_id()->key( 'suffix' ) ); ?>
                </label>
                <input type="text" class="merge-tag-support mt-position-right" name="<?php echo gp_unique_id()->key( 'suffix' ); ?>" id="<?php echo gp_unique_id()->key( 'suffix' ); ?>"
                        onkeyup="SetFieldProperty( '<?php echo gp_unique_id()->key( 'suffix' ); ?>', this.value );"
                        onchange="SetFieldProperty( '<?php echo gp_unique_id()->key( 'suffix' ); ?>', this.value );"
                        oninput="SetFieldProperty( '<?php echo gp_unique_id()->key( 'suffix' ); ?>', this.value );"
                        style="width:300px;" />
            </div>
        ...
        <script type='text/javascript'>
            ...
            ( function( $ ) {
                ...
                gpui = {
                    ...
                    key: function( key ) {
                        return '<?php echo gp_unique_id()->key( '' ); ?>' + key;
                    },
