...
namespace SearchWP\Dependencies\RtfHtmlPhp\Html;

use SearchWP\Dependencies\RtfHtmlPhp\Document;
class HtmlFormatter
{
    private $output = '';
    private $encoding;
    ...
    public function __construct($encoding = 'HTML-ENTITIES')
    {
        if ($encoding != 'HTML-ENTITIES') {
            ...
            if (!\extension_loaded('mbstring')) {
                \trigger_error("PHP mbstring extension not enabled, reverting back to HTML-ENTITIES");
                $encoding = 'HTML-ENTITIES';
                // Check if the encoding is reconized by mbstring extension
            } elseif (!\in_array($encoding, \mb_list_encodings())) {
                \trigger_error("Unrecognized Encoding, reverting back to HTML-ENTITIES");
                $encoding = 'HTML-ENTITIES';
            }
        }
    ...
    protected function LoadFont(\SearchWP\Dependencies\RtfHtmlPhp\Group $fontGroup)
    {
        ...
        foreach ($fontGroup->children as $child) {
            ...
            if ($child instanceof \SearchWP\Dependencies\RtfHtmlPhp\Text) {
    ...
    protected function ExtractFontTable($fontTblGrp)
    {
        ...
        foreach ($fontTblGrp->children as $child) {
            ...
            if (!$child instanceof \SearchWP\Dependencies\RtfHtmlPhp\Group) {
                continue;
            }
    ...
    protected function ExtractColorTable($colorTblGrp)
    {
        ...
        for ($i = 1; $i < $c; $i++) {
            ...
            if ($colorTblGrp[$i] instanceof \SearchWP\Dependencies\RtfHtmlPhp\ControlWord) {
                // Extract RGB color and convert it to hex string
                $color = \sprintf(
                    '#%02x%02x%02x',
                    // hex string format
                    $colorTblGrp[$i]->parameter,
                    // red
                    $colorTblGrp[$i + 1]->parameter,
                    // green
                    $colorTblGrp[$i + 2]->parameter
                );
                // blue
                $i += 2;
            } elseif ($colorTblGrp[$i] instanceof \SearchWP\Dependencies\RtfHtmlPhp\Text) {
                // This is a delimiter ';' so
                if ($i != 1) {
                    // Store the already extracted color
                    $colortbl[] = $color;
                } else {
                    // This is the 'auto' color
                    $colortbl[] = 0;
                }
            }
        }
    ...
    protected function ExtractImage($pictGrp)
    {
        ...
        foreach ($pictGrp as $child) {
            ...
            } elseif ($child instanceof \SearchWP\Dependencies\RtfHtmlPhp\Text) {
                // store Data
    ...
    {
        if (!$dest[1] instanceof \SearchWP\Dependencies\RtfHtmlPhp\ControlWord) {
            return;
        }
    ...
    {
        if ($entry instanceof \SearchWP\Dependencies\RtfHtmlPhp\Group) {
            $this->ProcessGroup($entry);
        } elseif ($entry instanceof \SearchWP\Dependencies\RtfHtmlPhp\ControlWord) {
            $this->FormatControlWord($entry);
        } elseif ($entry instanceof \SearchWP\Dependencies\RtfHtmlPhp\ControlSymbol) {
            $this->FormatControlSymbol($entry);
        } elseif ($entry instanceof \SearchWP\Dependencies\RtfHtmlPhp\Text) {
    ...
    protected function FormatControlWord($word)
    {
        switch ($word->word) {
            case 'plain':
            // Reset font formatting properties to default.
            case 'pard':
    ...
    protected function DecodeUnicode($code, $srcEnc = 'UTF-8')
    {
        ...
        if ($this->encoding == 'HTML-ENTITIES') {
            ...
            return $utf8 ? \mb_convert_encoding($utf8, $this->encoding, 'UTF-8') : \mb_convert_encoding("&#{$code};", $this->encoding, 'HTML-ENTITIES');
        }
    }
    protected function Write($txt)
    {
        ...
        if (!$this->state->equals($this->previousState) || $this->state->equals($this->previousState) && !$this->openedTags['span']) {
    ...
    protected function GetEncodingFromCharset($fcharset)
    {
        ...
        $charset = array(
            ...
            // Invalid
            77 => 'MAC',
            //*also [MacRoman]: Macintosh
            128 => 'CP932',
            //*or [Shift_JIS]?: Japanese
            129 => 'CP949',
            //*also [UHC]: Korean (Hangul)
            130 => 'CP1361',
            //*also [JOHAB]: Korean (Johab)
            134 => 'CP936',
            //*or [GB2312]?: Simplified Chinese
            136 => 'CP950',
            //*or [BIG5]?: Traditional Chinese
            161 => 'CP1253',
            // Greek
            162 => 'CP1254',
            // Turkish (latin 5)
