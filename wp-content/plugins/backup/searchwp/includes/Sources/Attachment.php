...
final class Attachment extends Post {
	...
	public function index_rebuild() {
		...
		if ( apply_filters( 'searchwp\source\attachment\attribute\document_content\reset', $default ) ) {
			$wpdb->delete( $wpdb->prefix . 'postmeta', [ 'meta_key' => SEARCHWP_PREFIX . 'content' ] );
		}

		if ( apply_filters( 'searchwp\source\attachment\attribute\pdf_metadata\reset', $default ) ) {
			$wpdb->delete( $wpdb->prefix . 'postmeta', [ 'meta_key' => SEARCHWP_PREFIX . 'content_pdf_metadata' ] );
		}

		if ( apply_filters( 'searchwp\source\attachment\attribute\image_exif\reset', $default ) ) {
			...
			function( \WP_Post $the_post, array $meta ) use ( $existing_content, $post ) {
				...
				if ( empty( trim( $existing_content ) ) ) {
					...
					if ( ! empty( $status->queued ) ) {
						?>
						<p><?php esc_html_e( 'This document is currently in the index queue, but has not yet been processed.', 'searchwp' ); ?></p>
						<?php
