...
class ModelFactory {
  public static function fetch($model, $id) {
    ...
    if(!class_exists($class)) {
      return new WP_Error(sprintf(__('A model for %s wasn\'t found', 'memberpress-courses'), $model));
    }
    ...
    if(isset($obj->ID) && $obj->ID <= 0) {
      return new WP_Error(sprintf(__('There was no %s with an id of %d found', 'memberpress-courses'), $model, $obj->ID));
    }
    else if(isset($obj->id) && $obj->id <= 0) {
      return new WP_Error(sprintf(__('There was no %s with an id of %d found', 'memberpress-courses'), $model, $obj->id));
    }
    else if(isset($obj->term_id) && $obj->term_id <= 0) {
      return new WP_Error(sprintf(__('There was no %s with an id of %d found', 'memberpress-courses'), $model, $obj->term_id));
    }
