...
class WC_Product_Reviews_Pro_Integrations {
	...
	public function __construct() {
		...
		if ( $this->is_jetpack_active() ) {
			$this->jetpack = wc_product_reviews_pro()->load_class( '/src/integrations/jetpack/class-wc-product-reviews-pro-integration-jetpack.php', 'WC_Product_Reviews_Pro_Integration_Jetpack' );
		}

		// Points and Rewards
		if ( $this->is_points_and_rewards_active() ) {
			$this->points_and_rewards = wc_product_reviews_pro()->load_class( '/src/integrations/woocommerce-points-and-rewards/class-wc-product-reviews-pro-integration-points-and-rewards.php', 'WC_Product_Reviews_Pro_Integration_Points_And_Rewards' );
		}

		// Product Vendors
		if ( $this->is_product_vendors_active() ) {
			$this->product_vendors = wc_product_reviews_pro()->load_class( '/src/integrations/woocommerce-product-vendors/class-wc-product-reviews-pro-integration-product-vendors.php', 'WC_Product_Reviews_Pro_Integration_Product_Vendors' );
		}

		// Tab Manager
		if ( $this->is_tab_manager_active() ) {
			$this->tab_manager = wc_product_reviews_pro()->load_class( '/src/integrations/woocommerce-tab-manager/class-wc-product-reviews-pro-integration-tab-manager.php', 'WC_Product_Reviews_Pro_Integration_Tab_Manager' );
		}
	}
