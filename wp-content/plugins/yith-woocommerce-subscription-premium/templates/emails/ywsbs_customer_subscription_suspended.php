...
<?php /* translators: %s: Customer first name */ ?>
	...
	<?php
		...
		printf( esc_html_x( 'Subscription %s has been suspended', 'the placeholder is the subscription number', 'yith-woocommerce-subscription' ), esc_html( $subscription->get_number() ) );
	?>
	</p>
	<p>
		<?php
	...
	<p style="padding:10px 0;"><a class="link"
			style="background-color:#eee;padding:8px 15px;color:#111;text-decoration:none;"
			href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>"><?php esc_html_e( 'pay now', 'yith-woocommerce-subscription' ); ?></a>
	</p>

	<h2><a class="link"
			href="<?php echo esc_url( ywsbs_get_view_subscription_url( $subscription->get_id() ) ); ?>">
			<?php
			// translators: the placeholder is the subscription number.
		...
		(<?php printf( '<time datetime="%s">%s</time>', wp_kses_post( date_i18n( 'c', time() ) ), wp_kses_post( date_i18n( wc_date_format(), time() ) ) ); ?>)</h2>
