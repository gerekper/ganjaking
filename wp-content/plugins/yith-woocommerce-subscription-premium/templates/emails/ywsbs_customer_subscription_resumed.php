...
<?php /* translators: %s: Customer first name */ ?>
	...
	<p>
		...
		printf( esc_html_x( 'Subscription %s has been resumed', 'the placeholder is the subscription number', 'yith-woocommerce-subscription' ), esc_html( $subscription->get_number() ) );
		?>
	</p>

	<h2>
		<a class="link" href="<?php echo esc_url( ywsbs_get_view_subscription_url( $subscription->get_id() ) ); ?>">
			<?php
			// translators: the placeholder is the subscription number.
			...
			?>
		</a>(<?php printf( '<time datetime="%s">%s</time>', esc_html( date_i18n( 'c', time() ) ), esc_html( date_i18n( wc_date_format(), time() ) ) ); ?>)
	</h2>
