...
class evoaup_frontend{
	...
	public function __construct(){
		...
		// woocommerce	
			...
			// checkout
				add_action('woocommerce_checkout_order_processed', array($this, 'alter_orders'), 10, 1);
				add_action('woocommerce_order_status_cancelled', array($this, 'update_order'), 10,1);
				add_action('woocommerce_order_status_refunded', array($this, 'update_order'), 10,1);
		...
		$this->optau = get_option('evcal_options_evoau_1');
	...
	// Scripts and styles
		...
		function scripts(){
			wp_enqueue_script('evoaup_script');				
		}
	...
	// form filtering
		function edit_event_form_args($args, $event_id, $sformat=''){
			...
			$args['hidden_fields']['submission_format'] = !empty($sformat)? $sformat:'regular';
		...
		// before form load send submission level field permissions array
			function field_permissions($permission_array, $editform, $event_id){
				$submission_level = get_post_meta($event_id, '_evoaup_submission_level', true);
				if(empty($submission_level)) return $permission_array;
		...
		// validation before form submission
			function before_form_submission($boolean, $post){
				...
				//$purchase_submissions_data = $this->have_valid_event_submissions($opt['evoaup_product_id']);
		...
		function form_submission($eventid, $formtype){
			if( !empty($_POST['submission_type']) && $_POST['submission_type']=='paid_submission'){
				...
				// regular submission
					...
					if($purchase_submissions_data['count']>0){
						$order_id = $purchase_submissions_data['order_id'];

						// reduce submissions left from order
						$new_count = (int)$purchase_submissions_data['count'] - 1;
		...
		// checking form before loading
			function form_check($boolean, $eid, $_EDITFORM, $atts){
				...
				$this->purchased_data = $this->have_valid_event_submissions($opt['evoaup_product_id']);
		...
		// messages if not qualify for form display
			function before_form(){
				...
				$this->purchased_data = $this->have_valid_event_submissions($product_id);
				...
				<?php if(!is_user_logged_in()):?>
					...
					<p><a class='evcal_btn' href='<?php echo wp_login_url( get_permalink());?>'><?php evo_lang_e('Login to use purchased submissions');?></a></p>
				<?php endif;?>
		...
		function have_valid_event_submissions($wcid){
			...
			if($customer_orders->have_posts()){
				...
				$existing_submission_levels = $this->fnc->get_submission_levels();


				while($customer_orders->have_posts()): $customer_orders->the_post();
					//$order = wc_get_order( $customer_orders->post );
					...
					if(!empty($submission_data) && sizeof($submission_data)>0 ){
						...
						$level_exists = $this->fnc->order_has_submission_levels_from_existing($submission_data);
						...
						// skip orders with no submissions left
						if($submission_count < 1) continue;
						
						$orderID = $customer_orders->post->ID;
				...
				// if there are submission level data that override 
				if($has_levelbased_submissions){
					return array(
						'allcount' =>$total_submission_left,
						'level_data'	=>$submission_order_data,
						'submission_format'		=> 'level_based',
					);
				}else{ // old method
					return array(
						'allcount'	=>$total_submission_left,
						// last available order and submission left info
						'count'		=>$current_submissions_left,	
						'level_data'	=>$submission_order_data,				
						'order_id'	=>$orderID,
						'submission_format'=>'regular'
					);
				}				
			}
		    return false;
	...
	// Woocomerce related functions
		function get_cart_item_meta_values($session_data, $values, $key){
	        ...
	        foreach(array(
	        	'evoaup_level','evoaup_name','evoaup_url'
	        ) as $meta_key){
		...
		function alter_orders($order_id){
		    ...
		    foreach ($items as $order_item_id=>$item) {	
		    	...
		    	$submission_level = wc_get_order_item_meta($order_item_id, '_evoaup_submission_level');
		    	if(!empty($submission_level)){
		    		$submission_level_data = $this->fnc->get_submission_level_data($submission_level);
		    		...
		    		$submission_data[$submission_level] = ((int)$item['qty']) * $submission_level_qty;
		    	}else{
		    		$submission_count += (int)$item['qty'];;
		    	}		    		

		    	// mark that order has been updated with submission data from order
		    	if(!$orderUpdated){
