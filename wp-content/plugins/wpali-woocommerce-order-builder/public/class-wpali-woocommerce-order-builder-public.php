...
class Wpali_Woocommerce_Order_Builder_Public {
	...
	public function enqueue_scripts() {
		...
		if ($this->is_wwob_enabled() == true){
			...
			wp_enqueue_script( 'sticky-sidebar-js', plugin_dir_url( __FILE__ ) . 'js/vendor/sticky-sidebar.js', array( 'jquery' ), $this->version, true );
			...
			<ul id="wwobform_totals_<?php echo $post->ID; ?>" class="side_wwobform_totals wwobform_fields ">
				<li class="wwobfield side-total-price">
					...
					<div class="wwobinput_container">
						<span class="wwob_currency_symbol"><?php echo get_woocommerce_currency_symbol(); ?></span><span class="formattedTotalPrice wwobinput_total"><?php echo number_format($product->get_price(), 2, '.', '') ?></span>
					</div>
