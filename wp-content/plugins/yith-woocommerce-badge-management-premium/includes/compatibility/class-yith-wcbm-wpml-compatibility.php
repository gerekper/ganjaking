...
if ( ! class_exists( 'YITH_WCBM_WPML_Compatibility' ) ) {
	/**
	 * WPML Compatibility Class
	 */
	class YITH_WCBM_WPML_Compatibility {
		 ...
		 *
		 * @param array $badge_options Badge Options.
		 *
		 * @return array
		 */
		public function badges_in_default_language_in_general_settings_panel( $badge_options ) {
			...
			if ( ! $badge_options && yith_wcbm_is_settings_panel() && $sitepress ) {
				...
				if ( isset( $_GET['tab'] ) && 'settings' !== $_GET['tab'] && $default_language !== $current_language ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
