			...
			if ( is_object( $ProductObj ) && check_if_variable_product( $ProductObj ) ) {
				...
				if ( is_shop() || is_product_category() || is_product_tag() ) {                                                                        //Shop and Category Page Message for Variable Product                            
					if ( 'yes' == get_option( 'rs_enable_display_earn_message_for_variation' ) && true == $bool ) {
						if ( '' != $varpointss ) {
							$earnmessage    = ( 'yes' == get_option( 'rs_product_purchase_activated' ) ) ? str_replace( '[variationrewardpoints]' , round_off_type( $varpointss ) , get_option( 'rs_message_for_single_product_variation' ) ) : '' ;
				...
				if ( $getshortcodevalues > 0 ) {
					...
					if ( is_product() ) {                              //Single Product and Custom Page Message for Simple Product
						...
						if ( false == $related_product && true == $bool ) {
							if ( ! $sumo_bookings_check ) {
								if ( '1' == get_option( 'rs_show_hide_message_for_shop_archive_single' ) ) {
			...
			if ( is_user_logged_in() ) {
				...
				if ( is_object( $checkproducttype ) && ( 'simple' == srp_product_type( $post->ID ) || 'subscription' == srp_product_type( $post->ID ) || 'bundle' == srp_product_type( $post->ID ) ) ) {
					...
					if ( $rewardpoints > 0 && ! $sumo_bookings_check ) {
						?>
						<div class="woocommerce-info rs_message_for_single_product"><?php echo wp_kses_post(do_shortcode( get_option( 'rs_message_for_single_product_point_rule' ) ) ); ?></div>
						<?php 
