...
if( ! class_exists( 'FPRewardSystem_Free_Product' ) ) {
    ...
    class FPRewardSystem_Free_Product {
        ...
        public static function send_mail_for_admin( $Points ) {
            ...
            $message = str_replace( array( '[username] , [current_level_name]' , '[product_id]' ) , array( get_option( 'woocommerce_email_from_name' ) , $levelname , $product_ids ) , get_option( 'rs_content_for_free_product_mail_send_admin' , 'Hi,<br/> Your user has got the product as free for reaching the configured level. Please check the below details,<br/> Username: [username]<br/>Product Name: [product_id]<br/>Level Name: [current_level_name].<br/>Thanks' ) ) ;
