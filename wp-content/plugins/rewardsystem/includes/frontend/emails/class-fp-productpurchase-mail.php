...
$site_referral_url = 'yes' == get_option( 'rs_restrict_referral_points_for_same_ip' ) ? esc_url_raw( add_query_arg( array( 'ref' => $UserInfo->user_login , 'ip' => base64_encode( get_referrer_ip_address() ) ) , $referral_url ) ) : esc_url_raw( add_query_arg( array( 'ref' => $UserInfo->user_login ) , $referral_url ) ) ;
...
$message       = ( '1' == $emails->rsmailsendingoptions ) ? str_replace( '{rs_earned_points}' , $PointsInOrder , $message ) : str_replace( '{rs_redeemed_points}' , $PointsInOrder , $message ) ;
