	...
	class RS_Rewardsystem_Shortcodes {
		...
		public static function shortcode_rs_list_enable_options() {
			...
			if ( 'yes' == get_option( 'rs_reward_action_activated' ) ) {
				...
				if ( 'yes' == get_option( 'rs_enable_product_review_points' ) && ''!=rs_get_product_review_reward_points( $post->ID ) ) {
					$ProReviewMsg = str_replace( '[rsreviewpoints]', round_off_type( rs_get_product_review_reward_points( $post->ID ) ), get_option( 'rs_msg_for_product_review' ) ) ;
