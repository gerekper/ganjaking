...
if ( ! class_exists( 'RSBuyingPointsFrontend' ) ) {
	...
	class RSBuyingPointsFrontend {
		...
		public static function msg_for_buying_points() {
			echo wp_kses_post(self::buying_point_msg_for_payment_plan_product() );
			echo wp_kses_post(self::buying_point_msg_for_product()) ;
		}
			...
			foreach ( $BuyingPoint as $ProductId => $Points ) {
				...
				if ( is_initial_payment( $ProductId ) ) {
					...
					if ( 1 == $ShowBuyingPointMsg ) {
						?>
						<div class="woocommerce-info rs_cart_message" ><?php echo wp_kses_post(do_shortcode( $BuyingPointMsg )) ; ?>  </div>
						<?php
					}
				} else {
