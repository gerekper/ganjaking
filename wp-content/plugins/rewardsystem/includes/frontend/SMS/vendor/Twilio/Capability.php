...
class Services_Twilio_Capability {
	...
	public function generateToken( $ttl = 3600) {
		...
		foreach ($this->scopes as $scope) {
			if ('outgoing' == $scope->privilege  && $this->clientName) {
...
class JWT {
	...
	public static function decode( $jwt, $key = null, $verify = true) {
		...
		if ($verify) {
			...
			if (self::sign("$headb64.$payloadb64", $key, $header->alg) != $sig) {
	...
	public static function jsonDecode( $input) {
		...
		if (function_exists('json_last_error') && $errno ) {
			self::handleJsonError($errno);
		} else if (null === $obj && 'null' !== $input) {
