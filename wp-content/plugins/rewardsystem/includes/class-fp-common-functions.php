...
if ( ! function_exists( 'is_payment_product' ) ) {
    ...
    function is_payment_product( $order_id , $product_id ) {
        ...
        if ( _sumo_pp_is_balance_payment_order( $order_id ) && 'yes' === get_post_meta( $order_id , 'is_sumo_pp_order' , 'yes' ) ){
            ...
            return $product_id == get_post_meta( $payment_id , '_product_id' , true ) ;
        }
    ...
    function rs_get_first_purchase_point( $order = false ) {
        ...
        if ( '1' == get_option( 'rs_global_reward_points_type' , 1 ) ) {
            ...
            if ( is_object( $order ) ) {
                ...
                $total        = 'yes' == get_option( 'rs_display_earn_point_tax_based' ) ? WC()->cart->subtotal - $subtotal_tax : WC()->cart->subtotal ;
