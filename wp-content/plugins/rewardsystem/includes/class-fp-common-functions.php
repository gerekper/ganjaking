...
if ( ! function_exists( 'block_points_for_renewal_order' ) ) {
	...
	function block_points_for_renewal_order( $order_id, $enable ) {
		if ( 'yes' == $enable && '' != get_post_meta( $order_id , 'sumo_renewal_order_date' , true ) ) {
			return false ;
		}
...
if ( ! function_exists( 'rs_get_reward_points_based_on_cart_total_for_referrer' ) ) {
	...
	/**
	 * Get cart total based referrer points.
	 *
	 * @return int/float
	 */
...
if ( ! function_exists( 'rs_get_reward_points_based_on_cart_total_for_referred' ) ) {
	...
	/**
	 * Get cart total based referred points.
	 *
	 * @return int/float
	 */
...
if ( ! function_exists( 'get_list_of_modules' ) ) {
	...
	function get_list_of_modules( $value = '' ) {
		return array(
			...
			'fppointprice'         => 'name' == $value ? __( 'Points Price' , 'rewardsystem' ) : get_option( 'rs_point_price_activated' ) ,
	...
	function rs_custom_search_fields( $args ) {
		...
		if ( ( float ) WC_VERSION <= ( float ) ( '2.2' ) ) {
			...
			<select <?php echo esc_attr( $args[ 'multiple' ] ? 'multiple="multiple"' : '' ) ; ?> name="
							   <?php 
								echo esc_attr( '' !== $args[ 'name' ] ? $args[ 'name' ] : $args[ 'id' ]  ) ;if ( $args[ 'multiple' ] ) {
									?>
				[]<?php } ?>" 
...
if ( ! function_exists( 'rs_get_earning_and_redeeming_level_id' ) ) {
	...
	/**
	 * Get Earning and Redeeming level id
	 *
	 * @return int.
	 * */
...
if ( ! function_exists( 'rs_get_manual_referral_link_rule_count' ) ) {
	...
	/**
	 * Get manual referral link count.
	 *
	 * @return int
	 */
...
if ( ! function_exists( 'rs_restrict_product_purchase_point_when_free_shipping_is_enabled' ) ) {
	...
	/**
	 * Restrict product purchase point when free shipping is enabled.
	 *
	 * @return bool
	 */
