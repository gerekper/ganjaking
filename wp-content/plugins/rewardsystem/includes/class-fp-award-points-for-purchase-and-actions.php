...
//                    add_action( 'woocommerce_thankyou' , array( __CLASS__ , 'update_earning_points_for_user' ) , 1 ) ; Commented because product and referral purchase points awarded twice.
        ...
        public static function available_points_for_user() {
            ...
            ?>
            <div class="woocommerce-info <?php echo implode( ' ' , $class_names ) ; ?>">
        ...
        public static function insert_earning_points( $args = array() ) {
            ...
            global $wpdb ;
            $table_name    = $wpdb->prefix . "rspointexpiry" ;
            $earned_points = get_option( 'rs_enable_round_off_type_for_calculation' ) == 'yes' ? round_off_type( $pointstoinsert , array() , false ) : ( float ) $pointstoinsert ;
        ...
        public static function insert_revised_points( $orderuserid , $order_id , $points_data ) {
            ...
            if ( ! empty( $restrictuserpoints ) ) {
                $points_to_revise = $wpdb->get_results( "SELECT SUM(earnedpoints) as earnedpoints FROM $table_name WHERE userid = $orderuserid and orderid = $order_id and expirydate NOT IN(0) and checkpoints IN('MREPFU','PPRP')" , ARRAY_A ) ;
        ...
        public static function insert_revised_points_based_on_carttotal( $orderuserid , $OrderId ) {
            ...
            if ( ! empty( get_option( 'rs_max_earning_points_for_user' ) ) ) {
                $PointsToRevise = $wpdb->get_results( "SELECT SUM(earnedpoints) as earnedpoints FROM $table_name WHERE userid = $orderuserid and orderid = $OrderId and expirydate NOT IN(0) and checkpoints IN('MREPFU','PPRP')" , ARRAY_A ) ;
