...
if ( ! class_exists( 'SRP_Update_Social_Reward_Points' ) ) {

	/**
	 * SRP_Update_Social_Reward_Points Class.
	 */
	class SRP_Update_Social_Reward_Points extends WP_Background_Process {
		...
		protected function complete() {
			...
			if ( srp_check_is_array( $SlicedArray ) ) {
				SRP_Background_Process::callback_to_update_points_for_social_reward( $offset ) ;
				...
				if ( srp_check_is_array( $ProductCat ) ) {
					...
					if ( srp_check_is_array( $CategoryList ) ) {
						foreach ( $CategoryList as $CategoryId ) {
							...
							if ( isset($_REQUEST[ 'enabledisablereward' ]) && '1' == wc_clean(wp_unslash($_REQUEST[ 'enabledisablereward' ])) ) {
		...
		public static function update_product_meta_for_social_reward( $ProductId, $ProductMetaKey ) {
			if ( '1' == get_option( 'fp_enable_reward' ) ) {
