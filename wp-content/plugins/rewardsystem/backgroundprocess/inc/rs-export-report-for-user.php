	...
	class RS_Export_Report_For_User extends WP_Background_Process {
		...
		protected function complete() {
			...
			if ( srp_check_is_array( $SlicedArray ) ) {
				SRP_Background_Process::callback_to_export_report_for_user( $offset ) ;
		...
		public function export_report_for_user( $UserId ) {
			if ( 'no_users' != $UserId  ) {
				...
				if ( '1' == $DateType  ) {
					$earned_points = $wpdb->get_col( $wpdb->prepare( "SELECT SUM(earnedpoints) FROM {$wpdb->prefix}rsrecordpoints WHERE userid = %d" , $UserId ) ) ;
								$redeem_points = $wpdb->get_col( $wpdb->prepare( "SELECT SUM(redeempoints) FROM {$wpdb->prefix}rsrecordpoints WHERE userid = %d" , $UserId ) ) ;
				} else {
					$StartTime     = strtotime( get_option( 'selected_report_start_date' ) 
												. ' ' 
												. '00:00:00' ) ;
					$EndTime       = strtotime( get_option( 'selected_report_end_date' ) 
												. ' ' 
												. '23:59:00' ) ;
										$earned_points = $wpdb->get_col( $wpdb->prepare( "SELECT SUM(earnedpoints) FROM {$wpdb->prefix}rsrecordpoints WHERE userid = %d AND earneddate >= %d AND earneddate <= %d", $UserId , $StartTime , $EndTime ) ) ;
										$redeem_points = $wpdb->get_col( $wpdb->prepare( "SELECT SUM(redeempoints) FROM {$wpdb->prefix}rsrecordpoints WHERE userid = %d AND earneddate >= %d AND earneddate <= %d", $UserId , $StartTime , $EndTime ) ) ;
				}
				...
				if ( srp_check_is_array( $total_user_points ) ) {
					...
					foreach ( $total_user_points as $Id => $Points ) {
						...
						if ( ( '0'  == $EarnPoints ) && ( '0' ==  $RedeemPoints ) && ( '0' == $TotalPoints ) ) {
