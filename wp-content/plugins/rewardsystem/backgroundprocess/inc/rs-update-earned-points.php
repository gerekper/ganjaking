...
if ( ! class_exists( 'RS_Update_Earned_Points' ) ) {
	...
	class RS_Update_Earned_Points extends WP_Background_Process {
		...
		protected function complete() {
			...
			$ids    = $wpdb->get_results( $wpdb->prepare("SELECT DISTINCT ID FROM {$wpdb->users} as p INNER JOIN {$wpdb->usermeta} as p1 ON p.ID=p1.user_id WHERE p1.meta_key = 'rs_expired_points_before_delete' AND p1.meta_value > '0' LIMIT %d,1000", $offset) ) ;
			if ( is_array( $ids ) && ! empty( $ids ) ) {
				SRP_Background_Process::callback_to_update_earned_points( $offset ) ;
			} else {
