...
if ( ! class_exists( 'RS_Bulk_Update_for_Purchase_Points' ) ) {

    /**
     * RS_Bulk_Update_for_Purchase_Points Class.
     */
    class RS_Bulk_Update_for_Purchase_Points extends WP_Background_Process {
        ...
        protected function complete() {
            ...
            if ( srp_check_is_array( $SlicedArray ) ) {
                RS_Main_Function_for_Background_Process::callback_to_update_points_for_product( $offset ) ;
                    ...
                    if ( srp_check_is_array( $CategoryList ) ) {
                        foreach ( $CategoryList as $CategoryId ) {
                            ...
                            if ( (check_if_variable_product( $checkproduct ) || ((srp_product_type( $ProductId ) == 'variable') || (srp_product_type( $ProductId ) == 'variation')) ) ) {
                                ...
                                $enablereward = (get_option( 'fp_enable_reward' ) == 1) ? 'yes' : 'no' ;
                                update_post_meta( $ProductId , '_rewardsystemcheckboxvalue' , $enablereward ) ;

                                $enablereferralreward = (get_option( 'fp_enable_referral_reward' ) == 1) ? 'yes' : 'no' ;
        ...
        public function update_product_meta_for_bulk_update( $ProductId , $ProductLevelMetaKey ) {
            $enablereward = ($ProductLevelMetaKey[ 'enablereward' ] == '_enable_reward_points') ? get_option( 'fp_enable_reward' ) : ((get_option( 'fp_enable_reward' ) == 1) ? 'yes' : 'no') ;
            update_post_meta( $ProductId , $ProductLevelMetaKey[ 'enablereward' ] , $enablereward ) ;

            $enablereferralreward = ($ProductLevelMetaKey[ 'enablereferralreward' ] == '_enable_referral_reward_points') ? get_option( 'fp_enable_referral_reward' ) : (get_option( 'fp_enable_referral_reward' ) == 1 ? 'yes' : 'no') ;
