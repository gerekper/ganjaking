	...
	class RS_Export_Log extends WP_Background_Process {
		...
		protected function complete() {
			...
			if ( srp_check_is_array( $SlicedArray ) ) {
				SRP_Background_Process::callback_to_export_log_for_user( $offset ) ;
		...
		public function export_logs_for_user( $UserId ) {
			if ( 'no_users' != $UserId) {
				...
				$data       = array() ;
				$datas      = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}rspointexpiry WHERE userid = %d" , $UserId ) , ARRAY_A ) ;
