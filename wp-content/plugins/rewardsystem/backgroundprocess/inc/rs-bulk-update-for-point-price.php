...
if ( ! class_exists( 'SRP_Update_Point_Price' ) ) {

	/**
	 * SRP_Update_Point_Price Class.
	 */
	class SRP_Update_Point_Price extends WP_Background_Process {
		...
		protected function complete() {
			...
			if ( srp_check_is_array( $SlicedArray ) ) {
				SRP_Background_Process::callback_to_update_point_price_for_product( $offset ) ;
					...
					if ( srp_check_is_array( $CategoryList ) ) {
						foreach ( $CategoryList as $CategoryId ) {
							...
							if ( ( check_if_variable_product( $checkproduct ) || ( ( 'variable' == srp_product_type( $ProductId ) ) || ( 'variation'  == srp_product_type( $ProductId ) ) ) ) ) {
								...
								$enablepointprice = ( 1 == get_option( 'fp_enable_point_price' ) ) ? 'yes' : 'no' ;
		...
		public function update_product_meta_for_bulk_update( $ProductId, $ProductLevelMetaKey ) {
			$enablepointprice = ( '_enable_reward_points_price' == $ProductLevelMetaKey[ 'enablepointprice' ] ) ? get_option( 'fp_enable_point_price' ) : ( ( 1 == get_option( 'fp_enable_point_price' ) ) ? 'yes' : 'no' ) ;
