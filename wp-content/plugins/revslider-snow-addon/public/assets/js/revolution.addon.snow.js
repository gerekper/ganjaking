...
;var RsSnowAddOn = function($, slider) {
	...
	var staticLayer = slider.find('rs-static-layers');
	
	// inject a static layers div if it doesn't exist
	if(!staticLayer.length) {
		
		var staticDiv = document.createElement('rs-static-layers');
		staticDiv.className = 'rs-snow-container';
		staticDiv.setAttribute('data-key', 'staticlayers');
		slider[0].appendChild(staticDiv);
		
	}
	else {
		
		staticLayer.addClass('rs-snow-container');
		
	}
	
	var options = opt.snow,
