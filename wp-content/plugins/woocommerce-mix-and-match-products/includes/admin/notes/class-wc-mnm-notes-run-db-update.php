...
class WC_MNM_Notes_Run_Db_Update {
	...
	public function __construct() {
		...
		add_action( 'wc_mnm_hide_update_notice', array( __CLASS__, 'set_notice_actioned' ) );
	...
	private static function update_needed_notice( $note_id = null ) {
		...
		$update_url =
			add_query_arg(
				array(
					'wc_mnm_update_action' => 'do_update_db',
				),
		...
		$note_actions = array(
			array(
				'name'         => 'update-db_run',
				'label'        => esc_html__( 'Update Database', 'woocommerce-mix-and-match-products' ),
				'url'          => $update_url,
				'status'       => 'unactioned',
				'primary'      => true,
				'nonce_action' => 'wc_mnm_update_action',
				'nonce_name'   => 'wc_mnm_update_action_nonce',
			),
			array(
				'name'    => 'update-db_learn-more',
				'label'   => esc_html__( 'Learn more about updates', 'woocommerce-mix-and-match-products' ),
		...
		$note->set_title( esc_html__( 'WooCommerce Mix and Match database update required', 'woocommerce-mix-and-match-products' ) );
		$note->set_content(
			esc_html__( 'WooCommerce Mix and Match has been updated! To keep things running smoothly, we have to update your database to the newest version.', 'woocommerce-mix-and-match-products' )
			. sprintf( ' ' . esc_html__( 'The database update process runs in the background and may take a little while, so please be patient.', 'woocommerce-mix-and-match-products' ) )
		);
	...
	private static function update_in_progress_notice( $note_id ) {
		...
		$pending_actions_url = admin_url( 'admin.php?page=wc-status&tab=action-scheduler&s=wc_mnm_run_update&status=pending' );
		$cron_disabled       = Constants::is_true( 'DISABLE_WP_CRON' );
		$cron_cta            = $cron_disabled ? __( 'You can manually run queued updates here.', 'woocommerce-mix-and-match-products' ) : __( 'View progress →', 'woocommerce-mix-and-match-products' );

		$note = new Note( $note_id );
		$note->set_title( __( 'WooCommerce Mix and Match database update in progress', 'woocommerce-mix-and-match-products' ) );
		$note->set_content( __( 'The database is updating in the background. The database update process may take a little while, so please be patient.', 'woocommerce-mix-and-match-products' ) );
	...
	private static function update_done_notice( $note_id ) {
		$hide_notices_url = html_entity_decode( // to convert &amp;s to normal &, otherwise produces invalid link.
			add_query_arg(
				array(
					'wc-mnm-hide-notice' => 'update',
				),
				wc_get_current_admin_url() ? remove_query_arg( 'wc_mnm_update_action', wc_get_current_admin_url() ) : admin_url( 'admin.php?page=wc-settings' )
			)
		);
		...
		$note_actions = array(
			array(
				...
				'status'       => 'actioned',
				'primary'      => true,
				'nonce_action' => 'wc_mnm_hide_notices',
				'nonce_name'   => '_wc_mnm_notice_nonce',
			),
		);
		...
		$note->set_title( __( 'WooCommerce Mix and Match database update done', 'woocommerce-mix-and-match-products' ) );
		$note->set_content( __( 'Database update complete. Thank you for updating to the latest version!', 'woocommerce-mix-and-match-products' ) );
	...
	public static function show_reminder() {
		$needs_db_update = \WC_MNM_Install::needs_db_update();
		...
		if ( ! $needs_db_update ) {
			...
			if ( $next_scheduled_date || ! empty( $_GET['wc_mnm_update_action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
