<?php
	...
	if( !class_exists('Wbcr_FactoryForms438_ListControl') ) {

		class Wbcr_FactoryForms438_ListControl extends Wbcr_FactoryForms438_Control {
				...
				if( $this->isAjax() ) {
					...
					<script>
						window['<?php echo $ajax_id ?>'] = {
							...
							'emptyList': '<?php echo $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_438') ) ?>'
						};
					</script>
					<?php
			...
			protected function checklistHtml()
			{
				...
				$emptyList = $this->getOption('empty', __('The list is empty.', 'wbcr_factory_forms_438'));
