...
if ( ! class_exists( 'YITH_WCWL_Estimate_Email' ) ) {
	...
	class YITH_WCWL_Estimate_Email extends YITH_WCWL_Mail {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_email_recipient_estimate_mail', array( $this, 'send_cc_copy_user' ), 10, 3 );
		}
		 ...
		 * @return string
		 */
		public function send_cc_copy_user( $recipient, $object, $email ) { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.objectFound
			if ( $this->enable_cc ) {
				$recipient = $recipient . ', ' . $email->reply_email;
			}
