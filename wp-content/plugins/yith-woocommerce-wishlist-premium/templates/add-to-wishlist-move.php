 ...
 * @var $label                     string Label for the button
 */
...
<div class="yith-wcwl-add-button">
	<span class="feedback">
		...
		<?php
		/**
		 * APPLY_FILTERS: yith_wcwl_in_your_wishlist_label
		 *
		 ...
		 * @return string
		 */
		echo esc_html( apply_filters( 'yith_wcwl_in_your_wishlist_label', __( 'In your wishlist', 'yith-woocommerce-wishlist' ) ) );
		?>
	</span>
	...
	<?php
	/**
	 * APPLY_FILTERS: yith_wcwl_move_to_another_wishlist_title
	 *
	 * Filter the label to move the item to another wishlist.
	 *
	 * @param string $label Label
	 *
	 * @return string
	...
	<a
		...
		class="move_to_another_wishlist <?php echo esc_attr( $link_classes ); ?> open-pretty-photo"
		data-rel="prettyPhoto[add_to_wishlist_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>]"
		data-title="<?php echo esc_attr( apply_filters( 'yith_wcwl_move_to_another_wishlist_title', __( 'Move to another wishlist', 'yith-woocommerce-list' ) ) ); ?>"
		rel="nofollow"
	>
	...
	<div id="add_to_wishlist_popup_<?php echo esc_attr( $product_id ); ?>_<?php echo esc_attr( $unique_id ); ?>" class="yith-wcwl-popup">
		...
		">
			<div class="yith-wcwl-popup-content">
				 ...
				 *
				 * Filter whether to show the icon in the 'Add to wishlist' popup.
				 *
				...
				<p class="form-row">
					...
					echo wp_kses_post( sprintf( __( 'This item is already in the <b>%s wishlist</b>.<br/>You can move it to another list:', 'yith-woocommerce-wishlist' ), $found_in_list->get_formatted_name() ) );
					?>
				</p>
				...
				<div class="yith-wcwl-second-row">
					<p class="form-row form-row-wide">
						 ...
						 */
						?>
						<input name="wishlist_name" type="text" class="wishlist-name input-text" placeholder="<?php echo esc_attr( apply_filters( 'yith_wcwl_new_list_title_text', __( 'Wishlist name', 'yith-woocommerce-wishlist' ) ) ); ?>"/>
					</p>
					<p class="form-row form-row-wide">
						<label>
			...
			<div class="yith-wcwl-popup-footer">
				<a
					class="wishlist-submit move_to_wishlist <?php echo esc_attr( $link_popup_classes ); ?>"
					data-product-id="<?php echo esc_attr( $product_id ); ?>"
					data-original-product-id="<?php echo esc_attr( $parent_product_id ); ?>"
					data-item-id="<?php echo $found_item ? esc_attr( $found_item->get_id() ) : ''; ?>"
					data-origin-wishlist-id="<?php echo $found_in_list ? esc_attr( $found_in_list->get_token() ) : ''; ?>"
					rel="nofollow"
				>
					<?php
					/**
					 * APPLY_FILTERS: yith_wcwl_move_label
					 *
					 * Filter the button text to move the item to another wishlist.
					 *
					 * @param string $text Button text
					 *
					 * @return string
					 */
					echo wp_kses_post( apply_filters( 'yith_wcwl_move_label', __( 'Move', 'yith-woocommerce-wishlist' ) ) );
					?>
				</a>
				...
				<div class="yith-wcwl-remove-button">
					...
					<a href="#" class="delete_item" data-item-id="<?php echo esc_attr( $found_item->get_id() ); ?>" data-product-id="<?php echo esc_attr( $product_id ); ?>" >
						<?php esc_html_e( 'click to remove it', 'yith-woocommerce-wishlist' ); ?>
					</a>
				</div>
			</div>
