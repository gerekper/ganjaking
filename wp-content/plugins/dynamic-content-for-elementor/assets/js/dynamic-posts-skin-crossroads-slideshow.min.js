...
move(direction,val){return new Promise((resolve,reject)=>{const tx=direction==='left'?'+='+val*-1:'+='+val;const duration=1.2;gsap.timeline({onComplete:resolve}).to(this.DOM.imgWrap,{duration:duration,x:tx,ease:"quart.inOut"},0).to(this.DOM.imgWrap,{duration:duration*0.5,scaleX:1.3,ease:"quart.in"},0).to(this.DOM.imgWrap,{duration:duration*0.5,scaleX:1,ease:"quart.out"},duration*0.5).to(this.DOM.number,{duration:duration,x:tx,ease:"quint.inOut"},0).to(this.DOM.subtitle,{duration:duration*1.1,x:tx,ease:"quart.inOut"},0).to(this.DOM.title,{duration:duration*1.05,x:tx,ease:"quart.inOut"},0)})}
...
gsap.timeline().to(this.centerSlide.DOM.imgWrap,{duration:0.7,ease:"expo.out",scale:1.02}).to(this.centerSlide.DOM.img,{duration:1.7,ease:"expo.out",scale:1.05},0)};this.DOM.interaction.center.addEventListener('mouseenter',this.mouseenterCenterFn);this.mouseleaveCenterFn=()=>{if(this.isAnimating){return}
gsap.timeline().to(this.centerSlide.DOM.imgWrap,{duration:0.7,ease:"expo.out",scale:1}).to(this.centerSlide.DOM.img,{duration:0.7,ease:"expo.out",scale:1},0)};this.DOM.interaction.center.addEventListener('mouseleave',this.mouseleaveCenterFn);this.resizeFn=()=>this.calculateGap();window.addEventListener('resize',this.resizeFn);this.DOM.contentItems.forEach(item=>{item.querySelector('.dce-img-wrap--content').addEventListener('click',()=>this.closeSlide())});document.addEventListener("keyup",(e)=>{if(e.keyCode==27){this.closeSlide()}
...
const revealer=new Revealer(document.querySelector('.dce-revealer__inner'));new Slideshow(document.querySelector('.dce-grid--slideshow'));imagesLoaded(document.querySelectorAll('.dce-img'),{background:!0},()=>document.body.classList.remove('loading'));[...document.querySelectorAll('.frame__mode input[type="radio"]')].forEach(radio=>radio.addEventListener('click',()=>{document.body.classList[radio.parentNode.classList.contains('frame__mode-item--dark')?'add':'remove']('dark-mode')}))};jQuery(window).on('elementor/frontend/init',function(){elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamicposts-v2.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-woo-products-cart.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-woo-products.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-dynamic-show-favorites.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-my-posts.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-sticky-posts.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-search-results.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-metabox-relationship.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler);elementorFrontend.hooks.addAction('frontend/element_ready/dce-acf-relationship.crossroadsslideshow',Widget_DCE_Dynamicposts_crossroadsslideshow_Handler)})