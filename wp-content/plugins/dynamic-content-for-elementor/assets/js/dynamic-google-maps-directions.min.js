...
if("OK"==data.status){try{var mapRequest=await this.elements.google_utility.makeGoogleMap(this.elements.map,this.elements.mapParams);var map=mapRequest.map;var data_to_send={map:map,results:{data_obj:{start:data.results.origin_address,end:data.results.final_address,}}}
this.geoLocalization(map,async(data)=>{if(data.status=="OK"){if("departure"==this.elements.use_geolocation_as){data_to_send.results.data_obj.start=data.geo_position}else if("destination"==this.elements.use_geolocation_as){data_to_send.results.data_obj.end=data.geo_position}
...
geoLocalization(map,callback){if(this.elements.geolocation){var data={map:map,buttons:{geo_button_text:this.elements.geolocation_button_text,id:this.getID()},}
this.elements.google_utility.geoLocalization(data,(data_geo_result)=>{if("OK"==data_geo_result.status){data.map=this.elements.google_utility.resetMap(this.elements.map,this.elements.mapParams)
...
if(this.elements.markers){var leg=result.routes[0].legs[0];this.makeMarker('start',map,leg.start_location,{url:this.elements.customMarkers.start.url,size:[30,30]},this.elements.customMarkers.start.title,this.elements.customMarkers.start.label,this.elements.customMarkers.start.size);this.makeMarker('end',map,leg.end_location,{url:this.elements.customMarkers.end.url,size:[30,30]},this.elements.customMarkers.end.title,this.elements.customMarkers.end.label,this.elements.customMarkers.end.size)}
this.refreshInstruction();this.refreshInfo()}})
return}
makeMarker(point,map,position,urlSizeObj,title,label,size){var icon={url:urlSizeObj.url,anchor:new google.maps.Point(0,0),scaledSize:new google.maps.Size(size,size),}
var marker=new google.maps.Marker({position:position,draggable:!1,map:map,icon:icon,title:title,label:label,animation:google.maps.Animation.DROP,});this.addInfoWindow(point,marker,position,map)}
async addInfoWindow(point,marker,location,map){var infoWindowContent="";if(this.elements.info_window){if("start"==point){infoWindowContent=this.getElementSettings().departure_info_window}else if("end"==point){infoWindowContent=this.getElementSettings().destination_info_window}}else{try{var latlng=new google.maps.LatLng(location.lat(),location.lng());const address=await this.elements.google_utility.findAddressCb(latlng);if("OK"==address.status){infoWindowContent=address.results.formatted_address}}catch(error){console.error(error.message)}}
