...
var ripetitore='repeater_shape_'+forma;eval('var repeaterShape = elementSettings.'+ripetitore);var contentElemsTotal=repeaterShape.length;var numberOfElements=repeaterShape.length;var shapes=[];var dceshape="#shape-"+id_scope;var dceshape_svg="#dce-svg-"+id_scope;if(tl){tl.kill($(dceshape))}
...
return easing_morph};var createTween=function(){if($("#shape-"+id_scope).length){var tweenSVG='tlpos';var tweenString='tl';$.each(shapes,function(i,el){var fill_element='fill:"'+shapes[i].fill.color+'", ';if(enable_image&&(shapes[i].fill.image||pattern_image)){fill_element='';$(dceshape).attr('fill','url(#pattern-'+id_scope+')')}
...
tl.eventCallback("onRepeat",myFunction1,["param1","param2"]);if(repeat_morph!=0){tl.repeat(repeat_morph);tlpos.repeat(repeat_morph)}
