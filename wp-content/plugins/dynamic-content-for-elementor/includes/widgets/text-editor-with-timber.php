...
class TextEditorWithTimber extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    {
        $this->start_controls_section('section_timber', ['label' => __('Text Editor with Timber', 'dynamic-content-for-elementor')]);
        $this->add_control('content', ['label' => '', 'type' => Controls_Manager::WYSIWYG, 'default' => __('Hello', 'dynamic-content-for-elementor') . ' {{ current_user.name }}!', 'description' => esc_html__('If you add custom html tags, you can use the class `dce-force-style` to apply the style as selected in the Style tab. ', 'dynamic-content-for-elementor'), 'dynamic' => ['active' => \true]]);
        ...
        $this->add_control('text_color', ['label' => __('Text Color', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::COLOR, 'default' => '', 'selectors' => ['{{WRAPPER}} .dce-content' => 'color: {{VALUE}};']]);
        ...
        $this->add_responsive_control('text_columns', ['label' => __('Columns', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'separator' => 'before', 'options' => $text_columns, 'selectors' => ['{{WRAPPER}} .dce-content' => 'columns: {{VALUE}};']]);
        $this->add_responsive_control('column_gap', ['label' => __('Columns Gap', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SLIDER, 'size_units' => ['px', '%', 'em', 'vw'], 'range' => ['px' => ['max' => 100], '%' => ['max' => 10, 'step' => 0.1], 'vw' => ['max' => 10, 'step' => 0.1], 'em' => ['max' => 10, 'step' => 0.1]], 'selectors' => ['{{WRAPPER}} .dce-content' => 'column-gap: {{SIZE}}{{UNIT}};']]);
        $this->end_controls_section();
    ...
    protected function safe_render()
    {
        ...
        echo $pdf_html_templates->timber_expand($content, []);
        echo '</div>';
    }
}
