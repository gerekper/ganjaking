 ...
 */
namespace DynamicOOOS\FontLib;

use DynamicOOOS\FontLib\Exception\FontNotFoundException;
/**
 * Generic font file.
 *
 * @package php-font-lib
...
class Font
{
    ...
    public static function load($file)
    {
        ...
        switch ($header) {
            ...
            case "true":
            case "typ1":
                $class = "DynamicOOOS\\TrueType\\File";
                break;
            case "OTTO":
                $class = "DynamicOOOS\\OpenType\\File";
                break;
            case "wOFF":
                $class = "DynamicOOOS\\WOFF\\File";
                break;
            case "ttcf":
                $class = "DynamicOOOS\\TrueType\\Collection";
                break;
            // Unknown type or EOT
            default:
                $magicNumber = \file_get_contents($file, \false, null, 34, 2);
                if ($magicNumber === "LP") {
                    $class = "DynamicOOOS\\EOT\\File";
                }
        }
        if ($class) {
            $class = "FontLib\\{$class}";
