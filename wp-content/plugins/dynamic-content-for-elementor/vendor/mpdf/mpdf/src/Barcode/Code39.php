 ...
 */
class Code39 extends \DynamicOOOS\Mpdf\Barcode\AbstractBarcode implements \DynamicOOOS\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
    ...
    private function init($code, $printRatio, $extended, $checksum)
    {
        ...
        if ($code === \false) {
            throw new \DynamicOOOS\Mpdf\Barcode\BarcodeException(\sprintf('Invalid CODE39 barcode value "%s"', $code));
        }
        if ($checksum) {
            // checksum
