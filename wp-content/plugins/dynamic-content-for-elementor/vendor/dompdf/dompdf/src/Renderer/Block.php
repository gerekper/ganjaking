 ...
 */
namespace DynamicOOOS\Dompdf\Renderer;
...
class Block extends AbstractRenderer
{
    ...
    function render(Frame $frame)
    {
        ...
        if ($node->nodeName === "body") {
            $h = $frame->get_containing_block("h") - (float) $style->length_in_pt(array($style->margin_top, $style->border_top_width, $style->border_bottom_width, $style->margin_bottom), (float) $style->length_in_pt($style->width));
        }
    ...
    protected function _render_border(AbstractFrameDecorator $frame, $border_box = null, $corner_style = "bevel")
    {
        ...
        // Do it the long way
        $widths = array((float) $style->length_in_pt($bp["top"]["width"]), (float) $style->length_in_pt($bp["right"]["width"]), (float) $style->length_in_pt($bp["bottom"]["width"]), (float) $style->length_in_pt($bp["left"]["width"]));
