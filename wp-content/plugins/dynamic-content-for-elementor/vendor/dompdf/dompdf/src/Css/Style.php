...
use DynamicOOOS\Dompdf\FontMetrics;
...
class Style
{
    ...
    // stylesheet this style is attached to
    /**
    ...
    function __set($prop, $val)
    {
        ...
        if ($prop !== "content" && \is_string($val) && \strlen($val) > 5 && \mb_strpos($val, "url") === \false) {
            $val = \mb_strtolower(\trim(\str_replace(array("\n", "\t"), array(" "), $val)));
            $val = \preg_replace("/([0-9]+) (pt|px|pc|em|ex|in|cm|mm|%)/S", "DynamicOOOS\\1\\2", $val);
        }
        $method = "set_{$prop}";
    ...
    function get_font_size()
    {
        ...
        $this->__font_size_calculated = \true;
        return $this->_props["font_size"];
    }
    /**
    ...
    function get_computed_border_radius($w, $h)
    {
        ...
        if ($rTL + $rTR + $rBL + $rBR == 0) {
            return $this->_computed_border_radius = array(0, 0, 0, 0, "top-left" => 0, "top-right" => 0, "bottom-right" => 0, "bottom-left" => 0);
        }
        ...
        return $this->_computed_border_radius = array($rTL, $rTR, $rBR, $rBL, "top-left" => $rTL, "top-right" => $rTR, "bottom-right" => $rBR, "bottom-left" => $rBL);
    }
    /**
    ...
    */
    ...
    protected function _set_style_side_width_important($style, $side, $val)
    {
        if ($side === "bottom") {
            $this->_computed_bottom_spacing = null;
            //reset cache for any bottom width changes
        }
        //see __set and __get, on all assignments clear cache!
    ...
    protected function _set_style($style, $val, $important)
    {
        if (!isset($this->_important_props[$style]) || $important) {
            if ($important) {
                $this->_important_props[$style] = \true;
            }
            //see __set and __get, on all assignments clear cache!
    ...
    protected function _set_border($side, $border_spec, $important)
    {
        ...
        foreach ($arr as $value) {
            ...
            if (\in_array($value, self::$BORDER_STYLES)) {
                ...
                if (\preg_match("/[.0-9]+(?:px|pt|pc|em|ex|%|in|mm|cm)|(?:thin|medium|thick)/", $value)) {
    ...
    function set_outline($val)
    {
        ...
        $props = array("outline_style", "outline_width", "outline_color");
    ...
    function __toString()
    {
        return \print_r(\array_merge(array("parent_font_size" => $this->_parent_font_size), $this->_props), \true);
    }
    /*DEBUGCSS*/
