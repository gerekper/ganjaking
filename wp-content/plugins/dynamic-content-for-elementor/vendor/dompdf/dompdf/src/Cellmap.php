...
use DynamicOOOS\Dompdf\FrameDecorator\Table as TableFrameDecorator;
use DynamicOOOS\Dompdf\FrameDecorator\TableCell as TableCellFrameDecorator;
/**
...
class Cellmap
{
     ...
     * @var array
     */
    protected static $_BORDER_STYLE_SCORE = array("inset" => 1, "groove" => 2, "outset" => 3, "ridge" => 4, "dotted" => 5, "dashed" => 6, "solid" => 7, "double" => 8, "hidden" => 9, "none" => 0);
    /**
    ...
    public function &get_column($i)
    {
        if (!isset($this->_columns[$i])) {
            $this->_columns[$i] = array("x" => 0, "min-width" => 0, "max-width" => 0, "used-width" => null, "absolute" => 0, "percent" => 0, "auto" => \true);
        }
    ...
    public function &get_row($j)
    {
        if (!isset($this->_rows[$j])) {
            $this->_rows[$j] = array("y" => 0, "first-column" => 0, "height" => null);
        }
        return $this->_rows[$j];
    }
    /**
    ...
    public function get_border($i, $j, $h_v, $prop = null)
    {
        if (!isset($this->_borders[$i][$j][$h_v])) {
            $this->_borders[$i][$j][$h_v] = array("width" => 0, "style" => "solid", "color" => "black");
        }
        if (isset($prop)) {
    ...
    {
        return array("top" => $this->get_border($i, $j, "horizontal"), "right" => $this->get_border($i, $j + 1, "vertical"), "bottom" => $this->get_border($i + 1, $j, "horizontal"), "left" => $this->get_border($i, $j, "vertical"));
    }
    /**
     * @param Frame $frame
     *
    ...
    public function add_frame(Frame $frame)
    {
        ...
        if (!$this->_columns_locked) {
            ...
            if ($this->_fixed_layout) {
                list($frame_min, $frame_max) = array(0, 1.0E-9);
            } else {
            ...
            if ($frame_min > $min) {
                ...
                $inc = $this->is_layout_fixed() ? 1.0E-9 : ($frame_min - $min) / $colspan;
                for ($c = 0; $c < $colspan; $c++) {
