...
use DOMXPath;
use DynamicOOOS\Dompdf\FrameDecorator\Page as PageFrameDecorator;
use DynamicOOOS\Dompdf\FrameReflower\Page as PageFrameReflower;
use DynamicOOOS\Dompdf\Positioner\AbstractPositioner;
/**
...
class Factory
{
    ...
    static function decorate_frame(Frame $frame, Dompdf $dompdf, Frame $root = null)
    {
        ...
        $decorator = "\\DynamicOOOS\\Dompdf\\FrameDecorator\\{$decorator}";
        $reflower = "\\DynamicOOOS\\Dompdf\\FrameReflower\\{$reflower}";
    ...
    protected static function getPositionerInstance($type)
    {
        if (!isset(self::$_positioners[$type])) {
            $class = '\\DynamicOOOS\\Dompdf\\Positioner\\' . $type;
