...
use MailPoetVendor\Egulias\EmailValidator\Result\Reason\ExpectingDTEXT;
use MailPoetVendor\Egulias\EmailValidator\Result\Reason\UnusualElements;
use MailPoetVendor\Egulias\EmailValidator\Warning\DomainLiteral as WarningDomainLiteral;
class DomainLiteral extends PartParser
{
 ...
 {
 $matchesIP = array();
 $IPv4Match = \preg_match('/\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/', $addressLiteralIPv4, $matchesIP);
 ...
 {
 $matchesIP = array();
 $IPv4Match = \preg_match('/\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/', $addressLiteral, $matchesIP);
 // Extract IPv4 part from the end of the address-literal (if there is one)
 if ($IPv4Match > 0) {
 ...
 {
 if ($this->lexer->token['type'] === EmailLexer::INVALID || $this->lexer->token['type'] === EmailLexer::C_DEL || $this->lexer->token['type'] === EmailLexer::S_LF || $this->lexer->token['type'] === EmailLexer::S_BACKSLASH) {
