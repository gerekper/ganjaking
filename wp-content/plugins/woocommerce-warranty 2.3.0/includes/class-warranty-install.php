...
class Warranty_Install {
	...
	public function install_notice() {
		...
		if ( 1 === (int) get_option( 'warranty_needs_update' ) ) {
			...
			<div id="message" class="updated">
				<p><strong><?php esc_html_e( 'WC Warranty Data Update Required', 'wc_warranty' ); ?></strong></p>
				<p class="submit"><a href="<?php echo esc_url( add_query_arg( 'warranty_update', 'true', admin_url( 'admin.php?page=warranties' ) ) ); ?>" class="warranty-update-now button-primary"><?php esc_html_e( 'Run the updater', 'wc_warranty' ); ?></a></p>
			</div>
			<script type="text/javascript">
				jQuery( '.warranty-update-now' ).click( 'click', function() {
					var answer = confirm( '<?php esc_html_e( 'It is strongly recommended that you backup your database before proceeding. Are you sure you wish to run the updater now?', 'wc_warranty' ); ?>' );
					return answer;
				} );
			</script>
			<?php
	...
	public function updated_notice() {
		?>
		<div id="message" class="updated">
			<p><?php esc_html_e( 'Data updates have been successfully applied!', 'wc_warranty' ); ?></p>
		</div>
		<?php
	}
	...
	public function install() {
		...
		if ( version_compare( $db_version, WooCommerce_Warranty::$db_version, '<' ) && null !== $db_version ) {
	...
	public function run_updates() {
		...
		if ( version_compare( $db_version, '20221123', '<' ) ) {
			include 'updates/update_20221123.php';
		}
	}
