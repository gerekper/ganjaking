...
class Warranty_Frontend {
	...
	public function display_notices() {
		...
		if ( function_exists( 'wc_add_notice' ) ) {
			...
			$woocommerce->add_message( esc_html( $error_text ) );
		}
	}
		...
		if ( isset( $request_data['req'] ) ) {
			...
			if ( 'new_warranty' === $request ) {
				...
				if ( ! empty( $warranty_key ) && '---' !== $warranty_key ) {
					...
					if ( empty( $check_orders ) ) {
						$back = add_query_arg(
							array(
								'wak'               => $warranty_key,
								'wc_warranty_error' => esc_html__( 'The warranty key is invalid!', 'wc_warranty' ),
							),
							$back_url
						);
				...
				if ( empty( $matched_orders ) ) {
					$back = add_query_arg(
						array(
							'wc_warranty_error' => esc_html__( 'The email or order is not found!', 'wc_warranty' ),
						),
						$back_url
					);
				...
				if ( ! empty( $item_idx ) ) {
					...
					if ( empty( $items[ $item_idx ] ) ) {
						$back = add_query_arg(
							array(
								'wc_warranty_error' => esc_html__( 'The item is not found in the order!', 'wc_warranty' ),
							),
							$back_url
						);
				...
				if ( is_wp_error( $status ) ) {
					...
					$back = add_query_arg(
						array(
							'wc_warranty_success' => esc_html__( 'Return request submitted successfully', 'wc_warranty' ),
						),
						$back_url
					);
