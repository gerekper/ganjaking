	...
	class YITH_WCCL_Admin {
		...
		public function enqueue_scripts() {
			...
			if ( ( ( 'edit-tags.php' == $pagenow || 'edit.php' == $pagenow || 'term.php' == $pagenow ) && isset( $_GET['post_type'] ) && 'product' == $_GET['post_type'] )
				...
				wp_enqueue_script( 'yith-wccl-admin', YITH_WCCL_URL . '/assets/js/yith-wccl-admin'.$min.'.js', array(
					'jquery',
					'wp-color-picker',
					'jquery-ui-dialog',
				), $this->version, true );
		...
		public function product_option_terms( $taxonomy, $i ) {
			...
			<button
				class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'yith-woocommerce-color-label-variations' ); ?></button>
			<button
				class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'yith-woocommerce-color-label-variations' ); ?></button>
			<button class="button fr plus yith_wccl_add_new_attribute"
				data-type_input="<?php echo esc_attr( $taxonomy->attribute_type ); ?>"><?php esc_html_e( 'Add new', 'yith-woocommerce-color-label-variations' ); ?></button>
