...
class RsChartsSliderFront extends RevSliderFunctions {
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['charts'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
	...
	public function write_init_script($slider, $id){
		...
		if(!empty($addOn)){
			...
			echo "\n";
			echo "\t\t\t\t\t\t" . 'if (revapi'.$id.' !== undefined) jQuery.fn.revolution.chartsInit(revapi'.$id.'[0].id);' . "\n";
