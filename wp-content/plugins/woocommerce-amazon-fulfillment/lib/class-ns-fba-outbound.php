...
if ( ! class_exists( 'NS_FBA_Outbound' ) ) {
	...
	class NS_FBA_Outbound {
		 ...
		 */
		public function create_service_outbound() {
			$config  = array(
		...
		public function test_api() {
			...
			$request->setQueryStartDateTime( gmdate( 'c', time() - 1 * HOUR_IN_SECONDS ) );
		 ...
		 */
		public function create_fulfillment_address( $order_id ) {
			// set initial address parameters and a copy to compare converted versus original (raw).
			$shipping_name = trim(
				get_post_meta( $order_id, '_shipping_first_name', true ) .
		 ...
		 *
		 * @param FBAOutboundServiceMWS_Model_Address $address The FBAOutboundServiceMWS_Model_Address model.
		 *
		 * @return string
		 */
		public function create_fulfillment_address_html( FBAOutboundServiceMWS_Model_Address $address ) {
		 ...
		 *
		 * @param int  $order_id The order id.
		 * @param bool $is_manual_send If this is a manual send. Defaults to false.
		 *
		 * @throws Exception When there is an error.
		 *
		 * @return mixed
		 */
		public function send_fulfillment_order( $order_id, $is_manual_send = false ) {
			...
			try {
				...
				foreach ( $order_items as $item ) {
					...
					if ( $is_order_item_fba ) {
						...
						// phpcs:ignore
						// error_log( "send_fulfillment_order SENDING to Amazon the SKU: " . $order_item_sku );
				...
				if ( ! $is_any_order_item_fba ) {
					...
					$request->setDisplayableOrderDateTime( gmdate( 'c', time() ) );
					...
					$request->setDisplayableOrderComment( $comment ? $comment : 'Thank you for your order!' );
					...
					$dom->preserveWhiteSpace = false; // phpcs:ignore WordPress.NamingConventions.ValidVariableName
					$dom->formatOutput       = true; // phpcs:ignore WordPress.NamingConventions.ValidVariableName
					...
					if ( $this->ns_fba->utils->isset_on( ( $this->ns_fba->options['ns_fba_update_inventory'] ) ) ) {
						...
						foreach ( $fba_skus as $fba_sku ) {
							...
							if ( '' === $inventory['message'] ) {
								...
								array_push(
									$inventory_updates,
									'Error updating ' . $fba_sku . ' stock to ' . $inventory['number'] .
		...
		/**
		 * Send fulfilment order.
		 *
		 * @param int $order_id The order id.
		 */
		public function maybe_send_fulfillment_order( $order_id ) {
		...
		/**
		 * Get fulfilment order shipping info.
		 *
		 * @param int $order_id The order id.
		 *
		 * @return string
			...
			foreach ( $orders as $f_order ) {
				...
				try {
					...
					foreach ( $statuses as $status ) {
						$fullfillment_status = $status->nodeValue; // phpcs:ignore WordPress.NamingConventions.ValidVariableName
						echo esc_html( $order_number . ' ' . $f_order->post_type . ' ' . $f_order->post_status . ' FBA status: ' . $fullfillment_status . '<br />' );
					}
		 ...
		 */
		public function set_order_status_safe( $order_id, $status ) {
			$order  = wc_get_order( $order_id );
