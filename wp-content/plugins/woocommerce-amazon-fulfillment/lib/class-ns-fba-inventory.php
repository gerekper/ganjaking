...
if ( ! class_exists( 'NS_FBA_Inventory' ) ) {
	...
	class NS_FBA_Inventory {
		 ...
		 */
		public function create_service_inventory() {
			$config  = array(
		 ...
		 */
		public function get_sku_inventory( $service, $sku ) {
			...
			try {
				...
				if ( $response->isSetListInventorySupplyResult() ) {
					$list_inventory_supply_result = $response->getListInventorySupplyResult();
					if ( $list_inventory_supply_result->isSetInventorySupplyList() ) {
						$inventory_supply_list = $list_inventory_supply_result->getInventorySupplyList();
						$member_list           = $inventory_supply_list->getmember();
		 ...
		 */
		public function sync_all_inventory( $next = '' ) {
			...
			error_log(
				"---------------------------------------------------------------------<br />\n" .
				'Inventory Sync Running on ' . gmdate( 'c', time() ) . "<br />\n" .
				"---------------------------------------------------------------------<br />\n",
				3,
			...
			if ( $inventory['number'] > 0 && '' === $inventory['message'] ) {
				...
				error_log(
					( $i++ ) . ': <span style="color:red;"> >> Error updating stock' .
		 ...
		 *
		 * @param object $service The service.
		 * @param string $next    Optional next value.
		 *
		 * @return  array   number = -1 for error otherwise number in inventory
		 *                  message = '' if success or error message if fail
		 */
		public function get_updated_inventory( $service, $next = '' ) {
			// setup global to support WPML with multiple translated IDs per product.
			// only if WPML is detected and Polylang does not trigger a false positive.
			if ( function_exists( 'icl_object_id' ) && ! defined( 'POLYLANG_BASENAME' ) ) {
				global $sitepress;
			...
			if ( '' === $next ) {
				...
				// ISO 8601 date.
				$request->setQueryStartDateTime( gmdate( 'c', strtotime( gmdate( 'Y-m-d' ) . $timeframe ) ) );
			...
			try {
				...
				if ( $results_available ) {
					if ( '' === $next ) {
						...
						$list_inventory_supply_result = $response->getListInventorySupplyByNextTokenResult();
					}
					if ( $list_inventory_supply_result->isSetInventorySupplyList() ) {
						$inventory_supply_list = $list_inventory_supply_result->getInventorySupplyList();
						$member_list           = $inventory_supply_list->getmember();
					...
					if ( $list_inventory_supply_result->isSetNextToken() ) {
						$inventory['next'] = $list_inventory_supply_result->getNextToken();
					} else {
						$inventory['next'] = '';
					}
