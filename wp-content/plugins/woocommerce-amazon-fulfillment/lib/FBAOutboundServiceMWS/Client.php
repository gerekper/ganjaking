...
class FBAOutboundServiceMWS_Client implements FBAOutboundServiceMWS_Interface
{
	...
	private function debug_log( $message ) {
		if ( $this->_config['IsDebugLogging'] ) {
			error_log( $message );
		}
	}
    ...
    private function _convertCancelFulfillmentOrder($request) {
        ...
        // Modified by Never Settle (AL) because of a bug in original Lib - there is no 'Marketplace' parameter in MWS
	    // therefore always map to the MarketplaceId supported parameter defined by MWS to prevent signature issues
        if ($request->isSetMarketplace()) {
            $parameters['MarketplaceId'] =  $request->getMarketplace();
        }
     ...
     * </ul>
     */
    public function __construct( $awsAccessKeyId, $awsSecretAccessKey, $config,
   ...
   private function collapseWhitespace($s) {
    ...
    private function _httpPost(array $parameters)
    {
	    ...
	    // Dump raw request to log if config debugging ON
	    $host = $url['host'];
        $agent = $this->_config['UserAgent'];
	    $queryStr = str_replace( "&", "\n\t\t\t\t&", $query );
	    $allHeadersStrStr = print_r( $allHeadersStr, true );
	    $this->debug_log ("Inside _httpPost:
	        URL: $scheme $host $uri : $port
	        AGENT: $agent
	        POSTF: $queryStr
	        HEADR: $allHeadersStrStr
	    ");
    ...
    */
    ...
    private function _addRequiredParameters(array $parameters)
    {
	    ...
	    // -----------------------------------------------------------------------------------------
	    // 3/1/2019 modified by Never Settle (AL) to offload signature processing for new auth model
	    // See wp-content/plugins/woocommerce-amazon-fulfillment/lib/class-ns-fba-utils.php LINE: 95
	    // for where we define USE_SIGNATURE_API as the value for the Secret Key WHEN
	    // the MWS Auth Token is present and we need to be in Signature API mode
	    // -----------------------------------------------------------------------------------------
	    // This provides support for legacy configurations for those who have their own Secret Key
	    if ( 'USE_SIGNATURE_API' == $this->_awsSecretAccessKey ) {
		    $parameters['Signature'] = $this->_signParametersAPI($parameters);
	    } else {
		    $parameters['Signature'] = $this->_signParameters($parameters, $this->_awsSecretAccessKey);
	    }
	
//	    $this->debug_log( '_awsAccessKeyID = ' . $this->_awsAccessKeyId );
//	    $this->debug_log( '_awsSecretAccessKey = ' . $this->_awsSecretAccessKey );
//	    $this->debug_log( 'MWS Auth Token = ' . NS_MWS_AUTH_TOKEN );
//	    $this->debug_log( "PARAMETERS: \n" . print_r ( $parameters, true ));
    ...
    /**
     * Get signature from NS FBAsig API endpoint
     *
     * @param $parameters
     *
     * @return bool
    ...
    {
        $parameters['NSFBAHost']       = get_site_url();
        $parameters['SignRequestType'] = 'outbound_service';
        $parameters['AmazonRegionUrl'] = $this->_config['ServiceURL'];
        $response                      = wp_remote_post(
            'https://fba.atouchpoint.com/api/calculate-mws-signature',
            array(
                'method'  => 'POST',
                'timeout' => 45,
	    ...
	    if ( ! empty( $response ) && ! is_wp_error( $response ) &&
	         is_array( $response ) && isset( $response['body'] )) {
		    $this->debug_log('Sign Parameters Outbound Service API START LOG: ');
		    $this->debug_log("PARAMETERS: \n" . print_r($parameters, true));
		    $response_body = json_decode($response['body']);
		    $this->debug_log("String to sign: \n\t" . str_replace( '&', "\n\t\t&",
				    str_replace("\n", "\n\t", $response_body->log_data->string_to_sign)));
		    $this->debug_log('Signature: ' . $response_body->signature);
		    $this->debug_log('Sign Parameters Outbound Service API END LOG.');
		    $response_body = json_decode($response['body']);
		    if ($response_body->success == true) {
			    return json_decode($response['body'])->signature;
		    } elseif ($response_body->success == false) {
			    $this->debug_log('API errors: \n' . print_r($response_body->errors, true));
			    throw new \FBAOutboundServiceMWS_Exception(array(
				    'ErrorCode' => 'GetMWSSignError',
				    'Message'   => reset($response_body->errors)[0]
			    ));
		    }
	    } else {
		    $msg = 'There is no connection with MWS Auth API.';
		    if ( is_wp_error( $response )) {
			    ...
			    if ( '7.34.0' > $curl_version ) {
			    	$msg .= ' The cURL version on your host server is: ' . $curl_version;
				    $msg .= ' which is older than version 7.34.0. Your connection to Amazon will not work, ';
				    $msg .= ' because your host is running an out of date and less secure version of cURL. ';
				    $msg .= ' You will need to ask your host to update cURL to at least 7.34.0 so that it ';
				    $msg .= ' will support TLS1.2 for secure communication with Amazon.';
			    } else {
				    $msg .= ' WP_ERROR: ' . $response->get_error_message();
			    }
		    }
		    $this->debug_log( 'No response from API. ' . $msg );
		    throw new \FBAOutboundServiceMWS_Exception(array(
			    'ErrorCode' => 'GetMWSSignError',
			    'Message'   => $msg
		    ));
    ...
    private function _signParameters(array $parameters, $key) {
        ...
        $this->debug_log( 'Sign Parameters Outbound Service LIB START LOG: ' );
        $this->debug_log( "PARAMETERS: \n" . print_r ( $parameters, true ));
	    $this->debug_log("String to sign: \n\t" . str_replace( '&', "\n\t\t&",
			    str_replace("\n", "\n\t", $stringToSign)));
        $this->debug_log( 'Signature: ' . $this->_sign($stringToSign, $key, $algorithm));
        $this->debug_log( 'Sign Parameters Outbound Service LIB END LOG.' );
