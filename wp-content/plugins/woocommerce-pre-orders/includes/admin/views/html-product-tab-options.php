...
<div id="wc_pre_orders_data" class="panel woocommerce_options_panel">
	<div class="options_group">
		...
		if ( $has_active_pre_orders ) {
			...
			echo '<p><strong>' . sprintf( esc_html__( 'There are active pre-orders for this product. To change the release date, %1$suse the Actions menu%2$s.', 'woocommerce-pre-orders' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc_pre_orders&tab=actions&section=change-date&action_default_product=' . $post->ID ) ) . '">', '</a>' ) . '</strong></p>';

			// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
			echo '<p><strong>' . sprintf( esc_html__( 'To change other settings, please %1$scomplete or cancel the active pre-orders%2$s first.', 'woocommerce-pre-orders' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc_pre_orders' ) ) . '">', '</a>' ) . '</strong></p>';
		}
		...
		<p class="form-field _wc_pre_orders_availability_datetime_field ">
			<label for="_wc_pre_orders_availability_datetime"><?php esc_html_e( 'Availability date/time', 'woocommerce-pre-orders' ); ?></label>
			...
			<img class="help_tip" data-tip="<?php echo wc_sanitize_tooltip( sprintf( __( 'Set the date & time that this pre-order will be available. The product will behave as a normal product when this date/time is reached.', 'woocommerce-pre-orders' ) ) ); ?>" src="<?php echo esc_url( $woocommerce->plugin_url() . '/assets/images/help.png' ); ?>" height="16" width="16" />
		</p>
		<?php
