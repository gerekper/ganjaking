 ...
 *
 * Override this template by copying it to [your theme]/woocommerce/yith-pdf-invoice/credit-note-details-with-products.php
 *
...
$order           = $document->order; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			...
			<tr>
				...
				<?php if ( ywpi_is_enabled_column_picture_credit_notes( $document ) ) : ?>
					<td class="column-picture">
						<?php
						/**
						 * APPLY_FILTERS: yith_ywpi_image_path
						 *
						 * Filter the product image path in the document.
						 *
						 * @param string $image_path the product image path.
			...
			<tr>
				...
				<td class="column-product">
					...
					if ( ywpi_is_enabled_column_product_name_credit_notes( $document ) ) :
						...
						$item_name = apply_filters( 'woocommerce_order_product_title', $item_name, $item );
					...
					<?php do_action( 'yith_ywpi_column_product_after_content', $document, $_product, $item_id, $item ); ?>
				</td>
				<td class="column-refund-text">
