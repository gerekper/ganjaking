...
$settings_escaped = $responsive = $custom_dots = $arr_style = $wrap_data = $design_style = '';
...
if ( $autoplay_speed ) {
	$settings_escaped['autoplaySpeed'] = absint( $autoplay_speed );
}
if ( $speed ) {
	$settings_escaped['speed'] = absint( $speed );
}
...
$settings_escaped['responsive'] = array(
	array(
		'breakpoint' => 1025,
		'settings'   => array(
...
if ( 'off' !== $dots_icon && '' !== $dots_icon ) {
	...
	$settings_escaped['customPaging'] = 'function(slider, i) { return \'<i type="button"' . $custom_dots . ' class="' . esc_attr( $dots_icon ) . '" data-role="none"></i>\'; }';
}

if ( function_exists( 'vc_is_inline' ) && vc_is_inline() ) {
	$wrap_data .= ' data-plugin-options="' . esc_attr( json_encode( $settings_escaped ) ) . '"';
}
...
if ( ! function_exists( 'vc_is_inline' ) || ! vc_is_inline() ) :
	...
	<script>
		jQuery(document).ready(function ($) {
			if ($.fn.slick) {
				$('.porto-carousel-<?php echo $uid_escaped; ?>').slick(<?php echo str_replace( array( '\"', '"', '$$', '\/' ), array( '$$', '', '"', '/' ), json_encode( $settings_escaped ) ); ?>);
			} else {
				...
				setTimeout(function() {
					if ($.fn.slick) { $('.porto-carousel-<?php echo $uid_escaped; ?>').slick(<?php echo str_replace( array( '\"', '"', '$$', '\/' ), array( '$$', '', '"', '/' ), json_encode( $settings_escaped ) ); ?>); }
				}, 300);
			}
		});
	</script>
