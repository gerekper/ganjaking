...
export default class PortoSpExcerpt extends portoComponent.shortcodeComponent {
	componentDidMount() {
		super.updateShortcodeToHtml( this.getShortcode(), this.refs.vcvhelper )
	}
	getShortcode() {
		return '[porto_single_product_excerpt]'
	}
	render() {
		const { id, atts, editor } = this.props
		let { spAlign, el_class } = atts // destructuring assignment for attributes from settings.json with access publc
		let wrappr_cls = "vc-sp-excerpt"
		wrappr_cls = wrappr_cls.concat( ` single-excerpt-${ id }` )
