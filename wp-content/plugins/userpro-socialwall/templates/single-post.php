<?php
/*
 * START - this code is commented to fix the issue for displaying followers post only.
	$following=0;
 ...
 * END - this code is commented to fix the issue for displaying followers post only.
 */

$user_info=array();
$user_info = get_userdata($post->post_author);

if(empty($args['role']) || $args['role']==implode(', ', $user_info->roles))
{
	global $userpro;
...
<i onclick="userwall_postlikecount_post(<?php echo $post->ID;?>,<?php echo get_current_user_id()?>,<?php if(empty($like_post)) echo 0; else echo  count($like_post);?>,<?php if (empty($dislike_post)) echo 0; else echo count($dislike_post);?>,<?php if(empty($timestamp)) echo ''; else echo  $timestamp;?>);" class="userpro-icon-thumbsup socialwall_postlikecount_post btn-like-dislike socialwall_postlikecount_post_first"></i> <i class="socialwall_postlikecount_post "><?php if(empty($like_post)) echo 0; else echo  count($like_post);?></i>
...
<div class="userwall_postlikecount_post" id=userwall_postlikecount_post<?php echo $post->ID?>>
<?php
echo '<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post userpro-icon-thumbsup socialwall_postlikecount_post_first"></i>'; ?>
