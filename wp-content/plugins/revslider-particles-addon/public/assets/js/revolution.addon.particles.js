...
(function () {
	...
	window.RsParticlesAddOn = function (slider) {
		...
		slides.each(function () {
			...
			if (data) {
				...
				data = convertOptions(data,slider[0].id);				
		...
		slides.each(function () {
			...
			if (shape.type === 'image') {
				...
				shape.image.src = shape.image.src.split('|__|');
				...
				for (i = 0; i < len; i++) {
					...
					options.particles.shape.sizes =options.particles.shape.sizes===undefined ? [] : options.particles.shape.sizes;
					if (svg.indexOf('path')>=0) {	
						
						var viewbox = svg.split('::'),
							dims=viewbox[1].split(" ");	
						options.particles.shape.sizes[i] = { width:parseInt(dims[0],0), height:parseInt(dims[1],0)};
						
						svg = '<svg xmlns="http://www.w3.org/2000/svg" data-custom="true" width="' + dims[0] + '" height="' + dims[1] + '" viewBox="0 0 '+viewbox[1]+'">' + viewbox[0] + '</svg>';						
						...
						if (!bWidth) {						
							...
							options.particles.shape.sizes[i] = { width:view, height:view};
							...
							options.particles.shape.sizes[i] = { width:size, height:size};
						}
		...
		slider.one('revolution.slide.onchange', function (e, data) {
			...
			if (opt.onBeforeSwap !== undefined) {
				onAfterSwap.call(this, null, opt.onBeforeSwap)
			}
			...
			slider.on('leftviewport', leftViewport)
				.on('enterviewport', enterViewport);
		});

		function leftViewport() {
			slides.each(function () {
				var slide = jQuery(this);
				var pjs = slide.data('pjs');
		...
		function enterViewport() {
			slides.each(function () {
				...
				if (pjs) {
					pjs.instance.pJS.fn.vendors.play();
				}
			});
		}

		function carouselChange(e, data) {
			var curSlide = data.slide[0];
			slides.each(function () {
				var slide = jQuery(this);
				var pjs = slide.data('pjs');
				...
				if (curSlide === slide[0]) {
					...
					if (!pjs && data) {
						var startDrawingParticles = false;
						data = jQuery.extend(true, {}, data);

						if ((opt.inviewport === !undefined && opt.inviewport === true) || opt.inviewport === undefined) {
							startDrawingParticles = true;
						} else {
							startDrawingParticles = false;
						}
						var zIndex = slide.attr('data-particlesindex'),
							pjs = particlesJSRs(slide, data, slide.attr('data-particlesid'), slider, zIndex, startDrawingParticles);

						slide.data('pjs', pjs);
						punchgs.TweenLite.to(pjs.el, 0.5, {
							opacity: 1,
		...
		function onBeforeSwap(e, data) {
			...
			if (pjs) {
				...
				punchgs.TweenLite.to(pjs.el, 0.3, {
					opacity: 0,
					ease: punchgs.Linear.easeNone,
					onComplete: function () {
	...
	function convertSvgs(val) {
		...
		if (val.indexOf(':RSV6.3.0+:')>=0) {			
			val = val.split('|__|');
			val[0] = val[0].replace(':RSV6.3.0+:','');
		} else {			
			val = val.split(',');
		}
	...
	function convertOptions(data,id) {
		...
		return {
			id:id,
			responsive:data.responsive=="true" || data.responsive==true ? true : false,
			zIndex: data.styles.particle.zIndex,
			particles: {
				number: {
					value: minMax(parseInt(data.particles.number, 10), 1, 500)
				},
				color: {
					value: data.styles.particle.color,
					ocolor : data.styles.particle.ocolor
				},
				shape: {
					type: 'image',
					stroke: {
						width: parseInt(borderSize, 10),
						color: data.styles.border.color,
						opacity: minMax(convertOpacity(data.styles.border.opacity), 0, 1)
					},
					image: {
						src: convertSvgs(data.particles.shape)
					}
				},
				opacity: {
					...
					anim: {
						enable: trueFalse(data.pulse.opacity.enable),
						speed: parseFloat(data.pulse.opacity.speed),
						opacity_min: minMax(convertOpacity(data.pulse.opacity.min), 0, 1),
						sync: trueFalse(data.pulse.opacity.sync)
					}
				},
				size: {
					...
					anim: {
						enable: trueFalse(data.pulse.size.enable),
						speed: parseFloat(data.pulse.size.speed),
						size_min: minMax(parseFloat(data.pulse.size.min), 0.1, 250),
						sync: trueFalse(data.pulse.size.sync)
					}
				},
				line_linked: {
					enable: trueFalse(data.styles.lines.enable),
					distance: parseInt(data.styles.lines.distance, 10),
					color: data.styles.lines.color,
					opacity: minMax(convertOpacity(data.styles.lines.opacity), 0, 1),
					width: parseInt(data.styles.lines.width, 10)
				},
				move: {
					enable: trueFalse(data.movement.enable),
					speed: minMax(parseInt(data.movement.speed, 10), 1, 50),
					direction: direction,
					random: randomMove,
					min_speed: minMax(parseInt(data.movement.speedMin, 10), 1, 50),
			...
			interactivity: {
				events: {
					onhover: {
						enable: onHover,
						mode: data.interactivity.hoverMode
					},
					onclick: {
						enable: onClick,
						mode: data.interactivity.clickMode
					}
				},
				modes: {
					grab: {
						distance: parseFloat(data.grab.distance),
						line_linked: {
							opacity: minMax(convertOpacity(data.grab.opacity), 0.1, 1)
						}
					},
					bubble: {
						distance: parseFloat(data.bubble.distance),
						size: parseFloat(data.bubble.size),
						opacity: minMax(convertOpacity(data.bubble.opacity), 0, 1)
					},
					repulse: {
						distance: parseFloat(data.repulse.distance),
						easing: parseInt(data.repulse.easing, 10)
					}
				}
			}
		};
	...
	var pJS = function (canvas_el, params, slider, revapi, startDrawingParticles) {		
		...
		pJS.fn.retinaInit = function () {
			...
			pJS.particles.line_linked.distanceSquare = pJS.particles.line_linked.distance * pJS.particles.line_linked.distance;
			pJS.interactivity.modes.grab.distance = pJS.tmp.obj.mode_grab_distance * pJS.canvas.pxratio;
			pJS.interactivity.modes.grab.distanceSquare = pJS.interactivity.modes.grab.distance * pJS.interactivity.modes.grab.distance;
			pJS.interactivity.modes.bubble.distance = pJS.tmp.obj.mode_bubble_distance * pJS.canvas.pxratio;
			pJS.interactivity.modes.bubble.distanceSquare = pJS.interactivity.modes.bubble.distance * pJS.interactivity.modes.bubble.distance;
		...
		pJS.fn.particle.prototype.drawSVG = function (img_obj, radius) {
			...
			pJS.canvas.ctx.drawImage(
				...
				this.y - radius,
				radius * 2,
				radius * 2 / (img_obj.width / img_obj.height)
			);
		...
		pJS.fn.particle.prototype.draw = function () {
			...
			var p = this,
				svg, radius, opacity, color_value;
		...
		pJS.fn.particlesUpdate = function () {
			...
			for (var i = 0; i < len; i++) {
				...
				p.bw = pJS.responsive ? jQuery.fn.revolution[pJS.sliderid].bw : 1;
		...
		pJS.fn.interact.linkParticles = function (p1, p2) {
			...
			if (dist <= pJS.particles.line_linked.distanceSquare) {

				var opacity_line = pJS.particles.line_linked.opacity - (dist / (1 / pJS.particles.line_linked.opacity)) / pJS.particles.line_linked.distanceSquare;
		...
		pJS.fn.interact.bounceParticles = function (p1, p2) {
			...
			var dx = p1.x - p2.x,
				dy = p1.y - p2.y,
				dist = dx * dx + dy * dy,
				dist_p = Math.pow(p1.radius + p2.radius, 2);
		...
		pJS.fn.modes.bubbleParticle = function (p) {
			...
			if (pJS.interactivity.events.onhover.enable && isInArray('bubble', pJS.interactivity.events.onhover.mode)) {
				...
				var ratio = 1 - dist_mouse / pJS.interactivity.modes.bubble.distanceSquare;


				/* mousemove - check ratio */
				if (dist_mouse <= pJS.interactivity.modes.bubble.distanceSquare) {
		...
		pJS.fn.modes.grabParticle = function (p) {
			...
			if (pJS.interactivity.events.onhover.enable && pJS.interactivity.status === 'mousemove') {
				...
				if (dist_mouse <= pJS.interactivity.modes.grab.distanceSquare) {

					var opacity_line = pJS.interactivity.modes.grab.line_linked.opacity - (dist_mouse / (1 / pJS.interactivity.modes.grab.line_linked.opacity)) / pJS.interactivity.modes.grab.distanceSquare;
		...
		pJS.fn.vendors.densityAutoParticles = function () {
			...
			if (pJS.particles.number.density.enable) {
				...
				if (pJS.tmp.retina) {
					area = area / (pJS.canvas.pxratio * pJS.canvas.pxratio);
				}
		...
		pJS.fn.vendors.destroypJS = function () {
			...
			window.removeEventListener("focus", pJS.fn.vendors.onFocus);
			pJS.fpsCounter.kill();
			pJS.fpsCounter = null;
			if(pJS.fpsOptiTween) {
				pJS.fpsOptiTween.kill();
				pJS.fpsOptiTween = null;
			}
			var prop;
			for (prop in pJS) {
				if (pJS.hasOwnProperty(prop)) delete pJS[prop];
			}
			for (prop in $this) {
		...
		pJS.fn.vendors.pause = function () {
			if (pJS.fn.drawAnimFrame) cancelAnimationFrame(pJS.fn.drawAnimFrame);
			pJS.fpsCounter.pause();
			if(pJS.fpsOptiTween) pJS.fpsOptiTween.pause();
		}

		pJS.fn.vendors.play = function () {
			pJS.fn.drawAnimFrame = requestAnimationFrame(pJS.fn.vendors.draw);
			pJS.fpsCount = 0;
			pJS.fpsTime = performance.now();
			pJS.fpsCounter.play();
		}
		...
		/* ---------- pJS - performance test ------------ */

		pJS.fpsCount = 0;

		pJS.fn.fpsCalculator = function () {
			pJS.fpsTime = (performance.now() - pJS.fpsTime) / 1000;

			var average = Math.round(pJS.fpsCount / pJS.fpsTime);
			pJS.fpsCount = 0;
			pJS.fpsTime = performance.now();

			if (average < 40) {
				var optiDuration = 5;
				var addAreaValue = (60 - average) * 5;
				pJS.fpsOptiTween = punchgs.gsap.to(pJS.particles.number.density, {
					duration: optiDuration,
					ease: "none",
					value_area: "+=" + addAreaValue,
					onUpdate: function(){
						pJS.fn.vendors.densityAutoParticles();
					}
				})
				...
				if(average < 20){
					...
					pJS.fpsCounter.repeatDelay(optiDuration);
				}
				
			} else{
				pJS.fpsCounter.repeatDelay(10);
			}
		}

		pJS.fpsCounter = punchgs.gsap.set({}, {
			paused: true,
			repeat: -1,
			repeatDelay: 5,
			onRepeat: pJS.fn.fpsCalculator
		});

		pJS.fn.vendors.onFocus = function(){
			pJS.fpsTime = performance.now();
			pJS.fpsCount = 0;
			pJS.fpsCounter.play();
		}

		window.addEventListener("focus", pJS.fn.vendors.onFocus);
		...
		function cacheTheSvg(svgXml, theFill, theStroke, size, realsizes,ocolor) {
			
			/* set color to svg element */
			var rgbHex = /#([0-9A-F]{3,6})/gi,
				coloredSvgXml = svgXml.indexOf("data-custom=")>=0 && ocolor==true ? svgXml : svgXml.replace(rgbHex, theFill).replace('{{stroke-color}}', theStroke);
			...
			img.width = realsizes.width;
			img.height = realsizes.height;
			canvas.width = (size/img.height)*img.width;
			canvas.height = size;
			img.addEventListener('load', function () {
				...
				ctx.drawImage(this, 0, 0, (size/img.height)*img.width, size);
		...
		if (pJS.particles.shape.type === 'image') {
			...
			for (var i = 0, j, k; i < len1; i++) {
				for (j = 0; j < len2; j++) {
					for (k = 0; k < len3; k++) {
						cacheTheSvg(shapes[i], fills[j], strokes[k], pJS.particles.size.drawSize, pJS.particles.shape.sizes[i],params.particles.color.ocolor);
					}
				}
			}
		} else {
	...
	function particlesJSRs(slide, data, ids, slider, zIndex, startDrawingParticles) {
		...
		return {
			instance: new pJS(canvas_el, data, container, slider, startDrawingParticles),
			el: jQuery(canvas_el)
		};
