...
class WC_Order_Status_Manager_Order_Statuses {
		...
		if ( isset( $core_order_statuses[ 'wc-' . $slug ] ) ) {
			...
			switch ( $slug ) {
				...
				case 'completed':
					...
					update_post_meta( $post_id, '_action_icon',        ( Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? 'dashicons dashicons-yes' : 'wcicon-check' ) );
					update_post_meta( $post_id, '_bulk_action',        'yes' );
					update_post_meta( $post_id, '_include_in_reports', 'yes' );
					update_post_meta( $post_id, '_is_paid',            'yes' );
	...
	public function get_core_order_status_color( $status ) {
		...
		switch ( $status ) {
			...
			case 'pending':
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#c6e1c6' : '#73a724';
			case 'processing':
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#c6e1c6' : '#73a724';
			case 'on-hold':
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#f8dda7' : '#999999';
			case 'completed':
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#c8d7e1' : '#2ea2cc';
			case 'cancelled':
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#e5e5e5' : '#2ea2cc';
			case 'refunded':
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#e5e5e5' : '#999999';
			case 'failed':
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#eba3a3' : '#d0c21f';
			default:
				return Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) ? '#e5e5e5' : '#999999';
		}
	}
