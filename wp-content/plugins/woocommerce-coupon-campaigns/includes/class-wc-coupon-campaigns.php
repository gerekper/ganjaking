<?php
/**
 * The Coupon Campaigns class file.
 *
...
class WC_Coupon_Campaigns {
	 ...
	 * The constructor.
	 *
	 * @param string $file The main plugin filename.
	 */
	...
	/**
	 * Register the scripts used in WooCommerce admin.
	 */
	public function register_script() {
		if ( ! method_exists( PageController::class, 'is_admin_or_embed_page' ) || ! PageController::is_admin_or_embed_page() ) {
			return;
		}

		$script_path       = '/assets/build/index.js';
		$script_asset_path = $this->dir . '/assets/build/index.asset.php';
		$script_asset      = file_exists( $script_asset_path ) ? require $script_asset_path : array(
			'dependencies' => array(),
			'version'      => filemtime( "{$this->dir}{$script_path}" ),
		);
		$script_url        = plugins_url( $script_path, $this->file );
		...
		wp_enqueue_script( 'wc-coupon-campaigns' );
	}
			...
			case 'reports':
				$url = add_query_arg(
					array(
						...
						'coupon_campaign' => intval( $term_id ),
					),
					admin_url( 'admin.php' )
				);
	...
	public function campaign_filter_action( $request ) {
		...
		if ( $typenow === $this->post_type ) {
			$selected_campaign = isset( $_GET[ $this->tax ] ) ? sanitize_text_field( wp_unslash( $_GET[ $this->tax ] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( $selected_campaign ) {
				...
				if ( isset( $request['tax_query'] ) ) {
					...
					$request['tax_query'] = $query_array; // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query
				}
			}
		}
	...
	public function admin_menu_highlight() {
		...
		if ( isset( $taxonomy ) ) {
			if ( $taxonomy === $this->tax ) {
				$submenu_file = 'edit-tags.php?taxonomy=' . esc_attr( $this->tax ); //phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
	...
	public function display_usage_notes( $post ) {
		...
		if ( $usage_limit ) {
			...
			$data .= sprintf( __( '%1$s / %2$s', 'wc_coupon_campaigns' ), $usage_count, $usage_limit );
		} else {
			/* translators: %s - usage count */
			$data .= sprintf( __( '%s / &infin;', 'wc_coupon_campaigns' ), $usage_count );
		}
	...
	public function save_usage_data( $order_id ) {
		...
		$processed = absint( $order->get_meta( '_coupons_processed', true ) );
		...
		$order->add_meta_data( '_coupons_processed', '1' );
		$order->save();
	}
	...
	private function save_coupon_note( $coupon, $discount, $order_id, $order_total, $user, $time ) {
		$coupon_id = $coupon->get_id();
		$order_url = OrderUtil::get_order_admin_edit_url( intval( $order_id ) );
		...
		if ( $notes ) {
			foreach ( $notes as $note ) {
				...
				<li rel="<?php echo absint( $note->comment_ID ); ?>" class="note">
					...
					<p class="meta">
						<?php /* translators: %s - time ago */ ?>
						<?php printf( esc_html__( 'added %s ago', 'wc_coupon_campaigns' ), esc_html( human_time_diff( strtotime( $note->comment_date_gmt ), time() ) ) ); ?>
					</p>
				</li>
				<?php
			}
	...
	public function display_order_campaigns( $order ) {
		...
		$campaigns = wp_get_post_terms( $order->get_id(), $this->tax );
	...
	public function coupon_info() {
		...
		if ( isset( $_GET['coupon'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended

			$coupon_code = esc_attr( sanitize_text_field( wp_unslash( $_GET['coupon'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	 ...
	 * @since 1.0.3
	 *
	 * @param WP_Comment_Query $comments_query The comments query object.
