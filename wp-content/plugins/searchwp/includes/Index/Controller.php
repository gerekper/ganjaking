...
class Controller extends BackgroundProcess {
	...
	protected function task( $batch ) {
		...
		$source_obj = $this->get_source_by_name( $source );

		if ( ! $source_obj instanceof Source ) {
			do_action( 'searchwp\debug\log', 'Invalid source during drop: ' . $source, 'index' );
		} else {
			$this->drop( $source_obj, $id, true );
		}
	...
	protected function complete() {
		...
		$more_to_do = ! $this->is_queue_empty();

		if ( $more_to_do ) {
			do_action( 'searchwp\debug\log', 'Dispatching next delta batch', 'indexer' );
			$this->dispatch();
		} else {
	...
	private function check_for_process_failure() {
		if ( $this->is_process_asleep() ) {
			do_action( 'searchwp\debug\log', 'Process has fallen asleep, waking up...', 'index' );
			$this->unlock_process();
			$this->_dispatch();
		}
	}
	...
	public function _add_hooks() {
		foreach ( $this->sources as $source ) {
			if ( ! Utils::any_engine_has_source( $source ) ) {
				continue;
			}
	...
	public function drop( Source $source, $id, $force = false ) {
		...
		$cache_key = SEARCHWP_PREFIX . 'index_drop' . $entry_id . $source_name . $site;
	 ...
	 */
	public function has_tokens( array $tokens, array $sources, array $sites ) {
		global $wpdb;
