...
class Parser
{
    ...
    public function parseContent($content)
    {
        ...
        \ob_start();
        @($parser = new \SearchWP\Dependencies\TCPDF_PARSER(\ltrim($content)));
    ...
    protected function parseTrailer($structure, $document)
    {
        ...
        foreach ($structure as $name => $values) {
            ...
            if (\is_numeric($values)) {
                $trailer[$name] = new \SearchWP\Dependencies\Smalot\PdfParser\Element\ElementNumeric($values, $document);
    ...
    {
        $header = new \SearchWP\Dependencies\Smalot\PdfParser\Header(array(), $document);
        $content = '';
            ...
            switch ($part[0]) {
                ...
                case 'stream':
                    ...
                    if ($header->get('Type')->equals('ObjStm')) {
                        ...
                        foreach ($xrefs as $xref) {
                            list($id, $position) = \explode(' ', \trim($xref));
                            $table[$position] = $id;
                        }
                        \ksort($table);
                        ...
                        foreach ($positions as $index => $position) {
                            ...
                            $object = \SearchWP\Dependencies\Smalot\PdfParser\PDFObject::factory($document, $sub_header, '');
                ...
                default:
                    if ($part != 'null') {
                        ...
                        if ($element) {
                            $header = new \SearchWP\Dependencies\Smalot\PdfParser\Header(array($element), $document);
                        }
                    }
                    break;
            }
        ...
        if (!isset($this->objects[$id])) {
            $this->objects[$id] = \SearchWP\Dependencies\Smalot\PdfParser\PDFObject::factory($document, $header, $content);
        }
    }
    /**
     * @param array    $structure
    ...
    protected function parseHeaderElement($type, $value, $document)
    {
        switch ($type) {
            ...
            case 'numeric':
                return new \SearchWP\Dependencies\Smalot\PdfParser\Element\ElementNumeric($value, $document);
            case 'boolean':
                return new \SearchWP\Dependencies\Smalot\PdfParser\Element\ElementBoolean($value, $document);
            case 'null':
                return new \SearchWP\Dependencies\Smalot\PdfParser\Element\ElementNull($value, $document);
            case '(':
