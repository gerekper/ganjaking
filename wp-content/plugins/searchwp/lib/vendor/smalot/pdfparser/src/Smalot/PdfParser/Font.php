...
class Font extends \SearchWP\Dependencies\Smalot\PdfParser\PDFObject
{
    ...
    public function loadTranslateTable()
    {
        ...
        $this->table = array();
        $this->tableSizes = array('from' => 1, 'to' => 1);
        if ($this->has('ToUnicode')) {
            ...
            if (\preg_match_all('/begincodespacerange(?P<sections>.*?)endcodespacerange/s', $content, $matches)) {
                foreach ($matches['sections'] as $section) {
                    ...
                    $this->tableSizes = array('from' => \max(1, \strlen(\current($matches['from'])) / 2), 'to' => \max(1, \strlen(\current($matches['to'])) / 2));
                    break;
                }
            }
    ...
    public function decodeText($commands)
    {
        ...
        foreach ($commands as $command) {
            switch ($command[\SearchWP\Dependencies\Smalot\PdfParser\PDFObject::TYPE]) {
                case 'n':
                    if (\floatval(\trim($command[\SearchWP\Dependencies\Smalot\PdfParser\PDFObject::COMMAND])) < $font_space) {
    ...
    protected function decodeContent($text, &$unicode)
    {
        if ($this->has('ToUnicode')) {
            ...
            if ($encoding instanceof \SearchWP\Dependencies\Smalot\PdfParser\Encoding) {
                if ($unicode) {
                    ...
                    if ($encoding->get('BaseEncoding')->equals('MacRomanEncoding')) {
                        // Edited from package for SearchWP.
                        // @link https://github.com/smalot/pdfparser/issues/229
                        // @link https://github.com/daneren2005/pdfparser/commit/ab83a1a6ec6914f84e0001215b2257ac9176a956
        ...
        if (!$unicode) {
            if ($this->get('Encoding') instanceof \SearchWP\Dependencies\Smalot\PdfParser\Element && $this->get('Encoding')->equals('MacRomanEncoding')) {
                // Edited from package for SearchWP.
                // @link https://github.com/smalot/pdfparser/issues/229
                // @link https://github.com/daneren2005/pdfparser/commit/ab83a1a6ec6914f84e0001215b2257ac9176a956
                $text = \mb_convert_encoding($text, 'UTF-8', 'ISO-8859-1');
