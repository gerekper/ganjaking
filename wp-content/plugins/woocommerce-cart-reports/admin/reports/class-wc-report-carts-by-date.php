...
class WC_Report_Carts_By_Date extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$abandoned_carts = $this->get_count_within_range( $abandoned_carts_items, $this->start_date, $this->end_date);

		$legend[] = array(
			'title' => sprintf( __( '%s abandoned carts in this period.', 'woocommerce_cart_reports' ), '<strong>' . count($abandoned_carts) . '</strong>' ),
	...
	public function get_count_within_range( $data, $start_date, $end_date) {
		$results = array();
		foreach ($data as $d) {
			$date_time = strtotime($d->post_modified);

			//A a day on to the end date to include the day
			if ($date_time > $start_date && $date_time < $end_date + 86400) {
				$results[] = $d;
			}
		}
		return $results;
	...
	public function get_main_chart() {
		...
		$order_counts            = $this->prepare_chart_data( $orders, 'post_modified', 'total_orders', $this->chart_interval, $this->start_date, $this->chart_groupby );
		$abandoned_carts_encoded = $this->prepare_chart_data( $ee_abandoned_posts, 'post_modified', 'abandoned_cart_itms', $this->chart_interval, $this->start_date, $this->chart_groupby );
