...
/**
 * weLaunch Framework is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * any later version.
 *
 * weLaunch Framework is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 * You should have received a copy of the GNU General Public License
 * along with weLaunch Framework. If not, see <http://www.gnu.org/licenses/>.
 *
 * @package     weLaunch_Framework
 * @subpackage  Core
 * @subpackage  Core
 * @author      weLaunch Framework Team
 */

// Exit if accessed directly.
defined( 'ABSPATH' ) || exit;

require_once dirname( __FILE__ ) . '/class-welaunch-core.php';

weLaunch_Core::$version    = '4.1.24';
weLaunch_Core::$welaunch_path = dirname( __FILE__ );
weLaunch_Core::instance();

// Don't duplicate me!
if ( ! class_exists( 'weLaunchFramework', false ) ) {

	/**
	 * Main weLaunchFramework class
	 *
	 * @since       1.0.0
	 */
	class weLaunchFramework {

		/**
		 * weLaunchFramework instance storage.
		 *
		 * @var null
		 * @access public
		 */
		...
		/**
		 * Absolute directory of the weLaunch instance.
		 *
		 * @var string
		 * @access public
		 */
		...
		/**
		 * Full URL of the weLaunch instance.
		 *
		 * @var string
		 * @access public
		 */
		 ...
		 * Init
		 *
		 * Backward compatibility for previous version of weLaunch.
		 */
		public static function init() {

			// Backward compatibility for extensions.
			self::$_version    = weLaunch_Core::$version;
			self::$_dir        = weLaunch_Core::$dir;
			self::$_url        = weLaunch_Core::$url;
			self::$_upload_dir = weLaunch_Core::$upload_dir;
			self::$_upload_url = weLaunch_Core::$upload_url;
			self::$_as_plugin  = weLaunch_Core::$as_plugin;
			self::$_is_plugin  = weLaunch_Core::$is_plugin;
		}
		...
		/**
		 * Pointer to the weLaunch_Options_Default class.
		 *
		 * @var null|weLaunch_Options_Defaults
		 */
		public $options_defaults_class = null;

		/**
		 * Pointer to the weLaunch_Options class.
		 *
		 * @var null|weLaunch_Options
		 */
		public $options_class = null;

		/**
		 * Pointer to the weLaunch_Required class
		 *
		 * @var null|weLaunch_Required
		 */
		public $required_class = null;

		/**
		 * Pointer to the weLaunch_Output class.
		 *
		 * @var null|weLaunch_Output
		 */
		public $output_class = null;

		/**
		 * Pointer to the weLaunch_Page_Render class.
		 *
		 * @var null|weLaunch_Page_Render
		 */
		public $render_class = null;

		/**
		 * Pointer to the weLaunch_Enqueue class.
		 *
		 * @var null|weLaunch_Enqueue
		 */
		public $enqueue_class = null;

		/**
		 * Pointer to the weLaunch_Transients class.
		 *
		 * @var null|weLaunch_Transients
		 */
		public $transient_class = null;

		/**
		 * Pointer to the weLaunch_wordPress_Data class.
		 *
		 * @var null|weLaunch_WordPress_Data
		 */
		public $wordpress_data = null;

		/**
		 * Poiner to the weLaunch_Validation class.
		 *
		 * @var null|weLaunch_Validation
		 */
		public $validate_class = null;

		/**
		 * Poiner to the weLaunch_Sanitize class.
		 *
		 * @var null|weLaunch_Validation
		 */
		...
		/**
		 * Pointer to the weLaunch_Args class.
		 *
		 * @var null|weLaunch_Args
		 */
		public $args_class = null;

		/**
		 * Array of active transients used by weLaunch.
		 *
		 * @var araray
		 */
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; eh?', 'welaunch-framework' ), '4.0' );
		}
		...
		public function __construct( $sections = array(), $args = array() ) {
			...
			if ( ! isset( weLaunch::$init[ $args['opt_name'] ] ) ) {
				// Let's go back to the weLaunch API instead of having it run directly.
				weLaunch_Functions_Ex::record_caller( $args['opt_name'] );
				weLaunch::set_args( $args['opt_name'], $args );
				if ( ! empty( $sections ) ) {
					weLaunch::set_sections( $args['opt_name'], $sections );
				}
				$sections = weLaunch::construct_sections( $args['opt_name'] );
				$args     = weLaunch::construct_args( $args['opt_name'] );
				weLaunch::set_defaults( $args['opt_name'] );
				weLaunch::$init[ $args['opt_name'] ] = 1;
			}

			$args             = new weLaunch_Args( $this, $args );
			$this->args_class = $args;
			$this->args       = $args->get;

			weLaunch_Core::core_construct( $this, $this->args );

			new weLaunch_Admin_Notices( $this );

			if ( ! empty( $this->args['opt_name'] ) ) {
				new weLaunch_Instances( $this );

				$this->filesystem = weLaunch_Filesystem::get_instance( $this );

				/**
				 * Filter 'welaunch/options/{opt_name}/sections'
				 *
				 * @param  array $sections field option sections
				 */

				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$this->sections = apply_filters( "welaunch/options/{$this->args['opt_name']}/sections", $sections );

				/**
				 * Construct hook
				 * action 'welaunch/construct'
				 *
				 * @param object $this weLaunchFramework
				 */

				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				do_action( 'welaunch/construct', $this );

				// Internataionalization.
				new weLaunch_I18n( $this, __FILE__ );

				$this->required_class  = new weLaunch_Required( $this );
				$this->transient_class = new weLaunch_Transients( $this );
				$this->wordpress_data  = new weLaunch_WordPress_Data( $this );
				$this->validate_class  = new weLaunch_Validation( $this );
				$this->sanitize_class  = new weLaunch_Sanitize( $this );

				// Register extra extensions.
				new weLaunch_Extensions( $this );

				// Grab database values.
				$this->options_defaults_class = new weLaunch_Options_Defaults();
				$this->options_class          = new weLaunch_Options_Constructor( $this );
				$this->options_class->get();

				$this->output_class  = new weLaunch_Output( $this );
				$this->render_class  = new weLaunch_Page_Render( $this );
				$this->enqueue_class = new weLaunch_Enqueue( $this );

				new weLaunch_AJAX_Save( $this );
				new weLaunch_AJAX_Typography( $this );
				new weLaunch_AJAX_Select2( $this );
			}

			/**
			 * Loaded hook
			 * action 'welaunch/loaded'
			 *
			 * @param  object $this weLaunchFramework
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( 'welaunch/loaded', $this );
		}

		/**
		 * Begin backward compatibility shims for weLaunch v3 configs and extensions.
		 */
		...
		public function _enqueue_output() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
			if ( empty( $this->output_class ) ) {
				$obj          = new weLaunchFramework( $this->sections, $this->args );
		...
		public function current_user_can( $permission ) {
			_deprecated_function( __FUNCTION__, '4.0.0', 'weLaunch_Helpers::current_user_can' );

			return weLaunch_Helpers::current_user_can( $permission );
		}

		/**
		 * End backward compatibility shims for weLaunch v3 configs and extensions.
		 */

		/**
		 * Pointer to the weLaunchFramework instance.
		 *
		 * @return weLaunchFramework
		 */
	...
	weLaunchFramework::init();

	/**
	 * Action 'welaunch/init'
	 *
	 * @param null
	 */
	do_action( 'welaunch/init' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName
}
