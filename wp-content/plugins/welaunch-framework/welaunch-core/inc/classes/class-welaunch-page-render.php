<?php
/**
 * weLaunch Page Render Class
 *
 * @class weLaunch_Page_Render
 * @version 3.0.0
...
if ( ! class_exists( 'weLaunch_Page_Render', false ) ) {

	/**
	 * Class weLaunch_Page_Render
	 */
	class weLaunch_Page_Render extends weLaunch_Class {
		...
		/**
		 * weLaunch_Page_Render constructor.
		 *
			...
			if ( 'hidden' === $core->args['menu_type'] ) {
				...
				if ( true === $core->args['allow_sub_menu'] ) {
					foreach ( $core->sections as $k => $section ) {
						...
						if ( isset( $section['permissions'] ) && ! weLaunch_Helpers::current_user_can( $section['permissions'] ) ) {
							continue;
						}
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				$dismiss = 'true';
				$s       = esc_html__( 'Enable', 'welaunch-framework' );

				// Values for disabling hints.
				if ( 'true' === $hint_status ) {
					$dismiss = 'false';
					$s       = esc_html__( 'Disable', 'welaunch-framework' );
				}

				// Make URL.
				$nonce = wp_create_nonce( 'welaunch_hint_toggle' );
				$url   = '<a class="welaunch_hint_status" href="?nonce=' . $nonce . '&amp;dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . esc_attr( $cur_page ) . '&amp;tab=' . esc_attr( $cur_tab ) . '">' . $s . ' hints</a>';

				$event = esc_html__( 'moving the mouse over', 'welaunch-framework' );
				if ( 'click' === $core->args['hints']['tip_effect']['show']['event'] ) {
					$event = esc_html__( 'clicking', 'welaunch-framework' );
				}

				// Construct message.
				// translators: %1$s: Mousr action.  %2$s: Hint status.
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'welaunch-framework' ), $event, weLaunch_Core::strtolower( $s ) ) . '<br/><br/>' . $url;

				// Construct hint tab.
				$tab = array(
					'id'      => 'welaunch-hint-tab',
					'title'   => esc_html__( 'Hints', 'welaunch-framework' ),
					'content' => '<p>' . $msg . '</p>',
				);
			...
			if ( '' !== $core->args['help_sidebar'] ) {
				...
				// about hints.
				if ( true === $this->show_hints ) {
					$screen->set_help_sidebar( '<p><strong>weLaunch Framework</strong><br/><br/>' . esc_html__( 'Hint Tooltip Preferences', 'welaunch-framework' ) . '</p>' );
				}
			}

			/**
			 * Action 'welaunch/page/{opt_name}/load'
			 *
			 * @param object $screen WP_Screen
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "welaunch/page/{$core->args['opt_name']}/load", $screen );
		}

		/**
		 * Class Add Sub Menu Function, creates options submenu in WordPress admin area.
		 *
		 * @param       object $core weLaunchFrameword core pointer.
		 *
		 * @since       3.1.9
		 * @access      private
		 * @return      void
		...
		private function submenu( $core ) {
			...
			$page_parent = weLaunch_Core::strtolower( $page_parent );
		...
		public function section_desc( $section ) {
			...
			if ( isset( $core->sections[ $id ]['desc'] ) && ! empty( $core->sections[ $id ]['desc'] ) ) {
				echo '<div class="welaunch-section-desc">' . wp_kses_post( $core->sections[ $id ]['desc'] ) . '</div>';
			}
		}
		...
		public function field_input( $field, $v = null, $metabox = false ) {
			...
			if ( isset( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
				...
				/**
				 * Action 'welaunch/field/{opt_name}/{field.type}/callback/before'
				 *
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"welaunch/field/{$core->args['opt_name']}/{$field['type']}/callback/before",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'welaunch/field/{opt_name}/callback/before'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"welaunch/field/{$core->args['opt_name']}/callback/before",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'welaunch/field/{opt_name}/{field.type}/callback/after'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"welaunch/field/{$core->args['opt_name']}/{$field['type']}/callback/after",
					array(
						&$field,
						&$value,
					)
				...
				/**
				 * Action 'welaunch/field/{opt_name}/callback/after'
				 *
				 * @param array  $field field data
				 * @param string $value field.id
				 */
				...
				do_action_ref_array(
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					"welaunch/field/{$core->args['opt_name']}/callback/after",
					array(
						&$field,
						&$value,
					)
			...
			if ( isset( $field['type'] ) ) {
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$class_file = apply_filters( "welaunch/{$core->args['opt_name']}/field/class/{$field['type']}", $filter_path, $field );
				...
				if ( $class_file ) {
					...
					if ( ! class_exists( $field_class ) ) {
						if ( file_exists( $class_file ) ) {
							...
							echo sprintf( esc_html__( 'Field %1$s could not be displayed. Field type %2$s was not found.', 'welaunch-framework' ), '<code>' . esc_attr( $field['id'] ) . '</code>', '<code>' . esc_attr( $field['type'] ) . '</code>' );
						}
					}
				}
				...
				if ( class_exists( $field_class ) ) {
					...
					/**
					 * Action 'welaunch/field/{opt_name}/{field.type}/render/before'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"welaunch/field/{$core->args['opt_name']}/{$field['type']}/render/before",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'welaunch/field/{$this->args['opt_name']}/render/before'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"welaunch/field/{$core->args['opt_name']}/render/before",
						array(
							&$field,
							&$value,
						)
					...
					$pro_field_loaded = weLaunch_Functions::load_pro_field( $data );
					...
					/**
					 * Filter 'welaunch/field/{opt_name}'
					 *
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "welaunch/field/{$core->args['opt_name']}", ob_get_contents(), $field );

					/**
					 * Filter 'welaunch/field/{opt_name}/{field.type}/render/after'
					 *
					 * @param       string        rendered field markup
					 * @param array $field field data
					 */
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "welaunch/field/{$core->args['opt_name']}/{$field['type']}/render/after", $_render, $field );

					/**
					 * Filter 'welaunch/field/{opt_name}/render/after'
					 *
					 * @param       string        rendered field markup
					 * @param array $field field data
					 */
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$_render = apply_filters( "welaunch/field/{$core->args['opt_name']}/render/after", $_render, $field );
					...
					/**
					 * Action 'welaunch/field/{opt_name}/{field.type}/fieldset/before/{opt_name}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"welaunch/field/{$core->args['opt_name']}/{$field['type']}/fieldset/before/{$core->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'welaunch/field/{opt_name}/fieldset/before/{opt_name}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"welaunch/field/{$core->args['opt_name']}/fieldset/before/{$core->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					...
					if ( isset( $field['full_width'] ) && true === $field['full_width'] ) {
						$class_string .= 'welaunch_remove_th';
					}
					...
					if ( weLaunch_Core::$pro_loaded ) {
						if ( $pro_field_loaded ) {
							$class_string .= ' welaunch-pro-field-init';
						}
					}

					echo '<fieldset id="' . esc_attr( $core->args['opt_name'] . '-' . $field['id'] ) . '" class="' . esc_attr( $hidden . esc_attr( $disabled ) . 'welaunch-field-container welaunch-field welaunch-field-init welaunch-container-' . $field['type'] . ' ' . $class_string ) . '" data-id="' . esc_attr( $field['id'] ) . '" data-type="' . esc_attr( $field['type'] ) . '">';
					...
					/**
					 * Action 'welaunch/field/{opt_name}/{field.type}/fieldset/after/{opt_name}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"welaunch/field/{$core->args['opt_name']}/{$field['type']}/fieldset/after/{$core->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
					...
					/**
					 * Action 'welaunch/field/{opt_name}/fieldset/after/{opt_name}'
					 *
					 * @param array  $field field data
					 * @param string $value field id
					 */
					...
					do_action_ref_array(
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						"welaunch/field/{$core->args['opt_name']}/fieldset/after/{$core->args['opt_name']}",
						array(
							&$field,
							&$value,
						)
		...
		public function add_menu() {
			...
			if ( $menu ) {
				...
				if ( isset( $submenu[ $core->args['page_slug'] ] ) && is_array( $submenu[ $core->args['page_slug'] ] ) ) {
					foreach ( $submenu[ $core->args['page_slug'] ] as $index => $welaunch_options_submenu ) {
						$subnodeargs = array(
							'id'     => esc_html( $core->args['page_slug'] . '_' . $index ),
							'title'  => esc_html( $welaunch_options_submenu[0] ),
							'parent' => esc_html( $core->args['page_slug'] ),
							'href'   => esc_url( admin_url( 'admin.php?page=' . $welaunch_options_submenu[2] ) ),
						);
		...
		/**
		 * Do action welaunch-admin-head for options page
		 *
		 * @since       1.0.0
		 * @access      public
		 * @return      void
		...
		public function admin_head() {
			...
			/**
			 * Action 'welaunch/page/{opt_name}/header'
			 *
			 * @param  object $this weLaunchFramework
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "welaunch/page/{$core->args['opt_name']}/header", $core );
		}
		...
		public function get_header_html( $field ) {
			...
			if ( isset( $field['title'] ) && isset( $field['type'] ) && 'info' !== $field['type'] && 'section' !== $field['type'] ) {
				...
				if ( isset( $field['hint'] ) && ! empty( $field['hint'] ) ) {
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$hint = apply_filters( 'welaunch/hints/html', $hint, $field, $core->args );
					...
					if ( 'true' === $meta_val || empty( $meta_val ) && empty( $hint ) ) {
						...
						$hint = '<div class="welaunch-hint-qtip" style="float:' . esc_attr( $core->args['hints']['icon_position'] ) . '; font-size: ' . esc_attr( $size ) . '; color:' . esc_attr( $hint_color ) . '; cursor: ' . $pointer . ';" qtip-title="' . esc_attr( $title_param ) . '" qtip-content="' . wp_kses_post( $content_param ) . '">&nbsp;<i class="' . ( isset( $core->args['hints']['icon'] ) ? esc_attr( $core->args['hints']['icon'] ) : '' ) . '"></i></div>';
					}
				}
			...
			if ( ! empty( $th ) ) {
				$th = '<div class="welaunch_field_th">' . $th . '</div>';
			}
		...
		private function get_default_output_string( $field ) {
			...
			if ( ! empty( $default_output ) ) {
				$default_output = esc_html__( 'Default', 'welaunch-framework' ) . ': ' . substr( $default_output, 0, - 2 );
			}
		...
		public function section_menu( $k, $section, $suffix = '', $sections = array() ) {
			...
			if ( isset( $section['type'] ) && 'divide' === $section['type'] ) {
				...
				$string            .= '<li id="' . esc_attr( $k . $suffix ) . '_section_group_li" class="welaunch-group-tab-link-li' . esc_attr( $hide_section ) . esc_attr( $section['class'] ) . esc_attr( $subsections_class ) . '">';
				$string            .= '<a href="javascript:void(0);" id="' . esc_attr( $k . $suffix ) . '_section_group_li_a" class="welaunch-group-tab-link-a" data-key="' . esc_attr( $k ) . '" data-rel="' . esc_attr( $k . $suffix ) . '">' . $extra_icon . $icon . '<span class="group_title">' . wp_kses_post( $section['title'] ) . '</span></a>';
				...
				if ( $subsections ) {
					...
					while ( $do_loop ) {
						...
						if ( count( $sections ) < $next_k || ! isset( $sections[ $next_k ] ) || ! isset( $sections[ $next_k ]['subsection'] ) || true !== $sections[ $next_k ]['subsection'] ) {
							...
							$string                      .= '<li id="' . esc_attr( $next_k . $suffix ) . '_section_group_li" class="welaunch-group-tab-link-li ' . esc_attr( $hide_sub ) . esc_attr( $section[ $next_k ]['class'] ) . ( $icon ? ' hasIcon' : '' ) . '">';
							$string                      .= '<a href="javascript:void(0);" id="' . esc_attr( $next_k . $suffix ) . '_section_group_li_a" class="welaunch-group-tab-link-a" data-key="' . esc_attr( $next_k ) . '" data-rel="' . esc_attr( $next_k . $suffix ) . '">' . $icon . '<span class="group_title">' . wp_kses_post( $sections[ $next_k ]['title'] ) . '</span></a>';
							$string                      .= '</li>';
						}
					}
