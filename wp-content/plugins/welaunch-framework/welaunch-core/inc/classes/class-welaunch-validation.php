<?php
/**
 * weLaunch Validation Class
 *
 * @class   weLaunch_Validation
 * @version 4.0.0
...
if ( ! class_exists( 'weLaunch_Validation', false ) ) {

	/**
	 * Class weLaunch_Validation
	 */
	class weLaunch_Validation extends weLaunch_Class {
					...
					foreach ( $section['fields'] as $fkey => $field ) {
						...
						if ( isset( $field['validate'] ) ) {
							...
							foreach ( $val_arr as $idx => $val ) {
								...
								$validate = 'weLaunch_Validation_' . $val;

								if ( ! class_exists( $validate ) ) {

									/**
									 * Filter 'welaunch/validate/{opt_name}/class/{field.validate}'
									 *
									...
									$class_file = apply_filters( "welaunch/validate/{$core->args['opt_name']}/class/{$val}", weLaunch_Core::$dir . "inc/validation/{$val}/class-welaunch-validation-{$file}.php", $validate );
