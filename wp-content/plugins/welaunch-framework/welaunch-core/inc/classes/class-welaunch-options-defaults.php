<?php
/**
 * weLaunch Option Defaults Class
 *
 * @class weLaunch_Options_Defaults
 * @version 4.0.0
...
if ( ! class_exists( 'weLaunch_Options_Defaults', false ) ) {
	...
	class weLaunch_Options_Defaults {
		...
		public function default_values( $opt_name, $sections = array(), $wp_data_class = null ) {
			...
			if ( isset( $_REQUEST['action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( weLaunch_Functions_Ex::string_ends_with( $action, '_select2' ) && weLaunch_Functions_Ex::string_starts_with( $action, 'welaunch_' ) ) {
					return;
				}
			}
		...
		public function field_default_values( $opt_name = '', $field, $wp_data_class = null ) {
			if ( null === $wp_data_class && class_exists( 'weLaunch_WordPress_Data' ) && ! ( 'select' === $field['type'] && isset( $field['ajax'] ) && $field['ajax'] ) ) {
				$wp_data_class = new weLaunch_WordPress_Data( $opt_name );
			}
			...
			if ( isset( $field['default'] ) ) {
				...
				$this->options_defaults[ $field['id'] ] = apply_filters( "welaunch/{$opt_name}/field/{$field['type']}/defaults", $field['default'], $field );
