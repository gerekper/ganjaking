<?php
/**
 * weLaunch Framework Admin Notice Class
...
// Don't duplicate me!
if ( ! class_exists( 'weLaunch_Admin_Notices', false ) ) {

	/**
	 * weLaunch Admin Notices Class
	 *
	 * @since       3.0.0
	 */
	class weLaunch_Admin_Notices extends weLaunch_Class {
		...
		/**
		 * weLaunch_Admin_Notices constructor.
		 *
		 * @param array $parent weLaunchFramework object.
		 * @access public
		 */
		public function __construct( $parent ) {
			parent::__construct( $parent );

			add_action( 'wp_ajax_welaunch_hide_admin_notice', array( $this, 'ajax' ) );
		...
		public function admin_notices( $notices = array() ) {
			...
			if ( isset( $_GET ) && isset( $_GET['page'] ) && $core->args['page_slug'] === $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification
				do_action( 'welaunch_admin_notices_run', $core->args );
				...
				if ( ! empty( $notices ) ) {
					if ( isset( $notices[ $core->args['page_slug'] ] ) ) {
						...
						foreach ( $notices[ $core->args['page_slug'] ] as $notice ) {
							...
							$add_style = '';
							if ( strpos( $notice['type'], 'welaunch-message' ) !== false ) {
							...
							if ( true === $notice['dismiss'] ) {
								...
								if ( ! get_user_meta( $userid, 'ignore_' . $notice['id'] ) ) {
									...
									if ( version_compare( $wp_version, '4.2', '>' ) ) {
										...
										$css_class = esc_attr( $notice['type'] ) . ' welaunch-notice notice is-dismissible welaunch-notice';
										...
										echo '<div ' . esc_html( $add_style ) . ' class="' . esc_attr( $notice['type'] ) . ' notice is-dismissable"><p>' . wp_kses_post( $notice['msg'] ) . '&nbsp;&nbsp;<a href="?dismiss=true&amp;id=' . esc_attr( $css_id ) . '">' . esc_html__( 'Dismiss', 'welaunch-framework' ) . '</a>.</p></div>';
									}
								}
							} else {
								// Standard notice.
							...
							<script>
								jQuery( document ).ready( function( $ ) {
									$( document.body ).on(
										'click', '.welaunch-notice.is-dismissible .notice-dismiss', function( e ) {
											e.preventDefault();
											var $data = $( this ).parent().find( '.dismiss_data' );
											$.post(
												ajaxurl, {
													action: 'welaunch_hide_admin_notice',
													id: $data.attr( 'id' ),
													nonce: $data.val()
												}
											);
		...
		private function dismiss_admin_notice() {
			...
			if ( isset( $_GET['dismiss'] ) && isset( $_GET['id'] ) ) {
				if ( isset( $_GET['nonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_GET['nonce'] ) ), 'welaunch_hint_toggle' ) ) {
					...
					wp_nonce_ays( 'welaunch_hint_toggle' );
				}
			}
		}
