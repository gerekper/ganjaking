<?php
/**
 * weLaunch Helper Class
 *
 * @class   weLaunch_Helpers
 * @version 3.0.0
...
// Don't duplicate me!
if ( ! class_exists( 'weLaunch_Helpers', false ) ) {

	/**
	 * weLaunch Helpers Class
	...
	class weLaunch_Helpers {
		...
		public static function section_from_field_id( $opt_name = '', $field_id = '' ) {
			if ( '' !== $opt_name ) {
				$welaunch = weLaunch::instance( $opt_name );

				if ( is_object( $welaunch ) ) {
					$sections = $welaunch->sections;
		...
		public static function tabFromField( $parent, $field ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0', 'weLaunch_Helpers::tab_from_field( $parent, $field )' );
		...
		public static function isFieldInUse( $parent, $field ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			// TODO - Uncomment this at release.
			// phpcs:ignore Squiz.PHP.CommentedOutCode
			// _deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0', 'weLaunch_Helpers::is_field_in_use( $parent, $field )' );
		 ...
		 *
		 * @return weLaunch_Helpers::is_local_host()
		 * @deprecated No longer using camelCase naming convention.
		 * @since      4.0
		 */
		public static function isLocalHost() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0', 'weLaunch_Helpers::is_local_host()' );
		 ...
		 *
		 * @return weLaunch_Helpers::is_wp_debug()
		 * @deprecated No longer using camelCase naming convention.
		 * @since      4.0
		 */
		public static function isWpDebug() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0', 'weLaunch_Functions_Ex::is_wp_debug()' );
		 ...
		 *
		 * @return weLaunch_Helpers::get_statistics_object()
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function getTrackingObject() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			// phpcs:ignore: Squiz.PHP.CommentedOutCode
			/* _deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::get_statistics_object()' ); */
		...
		public static function get_statistics_object() {
			...
			$demo_mode = get_option( 'weLaunchFrameworkPlugin', false );
			if ( ! empty( $demo_mode ) ) {
				$demo_mode = true;
			}
			...
			$data = array(
				...
				'localhost'       => weLaunch_Helpers::is_local_host(),
				'php'             => PHP_VERSION,
				'posts'           => $pts,
				'comments'        => array(
				...
				'options'         => apply_filters( 'welaunch/tracking/options', array() ), // TODO - What is this?!

				'welaunch_installed' => weLaunch_Core::$installed,
				'welaunch_version'   => weLaunch_Core::$version,
				'welaunch_demo_mode' => $demo_mode,
				'welaunch_plugin'    => weLaunch_Core::$as_plugin,
			...
			$parts    = explode( ' ', weLaunch_Core::$server['SERVER_SOFTWARE'] );
			$software = array();
			foreach ( $parts as $part ) {
				...
				if ( false !== strpos( $part, '/' ) ) {
					$chunk = explode( '/', $part );
					$software[ weLaunch_Core::strtolower( $chunk[0] ) ] = $chunk[1];
				}
			}
			$data['server']     = weLaunch_Core::$server['SERVER_SOFTWARE'];
			$data['db_version'] = $wpdb->db_version();

			$data['callers'] = self::process_welaunch_callers( true );
		...
		public static function get_extensions( $opt_name = '' ) {
			if ( empty( $opt_name ) ) {
				$instances = weLaunch_Instances::get_all_instances();
			} else {
				$instances = array(
					weLaunch_Instances::get_instance( $opt_name ),
				);
			}
		 ...
		 *
		 * @return weLaunch_Instances::isParentTheme( $file )
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function isParentTheme( $file ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::is_parent_theme( $file )' );
		...
		public static function is_parent_theme( $file ) {
			$file = weLaunch_Functions_Ex::wp_normalize_path( $file );
			$dir  = weLaunch_Functions_Ex::wp_normalize_path( get_template_directory() );
		 ...
		 * @param string $file Path to file.
		 *
		 * @return weLaunch_Instances::wp_normalize_path( $file )
		 * @deprecated Moved to another class.
		 */
		public static function wp_normalize_path( $file ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Functions_Ex::wp_normalize_path( $file )' );

			return weLaunch_Functions_Ex::wp_normalize_path( $file );
		}
		 ...
		 * @param string $file Path to file.
		 *
		 * @return weLaunch_Instances::is_child_theme( $file )
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function isChildTheme( $file ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::is_child_theme( $file )' );

			return self::is_child_theme( $file );
		}

		/**
		 * Deprecated. Returns true if weLaunch is running as a plugin.
		 *
		 * @return weLaunch_Helpers::()
		 * @deprecated No longer using camelCase naming convention.
		 */
		private static function welaunchAsPlugin() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Core::$as_plugin()' );

			return weLaunch_Core::$as_plugin;
		}
		...
		public static function is_child_theme( $file ) {
			$file = weLaunch_Functions_Ex::wp_normalize_path( $file );
			$dir  = weLaunch_Functions_Ex::wp_normalize_path( get_stylesheet_directory() );
		 ...
		 * @param string $file Path to file.
		 *
		 * @return weLaunch_Instances::is_theme( $file )
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function isTheme( $file ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			// phpcs:ignore Squiz.PHP.CommentedOutCode
			// _deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::is_theme( $file )' );
		 ...
		 *
		 * @return weLaunch_Functions_Ex::wp_normalize_path($path)
		...
		public static function cleanFilePath( $path ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			// TODO - Uncomment this at release.
			// phpcs:ignore Squiz.PHP.CommentedOutCode
			// _deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0', 'weLaunch_Functions_Ex::wp_normalize_path( $path )' );
			return weLaunch_Functions_Ex::wp_normalize_path( $path );
		}
		...
		public static function get_hash() {
			$remote_addr = isset( weLaunch_Core::$server['REMOTE_ADDR'] )
				? weLaunch_Core::$server['REMOTE_ADDR']
				: '127.0.0.1';
		...
		public static function get_wp_themes() {
			...
			foreach ( $themes as $theme ) {
				$path          = weLaunch_Functions_Ex::wp_normalize_path( trailingslashit( $theme->get_theme_root() ) . $theme->get_template() );
				$theme_paths[] = $path;

				if ( weLaunch_Functions_Ex::wp_normalize_path( realpath( $path ) ) !== $path ) {
					$theme_paths[] = weLaunch_Functions_Ex::wp_normalize_path( realpath( $path ) );
				}

				$wp_theme_paths[ $path ] = weLaunch_Functions_Ex::wp_normalize_path( realpath( $path ) );
			}
		...
		public static function path_info( $file ) {
			$theme_info  = weLaunch_Functions_Ex::is_inside_theme( $file );
			$plugin_info = weLaunch_Functions_Ex::is_inside_plugin( $file );
		...
		/**
		 * Compiles caller data for weLaunch.
		 *
		 * @param book $simple Mode.
		 *
		 * @return array
		 */
		public static function process_welaunch_callers( $simple = false ) {
			$data = array();

			foreach ( weLaunch_Core::$callers as $opt_name => $callers ) {
		 ...
		 *
		 * @return weLaunch_Helpers::make_bool_str( $var )
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function makeBoolStr( $var ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::make_bool_str( $var )' );
		...
		public static function localize( $localize ) {
			$welaunch = weLaunch::instance( $localize['args']['opt_name'] );
			$nonce = wp_create_nonce( 'welaunch-ads-nonce' );
			$base  = admin_url( 'admin-ajax.php' ) . '?t=' . $welaunch->core_thread . '&action=welaunch_p&nonce=' . $nonce . '&url=';
		...
		public static function get_request_headers( $args = array() ) {
			...
			$array = array(
				...
				'welaunch'      => weLaunch_Core::$version,
				'installed'  => weLaunch_Core::$installed,
				'debug'      => defined( 'WP_DEBUG' ) && WP_DEBUG ? true : false,
				'local'      => self::is_local_host(),
				'wordpress'  => get_bloginfo( 'version' ),
				'site'       => esc_url( home_url( '/' ) ),
				'auto_fonts' => get_option( 'auto_update_welaunch_google_fonts', false ),
		...
		public static function mokama() {
			if ( defined( 'RDX_MOKAMA' ) ) {
				return weLaunch_Functions_Ex::s();
			}
			return false;
		}
		 ...
		 *
		 * @return weLaunch_Helpers::compile_system_status( $json_output, $remote_checks )
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function compileSystemStatus( $json_output = false, $remote_checks = false ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::compile_system_status( $json_output, $remote_checks )' );
		...
		public static function compile_system_status( $json_output = false, $remote_checks = false ) {
			...
			$sysinfo['welaunch_ver']      = esc_html( weLaunch_Core::$version );
			$sysinfo['welaunch_data_dir'] = weLaunch_Core::$upload_dir;

			$fs        = weLaunch_Filesystem::get_instance();
			$test_file = weLaunch_Core::$upload_dir . 'test-log.log';
			...
			$sysinfo['welaunch_data_writeable'] = $res;
			...
			if ( ! class_exists( 'Browser' ) ) {
				require_once weLaunch_Core::$dir . 'inc/lib/browser.php';
			}
			...
			$sysinfo['server_info'] = esc_html( weLaunch_Core::$server['SERVER_SOFTWARE'] );
			...
			if ( true === $remote_checks ) {
				...
				$response = @wp_remote_get( 'https://raw.githubusercontent.com/dovy/welaunch-framework/master/CONTRIBUTING.md' );
			...
			$welaunch = weLaunch::all_instances();

			$sysinfo['welaunch_instances'] = array();

			if ( ! empty( $welaunch ) && is_array( $welaunch ) ) {
				foreach ( $welaunch as $inst => $data ) {
					weLaunch::init( $inst );

					$sysinfo['welaunch_instances'][ $inst ]['args']     = $data->args;
					$sysinfo['welaunch_instances'][ $inst ]['sections'] = $data->sections;
					foreach ( $sysinfo['welaunch_instances'][ $inst ]['sections'] as $key => $section ) {
						if ( isset( $section['fields'] ) && is_array( $section['fields'] ) ) {
							foreach ( $section['fields'] as $field_key => $field ) {
								if ( isset( $field['validate_callback'] ) ) {
									unset( $sysinfo['welaunch_instances'][ $inst ]['sections'][ $key ]['fields'][ $field_key ]['validate_callback'] );
								}
								if ( 'js_button' === $field['type'] ) {
									if ( isset( $field['script'] ) && isset( $field['script']['ver'] ) ) {
										unset( $sysinfo['welaunch_instances'][ $inst ]['sections'][ $key ]['fields'][ $field_key ]['script']['ver'] );
									}
								}
							}
						}
					...
					$sysinfo['welaunch_instances'][ $inst ]['extensions'] = weLaunch::get_extensions( $inst );
					...
					if ( isset( $data->extensions[ $metabox_key ] ) ) {
						...
						$sysinfo['welaunch_instances'][ $inst ][ $metabox_key ] = $data->extensions[ $metabox_key ]->boxes;
					}

					if ( isset( $data->args['templates_path'] ) && '' !== $data->args['templates_path'] ) {
						$sysinfo['welaunch_instances'][ $inst ]['templates'] = self::get_welaunch_templates( $data->args['templates_path'] );
					}
				}
			}
		...
		/**
		 * Deprecated. Returns array of weLaunch templates.
		 *
		 * @param string $custom_template_path Path to custom template.
		 *
		 * @return weLaunch_Helpers::get_welaunch_templates( $custom_template_path )
		 * @deprecated No longer using camelCase naming convention.
		 */
		private static function getweLaunchTemplates( $custom_template_path ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::get_welaunch_templates( $custom_template_path )' );

			return self::get_welaunch_templates( $custom_template_path );
		}

		/**
		 * Returns array of weLaunch templates.
		 *
		 * @param string $custom_template_path Path to template dir.
		 *
		 * @return array
		 */
		private static function get_welaunch_templates( $custom_template_path ) {
			$filesystem         = weLaunch_Filesystem::get_instance();
			$template_paths     = array( 'weLaunchFramework' => weLaunch_Core::$dir . 'templates/panel' );
			$scanned_files      = array();
			$found_files        = array();
			...
			foreach ( $scanned_files as $plugin_name => $files ) {
				foreach ( $files as $file ) {
					...
					if ( $theme_file ) {
						$core_version  = self::get_template_version( weLaunch_Core::$dir . 'templates/panel/' . $file );
						...
						if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
							...
							$found_files[ $plugin_name ][] = sprintf( '<code>%s</code> ' . esc_html__( 'version', 'welaunch-framework' ) . ' <strong style="color:red">%s</strong> ' . esc_html__( 'is out of date. The core version is', 'welaunch-framework' ) . ' %s', str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
						} else {
				...
				foreach ( $files as $key => $value ) {
					if ( ! in_array( $value, array( '.', '..' ), true ) ) {
						if ( is_dir( $template_path . DIRECTORY_SEPARATOR . $value ) ) {
							$sub_files = welaunch_scan_template_files( $template_path . DIRECTORY_SEPARATOR . $value );
		...
		public static function get_extension_dir( $dir ) {
			return trailingslashit( weLaunch_Functions_Ex::wp_normalize_path( dirname( $dir ) ) );
		}
		...
		public static function get_extension_url( $dir ) {
			...
			$ext_url = str_replace( weLaunch_Functions_Ex::wp_normalize_path( WP_CONTENT_DIR ), WP_CONTENT_URL, $ext_dir );
		...
		public static function google_fonts_update_needed() {
			...
			$path = trailingslashit( weLaunch_Core::$upload_dir ) . 'google_fonts.json';
			$now  = time();
			$secs = 60 * 60 * 24 * 7;
			if ( file_exists( $path ) ) {
		...
		public static function get_developer_keys() {
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$data = array( apply_filters( 'welaunch/tracking/developer', array() ) );
			if ( 1 === count( $data ) ) {
				if ( empty( $data[0] ) ) {
					$data = array();
				}
		...
		public static function google_fonts_array( $download = false ) {
			if ( ! empty( weLaunch_Core::$google_fonts ) && ! self::google_fonts_update_needed() ) {
				return weLaunch_Core::$google_fonts;
			}
			...
			if ( ! file_exists( $path ) || ( file_exists( $path ) && $download && self::google_fonts_update_needed() ) ) {
				if ( $download ) {
					$url = 'http://api.welaunch.io/gfonts';
					...
					if ( ! is_wp_error( $request ) ) {
						...
						if ( ! empty( $body ) ) {
							...
							weLaunch_Core::$google_fonts = json_decode( $body, true );
						}
					} else {
						return $request;
					}
				...
				weLaunch_Core::$google_fonts = json_decode( $filesystem->get_contents( $path ), true );
				if ( empty( weLaunch_Core::$google_fonts ) ) {
		...
		/**
		 * Deprecated. Gets all weLaunch instances
		 *
		 ...
		 */
		public static function getweLaunchInstances() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'weLaunch_Instances::get_all_instances()' );
		...
		public static function is_inside_plugin( $file ) {
			...
			if ( weLaunch_Functions_Ex::wp_normalize_path( $file ) !== '/' . $plugin_basename ) {
				...
				return array(
					...
					'path'      => weLaunch_Functions_Ex::wp_normalize_path( $file ),
					'url'       => plugins_url( $plugin_basename ),
					'real_path' => weLaunch_Functions_Ex::wp_normalize_path( dirname( realpath( $file ) ) ),
				);
			}
		...
		public static function is_inside_theme( $file = '' ) {
			$theme_paths = array(
				weLaunch_Functions_Ex::wp_normalize_path( get_template_directory() )   => get_template_directory_uri(),
				weLaunch_Functions_Ex::wp_normalize_path( get_stylesheet_directory() ) => get_stylesheet_directory_uri(),
			);

			$theme_paths = array_unique( $theme_paths );

			$file_path = weLaunch_Functions_Ex::wp_normalize_path( $file );
			$filename  = explode( '/', $file_path );
			$filename  = end( $filename );
			foreach ( $theme_paths as $theme_path => $url ) {

				$real_path = weLaunch_Functions_Ex::wp_normalize_path( realpath( $theme_path ) );

				if ( strpos( $file_path, trailingslashit( $real_path ) ) !== false ) {
					$slug = explode( '/', weLaunch_Functions_Ex::wp_normalize_path( $theme_path ) );
					if ( empty( $slug ) ) {
						continue;
					}
					$slug          = end( $slug );
		...
		public static function get_plugin_options() {
			...
			if ( is_multisite() ) {
				...
				foreach ( $plugins as $file => $plugin ) {
					if ( strpos( $file, 'welaunch-framework.php' ) !== false ) {
						...
						$options                  = get_site_option( 'weLaunchFrameworkPlugin', $defaults );
					}
				}
			}
			...
			if ( empty( $options ) ) {
				$options = get_option( 'weLaunchFrameworkPlugin', $defaults );
			}
		...
		public static function hash_arg() {
			echo esc_html( md5( weLaunch_Functions_Ex::hash_key() . '-welaunch' ) );
			die();
		}

		/**
		 * Adds stats parameters for weLaunch settings. Outside of the main class as the class could also be in use in other ways.
		 *
		 * @param array $options Stats options.
		 *
		 * @return array
		 */
		public static function welaunch_stats_additions( $options ) {
			$options['welaunch'] = array(
				'demo_mode' => get_option( 'weLaunchFrameworkPlugin' ),
			);
		...
		public static function support_args() {
			...
			if ( isset( $_REQUEST['i'] ) && ! empty( $_REQUEST['i'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				$array['key'] = md5( weLaunch_Functions_Ex::hash_key() );
			}
