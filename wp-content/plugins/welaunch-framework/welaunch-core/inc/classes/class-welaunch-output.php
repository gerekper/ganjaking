<?php
/**
 * weLaunch Output Class
 *
 * @class   weLaunch_Output
 * @version 3.0.0
...
if ( ! class_exists( 'weLaunch_Output', false ) ) {

	/**
	 * Class weLaunch_Output
	 */
	class weLaunch_Output extends weLaunch_Class {

		/**
		 * weLaunch_Output constructor.
		 *
		...
		public function __construct( $parent ) {
			...
			do_action( "welaunch/output/{$this->parent->args['opt_name']}/construct", $this );
		...
		public function enqueue() {
					...
					foreach ( $section['fields'] as $fieldk => $field ) {
						if ( isset( $field['type'] ) && 'callback' !== $field['type'] ) {
							...
							if ( ! empty( $core->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class, 'output' ) && $this->can_output_css( $core, $field ) ) {
								...
								// phpcs:ignore WordPress.NamingConventions.ValidHookName
								$field = apply_filters( "welaunch/field/{$core->args['opt_name']}/output_css", $field );
							...
							// phpcs:ignore WordPress.NamingConventions.ValidHookName
							do_action( "welaunch/field/{$core->args['opt_name']}/output_loop", $core, $field, $value, $style_data );

							// phpcs:ignore WordPress.NamingConventions.ValidHookName
							do_action( "welaunch/field/{$core->args['opt_name']}/output_loop/{$field['type']}", $core, $field, $value, $style_data );
			...
			if ( ! empty( $core->typography ) && ! empty( $core->typography ) && filter_var( $core->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
				...
				if ( ! class_exists( 'weLaunch_Typography' ) ) {
					require_once weLaunch_Core::$dir . '/inc/fields/typography/class-welaunch-typography.php';
				}
				$typography = new weLaunch_Typography( null, null, $core );
				...
				if ( $core->args['async_typography'] && ! empty( $core->typography ) ) {
					...
					wp_enqueue_style( 'welaunch-google-fonts-' . $core->args['opt_name'], $url, array(), $version, 'all' );
		...
		public function add_style_attributes( $html = '', $handle = '', $href = '', $media = '' ) {
			if ( weLaunch_Functions_Ex::string_starts_with( $handle, 'welaunch-google-fonts-' ) ) {
		 ...
		 *
		 * @param array  $core       weLaunchFramework core pointer.
		...
		public function output_css() {
			...
			if ( ! empty( $core->outputCSS ) && ( true === $core->args['output_tag'] || ( isset( $_POST['customized'] ) && isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'preview-customize_' . wp_get_theme()->get_stylesheet() ) ) ) ) {
				// phpcs:ignore WordPress.NamingConventions.ValidVariableName, WordPress.Security.EscapeOutput
				echo '<style id="' . esc_attr( $core->args['opt_name'] ) . '-dynamic-css" title="dynamic-css" class="welaunch-options-output">' . $core->outputCSS . '</style>';
			}
		}
		 ...
		 *
		 * @param object $core  weLaunchFramework core pointer.
		...
		private function can_output_css( $core, $field ) {
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$field = apply_filters( "welaunch/field/{$core->args['opt_name']}/_can_output_css", $field );
