<?php
/**
 * weLaunch Enable Gutenberg Class
 *
 * @class   weLaunch_Enable_Gutenberg
 * @version 4.0.0
 * @package weLaunch Framework
 * @author  Dovy Pauksts of weLaunch.io
 */
...
if ( ! class_exists( 'weLaunch_Enable_Gutenberg', false ) ) {
	...
	class weLaunch_Enable_Gutenberg {
		...
		public function __construct( $args = array() ) {
			...
			if ( empty( $args['slug'] ) ) {
				echo 'You must pass a slug to the weLaunch_Enable_Gutenberg() constructor.';
		...
		public static function cleanup_options( $slug = '' ) {
			if ( ! empty( $slug ) ) {
				$obj = new weLaunch_Enable_Gutenberg(
					array(
						'slug' => $slug,
						'name' => '',
					)
		...
		public function display_admin_notice() {
			...
			$data = array(
				'url'     => '',
				'content' => '',
				'header'  => __( 'Gutenberg is currently disabled!', 'welaunch-framework' ),
				'button'  => '',
			);
			...
			if ( isset( $_GET[ $this->decativate_option ] ) || empty( self::$known_plugins ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( isset( $_GET[ $this->decativate_option ] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					// That didn't work.
					$data['header']  = __( 'Hmm, it seems something else is disabling Gutenberg...', 'welaunch-framework' );
					$data['content'] = sprintf( '<p>Well seems like we have more to do. Don\'t worry, we can still fix this! Click the <strong>Enable Gutenberg</strong> button and weLaunch will enable Gutenberg for you.</p>' );
				} elseif ( self::$theme_disabled ) {
					$data['header']   = __( 'Your theme author has disabled Gutenberg!', 'welaunch-framework' );
					$data['content'] .= sprintf( '<p>It looks like your theme has disabled Gutenberg. Don\'t panic though! Click the <strong>Enable Gutenberg</strong> button to the right and weLaunch will enable Gutenberg for you.</p>' );
				} else {
					$data['header']   = __( 'Looks like something has disabled Gutenberg?', 'welaunch-framework' );
					$data['content'] .= sprintf( '<p>Did you know that Gutenberg is disabled? If that\'s intended you can dismiss this notice, not what you intended? Click <strong>Enable Gutenberg</strong> and weLaunch will automatically fix this for you.</p>' );
				}

				$data['url']    = $auto_enable_url;
				$data['button'] = __( 'Enable Gutenberg', 'welaunch-framework' );

			} elseif ( empty( self::$known_plugins ) ) {
				// Disabled by the theme or other.
				$data['header'] = __( 'Your theme', 'welaunch-framework' );
				$data['url']    = $auto_enable_url;
				$data['button'] = __( 'Enable Gutenberg', 'welaunch-framework' );
			} else {
				// Disable Plugins!
				...
				if ( 1 === count( self::$known_plugins ) ) {
					$data['button']  = __( 'Disable Plugin', 'welaunch-framework' );
					$data['content'] = sprintf( '<p>The following plugin is preventing Gutenberg from working: %s. To automatically fix the issue click the <strong>Disable Plugin</strong> button on the right and weLaunch will enable it for you.</p>', $plugins, esc_url( 'https://kinsta.com/blog/gutenberg-wordpress-editor/' ) );
				} else {
					$data['button']  = __( 'Disable Plugins', 'welaunch-framework' );
					$data['content'] = sprintf( '<p>The following plugin is preventing Gutenberg from working: %s. To automatically fix the issue click the <strong>Disable Plugins</strong> button on the right and weLaunch will enable it for you.</p>', $plugins, esc_url( 'https://kinsta.com/blog/gutenberg-wordpress-editor/' ) );
				}
			}
			...
			<div class="notice updated welaunch-notice">
				<div class="welaunch-notice-inner">
					<div class="welaunch-notice-icon">
						...
						<img src="<?php echo esc_url( weLaunch_Core::$url . '/assets/img/icon--color.svg' ); ?>" alt="<?php echo esc_attr__( 'weLaunch WordPress Plugin', 'welaunch-framework' ); ?>"/>
					</div>
					...
					<div class="welaunch-install-now">
						<?php printf( '<a href="%1$s" class="button button-primary welaunch-install-button">%2$s</a>', esc_url( $data['url'] ), esc_html( $data['button'] ) ); ?>
						<a href="<?php echo esc_url( $no_bug_url ); ?>" class="no-thanks"><?php echo esc_html__( 'No Thanks', 'welaunch-framework' ); ?></a>
					</div>
				</div>
			</div>
			<?php
	...
	/*
	 * Instantiate the weLaunch_Enable_Gutenberg class.
	 */
	new weLaunch_Enable_Gutenberg(
		array(
			'slug' => 'welaunch-framework',
