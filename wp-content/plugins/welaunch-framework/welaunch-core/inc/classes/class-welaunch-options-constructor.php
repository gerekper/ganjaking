<?php
/**
 * weLaunch Options Class
 *
 * @class weLaunch_Options
 * @version 3.0.0
...
if ( ! class_exists( 'weLaunch_Options_Object', false ) ) {
	 ...
	 * Class weLaunch_Options
	 */
	class weLaunch_Options_Constructor extends weLaunch_Class {
			...
			foreach ( $sections as $key => $section ) {
				if ( 'fields' === $key ) {
					foreach ( $section as $field ) {
						if ( ! empty( $field['id'] ) && ! empty( $field['data'] ) && ! empty( $options_values[ $field['id'] ] ) && \weLaunch_Helpers::is_integer( $options_values[ $field['id'] ] ) ) {
		...
		public function get() {
			...
			/**
			 * Action 'welaunch/options/{opt_name}/options'
			 *
			 * @param mixed $value option values
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$core->options = apply_filters( "welaunch/options/{$core->args['opt_name']}/options", $core->options, $core->sections );
		 ...
		 * Set a global variable by the global_variable argument
		 *
		 * @param object $core weLaunchFramework core object.
		 *
		 * @since   3.1.5
		 *
		...
		private function set_global_variable( $core ) {
			if ( ! empty( $core->args['global_variable'] ) ) {
				...
				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$GLOBALS[ $options_global ] = apply_filters( "welaunch/options/{$core->args['opt_name']}/global_variable", $core->options );

				// Last save key.
				if ( isset( $core->transients['last_save'] ) ) {
					$GLOBALS[ $options_global ]['WELAUNCH_LAST_SAVE'] = $core->transients['last_save'];
				}

				// Last compiler hook key.
				if ( isset( $core->transients['last_compiler'] ) ) {
					$GLOBALS[ $options_global ]['WELAUNCH_LAST_COMPILER'] = $core->transients['last_compiler'];
				}
		...
		public function register() {
			...
			foreach ( $core->sections as $k => $section ) {
				...
				/**
				 * Filter 'welaunch/options/{opt_name}/section/{section.id}'
				 *
				 * @param array $section section configuration
				 */
				if ( isset( $section['id'] ) ) {
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$section = apply_filters( "welaunch/options/{$core->args['opt_name']}/section/{$section['id']}", $section );
				}
				...
				if ( isset( $section['permissions'] ) && false !== $section['permissions'] ) {
					if ( ! weLaunch_Helpers::current_user_can( $section['permissions'] ) ) {
				...
				if ( isset( $section['fields'] ) ) {
					foreach ( $section['fields'] as $fieldk => $field ) {
						...
						/**
						 * Filter 'welaunch/options/{opt_name}/field/{field.id}'
						 *
						 * @param array $field field config
						 */

						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						$field = apply_filters( "welaunch/options/{$core->args['opt_name']}/field/{$field['id']}/register", $field );
						...
						if ( isset( $field['permissions'] ) && false !== $field['permissions'] ) {
							if ( ! weLaunch_Helpers::current_user_can( $field['permissions'] ) ) {
						...
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						$field = apply_filters( "welaunch/options/{$core->args['opt_name']}/field/{$field['id']}", $field );
						...
						/**
						 * Action 'welaunch/options/{opt_name}/field/field.type}/register'
						 */

						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						do_action( "welaunch/options/{$core->args['opt_name']}/field/{$field['type']}/register", $field );
						...
						if ( ! $display || isset( $this->no_panel_section[ $k ] ) ) {
							...
							if ( isset( $field['disabled'] ) && $field['disabled'] ) {
								$field['label_for'] = 'welaunch_disable_field';
							}

							if ( isset( $field['hidden'] ) && $field['hidden'] ) {
								$field['label_for'] = 'welaunch_hide_field';
							}
			...
			/**
			 * Action 'welaunch/options/{opt_name}/register'
			 *
			 * @param array option sections
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action( "welaunch/options/{$core->args['opt_name']}/register", $core->sections );
		...
		public function default_values() {
			...
			/**
			 * Filter 'welaunch/options/{opt_name}/defaults'
			 *
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$core->options_defaults = apply_filters( "welaunch/options/{$core->args['opt_name']}/defaults", $core->options_defaults, $core->transients['changed_values'] );
		...
		public function validate_options( $plugin_options ) {
			...
			if ( isset( $plugin_options['welaunch-no_panel'] ) ) {
				$keys = explode( '|', $plugin_options['welaunch-no_panel'] );
				foreach ( $keys as $key ) {
					$plugin_options[ $key ] = $core->options[ $key ];
				}
				if ( isset( $plugin_options['welaunch-no_panel'] ) ) {
					unset( $plugin_options['welaunch-no_panel'] );
				}
			}
			...
			if ( is_array( $imported_options ) && ! empty( $imported_options ) && isset( $imported_options['welaunch-backup'] ) && ( 1 === $imported_options['welaunch-backup'] || '1' === $imported_options['welaunch-backup'] ) ) {
				 ...
				 *
				 * @param  &array [&$plugin_options, welaunch_options]
				 */

				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				do_action_ref_array(
					"welaunch/options/{$core->args['opt_name']}/import", // phpcs:ignore WordPress.NamingConventions.ValidHookName
					array(
						&$plugin_options,
						$imported_options,
				...
				setcookie( 'welaunch_current_tab_' . $core->args['opt_name'], '', 1, '/', $time + 1000, '/' );
				$_COOKIE[ 'welaunch_current_tab_' . $core->args['opt_name'] ] = 1;
			...
			if ( ! empty( $plugin_options['defaults'] ) ) {
				...
				/**
				 * Filter: 'welaunch/validate/{opt_name}/defaults'.
				 *
				 * @param  &array [ $this->options_defaults, $plugin_options]
				 */

				// phpcs:ignore WordPress.NamingConventions.ValidHookName
				$plugin_options = apply_filters( "welaunch/validate/{$core->args['opt_name']}/defaults", $core->options_defaults );
			...
			if ( ! empty( $plugin_options['defaults-section'] ) ) {
				if ( isset( $plugin_options['welaunch-section'] ) && isset( $core->sections[ $plugin_options['welaunch-section'] ]['fields'] ) ) {
					if ( empty( $core->options_defaults ) ) {
						$core->options_defaults = $core->_default_values();
					}

					foreach ( $core->sections[ $plugin_options['welaunch-section'] ]['fields'] as $field ) {
					...
					/**
					 * Filter: 'welaunch/validate/{opt_name}/defaults_section'.
					 *
					 * @param  &array [ $this->options_defaults, $plugin_options]
					 */

					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$plugin_options = apply_filters( "welaunch/validate/{$core->args['opt_name']}/defaults_section", $plugin_options );
				}
				...
				unset( $plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['welaunch-section'] );
			...
			/**
			 * Filter: 'welaunch/validate/{opt_name}/before_validation'
			 *
			 * @param  &array [&$plugin_options, welaunch_options]
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$plugin_options = apply_filters( "welaunch/validate/{$core->args['opt_name']}/before_validation", $plugin_options, $core->options );
			...
			/**
			 * Action 'welaunch/options/{opt_name}/validate'
			 *
			 * @param  &array [&$plugin_options, welaunch_options]
			 */
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			do_action_ref_array(
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
				"welaunch/options/{$core->args['opt_name']}/validate",
				array(
					&$plugin_options,
					$core->options,
					$core->transients['changed_values'],
