...
if ( ! class_exists( 'weLaunch_Args', false ) ) {
	...
	class weLaunch_Args {
		...
		/**
		 * Switch to omit social icons if dev_mode is set to true and weLaunch defaults are used.
		 *
		 * @var bool
		 */
		...
		/**
		 * Switch to omit support menu items if dev_mode is set to true and welaunch defaults are used.
		 *
		 * @var bool
		 */
		...
		public function __construct( $parent, $args ) {
			...
			$default = array(
				...
				'font_weights'                     => array(
					array(
						'id'   => '400',
						'name' => __( 'Regular 400', 'welaunch-framework' ),
					),
					array(
						'id'   => '400italic',
						'name' => __( 'Regular 400 Italic', 'welaunch-framework' ),
					),
					array(
						'id'   => '700',
						'name' => __( 'Bold 700', 'welaunch-framework' ),
					),
					array(
						'id'   => '700italic',
						'name' => __( 'Bold 700 Italic', 'welaunch-framework' ),
					),
				),
			...
			$default = apply_filters( 'welaunch/pro/args/defaults', $default );

			$args = weLaunch_Functions::parse_args( $args, $default );
			...
			if ( 'welaunch_extensions_demo' !== $args['opt_name'] && 'welaunch_demo' !== $args['opt_name'] ) {
		...
		private function filters( $args ) {
			/**
			 * Filter 'welaunch/args/{opt_name}'
			 *
			 * @param     array     $args weLaunchFramework configuration
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$args = apply_filters( "welaunch/args/{$args['opt_name']}", $args );

			/**
			 * Filter 'welaunch/options/{opt_name}/args'
			 *
			 * @param     array     $args weLaunchFramework configuration
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$args = apply_filters( "welaunch/options/{$args['opt_name']}/args", $args );
		...
		private function no_errors_please( $args ) {
			...
			if ( empty( $args['menu_title'] ) ) {
				$args['menu_title'] = esc_html__( 'Options', 'welaunch-framework' );
			}

			if ( empty( $args['page_title'] ) ) {
				$args['page_title'] = esc_html__( 'Options', 'welaunch-framework' );
			}
		...
		private function change_demo_defaults( $args ) {
			if ( $args['dev_mode'] || true === weLaunch_Helpers::is_local_host() ) {
				if ( ! empty( $args['admin_bar_links'] ) ) {
					foreach ( $args['admin_bar_links'] as $idx => $arr ) {
						if ( is_array( $arr ) && ! empty( $arr ) ) {
							foreach ( $arr as $x => $y ) {
								if ( strpos( weLaunch_Core::strtolower( $y ), 'welaunch' ) !== false ) {
									$msg = '<strong>' . esc_html__(
										'weLaunch Framework Notice',
										'welaunch-framework'
									) . ' </strong>' . esc_html__(
