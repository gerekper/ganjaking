<?php
/**
 * weLaunch Select2 AJAX Class
 *
 * @class   weLaunch_AJAX_Select2
 * @version 4.0.0
...
if ( ! class_exists( 'weLaunch_AJAX_Select2', false ) ) {

	/**
	 * Class weLaunch_AJAX_Select2
	 */
	class weLaunch_AJAX_Select2 extends weLaunch_Class {

		/**
		 * weLaunch_AJAX_Select2 constructor.
		 *
		...
		public function __construct( $parent ) {
			...
			add_action( "wp_ajax_welaunch_{$parent->args['opt_name']}_select2", array( $this, 'ajax' ) );
		}
		...
		public function ajax() {
			...
			if ( isset( $_REQUEST['nonce'] ) && isset( $_REQUEST['action'] ) ) {
				if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['nonce'] ) ), sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ) ) ) {
					wp_send_json_error( esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'welaunch-framework' ) );
				}

				if ( ! weLaunch_Helpers::current_user_can( $this->parent->args['page_permissions'] ) ) {
					wp_send_json_error( esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'welaunch-framework' ) );
				}
