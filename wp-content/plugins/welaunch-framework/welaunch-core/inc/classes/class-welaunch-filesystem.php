<?php
/**
 * weLaunch Filesystem Class
 *
 * @class weLaunch_Filesystem
 * @version 4.0.0
...
if ( ! class_exists( 'weLaunch_Filesystem', false ) ) {
	...
	class weLaunch_Filesystem {
		...
		public function __construct( $force_no_fs = false ) {
			...
			$this->cache_folder = trailingslashit( $uploads_dir['basedir'] ) . 'welaunch/';
		...
		public function ftp_form() {
			if ( isset( $this->parent->ftp_form ) && ! empty( $this->parent->ftp_form ) ) {
				...
				echo '<strong>' . esc_html__( 'File Permission Issues', 'welaunch-framework' ) . '</strong><br/>' . sprintf( esc_html__( 'We were unable to modify required files. Please ensure that %1$s has the proper read-write permissions, or modify your wp-config.php file to contain your FTP login credentials as %2$s.', 'welaunch-framework' ), '<code>' . esc_url( weLaunch_Functions_Ex::wp_normalize_path( trailingslashit( WP_CONTENT_DIR ) ) . '/uploads/' ) . '</code>', ' <a href="https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants" target="_blank">' . esc_html__( 'outlined here', 'welaunch-framework' ) . '</a>' );
				echo '</p>';
				echo '</div>';
				echo '<h2></h2>';
				echo '</div>';
		...
		public function execute( $action, $file = '', $params = '' ) {
			...
			if ( empty( $this->wp_filesystem ) ) {
				...
				$url = wp_nonce_url( $base, 'welaunch-options' );
		...
		/**
		 * Generates the default weLaunch cache folder.
		 *
		 * @return void
		 */
		private function generate_default_files() {
			...
			if ( ! $this->is_dir( weLaunch_Core::$upload_dir ) ) {
				$this->mkdir( weLaunch_Core::$upload_dir );
			}

			$hash_path = trailingslashit( weLaunch_Core::$upload_dir ) . 'hash';
			if ( ! $this->file_exists( $hash_path ) ) {
				$this->put_contents( $hash_path, weLaunch_Helpers::get_hash() );
			}

			$version_path = trailingslashit( weLaunch_Core::$upload_dir ) . 'version';
			if ( ! $this->file_exists( $version_path ) ) {
				$this->put_contents( $version_path, weLaunch_Core::$version );
			} else {
				$version_compare = $this->get_contents( $version_path );
				if ( weLaunch_Core::$version !== $version_compare ) {
					$this->put_contents( $version_path, weLaunch_Core::$version );
				}
			}
		}
		...
		public function do_action( $action, $file = '', $params = '' ) {
			...
			if ( ! $res ) {
				...
				// translators: %1$s: Upload URL.  %2$s: Codex URL.
				$msg = '<strong>' . esc_html__( 'File Permission Issues', 'welaunch-framework' ) . '</strong><br/>' . sprintf( esc_html__( 'We were unable to modify required files. Please ensure that %1$s has the proper read-write permissions, or modify your wp-config.php file to contain your FTP login credentials as %2$s.', 'welaunch-framework' ), '<code>' . esc_url( weLaunch_Functions_Ex::wp_normalize_path( trailingslashit( WP_CONTENT_DIR ) ) ) . '/uploads/</code>', '<a href="https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants" target="_blank">' . esc_html__( 'outlined here', 'welaunch-framework' ) . '</a>' );

				$data = array(
					'parent'  => self::$instance->parent,
					'type'    => 'error',
					'msg'     => $msg,
					'id'      => 'welaunch-wp-login',
					'dismiss' => false,
				);
		...
		public function get_template( $file ) {
			$panel = new weLaunch_Panel( $this );
			return $panel->get_template( $file );
		}

	}

	weLaunch_Filesystem::get_instance();
}
