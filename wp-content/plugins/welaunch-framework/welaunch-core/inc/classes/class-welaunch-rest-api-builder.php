...
class weLaunch_Rest_Api_Builder {
	...
	/**
	 * weLaunch_Rest_Api_Builder constructor.
	 */
	...
	public function rest_api_init() {
		...
		register_rest_route(
			...
			array(
				'args'                => array(
					'type' => array(
						'description' => __( 'The field type', 'welaunch-framework' ),
						'type'        => 'string',
					),
				),
				'methods'             => WP_REST_Server::READABLE,
	...
	/**
	 * Helper to get the weLaunch fields paths.
	 *
	 * @return array
	 */
	public function get_field_paths() {
		$fields     = array();
		$fields_dir = trailingslashit( weLaunch_Core::$dir ) . 'inc' . DIRECTORY_SEPARATOR . 'fields' . DIRECTORY_SEPARATOR;
		...
		foreach ( $dirs as $path ) {
			...
			$files    = array(
				...
				trailingslashit( $path ) . 'class-welaunch-' . $folder . '.php',
			);
		...
		$data = apply_filters( 'welaunch/fields', $data );
	...
	public function list_fields() {
		...
		foreach ( $field_classes as $folder => $filename ) {
			$class = 'weLaunch_' . ucwords( str_replace( '-', '_', $folder ) );
			...
			if ( $field_class && is_subclass_of( $class, 'weLaunch_Field' ) ) {
	 ...
	 *
	 * @param array $request Pointer to weLaunchFramework object.
	 *
	 * @return array
	 */
	public function get_field( $request = array() ) {
		...
		if ( isset( $field_classes[ mb_strtolower( $type ) ] ) ) {
			$class = 'weLaunch_' . ucwords( str_replace( '-', '_', $type ) );
			if ( ! class_exists( $class ) ) {
				require_once $field_classes[ mb_strtolower( $type ) ];
			}
			$field_class = array( 'weLaunch_' . ucwords( $type ), 'weLaunchFramework_' . ucwords( $type ) );
			$field_class = weLaunch_Functions::class_exists_ex( $field_class );

			if ( $field_class && is_subclass_of( $field_class, 'weLaunch_Field' ) ) {
				/**
				 * Test if the field exists
				 *
				 * @var weLaunch_Descriptor $descriptor
				 */
	...
	public function render_field( $request = array() ) {
		...
		if ( isset( $field_classes[ mb_strtolower( $type ) ] ) ) {
			...
			if ( $field_class && is_subclass_of( $field_class, 'weLaunch_Field' ) ) {
				...
				try {
					$class = new ReflectionClass( 'weLaunchFramework_' . $type );
				 ...
				 * @var weLaunch_Descriptor $descriptor
				 */
				$descriptor = call_user_func( array( 'weLaunchFramework_' . $type, 'get_descriptor' ) );
				$opt_name   = 'welaunch_builder_api';

				$welaunch_instance = new weLaunchFramework( array(), array( 'opt_name' => $opt_name ) );
				...
				$req['id']   = isset( $request['id'] ) ? $request['id'] : 'welaunch_field';
				$req['name'] = isset( $request['name'] ) ? $request['name'] : $req['id'];

				$field = $class->newInstance( $req, $request['example_values'], $welaunch_instance );
				ob_start();
				$field->render();
