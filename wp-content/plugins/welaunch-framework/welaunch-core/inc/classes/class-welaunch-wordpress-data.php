<?php
/**
 * weLaunch WordPress Data Class
 *
 * @class   weLaunch_WordPress_Data
 * @version 3.0.0
...
if ( ! class_exists( 'weLaunch_WordPress_Data', false ) ) {

	/**
	 * Class weLaunch_WordPress_Data
	 */
	class weLaunch_WordPress_Data extends weLaunch_Class {
		...
		/**
		 * weLaunch_WordPress_Data constructor.
		 *
		 * @param mixed $parent weLaunchFramework pointer or opt_name.
		 */
		...
		public function get( $type = false, $args = array(), $current_value = '', $ajax = false ) {
			...
			/**
			 * Filter 'welaunch/options/{opt_name}/pre_data/{type}'
			 *
			 * @param string $data
			 */
			$pre_data = apply_filters( "welaunch/options/{$opt_name}/pre_data/$type", null ); // phpcs:ignore WordPress.NamingConventions.ValidHookName
			...
			$current_data = array();
			if ( empty( $current_value ) && ! weLaunch_Helpers::is_integer( $current_value ) ) {
				$current_value = null;
			} else {
			...
			/**
			 * Filter 'welaunch/options/{opt_name}/data/{type}'
			 *
			 * @param string $data
			 */

			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$data = apply_filters( "welaunch/options/{$opt_name}/data/$type", $data );
			...
			switch ( $type ) {
				...
				case 'menu_location':
					...
					foreach ( $_wp_registered_nav_menus as $k => $v ) {
						$data[ $k ] = $v;
						if ( ! has_nav_menu( $k ) ) {
							$data[ $k ] .= ' ' . __( '[unassigned]', 'welaunch-framework' );
						}
					}
					break;
				...
				case 'font-icons':
					...
					$fonts = $fs->get_contents( weLaunch_Core::$dir . 'assets/css/vendor/elusive-icons.css' );
					if ( ! empty( $fonts ) ) {
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$font_icons = apply_filters( 'welaunch/font-icons', $data );

					/**
					 * Filter 'welaunch/{opt_name}/field/font/icons'
					 *
					 * @param array $font_icons array of elusive icon classes
					 *
					 * @deprecated
					...
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$font_icons = apply_filters( "welaunch/{$opt_name}/field/font/icons", $font_icons );
		...
		private function maybe_translate( &$value, $post_type ) {
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$value = apply_filters( "welaunch/options/{$this->opt_name}/wordpress_data/translate/post_type_value", $value, $post_type );
			...
			if ( function_exists( 'icl_object_id' ) ) {
				if ( has_filter( 'wpml_object_id' ) ) {
					if ( weLaunch_Helpers::is_integer( $value ) ) {
