<?php
/**
 * weLaunch Required Class
 *
 * @class weLaunch_Required
 * @version 4.0.0
...
if ( ! class_exists( 'weLaunch_Required', false ) ) {

	/**
	 * Class weLaunch_Required
	 */
	class weLaunch_Required extends weLaunch_Class {
			...
			switch ( $operation ) {
				...
				case 'equals':
					...
					if ( is_array( $parent_value ) ) {
						foreach ( $parent_value as $idx => $val ) {
							if ( is_array( $check_value ) ) {
								foreach ( $check_value as $i => $v ) {
									if ( weLaunch_Helpers::make_bool_str( $val ) === weLaunch_Helpers::make_bool_str( $v ) ) {
										$return = true;
									}
								}
							} else {
								if ( weLaunch_Helpers::make_bool_str( $val ) === weLaunch_Helpers::make_bool_str( $check_value ) ) {
									$return = true;
								}
							}
						}
						...
						if ( is_array( $check_value ) ) {
							foreach ( $check_value as $i => $v ) {
								if ( weLaunch_Helpers::make_bool_str( $parent_value ) === weLaunch_Helpers::make_bool_str( $v ) ) {
									$return = true;
								}
							}
						} else {
							if ( weLaunch_Helpers::make_bool_str( $parent_value ) === weLaunch_Helpers::make_bool_str( $check_value ) ) {
								$return = true;
							}
						}
					}
