...
/**
 * weLaunch Framework API Class
 * Makes instantiating a weLaunch object an absolute piece of cake.
 *
...
// Don't duplicate me!
if ( ! class_exists( 'weLaunch', false ) ) {

	/**
	 * weLaunch API Class
	 * Simple API for weLaunch Framework
	 *
	 * @since       3.0.0
	 */
	class weLaunch {
		...
		public static function load() {
			add_action( 'after_setup_theme', array( 'weLaunch', 'create_welaunch' ) );
			add_action( 'init', array( 'weLaunch', 'create_welaunch' ) );
			add_action( 'switch_theme', array( 'weLaunch', 'create_welaunch' ) );

			if ( version_compare( PHP_VERSION, '5.5.0', '<' ) ) {
				include_once weLaunch_Core::$dir . 'inc/lib/array-column.php';
			}
		}

		/**
		 * Delay init action function
		 * Delays all weLaunch objects from loaded before `plugins_loaded` runs.
		 */
		public static function delay_init() {
			if ( ! empty( self::$delay_init ) ) {
				...
				foreach ( self::$delay_init as $opt_name ) {
					self::init( $opt_name );
					$parent = weLaunch_Instances::get_instance( $opt_name );
					// translators: This is only shown to developers, should not impact users.
					$msg = sprintf(
						'<strong>%s</strong><br /><code>%s</code> %s',
						esc_html__( 'Warning, Premature Initialization', 'welaunch-framework' ),
						'self::init("' . esc_html( $opt_name ) . '")',
						// translators: This is only shown to developers, should not impact users.
						sprintf( esc_html__( 'was run before the %s hook and was delayed to avoid errors.', 'welaunch-framework' ), '<code>plugins_loaded</code>' )
					);
		...
		public static function init( $opt_name = '' ) {
			if ( ! empty( $opt_name ) ) {
				if ( ! did_action( 'plugins_loaded' ) ) {
					...
					add_action( 'plugins_loaded', array( 'weLaunch', 'delay_init' ) );
				} else {
					// The hook `plugins_loaded` has run, let's get going!
					self::load_welaunch( $opt_name );

					remove_action( 'setup_theme', array( 'weLaunch', 'create_welaunch' ) );
				}
			}
		}

		/**
		 * Retrive weLaunchFramework object.
		 *
		 * @param string $opt_name Panel opt_name.
		 *
		...
		/**
		 * Retrieve all weLaunchFramework Instances.
		 *
		 * @return array|weLaunchFramework[]
		 */
		 ...
		 *
		 * @param object $welaunch_framework weLaunchFramework object.
		 *
		 * @deprecated No longer using camcelCase naming convensions.
		 */
		public static function loadExtensions( $welaunch_framework ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			self::load_extensions( $welaunch_framework );
		}
		 ...
		 * @param object $welaunch_framework weLaunchFramework object.
		 */
		public static function load_extensions( $welaunch_framework ) {
			$instance_extensions = self::get_extensions( $welaunch_framework->args['opt_name'], '' );

			if ( $instance_extensions ) {
				foreach ( $instance_extensions as $name => $extension ) {
					$old_class = str_replace( 'weLaunch_', 'weLaunchFramework_', $extension['class'] );

					if ( ! class_exists( $extension['class'] ) && ! class_exists( $old_class ) ) {
						// In case you wanted override your override, hah.
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						$extension['path'] = apply_filters( 'welaunch/extension/' . $welaunch_framework->args['opt_name'] . '/' . $name, $extension['path'] );
					...
					if ( ! isset( $welaunch_framework->extensions[ $name ] ) ) {
						$field_classes = array( $extension['class'], $old_class );
						$ext_class     = weLaunch_Functions::class_exists_ex( $field_classes );
						if ( false !== $ext_class ) {
							$welaunch_framework->extensions[ $name ] = new $ext_class( $welaunch_framework );
							// Backwards compatibility for extensions.
							if ( ! is_subclass_of( $welaunch_framework->extensions[ $name ], 'weLaunch_Extension_Abstract' ) ) {
								$new_class_name                       = $ext_class . '_extended';
								self::$extension_compatibility        = true;
								$welaunch_framework->extensions[ $name ] = weLaunch_Functions_Ex::extension_compatibility( $welaunch_framework, $extension['path'], $ext_class, $new_class_name, $name );
							}
						} elseif ( is_admin() && true === $welaunch_framework->args['dev_mode'] ) {
		...
		/**
		 * Deprecated function of Load weLaunch Framework.
		 *
		 * @param string $opt_name Panrl opt_name.
		 *
		 * @deprecated No longer using camelCase naming convensions.
		 */
		public static function loadweLaunch( $opt_name = '' ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			self::load_welaunch( $opt_name );
		}
		...
		public static function set_defaults( $opt_name = '' ) {
			// Try to load the class if in the same directory, so the user only have to include the weLaunch API.
			if ( ! class_exists( 'weLaunch_Options_Defaults' ) ) {
				$file_check = trailingslashit( dirname( __FILE__ ) ) . 'class-welaunch-options-defaults.php';
				if ( file_exists( dirname( $file_check ) ) ) {
					include_once $file_check;
					$file_check = trailingslashit( dirname( __FILE__ ) ) . 'class-welaunch-wordpress-data.php';
					if ( file_exists( dirname( $file_check ) ) ) {
						include_once $file_check;
					}
				}
			...
			if ( class_exists( 'weLaunch_Options_Defaults' ) && ! isset( self::$options_defaults[ $opt_name ] ) ) {
				$sections                            = self::construct_sections( $opt_name );
				$wordpress_data                      = ( ! class_exists( 'weLaunch_WordPress_Data' ) ) ? null : new weLaunch_WordPress_Data( $opt_name );
				$options_defaults_class              = new weLaunch_Options_Defaults();
				...
				if ( isset( self::$args[ $opt_name ]['global_variable'] ) && self::$args[ $opt_name ]['global_variable'] ) {
					...
					/**
					 * Filter 'welaunch/options/{opt_name}/global_variable'
					 *
					 * @param array $value option value to set global_variable with
					 */ global $$option_global;

					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					$$option_global = apply_filters( 'welaunch/options/' . $opt_name . '/global_variable', self::$options_defaults[ $opt_name ] );
				}
			}
		}

		/**
		 * Load weLaunch Framework.
		 *
		 * @param string $opt_name Panel opt_name.
		 */
		public static function load_welaunch( $opt_name = '' ) {
			if ( empty( $opt_name ) ) {
				return;
			}
			...
			if ( class_exists( 'weLaunchFramework' ) ) {
				...
				echo '<div id="message" class="error"><p>' . esc_html__( 'weLaunch Framework is not installed. Please install it.', 'welaunch-framework' ) . '</p></div>';
			...
			weLaunch_Functions_Ex::record_caller( $opt_name );
			...
			if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
				add_action( "welaunch/extensions/{$opt_name}/before", array( 'weLaunch', 'loadExtensions' ), 0 );
			}

			$welaunch                   = new weLaunchFramework( $sections, $args );
			self::$init[ $opt_name ] = 1;

			if ( isset( $welaunch->args['opt_name'] ) && $welaunch->args['opt_name'] !== $opt_name ) {
				self::$init[ $welaunch->args['opt_name'] ] = 1;
			}
		}

		/**
		 * Deprecated Create weLaunch instance.
		 *
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function createweLaunch() {       // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			self::create_welaunch();
		}

		/**
		 * Create weLaunch instance.
		 */
		public static function create_welaunch() {
			foreach ( self::$sections as $opt_name => $the_sections ) {
				if ( ! empty( $the_sections ) ) {
					if ( ! self::$init[ $opt_name ] ) {
						self::loadweLaunch( $opt_name );
					}
				}
			}
		}
		...
		public static function get_section( $opt_name = '', $id = '' ) {
			...
			if ( ! empty( $opt_name ) && ! empty( $id ) ) {
				if ( ! isset( self::$sections[ $opt_name ][ $id ] ) ) {
					$id = weLaunch_Core::strtolower( sanitize_html_class( $id ) );
				}
		...
		public static function set_section( $opt_name = '', $section = array(), $replace = false ) {
			...
			if ( ! isset( $section['id'] ) ) {
				if ( isset( $section['type'] ) && 'divide' === $section['type'] ) {
					...
					if ( isset( $section['title'] ) ) {
						$section['id'] = weLaunch_Core::strtolower( sanitize_title( $section['title'] ) );
					} else {
						$section['id'] = time();
					}
				}
			...
			if ( ! empty( $opt_name ) && is_array( $section ) && ! empty( $section ) ) {
				if ( ! isset( $section['id'] ) && ! isset( $section['title'] ) ) {
					self::$errors[ $opt_name ]['section']['missing_title'] = esc_html__( 'Unable to create a section due to missing id and title.', 'welaunch-framework' );
				...
				self::$errors[ $opt_name ]['section']['empty'] = esc_html__( 'Unable to create a section due an empty section array or the section variable passed was not an array.', 'welaunch-framework' );
		...
		public static function get_post_meta( $opt_name = '', $the_post = array(), $key = '', $default = null ) {
			...
			$welaunch = weLaunchFrameworkInstances::get_instance( $opt_name );
			...
			$metaboxes = $welaunch->extensions['metaboxes'];
			...
			if ( ! empty( $opt_name ) && ! empty( $key ) ) {
				...
				if ( ! isset( $values[ $key ] ) ) {
					if ( null === $default ) {
						...
						if ( false !== $field ) {
							$defaults_class = new weLaunch_Options_Defaults();
							$sections       = self::construct_sections( $opt_name );
		...
		public static function set_option( $opt_name = '', $key = '', $option = '' ) {
			...
			if ( '' !== $opt_name && '' !== $key ) {
				$welaunch         = get_option( $opt_name );
				$welaunch[ $key ] = $option;

				return update_option( $opt_name, $welaunch );
			} else {
				return false;
			}
		}
		 ...
		 * @param string $instance   weLaunch instance.
		 */
		private static function check_extension_class_file( $opt_name, $name = '', $class_file = '', $instance = '' ) {
			if ( file_exists( $class_file ) ) {
				...
				$version = weLaunch_Helpers::get_template_version( $class_file );
				...
				$api_check = str_replace(
					array(
						'extension_' . $name,
						'class-welaunch-extension-' . $new_name,
					),
					array(
						$name . '_api',
						'class-welaunch-' . $new_name . '-api',
					),
					$class_file
				);

				if ( file_exists( $api_check ) && ! class_exists( 'weLaunch_' . ucfirst( $name ) ) ) {
		...
		public static function set_extensions( $opt_name, $path, $force = false ) {
			...
			if ( is_dir( $path ) ) {
				...
				$files = array(
					...
					$path . 'class-welaunch-extension-' . $folder_fix . '.php',
				);

				$ext_file = weLaunch_Functions::file_exists_ex( $files );
			...
			if ( true === $force ) {
				if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
					$welaunch = self::instance( $opt_name );

					if ( isset( $welaunch ) ) {
						self::load_extensions( $welaunch );
					}
				}
			}
		}
		...
		private static function get_all_extension() {
			$welaunch = self::all_instances();

			foreach ( $welaunch as $instance ) {
		...
		/**
		 * Gets all loaded extension for the passed weLaunchFramework instance.
		 *
		 * @param string $opt_name Panel opt_name.
		 * @param array  $instance weLaunchFramework instance.
		 */
		public static function get_instance_extension( $opt_name, $instance = array() ) {
			...
			foreach ( $instance->extensions as $name => $extension ) {
				if ( 'widget_areas' === $name ) {
					$new = new weLaunch_Widget_Areas( $instance );
				}
		...
		public static function getExtensions( $opt_name = '', $key = '' ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			_deprecated_function( __CLASS__ . '::' . __FUNCTION__, 'weLaunch 4.0.0', 'self::get_extensions( $opt_name, $key )' );
		...
		public static function get_extensions( $opt_name = '', $key = '' ) {
			if ( empty( $opt_name ) ) {
				...
				foreach ( self::$uses_extensions[ $opt_name ] as $extension ) {
					...
					$extension_class = 'weLaunch_Extension_' . $name;
					$the_data        = array(
						'path'    => $class_file,
						'dir'     => $directory,
		...
		/**
		 * Method to disables weLaunch demo mode popup.
		 */
		public static function disable_demo() {
			add_action( 'weLaunchFrameworkPlugin_admin_notice', 'weLaunch::remove_demo' );
			add_action( 'welaunch_framework_plugin_admin_notice', 'weLaunch::remove_demo' );
		}

		/**
		 * Callback used by self::disable_demo() to remove the demo mode notice from weLaunch.
		 */
		public static function remove_demo() {
			update_option( 'weLaunchFrameworkPlugin_ACTIVATED_NOTICES', '' );
		}

		/**
		 * Function which forces a panel/page to render.
		 *
		 * @param string $welaunch Panel opt_name or weLaunch object.
		 */
		public static function render( $welaunch = '' ) {
			if ( is_string( $welaunch ) ) {
				$welaunch = weLaunch_Instances::get_instance( $welaunch );
				if ( empty( $welaunch ) ) {
					return;
				}
			}
			$enqueue = new weLaunch_Enqueue( $welaunch );
			$enqueue->init();
