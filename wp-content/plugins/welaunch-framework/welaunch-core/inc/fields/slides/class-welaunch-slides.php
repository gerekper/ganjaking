...
// Don't duplicate me!
if ( ! class_exists( 'weLaunch_Slides', false ) ) {

	/**
	 * Main weLaunch_slides class
	 *
	 * @since       1.0.0
	 */
	class weLaunch_Slides extends weLaunch_Field {
		...
		public function set_defaults() {
			$defaults = array(
				...
				'content_title' => __( 'Slide', 'welaunch-framework' ),
			);
		...
		public function render() {
			...
			echo '<div class="welaunch-slides-accordion" data-new-content-title="' . esc_attr( sprintf( __( 'New %s', 'welaunch-framework' ), $this->field['content_title'] ) ) . '">';
			...
			if ( isset( $this->value ) && is_array( $this->value ) && ! empty( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<div class="welaunch-slides-accordion-group"><fieldset class="welaunch-field" data-id="' . esc_attr( $this->field['id'] ) . '"><h3><span class="welaunch-slides-header">' . esc_html( $slide['title'] ) . '</span></h3><div>';
					...
					echo '<div class="welaunch_slides_add_remove">';

					echo '<span class="button media_upload_button" id="add_' . esc_attr( $x ) . '">' . esc_html__( 'Upload', 'welaunch-framework' ) . '</span>';
					...
					echo '<span 
							...
							esc_html__( 'Remove', 'welaunch-framework' ) . '</span>';

					echo '</div>' . "\n";

					echo '<ul id="' . esc_attr( $this->field['id'] ) . '-ul" class="welaunch-slides-list">';
					...
					$placeholder = ( isset( $this->field['placeholder']['title'] ) ) ? esc_attr( $this->field['placeholder']['title'] ) : __( 'Title', 'welaunch-framework' );
					echo '<li>';
					echo '<input 
					...
					if ( $this->field['show']['description'] ) {
						$placeholder = ( isset( $this->field['placeholder']['description'] ) ) ? esc_attr( $this->field['placeholder']['description'] ) : __( 'Description', 'welaunch-framework' );
						echo '<li>';
						echo '<textarea 
					...
					$placeholder = ( isset( $this->field['placeholder']['url'] ) ) ? esc_attr( $this->field['placeholder']['url'] ) : __( 'URL', 'welaunch-framework' );
					...
					echo '<li>';
					echo '<a href="javascript:void(0);" class="button deletion welaunch-slides-remove">' . esc_html__( 'Delete', 'welaunch-framework' ) . '</a>';
					echo '</li>';
			...
			if ( 0 === $x ) {
				echo '<div class="welaunch-slides-accordion-group">';
				echo '<fieldset class="welaunch-field" data-id="' . esc_attr( $this->field['id'] ) . '">';
				echo '<h3>';

				// translators:  Content title for new accordion.
				echo '<span class="welaunch-slides-header">' . esc_html( sprintf( __( 'New %s', 'welaunch-framework' ), esc_attr( $this->field['content_title'] ) ) ) . '</span>';
				echo '</h3>';
				echo '<div>';
				...
				echo '<img class="welaunch-slides-image" id="image_image_id_' . esc_attr( $x ) . '" src="" alt="placeholder" target="_blank" rel="external" />';
				echo '</a>';
				echo '</div>';
				...
				echo '<span class="button remove-image' . esc_attr( $hide ) . '" id="reset_' . esc_attr( $x ) . '" rel="' . esc_attr( $this->parent->args['opt_name'] . '[' . $this->field['id'] ) . '][attachment_id]">' . esc_html__( 'Remove', 'welaunch-framework' ) . '</span>';
			...
			echo '<a href="javascript:void(0);" class="button welaunch-slides-add button-primary" rel-id="' . esc_attr( $this->field['id'] ) . '-ul" rel-name="' . esc_attr( $this->field['name'] . '[title][]' . $this->field['name_suffix'] ) . '">' . esc_html( sprintf( __( 'Add %s', 'welaunch-framework' ), esc_html( $this->field['content_title'] ) ) ) . '</a>';
			echo '<br/>';
		}
		...
		public function enqueue() {
			...
			if ( $this->parent->args['dev_mode'] ) {
				...
				wp_enqueue_style(
					'welaunch-field-slides-css',
					weLaunch_Core::$url . 'inc/fields/slides/welaunch-slides.css',
					array(),
					$this->timestamp,
					'all'
				);
			...
			wp_enqueue_script(
				'welaunch-field-slides-js',
				weLaunch_Core::$url . 'inc/fields/slides/welaunch-slides' . weLaunch_Functions::is_min() . '.js',
				array( 'jquery', 'jquery-ui-core', 'jquery-ui-accordion', 'jquery-ui-sortable', 'welaunch-field-media-js' ),
				$this->timestamp,
				true
			);
		}
...
class_alias( 'weLaunch_Slides', 'weLaunchFramework_Slides' );
