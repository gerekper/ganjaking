...
if ( ! class_exists( 'weLaunch_Dimensions', false ) ) {

	/**
	 * Class weLaunch_Dimensions
	 */
	class weLaunch_Dimensions extends weLaunch_Field {
		...
		public function render() {
			...
			$arr_units = weLaunch_Helpers::$array_units;

			$unit_check   = $arr_units;
			$unit_check[] = false;

			// If units field has a value but is not an acceptable value, unset the variable.
			if ( isset( $this->field['units'] ) && ! weLaunch_Helpers::array_in_array( $this->field['units'], $unit_check ) ) {
				unset( $this->field['units'] );
			}

			// If there is a default unit value  but is not an accepted value, unset the variable.
			if ( isset( $this->value['units'] ) && ! weLaunch_Helpers::array_in_array( $this->value['units'], $unit_check ) ) {
			...
			echo '<fieldset id="' . esc_attr( $this->field['id'] ) . '-fieldset" class="welaunch-dimensions-container" data-id="' . esc_attr( $this->field['id'] ) . '">';
			...
			if ( true === $this->field['width'] ) {
				...
				echo '<input 
						type="text" 
						class="welaunch-dimensions-input welaunch-dimensions-width mini ' . esc_attr( $this->field['class'] ) . '" 
						placeholder="' . esc_html__( 'Width', 'welaunch-framework' ) . '" 
			...
			if ( true === $this->field['height'] ) {
				...
				echo '<input 
						type="text" 
						class="welaunch-dimensions-input welaunch-dimensions-height mini ' . esc_attr( $this->field['class'] ) . '" 
			...
			if ( isset( $this->field['units'] ) && false !== $this->field['units'] ) {
				echo '<div 
						...
						original-title="' . esc_html__( 'Units', 'welaunch-framework' ) . '">';

				echo '<select 
						data-id="' . esc_attr( $this->field['id'] ) . '" 
						data-placeholder="' . esc_html__( 'Units', 'welaunch-framework' ) . '" 
						class="welaunch-dimensions welaunch-dimensions-units select ' . esc_attr( $this->field['class'] ) . '" 
						original-title="' . esc_html__( 'Units', 'welaunch-framework' ) . '" 
		...
		public function enqueue() {
			...
			wp_enqueue_script(
				'welaunch-field-dimensions-js',
				weLaunch_Core::$url . 'inc/fields/dimensions/welaunch-dimensions' . weLaunch_Functions::is_min() . '.js',
			...
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					'welaunch-field-dimensions-css',
					weLaunch_Core::$url . 'inc/fields/dimensions/welaunch-dimensions.css',
					array(),
					$this->timestamp,
					'all'
				);
...
class_alias( 'weLaunch_Dimensions', 'weLaunchFramework_Dimensions' );
