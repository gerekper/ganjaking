...
class Parsedown {
	...
	protected function blockMarkup( $Line ) {
		...
		if ( preg_match( '/^<[\/]?+(\w*)(?:[ ]*+' . $this->regexHtmlAttribute . ')*+[ ]*+(\/)?>/', $Line['text'], $matches ) ) {
			$element = weLaunch_Core::strtolower( $matches[1] );
	...
	protected function blockReference( $Line ) {
		if ( strpos( $Line['text'], ']' ) !== false
			...
			$id = weLaunch_Core::strtolower( $matches[1] );
	...
	protected function inlineLink( $Excerpt ) {
		...
		if ( preg_match( '/^[(]\s*+((?:[^ ()]++|[(][^ )]+[)])++)(?:[ ]+("[^"]*+"|\'[^\']*+\'))?\s*+[)]/', $remainder, $matches ) ) {
			...
			if ( preg_match( '/^\s*\[(.*?)\]/', $remainder, $matches ) ) {
				...
				$definition = weLaunch_Core::strtolower( $definition );

				$extent += strlen( $matches[0] );
			} else {
				$definition = weLaunch_Core::strtolower( $Element['handler']['argument'] );
			}
	...
	protected static function striAtStart( $string, $needle ) {
		...
		if ( $len > strlen( $string ) ) {
			...
			return weLaunch_Core::strtolower( substr( $string, 0, $len ) ) === weLaunch_Core::strtolower( $needle );
		}
	}
