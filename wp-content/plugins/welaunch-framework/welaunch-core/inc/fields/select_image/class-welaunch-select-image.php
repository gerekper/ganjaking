...
if ( ! class_exists( 'weLaunch_Select_Image', false ) ) {

	/**
	 * Class weLaunch_Select_Image
	 */
	class weLaunch_Select_Image extends weLaunch_Field {
		...
		public function render() {
			...
			if ( ! empty( $this->field['options'] ) ) {
				...
				echo '<select 
						...
						class="welaunch-select-item welaunch-select-images ' . esc_attr( $this->field['class'] ) . '"' .
				...
				if ( '' === $this->value ) {
					echo '<img src="#" class="welaunch-preview-image" style="visibility:hidden;" id="image_' . esc_attr( $this->field['id'] ) . '">';
				} else {
					echo '<img src=' . esc_url( $this->value ) . ' class="welaunch-preview-image" id="image_' . esc_attr( $this->field['id'] ) . '">';
				}
		...
		public function enqueue() {
			...
			wp_enqueue_script(
				'welaunch-field-select-image-js',
				weLaunch_Core::$url . 'inc/fields/select_image/welaunch-select-image' . weLaunch_Functions::is_min() . '.js',
			...
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					'welaunch-field-select-image-css',
					weLaunch_Core::$url . 'inc/fields/select_image/welaunch-select-image.css',
					array(),
					$this->timestamp,
					'all'
				);
...
class_alias( 'weLaunch_Select_Image', 'weLaunchFramework_Select_Image' );
