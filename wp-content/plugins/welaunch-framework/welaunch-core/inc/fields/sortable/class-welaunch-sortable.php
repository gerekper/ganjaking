...
if ( ! class_exists( 'weLaunch_Sortable', false ) ) {

	/**
	 * Class weLaunch_Sortable
	 */
	class weLaunch_Sortable extends weLaunch_Field {

		/**
		 * Field Render Function.
		 * Takes the vars and outputs the HTML for the field in the settings
		 *
		 * @since weLaunch_Options 2.0.1
		 */
		public function render() {
			...
			echo '<ul id="' . esc_attr( $this->field['id'] ) . '-list" class="welaunch-sortable ' . esc_attr( $class ) . ' ' . esc_attr( $label_class ) . '">';
		...
		public function enqueue() {
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					'welaunch-field-sortable-css',
					weLaunch_Core::$url . 'inc/fields/sortable/welaunch-sortable.css',
					array(),
					$this->timestamp,
					'all'
				);
			...
			wp_enqueue_script(
				'welaunch-field-sortable-js',
				weLaunch_Core::$url . 'inc/fields/sortable/welaunch-sortable' . weLaunch_Functions::is_min() . '.js',
...
class_alias( 'weLaunch_Sortable', 'weLaunchFramework_Sortable' );
