...
// Don't duplicate me!
if ( ! class_exists( 'weLaunch_Border', false ) ) {

	/**
	 * Class weLaunch_Border
	 */
	class weLaunch_Border extends weLaunch_Field {
		...
		public function render() {
			...
			if ( isset( $this->field['all'] ) && true === $this->field['all'] ) {
				echo '<div class="field-border-input input-prepend"><span class="add-on"><i class="el el-fullscreen icon-large"></i></span><input type="text" class="welaunch-border-all welaunch-border-input mini ' . esc_attr( $this->field['class'] ) . '" placeholder="' . esc_html__( 'All', 'welaunch-framework' ) . '" rel="' . esc_attr( $this->field['id'] ) . '-all" value="' . esc_attr( $this->value['top'] ) . '"></div>';
			}

			echo '<input type="hidden" class="welaunch-border-value" id="' . esc_attr( $this->field['id'] ) . '-top" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[border-top]" value="' . ( isset( $this->value['top'] ) && '' !== $this->value['top'] ? esc_attr( $this->value['top'] ) . 'px' : '' ) . '">';
			echo '<input type="hidden" class="welaunch-border-value" id="' . esc_attr( $this->field['id'] ) . '-right" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[border-right]" value="' . ( isset( $this->value['right'] ) && '' !== $this->value['right'] ? esc_attr( $this->value['right'] ) . 'px' : '' ) . '">';
			echo '<input type="hidden" class="welaunch-border-value" id="' . esc_attr( $this->field['id'] ) . '-bottom" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[border-bottom]" value="' . ( isset( $this->value['bottom'] ) && '' !== $this->value['bottom'] ? esc_attr( $this->value['bottom'] ) . 'px' : '' ) . '">';
			echo '<input type="hidden" class="welaunch-border-value" id="' . esc_attr( $this->field['id'] ) . '-left" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[border-left]" value="' . ( isset( $this->value['left'] ) && '' !== $this->value['left'] ? esc_attr( $this->value['left'] ) . 'px' : '' ) . '">';
			...
			if ( ! isset( $this->field['all'] ) || true !== $this->field['all'] ) {
				...
				if ( true === $this->field['top'] ) {
					echo '<div class="field-border-input input-prepend">
                            ...
                            <input type="text" class="welaunch-border-top welaunch-border-input mini ' . esc_attr( $this->field['class'] ) . '" placeholder="' . esc_html__( 'Top', 'welaunch-framework' ) . '" rel="' . esc_attr( $this->field['id'] ) . '-top" value="' . esc_attr( $this->value['top'] ) . '">
                         </div>';
				}
				...
				if ( true === $this->field['right'] ) {
					echo '<div class="field-border-input input-prepend">
                            ...
                            <input type="text" class="welaunch-border-right welaunch-border-input mini ' . esc_attr( $this->field['class'] ) . '" placeholder="' . esc_html__( 'Right', 'welaunch-framework' ) . '" rel="' . esc_attr( $this->field['id'] ) . '-right" value="' . esc_attr( $this->value['right'] ) . '">
                        </div>';
				}
				...
				if ( true === $this->field['bottom'] ) {
					echo '<div class="field-border-input input-prepend">
                            ...
                            <input type="text" class="welaunch-border-bottom welaunch-border-input mini ' . esc_attr( $this->field['class'] ) . '" placeholder="' . esc_html__( 'Bottom', 'welaunch-framework' ) . '" rel="' . esc_attr( $this->field['id'] ) . '-bottom" value="' . esc_attr( $this->value['bottom'] ) . '">
                        </div>';
				}
				...
				if ( true === $this->field['left'] ) {
					echo '<div class="field-border-input input-prepend">
                            ...
                            <input type="text" class="welaunch-border-left welaunch-border-input mini ' . esc_attr( $this->field['class'] ) . '" placeholder="' . esc_html__( 'Left', 'welaunch-framework' ) . '" rel="' . esc_attr( $this->field['id'] ) . '-left" value="' . esc_attr( $this->value['left'] ) . '">
                        </div>';
				}
			}
			...
			if ( false !== $this->field['style'] ) {
				$options = array(
					'solid'  => esc_html__( 'Solid', 'welaunch-framework' ),
					'dashed' => esc_html__( 'Dashed', 'welaunch-framework' ),
					'dotted' => esc_html__( 'Dotted', 'welaunch-framework' ),
					'double' => esc_html__( 'Double', 'welaunch-framework' ),
					'none'   => esc_html__( 'None', 'welaunch-framework' ),
				);

				echo '<select data-placeholder="' . esc_html__( 'Border style', 'welaunch-framework' ) . '" id="' . esc_attr( $this->field['id'] ) . '[border-style]" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[border-style]" class="tips welaunch-border-style ' . esc_attr( $this->field['class'] ) . '" rows="6" data-id="' . esc_attr( $this->field['id'] ) . '"' . esc_attr( $select2_data ) . '>';
			...
			if ( false !== $this->field['color'] ) {
				...
				echo 'class="color-picker welaunch-border-color welaunch-color welaunch-color-init ' . esc_attr( $this->field['class'] ) . '"';
				echo 'type="text"';
		...
		public function enqueue() {
			...
			$dep_array = array( 'jquery', 'select2-js', 'wp-color-picker', 'welaunch-js' );

			wp_enqueue_script(
				'welaunch-field-border-js',
				weLaunch_Core::$url . 'inc/fields/border/welaunch-border' . $min . '.js',
				$dep_array,
				$this->timestamp,
				true
			);
			...
			if ( $this->parent->args['dev_mode'] ) {
				if ( ! wp_style_is( 'welaunch-color-picker-css' ) ) {
					wp_enqueue_style( 'welaunch-color-picker-css' );
				}

				wp_enqueue_style(
					'welaunch-field-border-css',
					weLaunch_Core::$url . 'inc/fields/border/welaunch-border.css',
					array(),
					$this->timestamp,
					'all'
				);
...
class_alias( 'weLaunch_Border', 'weLaunchFramework_Border' );
