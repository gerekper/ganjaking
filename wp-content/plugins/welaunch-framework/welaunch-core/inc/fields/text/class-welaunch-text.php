...
use weLaunch_Descriptor_Types as RDT;

if ( ! class_exists( 'weLaunch_Text', false ) ) {

	/**
	 * Class weLaunch_Text
	 */
	class weLaunch_Text extends weLaunch_Field {
		...
		public static function make_descriptor() {
			...
			$d->set_info( 'Text', __( 'The Text field accepts any form of text and optionally validates the text before saving the value.', 'welaunch-framework' ) );

			$d->add_field( 'text_hint', __( 'Display a qTip div when active on the text field.', 'welaunch-framework' ), RDT::TEXT )->set_order( 100 )->set_required();
			$d->add_field( 'placeholder', __( 'Placeholder text.', 'welaunch-framework' ), RDT::TEXT )->set_order( 100 )->set_required();
		}
		...
		public function enqueue() {
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style( 'welaunch-field-text-css', weLaunch_Core::$url . 'inc/fields/text/welaunch-text.css', array(), $this->timestamp, 'all' );
			}
		}
...
class_alias( 'weLaunch_Text', 'weLaunchFramework_Text' );
