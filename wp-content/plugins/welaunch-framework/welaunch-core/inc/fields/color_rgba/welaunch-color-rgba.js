...
(function( $ ) {
	...
	welaunch.field_objects.color_rgba         = welaunch.field_objects.color_rgba || {};
	welaunch.field_objects.color_rgba.fieldID = '';

	welaunch.field_objects.color_rgba.hexToRGBA = function( hex, alpha ) {
		var result;
		var r;
		var g;
		var b;
	...
	welaunch.field_objects.color_rgba.init = function( selector ) {
		selector = $.welaunch.getSelector( selector, 'color_rgba' );
		...
		$( selector ).each(
			function() {
				...
				welaunch.field_objects.color_rgba.modInit( el );
				welaunch.field_objects.color_rgba.initColorPicker( el );
			}
		);
	};

	welaunch.field_objects.color_rgba.modInit = function( el ) {
		welaunch.field_objects.color_rgba.fieldID = el.find( '.welaunch-color_rgba-container' ).data( 'id' );
	};

	// Initialize colour picker.
	welaunch.field_objects.color_rgba.initColorPicker = function( el ) {

		// Get field ID.
		var field_id = welaunch.field_objects.color_rgba.fieldID;

		// Get the color scheme container.
		var colorpickerInput = el.find( '.welaunch-color-rgba' );
		...
		var container = el.find( '.welaunch-color-rgba-container' );
		...
		colorpickerInput.spectrum(
			{
				...
				localStorageKey: 'welaunch.color-rgba.' + field_id,
				palette: palette,
				inputText: inputText,
				...
				change: function( color ) {
					...
					if ( 'transparent' !== colorVal ) {
						rgbaVal = welaunch.field_objects.color_rgba.hexToRGBA( colorVal, alphaVal );
					} else {
						rgbaVal = 'transparent';
					}
					...
					welaunch_change( el.find( '.welaunch-color-rgba-container' ) );
				}
			}
		);
	};
