...
// Don't duplicate me!
if ( ! class_exists( 'weLaunch_Background', false ) ) {

	/**
	 * Main weLaunch_background class
	 *
	 * @since       3.1.5
	 */
	class weLaunch_Background extends weLaunch_Field {
		...
		public function render() {
			...
			if ( true === $this->field['background-color'] ) {
				...
				echo 'class="color-picker welaunch-color welaunch-background-input welaunch-color-init ' . esc_attr( $this->field['class'] ) . '"';
			...
			if ( $this->field['background-image'] ) {
				if ( true === $this->field['background-repeat'] ) {
					$array = array(
						'no-repeat' => esc_html__( 'No Repeat', 'welaunch-framework' ),
						'repeat'    => esc_html__( 'Repeat All', 'welaunch-framework' ),
						'repeat-x'  => esc_html__( 'Repeat Horizontally', 'welaunch-framework' ),
						'repeat-y'  => esc_html__( 'Repeat Vertically', 'welaunch-framework' ),
						'inherit'   => esc_html__( 'Inherit', 'welaunch-framework' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-repeat-select" data-placeholder="' . esc_html__( 'Background Repeat', 'welaunch-framework' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-repeat]" class="welaunch-select-item welaunch-background-input welaunch-background-repeat ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-clip'] ) {
					$array = array(
						'inherit'     => esc_html__( 'Inherit', 'welaunch-framework' ),
						'border-box'  => esc_html__( 'Border Box', 'welaunch-framework' ),
						'content-box' => esc_html__( 'Content Box', 'welaunch-framework' ),
						'padding-box' => esc_html__( 'Padding Box', 'welaunch-framework' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-clip-select" data-placeholder="' . esc_html__( 'Background Clip', 'welaunch-framework' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-clip]" class="welaunch-select-item welaunch-background-input welaunch-background-clip ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-origin'] ) {
					...
					echo '<select id="' . esc_attr( $this->field['id'] ) . '-origin-select" data-placeholder="' . esc_html__( 'Background Origin', 'welaunch-framework' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-origin]" class="welaunch-select-item welaunch-background-input welaunch-background-origin ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-size'] ) {
					$array = array(
						'inherit' => esc_html__( 'Inherit', 'welaunch-framework' ),
						'cover'   => esc_html__( 'Cover', 'welaunch-framework' ),
						'contain' => esc_html__( 'Contain', 'welaunch-framework' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-size-select" data-placeholder="' . esc_html__( 'Background Size', 'welaunch-framework' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-size]" class="welaunch-select-item welaunch-background-input welaunch-background-size ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-attachment'] ) {
					$array = array(
						'fixed'   => esc_html__( 'Fixed', 'welaunch-framework' ),
						'scroll'  => esc_html__( 'Scroll', 'welaunch-framework' ),
						'inherit' => esc_html__( 'Inherit', 'welaunch-framework' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-attachment-select" data-placeholder="' . esc_html__( 'Background Attachment', 'welaunch-framework' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-attachment]" class="welaunch-select-item welaunch-background-input welaunch-background-attachment ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
				...
				if ( true === $this->field['background-position'] ) {
					$array = array(
						'left top'      => esc_html__( 'Left Top', 'welaunch-framework' ),
						'left center'   => esc_html__( 'Left center', 'welaunch-framework' ),
						'left bottom'   => esc_html__( 'Left Bottom', 'welaunch-framework' ),
						'center top'    => esc_html__( 'Center Top', 'welaunch-framework' ),
						'center center' => esc_html__( 'Center Center', 'welaunch-framework' ),
						'center bottom' => esc_html__( 'Center Bottom', 'welaunch-framework' ),
						'right top'     => esc_html__( 'Right Top', 'welaunch-framework' ),
						'right center'  => esc_html__( 'Right center', 'welaunch-framework' ),
						'right bottom'  => esc_html__( 'Right Bottom', 'welaunch-framework' ),
					);

					echo '<select id="' . esc_attr( $this->field['id'] ) . '-position-select" data-placeholder="' . esc_html__( 'Background Position', 'welaunch-framework' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-position]" class="welaunch-select-item welaunch-background-input welaunch-background-position ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
					echo '<option></option>';
			...
			if ( $this->field['background-image'] ) {
				...
				$placeholder = isset( $this->field['placeholder'] ) ? $this->field['placeholder'] : esc_html__( 'No media selected', 'welaunch-framework' );

				echo '<input placeholder="' . esc_html( $placeholder ) . '" type="text" class="welaunch-background-input ' . esc_attr( $hide ) . 'upload ' . esc_attr( $this->field['class'] ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-image]" id="' . esc_attr( $this->parent->args['opt_name'] ) . '[' . esc_attr( $this->field['id'] ) . '][background-image]" value="' . esc_url( $this->value['background-image'] ) . '" />';
				...
				echo '<img class="welaunch-option-image" id="image_' . esc_attr( $this->value['media']['id'] ) . '" src="' . esc_url( $this->value['media']['thumbnail'] ) . '" alt="' . esc_attr( $alt ) . '" target="_blank" rel="external" />';
				echo '</a>';
				echo '</div>';
				...
				echo '<span class="button welaunch-background-upload" id="' . esc_attr( $this->field['id'] ) . '-media">' . esc_html__( 'Upload', 'welaunch-framework' ) . '</span>';
				...
				echo '<span class="button removeCSS welaunch-remove-background' . esc_attr( $hide ) . '" id="reset_' . esc_attr( $this->field['id'] ) . '" rel="' . esc_attr( $this->field['id'] ) . '">' . esc_html__( 'Remove', 'welaunch-framework' ) . '</span>';
		...
		public function enqueue() {
			...
			$dep_array = array( 'jquery', 'wp-color-picker', 'select2-js', 'welaunch-js' );

			wp_enqueue_script(
				'welaunch-field-background-js',
				weLaunch_Core::$url . 'inc/fields/background/welaunch-background' . weLaunch_Functions::is_min() . '.js',
				$dep_array,
				$this->timestamp,
				true
			);
			...
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					'welaunch-field-background-css',
					weLaunch_Core::$url . 'inc/fields/background/welaunch-background.css',
					array(),
					$this->timestamp,
					'all'
				);
...
class_alias( 'weLaunch_Background', 'weLaunchFramework_Background' );
