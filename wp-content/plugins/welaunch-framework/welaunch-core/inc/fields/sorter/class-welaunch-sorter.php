...
if ( ! class_exists( 'weLaunch_Sorter', false ) ) {

	/**
	 * Class weLaunch_Sorter
	 */
	class weLaunch_Sorter extends weLaunch_Field {
		...
		private function replace_id_with_slug( $arr ) {
			...
			if ( ! empty( $arr ) ) {
				foreach ( $arr as $id => $name ) {
					...
					if ( is_numeric( $id ) ) {
						$slug = weLaunch_Core::strtolower( $name );
		...
		public function render() {
			...
			if ( is_array( $sortlists ) ) {
				...
				if ( $sortlists ) {
					echo '<fieldset id="' . esc_attr( $this->field['id'] ) . '" class="welaunch-sorter-container welaunch-sorter">';
		...
		public function enqueue() {
			if ( $this->parent->args['dev_mode'] ) {
				wp_enqueue_style(
					'welaunch-field-sorder-css',
					weLaunch_Core::$url . 'inc/fields/sorter/welaunch-sorter.css',
					array(),
					$this->timestamp,
					'all'
				);
			...
			wp_enqueue_script(
				'welaunch-field-sorter-js',
				weLaunch_Core::$url . 'inc/fields/sorter/welaunch-sorter' . weLaunch_Functions::is_min() . '.js',
				array( 'jquery', 'welaunch-js', 'jquery-ui-sortable' ),
				$this->timestamp,
				true
			);
		}
		 ...
		 * @since  weLaunch_Framework 3.1.5
		 */
...
class_alias( 'weLaunch_Sorter', 'weLaunchFramework_Sorter' );
