...
class weLaunch_Customizer_Section extends WP_Customize_Section {
	...
	protected function render_fallback() {
		$classes = 'accordion-section welaunch-section control-section control-section-' . $this->type;
		?>
		<li id="accordion-section-<?php echo esc_attr( $this->id ); ?>" class="<?php echo esc_attr( $classes ); ?>">
			<h3 class="accordion-section-title" tabindex="0">
				...
				?>
				<span class="screen-reader-text"><?php esc_attr_e( 'Press return or enter to expand', 'welaunch-framework' ); ?></span>
			</h3>
			<ul class="accordion-section-content welaunch-main">
				<?php
				if ( isset( $this->opt_name ) && isset( $this->section ) ) {
					// phpcs:ignore WordPress.NamingConventions.ValidHookName
					do_action( "welaunch/page/{$this->opt_name}/section/before", $this->section );
				}
				?>
	...
	protected function render_template() {
		...
		<li id="accordion-section-{{ data.id }}"
			class="welaunch-standalone-section welaunch-customizer-opt-name welaunch-section accordion-section control-section control-section-{{ data.type }}"
			data-opt-name="{{ data.opt_name }}">
			<h3 class="accordion-section-title" tabindex="0">
				{{ data.title }}
				<span class="screen-reader-text"><?php esc_html_e( 'Press return or enter to open', 'welaunch-framework' ); ?></span>
			</h3>
			<ul class="accordion-section-content welaunch-main">
				...
				<li class="customize-section-description-container">
					<div class="customize-section-title">
						<button class="customize-section-back" tabindex="-1">
							<span class="screen-reader-text"><?php esc_html_e( 'Back', 'welaunch-framework' ); ?></span>
						</button>
						<h3>
