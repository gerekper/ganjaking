...
class VI_WNOTIFICATION_Frontend_Notify {
	...
	protected function get_product() {
		...
		if ( $enable_single_product && ( is_product() || isset( $_POST['viwn_pd_id'] ) ) ) {
			...
			if ( is_array( $transient_products ) && count( $transient_products ) ) {
...
//								'time'       => $this->time_substract( $order->get_date_created()->date_i18n( "Y-m-d H:i:s" ) ),
//								'time_org'   => $order->get_date_created()->date_i18n( "Y-m-d H:i:s" ),
//								'first_name' => base64_encode( ucfirst( $order->get_meta( '_billing_first_name' ) ) ),
//								'last_name'  => base64_encode( ucfirst( $order->get_meta( '_billing_last_name' ) ) ),
//								'city'       => base64_encode( ucfirst( $order->get_meta( '_billing_city' ) ) ),
//								'state'      => base64_encode( ucfirst( $order->get_meta( '_billing_state' ) ) ),
//								'country'    => base64_encode( ucfirst( WC()->countries->countries[ $order->get_meta( '_billing_country' ) ] ) ),
//							);
//							if ( ! $product_tmp['thumb'] && $p_data->is_type( 'variation' ) ) {
//								$parent_id = $p_data->get_parent_id();
//								if ( $parent_id ) {
