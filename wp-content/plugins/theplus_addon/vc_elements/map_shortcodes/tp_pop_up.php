<?php 
// Pop Up Elements
if(!class_exists("ThePlus_pop_up")){
	class ThePlus_pop_up{
		function __construct(){
			add_action( 'init', array($this, 'init_tp_pop_up') );
			add_shortcode( 'tp_pop_up',array($this,'tp_pop_up_shortcode'));
		}
		function tp_pop_up_shortcode($atts,$content = null){
			extract( shortcode_atts( array(
				  ...
				  'animation_delay'=>'50',
					...
					$uniqid = uniqid('pop_up');
							
							$data_attr=$pop_button='';
				   ...
				   $icons_after = '<i class="btn-icon button-'.esc_attr($before_after).' '.esc_attr($icon_class).'"></i>';
					...
					$the_button ='<div class="'.esc_attr($btn_align).' ts-button">';
						$the_button .='<div class="pt_plus_button '.$data_class.' " '.$data_attr.'>';
							$the_button .='<div class="button-link-wrap"'.$button_hover_text.'>';
					...
					$the_button .='</div>';		
							...
							if($pop_btn=='image'){
								$bg_image = wp_get_attachment_image_src( $btn_image, 'full',true);
								$bg_image = $bg_image[0];
								$pop_button= '<img src="'.esc_url($bg_image).'" alt="" />';
								$align_image = $img_align;
							}else{
								$align_image = '';
								$pop_button = $the_button;
							}
							$pop_up='<div class="theservice-pop-up '.esc_attr($uniqid).'  '.esc_attr($el_class).' '.esc_attr($animated_class).'" '.$data_attr.' data-animate-type="'.esc_attr($animation_effects).'" data-animate-delay="'.esc_attr($animation_delay_time).'">';
							$pop_up .='<div class="pop-up-btn  '.esc_attr($align_image).' '.esc_attr($btn_align).'">';
								$pop_up .='<a href="#'.esc_attr($uniqid).'" class="theservice-pop-up-'.esc_attr($pop_btn).'" data-lity >'.$pop_button.'</a>' ;
							$pop_up .='</div>';	
							$pop_up .='<div class="lity-hide">';
							$pop_up .='<div id="'.esc_attr($uniqid).'" class="'.esc_attr($el_class).' ">';
								$pop_up .= do_shortcode($content);
							$pop_up .= '</div>';
							$pop_up .= '</div>';
							$pop_up .= '</div>';
					$css_rule = '';	
					...
					$css_rule .= '</style>';
				return $css_rule.$pop_up;
		}
		function init_tp_pop_up(){
			if(function_exists("vc_map"))
			{
				vc_map(array(
						"name" => __('Modal Popups', 'pt_theplus'),
						'base' => "tp_pop_up",
						'icon' => 'tp-popup',
						"description" => esc_html__('Show Elements on Lightbox', 'pt_theplus'),
						'as_parent' => array(
							'except' => 'vc_gmaps'
						),
						...
						'params' => array(
												array(
													'type' => 'dropdown',
													'heading' =>  __('<span class="pt_theplus-vc-toolip tooltip-bottom"><i class="fa fa-question" aria-hidden="true"></i><span class="pt_theplus-vc-tooltip-text">'.esc_html__('You can choose Image or Button on click of that Pop up will be open.','pt_theplus').'</span></span>'.esc_html__('Pop Up Initiator', 'pt_theplus')), 
													'param_name' => 'pop_btn',
													'value' => array(
														__( 'Button', 'pt_theplus' ) => 'text',
												...
												array(
													'type' => 'attach_image',
													'heading' =>  __('<span class="pt_theplus-vc-toolip "><i class="fa fa-question" aria-hidden="true"></i><span class="pt_theplus-vc-tooltip-text">'.esc_html__('This Image will work as a popup initiator. You can upload .jpg, .png, .gif formats.','pt_theplus').'</span></span>'.esc_html__('Upload Image', 'pt_theplus')), 
								...
								array(
								'type' => 'dropdown',
								'heading' =>  __('<span class="pt_theplus-vc-toolip"><i class="fa fa-question" aria-hidden="true"></i><span class="pt_theplus-vc-tooltip-text">'.esc_html__('Choose alignment from Right, Left or Center.','pt_theplus').'</span></span>'.esc_html__('Alignment', 'pt_theplus')), 
							...
							array(
								...
								'heading' =>  __('<span class="pt_theplus-vc-toolip"><i class="fa fa-question" aria-hidden="true"></i><span class="pt_theplus-vc-tooltip-text">'.esc_html__('You can Select Button Styles using this option.','pt_theplus').'</span></span>'.esc_html__('Button Style', 'pt_theplus')), 
								'param_name'  => 'style',
								'admin_label' =>false,
								'simple_mode' => false,
								'dependency' => array(
	...
	if(class_exists('WPBakeryShortCodesContainer') && !class_exists('WPBakeryShortCode_tp_pop_up'))
	{
		class WPBakeryShortCode_tp_pop_up extends WPBakeryShortCodesContainer
		{
