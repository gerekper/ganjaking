...
class WC_Nested_Category_Layout extends Framework\SV_WC_Plugin
{
	...
	public function init_plugin() {
		...
		if ( ! is_admin() ) {
			...
			add_action( 'wp_head', [ $this, 'layout_style' ] );
	...
	public function wp_init() {
		...
		// or a product category page
			...
			// this prevents outputting an empty <ul> element in pages handled by our plugin template
	...
	public function remove_category_template_unnecessary_actions() {
		...
		wc_set_loop_prop( 'total', 0 ); // only if filter added
	...
	public function woocommerce_product_query( $wp_query ) {
		...
		// bail if this is neither a shop page nor a category page
	...
	public function list_categories_and_products() {
		...
		if ( ! $wp_the_query || $wp_the_query->is_home() || ( $wp_the_query->is_front_page() && wc_get_page_id( 'shop' ) !== (int) get_option( 'page_on_front' ) ) ) {
			return;
		}
		...
		$current_category_args = [
			...
			'echo'       => false,
			'show_count' => 0, // note: originally true, which worked fine, except for leaf categories, where it would generate a WordPress SQL Error notice due to the custom taxonomy 'product_cat' with 'term_taxonomy_id IN ()'
		];
		...
		if ( Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.6.0' ) ) {
			$current_category_args['orderby'] = 'menu_order';
			$args['orderby']                  = 'menu_order';
		}

		/**
		 * Filters the main category layout list query args.
		 *
		 * @since 1.18.1
		 *
		 * @param array $args associative array for current category wp_list_categories()
		 */
		$current_category_args = (array) apply_filters( 'wc_nested_category_layout_main_category_args', $current_category_args );
		...
		if ( $is_product_category ) {
			wp_list_categories( $current_category_args );
		}
	 ...
	 *
	 * @internal
	 * @TODO perhaps move this method into an integration class
	 *
	 * @since 1.17.3
	 *
	 * @param \WP_Query|mixed $wp_query WordPress query object
	 * @return void
	 */
	public function handle_third_party_plugins_compatibility( $wp_query ) : void {
		...
		if ( ! $wp_query->is_tax( 'product_cat' ) && $this->is_layered_nav_active() ) {
			$this->disable_nested_categories();
		}
	}
	...
	/**
	 * Determines if it's the main query for products.
	 *
	 * @since 1.17.4
	 *
	 * @param null|mixed|\WP_Query $query WordPress query object
	 * @return bool
	 */
	public function is_product_main_query( $query = null ) : bool {
		global $wp_query;

		if ( ! $query instanceof \WP_Query && $wp_query ) {
			$query = $wp_query;
		}
	...
	/**
	 * Determines whether product filters are active.
	 *
	 * @since 1.19.1
	 *
	 * @return bool
	 */
	public function is_layered_nav_active() : bool {

		return class_exists( 'WC_Query' )
			&& is_callable( 'WC_Query::get_layered_nav_chosen_attributes' )
			&& ! empty( WC_Query::get_layered_nav_chosen_attributes() );
	}
	 ...
	 * @TODO remove this method by version 2.0.0 or by December 2024
	 *
	 * @return bool
	 */
	public function is_woo_product_filter_active() : bool {

		wc_deprecated_function( __METHOD__ , '', __CLASS__ . '::is_layered_nav()' );

		return $this->is_layered_nav_active();
	}


	/**
	 * Nested Category Layout style.
	 *
	 * @since 1.18.1
	 *
	 * @internal
