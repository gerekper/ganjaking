...
if ( ! function_exists( 'woocommerce_nested_category_products_content_section' ) ) {
	...
	function woocommerce_nested_category_products_content_section( $categories, $product_category_ids ) {
		...
		if ( $thumbnail_id = get_term_meta( $category->term_id, 'thumbnail_id', true ) ) {
			 ...
			 * @since 1.0
			 *
			 * @param string $image current category image HTML
			 * @param \WP_Term $category current category object
			 */
			echo (string) apply_filters( 'wc_nested_category_layout_category_image', '<img src="' . $image[0] . '" alt="' . $category->name . '" class="wc-nested-category-layout-category-image" style="padding-bottom: 10px;" />', $category );
		}
		...
		wc_get_template( 'loop/nested-category.php', [ 'woocommerce_product_category_ids' => $product_category_ids, 'category' => $title_product_category ], '', wc_nested_category_layout()->get_plugin_path() . '/templates/' );
	}
