...
class WP_List_Table {
	...
	public function search_box( $text, $input_id ) {
		...
		if ( ! empty( $_REQUEST['post_mime_type'] ) ) // phpcs:ignore WordPress.Security.NonceVerification
			echo '<input type="hidden" name="post_mime_type" value="' . esc_attr( sanitize_text_field( wp_unslash( $_REQUEST['post_mime_type'] ) ) ) . '" />'; // phpcs:ignore WordPress.Security.NonceVerification
		if ( ! empty( $_REQUEST['detached'] ) ) // phpcs:ignore WordPress.Security.NonceVerification
			echo '<input type="hidden" name="detached" value="' . esc_attr( sanitize_text_field( wp_unslash( $_REQUEST['detached'] ) ) ) . '" />'; // phpcs:ignore WordPress.Security.NonceVerification
?>
<p class="search-box">
...
<?php
	...
	public function current_action() {
		if ( isset( $_REQUEST['filter_action'] ) && ! empty( $_REQUEST['filter_action'] ) ) // phpcs:ignore WordPress.Security.NonceVerification
			return false;

		if ( isset( $_REQUEST['action'] ) && -1 != $_REQUEST['action'] ) // phpcs:ignore WordPress.Security.NonceVerification
			return sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ); // phpcs:ignore WordPress.Security.NonceVerification

		if ( isset( $_REQUEST['action2'] ) && -1 != $_REQUEST['action2'] ) // phpcs:ignore WordPress.Security.NonceVerification
			return sanitize_text_field( wp_unslash( $_REQUEST['action2'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
	...
	protected function months_dropdown( $post_type ) {
		...
		if ( ! isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) { // phpcs:ignore WordPress.Security.NonceVerification
			$extra_checks .= " AND post_status != 'trash'";
		} elseif ( isset( $_GET['post_status'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$extra_checks = $wpdb->prepare( ' AND post_status = %s', sanitize_text_field( wp_unslash( $_GET['post_status'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification
		}
...
<?php
		foreach ( $months as $arc_row ) {
			...
			echo wp_kses_post( sprintf( "<option %s value='%s'>%s</option>\n",
...
<?php
	...
	protected function comments_bubble( $post_id, $pending_comments ) {
		...
		if ( $pending_comments ) {
			echo wp_kses_post( sprintf( '<a href="%s" class="post-com-count post-com-count-pending"><span class="comment-count-pending" aria-hidden="true">%s</span><span class="screen-reader-text">%s</span></a>',
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( isset( $_GET['orderby'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$current_orderby = sanitize_text_field( wp_unslash( $_GET['orderby'] ) ); // phpcs:ignore WordPress.Security.NonceVerification
		} else {
			$current_orderby = '';
		}
	...
	protected function single_row_columns( $item ) {
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( 'cb' === $column_name ) {
				...
				echo wp_kses_post( call_user_func(
