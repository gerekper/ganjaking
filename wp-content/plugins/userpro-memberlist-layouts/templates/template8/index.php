	...
	<section id=upmle-members">
		...
		<?php }?>
			...
			<div class="row1">
				...
				<!-- Member 1 -->
				<div class="col-md-4 upmle-user">
				<div class="member-inner-div">
                                    <?php
                                        $default_background_img = userpro_get_option('default_background_img');
                                         $upmle_background_image = $userpro->correct_space_in_url(  $default_background_img );
                                        if ( (isset($user_id) && $userpro->has('custom_profile_bg', $user_id)) || !empty($default_background_img) ) {
                                            $upmle_background_image = $userpro->correct_space_in_url(  userpro_profile_data('custom_profile_bg', $user_id) );
                                        }
                                        ?>
					<div class="member_img" style="background-image:url(<?php echo $upmle_background_image; ?>)">
					...
					<div class="member-details">
						<div class="name"><a href="<?php echo $userpro->permalink($user_id); ?>" class="upmle-name <?php userpro_user_via_popup($args); ?> userpro-transition" data-up_username="<?php echo $userpro->id_to_member($user_id); ?>"><?php echo userpro_profile_data('display_name', $user_id); ?></a></div>
						<div class="title"><a href="<?php echo $userpro->permalink($user_id); ?>" class="upmle-view userpro-flat-btn userpro-transition"><?php _e('View Profile','userpro'); ?></a></div>
