...
namespace Elementor;
class Appside_Icon_Box_Seven_Widget extends Widget_Base {
	...
	public function get_name() {
		return 'appside-icon-box-seven-widget';
	}
	...
	public function get_title() {
		return esc_html__( 'Icon Box:07', 'aapside-master' );
	}
	...
	protected function _register_controls() {
        ...
        $this->add_group_control(Group_Control_Background::get_type(),[
           ...
           'selector' => "{{WRAPPER}} .single-icon-item-07"
        ]);
        $this->add_control(
	        'padding',
	        [
		        ...
		        'size_units' => [ 'px', '%', 'em' ],
		        'selectors' => [
			        '{{WRAPPER}} .single-icon-item-07' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
		        ],
	        ]
        );
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-07' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			'icon_bottom_space',
			[
				'label' => esc_html__( 'Icon Bottom Space', 'plugin-domain' ),
				...
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-07 .img-icon' => 'margin-bottom: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .single-icon-item-07 .icon' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_control( 'icon_color', [
			...
			'selectors' => [
				"{{WRAPPER}} .single-icon-item-07 .icon"          => "color: {{VALUE}}",
				"{{WRAPPER}} .single-icon-item-07 .icon svg path" => "fill: {{VALUE}}",
			]
		] );
		$this->add_control( 'title_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				"{{WRAPPER}} .single-icon-item-07 .content .title" => "color: {{VALUE}}"
			]
		] );
		$this->add_control( 'description_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				"{{WRAPPER}} .single-icon-item-07 .content p" => "color: {{VALUE}}"
			]
		] );
		...
		$this->add_group_control( Group_Control_Typography::get_type(), [
			...
			'selector' => "{{WRAPPER}} .single-icon-item-07 .content .title"
		] );
		$this->add_group_control( Group_Control_Typography::get_type(), [
			'name'     => 'description_typography',
			'label'    => esc_html__( 'Description Typography', 'aapside-master' ),
			'selector' => "{{WRAPPER}} .single-icon-item-07 .content p"
		] );
		$this->end_controls_section();
	}
	...
	protected function render() {
		...
		$this->add_render_attribute( 'icon_box_wrapper', 'class', 'single-icon-item-07' );
		$this->add_render_attribute( 'icon_box_wrapper', 'class', $settings['theme'] );
...
Plugin::instance()->widgets_manager->register_widget_type( new Appside_Icon_Box_Seven_Widget() );