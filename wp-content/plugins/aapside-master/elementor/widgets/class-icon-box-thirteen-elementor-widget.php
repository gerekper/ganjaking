...
namespace Elementor;
class Appside_Icon_Box_Thireen_Widget extends Widget_Base {
	...
	public function get_name() {
		return 'appside-icon-box-thireen-widget';
	}
	...
	public function get_title() {
		return esc_html__( 'Icon Box: 13', 'aapside-master' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default'     => esc_html__( 'Students', 'aapside-master' )
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'     => esc_html__( 'Inspiration comes in many ways and you like.', 'aapside-master' )
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-13 .icon' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-13 .icon' => 'height: {{SIZE}}{{UNIT}};line-height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-13 .icon' => 'font-size: {{SIZE}}{{UNIT}};',
				],
			]
		);
        $this->add_group_control(Group_Control_Background::get_type(),[
           ...
           'selector' => "{{WRAPPER}} .single-icon-item-13 .icon"
        ]);
		$this->add_control(
			'icon_border_radius',
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-13 .icon' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-13 .img-icon' => 'margin-bottom: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .single-icon-item-13 .icon' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_control( 'icon_color', [
			...
			'selectors' => [
				"{{WRAPPER}} .single-icon-item-13 .icon"          => "color: {{VALUE}}",
				"{{WRAPPER}} .single-icon-item-13 .icon svg path" => "fill: {{VALUE}}",
			]
		] );
		$this->end_controls_section();
		$this->start_controls_section(
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .single-icon-item-13 .content .title' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_control( 'title_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				"{{WRAPPER}} .single-icon-item-13 .content .title" => "color: {{VALUE}}"
			]
		] );
		$this->add_control( 'description_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				"{{WRAPPER}} .single-icon-item-13 .content p" => "color: {{VALUE}}"
			]
		] );
		$this->end_controls_section();
		$this->start_controls_section(
		...
		$this->add_group_control( Group_Control_Typography::get_type(), [
			...
			'selector' => "{{WRAPPER}} .single-icon-item-13 .content .title"
		] );
		$this->add_group_control( Group_Control_Typography::get_type(), [
			'name'     => 'description_typography',
			'label'    => esc_html__( 'Description Typography', 'aapside-master' ),
			'selector' => "{{WRAPPER}} .single-icon-item-13 .content p"
		] );
		$this->end_controls_section();
	}
	...
	protected function render() {
		...
		$this->add_render_attribute( 'icon_box_wrapper', 'class', 'single-icon-item-13' );
...
Plugin::instance()->widgets_manager->register_widget_type( new Appside_Icon_Box_Thireen_Widget() );