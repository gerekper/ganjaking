...
namespace Elementor;
class Appside_Testimonial_Five_Widget extends Widget_Base {
	...
	public function get_name() {
		return 'appside-testimonial-five-widget';
	}
	...
	public function get_title() {
		return esc_html__( 'Testimonial: 05', 'aapside-master' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px','%' ],
				'selectors' => [
					"{{WRAPPER}} .single-testimonial-item-04 .author-meta" => 'margin-top: {{SIZE}}{{UNIT}};'
				]
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px','%' ],
				'selectors' => [
					"{{WRAPPER}} .single-testimonial-item-04 .content-area .ratings" => 'margin-bottom: {{SIZE}}{{UNIT}};margin-top: 0;'
				]
			]
		);
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px','%' ],
				'selectors' => [
					"{{WRAPPER}} .single-testimonial-item-04 .content-area .ratings i+i" => 'margin-left: {{SIZE}}{{UNIT}};'
				]
			]
		);
		$this->add_control(
			...
			[
				...
				'toggle' => true,
				'selectors' => [
					"{{WRAPPER}} .single-testimonial-item-04" => "text-align:{{VALUE}}"
				]
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .single-testimonial-item-04 .content-area .ratings' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		$autoplaytimeout = isset($settings['autoplaytimeout']['size']) ? $settings['autoplaytimeout']['size'] : 3000;
		?>
        <div class="testimonial-carousel-wrapper appside-rtl-slider">
            <div class="appside-testimonial-carousel-04 owl-carousel"
                 ...
                 data-margin="<?php echo esc_attr( isset($settings['margin']['size']) ? $settings['margin']['size'] : 0 ); ?>"
            ...
            >
				...
				foreach ( $all_testimonial_items as $item ):
					...
					$image_url = !empty($image_id) ? wp_get_attachment_image_src( $image_id, 'thumbnail', false )[0] : '';
					$image_alt = !empty($image_id) ? get_post_meta( '_wp_attachment_image_alt', $image_id, true ) : '';
					?>
...
Plugin::instance()->widgets_manager->register_widget_type( new Appside_Testimonial_Five_Widget() );
