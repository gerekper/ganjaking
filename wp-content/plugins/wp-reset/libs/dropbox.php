...
class WPRDropboxClient {
	...
	public function SetUseCUrl( $use_it, $curlOptions = array() ) {
		if ( ! $use_it && ! empty( $curlOptions ) ) {
			throw new WPRDropboxException( "not using cURL but specified cURL options" );
		}
	...
	public function DownloadFile( $path, $dest_path = '', $rev = null, $progress_changed_callback = null ) {
		...
		if ( $this->useCurl ) {
			...
			if ( $rh === false ) {
				throw new WPRDropboxException( "HTTP request to $url failed!" );
			}
			...
			while ( ! feof( $rh ) ) {
				if ( ( $s = fwrite( $fh, fread( $rh, self::BUFFER_SIZE ) ) ) === false ) {
					@fclose( $rh );
					@fclose( $fh );
					throw new WPRDropboxException( "Writing to file $dest_path failed!'" );
				}
				$bytes_loaded += $s;
		...
		if ( $meta->size != $bytes_loaded ) {
			throw new WPRDropboxException( "Download size mismatch! (header:{$meta->size} vs actual:{$bytes_loaded}; curl:{$this->useCurl})" );
		}
	...
	private static function &execCurlAndClose( $ch, &$out_response_headers = null ) {
		...
		curl_close( $ch );
		if ( $err_no || $res === false ) {
			throw new WPRDropboxException( "cURL-Error ($err_no): $err_str" );
		}
	...
	static function checkForError(
		...
		if ( ! empty( $resp->error ) ) {
			throw new WPRDropboxException( $resp, $context );
		}
	...
	private function doSingleCall( $path, $params = array(), $content_call = false, &$content = null ) {
		...
		if ( is_null( $resp ) && ! empty( $json ) ) {
			throw new WPRDropboxException( "apiCall($path) failed: $json (URL was $url)" );
		}
		if ( ( $resp === false || is_null( $resp ) ) && ! empty( $json ) && ! $content_call ) {
			throw new WPRDropboxException( "Error apiCall($path): $json" );
		}
	...
	function GetRequestToken() {
		throw new WPRDropboxException( 'GetRequestToken() has been removed with v2 API. Request tokens do not exist in OAuth2 anymore.' );
	}
	...
	function GetAccessToken() {
		throw new WPRDropboxException( 'GetAccessToken() has been removed with v2 API. Use GetBearerToken() instead!' );
	}

}

class WPRDropboxException extends Exception {
	private $tag;
