...
class WPR_Shuttle_Dump_File_Gzip extends WPR_Shuttle_Dump_File {
	function open() {
		return gzopen($this->file_location, 'a');
	}
	function write($string) {
...
class WPR_Shuttle_Dumper_ShellCommand extends WPR_Shuttle_Dumper {
	function dump($export_file_location, $table_prefix='') {
        global $wp_reset;
        $wp_reset->log('success', 'Dump table ' . $table_prefix . ' via shell to ' . $export_file_location);
        $command = 'mysqldump --max_allowed_packet=1000000000 -h ' . escapeshellarg($this->db->host) .
		...
		if ($return_val !== 0) {
            ...
            $wp_reset->log('error', 'Dump error ' . $error_text);
...
class WPR_Shuttle_Dumper_Native extends WPR_Shuttle_Dumper {
	public function dump($export_file_location, $table_prefix='') {
        ...
        $wp_reset->log('success', 'Exporting ' . $table_prefix . ' to ' . $export_file_location . ' which has a size of ' . (file_exists($export_file_location)?filesize($export_file_location):'0'));
        ...
        $this->dump_file->write("-- Port: " . $this->db->port . $eol);
        ...
        $this->dump_file->end();
        $wp_reset->log('success', 'Closed export of ' . $table_prefix . ' to ' . $export_file_location . ' which has a size of ' . filesize($export_file_location));
	...
	protected function dump_table($table) {
        global $wp_reset;
        $eol = $this->eol;
        $wp_reset->log('success', 'Dump table ' . $table);
