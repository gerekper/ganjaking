...
/**
 * WC_Newsletter_Subscription_Admin_Notices Class.
 */
class WC_Newsletter_Subscription_Admin_Notices {
	...
	public static function load_dismiss_notices() {
		...
		foreach ( $notices as $notice ) {
			if ( 0 !== strpos( $notice, 'wc_newsletter_subscription_' ) ) {
				continue;
			}
	...
	protected static function get_notice_content_from_file( $notice ) {
		$file = str_replace( array( 'wc_newsletter_subscription_', '_' ), array( '', '-' ), $notice );
...
WC_Newsletter_Subscription_Admin_Notices::init();
