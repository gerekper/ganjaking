...
class THEMECOMPLETE_EPO_Associated_Products {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_cart_shipping_packages', [ $this, 'woocommerce_cart_shipping_packages' ], 5 );
	...
	public function wc_epo_get_associated_product_html() {
		...
		if ( isset( $_REQUEST['layout_mode'] ) && isset( $_REQUEST['product_id'] ) && isset( $_REQUEST['name'] ) && isset( $_REQUEST['uniqid'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$counter          = isset( $_REQUEST['counter'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['counter'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( ! empty( $product ) && is_object( $product ) ) {
				...
				$args = [
					'tm_element_settings'               => [ 'uniqid' => $uniqid ],
					'option'                            => [
				...
				if ( ! empty( $package['contents'] ) ) {
					...
					foreach ( $package['contents'] as $cart_item_key => $cart_item_data ) {
						...
						if ( isset( $cart_item_data['associated_products'] ) && ! empty( $cart_item_data['associated_products'] ) ) {
							...
							$main_product     = $cart_item_data['data'];
							...
							if ( $main_product->needs_shipping() ) {
								...
								if ( $associated_value > 0 ) {
									...
									$main_product->set_price( (float) $main_product_price + $associated_value / $main_product_qty );
								}
	...
	public function associated_woocommerce_add_to_cart( $parent_cart_key, $parent_id, $parent_quantity, $variation_id, $variation, $cart_item_data ) {
		...
		if ( isset( $cart_item_data['tmproducts'] ) && ! empty( $cart_item_data['tmproducts'] ) && is_array( $cart_item_data['tmproducts'] ) ) {
			...
			foreach ( $cart_item_data['tmproducts'] as $key => $associated_data ) {
				...
				$associated_item_cart_data['associated_formprefix']           = str_replace( [ '.', ' ', '[' ], '', $associated_data['form_prefix'] );
