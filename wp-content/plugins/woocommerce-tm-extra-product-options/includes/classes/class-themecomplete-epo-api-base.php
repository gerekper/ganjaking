...
final class THEMECOMPLETE_EPO_API_Base {
	...
	public function get_epos( $epos = [], $option_id = 'all', $output_format = 'array', $mt_prefix = '', $product_id = 0 ) {
		...
		$all_epos = $this->get_epo_data_from_order( $epos, $option_id, $mt_prefix, $wpml_translation_by_id );
	...
	public function get_epos_data( $item = [], $item_meta = [], $option_id = 'all', $_product = false, $mt_prefix = '', $wpml_translation_by_id = [] ) {
		...
		return $this->get_epo_data_from_order( $epos, $option_id, $mt_prefix, $wpml_translation_by_id, $args );
	}
	 ...
	 * @return array<mixed>
	 */
	public function get_epo_data_from_order( $epos = [], $option_id = 'all', $mt_prefix = '', $wpml_translation_by_id = [], $args = [] ) {
		$all_epos = [];
		if ( $epos && is_array( $epos ) ) {
			...
			foreach ( $epos as $key => $epo ) {
				if ( $epo && is_array( $epo ) ) {
					...
					if ( 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_show_image_replacement' ) && ! empty( $epo['use_images'] ) && ! empty( $epo['images'] ) && 'images' === $epo['use_images'] ) {
					...
					if ( 'no' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_show_hide_uploaded_file_url_cart' ) && 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_show_upload_image_replacement' ) && isset( $epo['element'] ) && isset( $epo['element']['type'] ) && 'upload' === $epo['element']['type'] ) {
					...
					if ( isset( $item ) && isset( $item_meta ) && isset( $_product ) ) {
	...
	public function get_option( $order_id, $option_id = '', $return_type = 'array' ) {
		...
		foreach ( $line_items as $item_id => $item ) {
			...
			if ( $has_epo ) {
				...
				$all_epos[ $item_id ] = $this->get_epo_data_from_order( $epos, $option_id, $mt_prefix, $wpml_translation_by_id, $args );
			}
