...
class THEMECOMPLETE_EPO_FIELDS {
	 ...
	 * @param array<mixed>|false $element The element array.
	...
	public function __construct( $product_id = false, $element = false, $per_product_pricing = false, $cpf_product_price = false, $variation_id = false, $post_data = null ) {
		...
		if ( is_null( $post_data ) && isset( $_POST ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$post_data = stripslashes_deep( $_POST ); // phpcs:ignore WordPress.Security.NonceVerification
		}
		if ( empty( $post_data ) && isset( $_REQUEST['tcajax'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$post_data = stripslashes_deep( $_REQUEST ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		}
	 ...
	 *
	 * @param array<mixed>|false $epo_post_fields The posted fields array.
	 * @param array<mixed>|false $element The element array.
	...
	final public function add_cart_item_data( $attribute = false, $key = false, $key_id = 0, $keyvalue_id = 0 ) {
		...
		if ( ! is_array( $string ) ) {
			$last_underscore_pos = strrpos( $string, '_' );

			if ( false !== $last_underscore_pos && substr_count( $string, '_' ) > 1 ) {
				$result    = substr( $string, 0, $last_underscore_pos );
				$this->key = $result;
			}
		}
		...
		if ( false !== $ret ) {
			...
			if ( 'math' === $_price_type ) {
				...
				if ( str_contains( $_price, '{quantity}' ) || str_contains( $_price, '{product_price}' ) || THEMECOMPLETE_EPO()->has_math_special_variable( $_price ) ) {
