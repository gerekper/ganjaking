...
<?php else : ?>
	...
	$input_args = [
		...
		'tags'       => [
			'id'                    => $id,
			'name'                  => $name,
			'class'                 => $fieldtype . ' tm-epo-field tmcp-checkbox' . $use,
			'data-price'            => '',
			'data-rules'            => $rules,
			'data-original-rules'   => $original_rules,
			'data-rulestype'        => $rules_type,
			'data-limit'            => $limit,
			'data-exactlimit'       => $exactlimit,
			'data-minimumlimit'     => $minimumlimit,
			'data-image'            => $image,
			'data-imagec'           => $imagec,
			'data-imagep'           => $imagep,
			'data-imagel'           => $imagel,
	...
	if ( isset( $label_mode ) && ! empty( $label_mode ) && 'text' !== $label_mode ) {
		...
		switch ( $label_mode ) {
			case 'images':
				if ( ! empty( $src ) ) {
					echo '<img class="' . esc_attr( $border_type ) . esc_attr( $tmlazy ) . 'checkbox-image' . esc_attr( $swatch_class ) . '" '
						. 'alt="' . esc_attr( wp_strip_all_tags( $label_to_display ) ) . '" ';
					echo wp_kses_post( $src );
					echo wp_kses_post( $swatch_html );
					echo ' />';
				}
				echo '<span class="tc-label checkbox-image-label">' . apply_filters( 'wc_epo_kses', wp_kses_post( $label_to_display ), $label_to_display ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput
				break;
			case 'startimages':
				if ( ! empty( $src ) ) {
					echo '<img class="' . esc_attr( $border_type ) . esc_attr( $tmlazy ) . 'checkbox-image' . esc_attr( $swatch_class ) . '" '
				...
				if ( ! empty( $label_to_display ) ) {
					echo '<span class="tc-label checkbox-image-label-inline">' . apply_filters( 'wc_epo_kses', wp_kses_post( $label_to_display ), $label_to_display ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput
				}
				break;
			case 'endimages':
				if ( ! empty( $label_to_display ) ) {
				...
				break;
			case 'color':
				echo '<span class="tmhexcolorimage ' . esc_attr( $border_type ) . 'checkbox-image' . esc_attr( $swatch_class ) . '" '
					. 'alt="' . esc_attr( wp_strip_all_tags( $label_to_display ) ) . '" ';
				echo wp_kses_post( $swatch_html );
				echo '></span>'
					. '<span class="tc-label checkbox-image-label">' . apply_filters( 'wc_epo_kses', wp_kses_post( $label_to_display ), $label_to_display ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput
				break;
			case 'startcolor':
				echo '<span class="tmhexcolorimage ' . esc_attr( $border_type ) . 'checkbox-image' . esc_attr( $swatch_class ) . '" '
					. 'alt="' . esc_attr( wp_strip_all_tags( $label_to_display ) ) . '" ';
