...
<li class="<?php echo esc_attr( $liclass ); ?>"><div class="tmcp-field-wrap-inner">
	...
	<?php else : ?>
		...
		if ( apply_filters( 'wc_epo_checkbox_print_required_attribute', true ) && 'yes' === THEMECOMPLETE_EPO()->tm_epo_global_enable_validation && isset( $required ) && ! empty( $required ) ) {
		...
		if ( isset( $label_mode ) && ! empty( $label_mode ) && 'text' !== $label_mode ) {
			...
			$text_label_class = 'checkbox-image-label' . ( ( THEMECOMPLETE_EPO_HELPER()->str_startswith( $label_mode, 'start' ) || THEMECOMPLETE_EPO_HELPER()->str_startswith( $label_mode, 'end' ) ) ? '-inline' : '' );
			switch ( $label_mode ) {
				case 'images':
				case 'startimages':
				case 'endimages':
