...
class RsExplodinglayersSliderFront extends RevSliderFunctions {
		...
		foreach($slides as $slide){
			...
			foreach($layers as $layer){
				...
				foreach($frames as $frame){
					if($this->get_val($frame, array('explodinglayers', 'use'), false) === true){
						$enabled = true;
						break;
					}
				}
	...
	public function add_scripts(){
		...
		$_jsPathMin = file_exists(RS_EXPLODINGLAYERS_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['explodinglayers'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
