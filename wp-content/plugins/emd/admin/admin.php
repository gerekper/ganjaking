...
class userpro_ed_admin {
	...
	function __construct() {
		...
		$this->slug = 'userpro';
		$this->subslug = 'userpro-emd';
		require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		$this->plugin_data = get_plugin_data( userpro_ed_path . 'index.php', false, false);
	...
	function admin_init() {
		...
		$this->tabs = array(
			'settings' => __('EMD Settings','userpro')
		);
		$this->default_tab = 'settings';
		
		$this->options = get_option('userpro_ed');
		if (!get_option('userpro_ed')) {
			update_option('userpro_ed', userpro_ed_default_options() );
		}
	...
	function add_menu() {
		add_submenu_page( 'userpro', __('Enhanced Member Directory','userpro'), __('Enhanced Member Directory','userpro'), 'manage_options', 'userpro-emd', array(&$this, 'admin_page') );
	}
	...
	function get_tab_content() {
		...
		if( strstr($screen->id, $this->subslug ) ) {
			...
			require_once userpro_ed_path.'admin/panels/'.$tab.'.php';
		}
	}
	...
	function save() {
		...
		update_option('userpro_ed', $this->options);
		echo '<div class="updated"><p><strong>'.__('Settings saved.','userpro').'</strong></p></div>';
	}

	function reset() {
		update_option('userpro_ed', userpro_ed_default_options() );
		$this->options = array_merge( $this->options, userpro_ed_default_options() );
	...
	function admin_page() {
        if ( current_user_can('manage_options') ) {
            ...
            if ( isset( $_POST['reset-options'] ) && wp_verify_nonce( $_POST['user_pro_nonce'], 'user_pro_nonce' ) ) {
                $this->reset();
            }
...
$userpro_ed_admin = new userpro_ed_admin();