...
class Advanced_Counter extends Module_Base {
				...
				<div class="bdt-ep-advanced-counter-icon-wrap">
					<span class="bdt-ep-advanced-counter-icon">
						...
						<?php } elseif ($has_image and 'image' == $settings['icon_type']) { 
							...
							if (!$thumb_url) {
							printf('<img src="%1$s" alt="%2$s">', $settings['image']['url'], esc_html($settings['content_text']));
							} else {
	...
	?>
		...
		<?php if ($settings['content_number']) : ?>
			<div <?php echo $this->get_render_attribute_string('number'); ?>>
				<span class="bdt-count-this" id="bdt-ep-advanced-counter-data-<?php echo esc_attr($this->get_id()); ?>" <?php echo $this->get_render_attribute_string('content_number'); ?>>
					<?php echo wp_kses($settings['content_number'], element_pack_allow_tags('title')); ?>
				</span>
			</div>
		<?php endif; ?>
