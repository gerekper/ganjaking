...
class Acf_Slider extends Module_Base {
	...
	public function get_script_depends() {
		if ($this->ep_is_edit_mode()) {
			...
			return ['imagesloaded', 'ep-acf-slider'];
		}
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
            'link',
            [
                'label' => __('Link', 'bdthemes'),
                'dynamic' => ['active' => false],
                'type'        => Dynamic_Select::TYPE,
                'label_block' => true,
                'placeholder' => __('Type and select the link field...', 'bdthemes-element-pack'),
                'query_args'  => [
                    'query'        => 'acf',
                    //'field_type' => 'repeater',
                ],
						...
						<?php endif; ?>
							...
							<div class="bdt-slide-desc bdt-position-large bdt-position-<?php echo ($settings['origin']); ?> bdt-position-z-index">
								...
								<?php if (!empty($value[$content])) : ?>
									<div class="bdt-slide-text"><?php echo $this->parse_text_editor($value[$content]); ?></div>
								<?php endif; ?>
