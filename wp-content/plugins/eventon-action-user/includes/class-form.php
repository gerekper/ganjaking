					...
					if( ($SELECTED_FIELDS && in_array($INDEX, $SELECTED_FIELDS) )
						...
						// Other general fields
							...
							case 'virtual':
								...
								$evo_vir_fields = array();
								$evo_vir_fields['_vir_url']= array(
									'type'=>	'text',
									'name'=>	evo_lang('Virtual Event URL'),
									'value'=>	($this->EVENT ? $this->EVENT->get_prop('_vir_url'):''),
								);
								$evo_vir_fields['_vir_pass']= array(
									'type'=>	'text',
								...
								?><input type='hidden' name='_virtual_type' value='other_live'/><?php 
									...
									foreach($evo_vir_fields as $field=>$data){
										echo $this->get_form_html($field, $data);
									}
								echo "</div>";
							break;
						}
		...
		function _print_date_time_picker($A){
			...
			"<span class='evo_date_time_select' data-sow='{$sow}'> 
				<span class='evo_date_select'>
					<input id='evoAU_end_date_{$rand}' class='". ($disable_date_editing?'':"datepicker{$type}date")." ". ($assoc != 'rp'? 'req':'')." end evoau_dpicker ' readonly='true' type='text' data-role='none' name='event_{$type}_date' value='".$date_val."' data-assoc='{$assoc}'/>
...
// form footer
	function get_form_footer($atts, $_EDITFORM, $LIMITSUB){
		...
		<div class='evoau_json' style='display:none' data-j='<?php 
				$nofs = array(
