	...
	// submission form
		function output_event_submission_form($atts){
			...
			if(!empty($atts['lightbox']) && $atts['lightbox']=='yes'){
				?>
				<a id='evoau_lightbox_form_btn' data-mce='0' class='evoAU_form_trigger_btn ' data-d='<?php echo json_encode($atts);?>'><?php echo !empty($atts['btntxt'])?$atts['btntxt']:'Submit New Event';?></a>
				<?php
			}else{
		...
		function save_form_submissions(){
			...
			if($created_event_id){	
				...
				// Save limit submissions value
					if( isset($_POST['evoau_limit_submissions']) && $_POST['evoau_limit_submissions'] == 'yes' && $is_user_logged_in){
				...
				// save language of the event
					if(!empty($_POST['_evo_lang']))
						$this->create_custom_fields($created_event_id, '_evo_lang', $_POST['_evo_lang']);
