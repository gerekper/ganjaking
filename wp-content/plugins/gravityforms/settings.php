...
class GFSettings {
	...
	private static function plugin_settings_fields() {
		...
		return array(
					...
					array(
						...
						'after_input'       => function () {
							...
							if ( $version_info['is_valid_key'] ) {
								$license_key_alert = sprintf( '<div class="alert success">%s</div>', esc_html__( 'Your support license key has been successfully validated.', 'gravityforms' ) );
							} else if ( ! $version_info['is_valid_key'] && ! empty( $license_key ) ) {
								$license_key_alert = sprintf( '<div class="alert error">%s</div>', esc_html__( 'The provided license key is invalid.', 'gravityforms' ) );
							}
			...
			array(
				...
				'description'   => esc_html__( 'Enable this option if you would like to disable the plugin from outputting HTML5 form fields.', 'gravityforms' ),
				'class'         => 'gform-settings-panel--half',
				'default_value' => true,
				'fields'        => array(
					array(
	...
	public static function initialize_plugin_settings() {
		...
		$initial_values = array(
			...
			'enable_html5'              => (bool) get_option( 'rg_gforms_enable_html5' ),
		...
		$renderer = new Rocketgenius\Gravity_Forms\Settings(
			array(
				...
				'save_callback'     => function( $values ) {
					...
					// Enable HTML5.
					update_option( 'rg_gforms_enable_html5', (bool) rgar( $values, 'enable_html5' ) );
