...
class GF_Field_FileUpload extends GF_Field {
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$default_describedby = "gfield_description_{$this->formId}_{$this->id}";
		...
		if ( $multiple_files ) {
			...
			$upload             = "<div id='{$container_id}' data-settings='{$plupload_init_json}' class='gform_fileupload_multifile'>
										<div id='{$drag_drop_id}' class='gform_drop_area'>
											...
											<button id='{$browse_button_id}' class='button gform_button_select_files' aria-describedby='{$rules_messages_id} {$default_describedby} {$validation_message_id}' {$tabindex} >{$select_files_text}</button>
										</div>
									</div>";
			...
			$upload .= sprintf( "<input name='input_%d' id='%s' type='file' class='%s' aria-describedby='%s %s %s %s' onchange='javascript:gformValidateFileSize( this, %s );' {$tabindex} %s/>", $id, $field_id, esc_attr( $class ), $default_describedby, $validation_message_id, $live_validation_message_id, $rules_messages_id, esc_attr( $max_upload_size ), $disabled_text );
