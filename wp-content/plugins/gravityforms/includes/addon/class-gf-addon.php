...
abstract class GFAddOn {
	...
	public function set_settings_renderer( $renderer ) {
		...
		// Ensure renderer is an instance of Rocketgenius\Gravity_Forms\Settings.
	...
	public function prepare_settings_fields( &$fields = array() ) {
		...
		foreach ( $fields as &$field ) {
			...
			if ( $this->method_is_overridden( 'settings_' . $field['type'] ) ) {
				$field['callback'] = array( $this, 'settings_' . $field['type'] );
			} else if ( method_exists( $this, 'settings_' . $field['type'] ) && ! method_exists( get_parent_class( $this ), 'settings_' . $field['type'] ) ) {
				$field['callback'] = array( $this, 'settings_' . $field['type'] );
			}

			if ( $this->method_is_overridden( 'validate_' . $field['type'] ) ) {
				$field['validation_callback'] = array( $this, 'validate_' . $field['type'] );
			} else if ( method_exists( $this, 'validate_' . $field['type'] ) && ! method_exists( get_parent_class( $this ), 'validate_' . $field['type'] ) ) {
				$field['validation_callback'] = array( $this, 'validate_' . $field['type'] );
			}
	...
	public function single_setting_row( $field ) {
		...
		if ( $this->get_settings_renderer() ) {
			...
			// Initialize field.
			$field = \Rocketgenius\Gravity_Forms\Settings\Fields::create( $field, $this->get_settings_renderer() );
	...
	public function settings_text( $field, $echo = true ) {
		...
		$field = \Rocketgenius\Gravity_Forms\Settings\Fields::create(
			$field,
	...
	public function checkbox_input( $choice, $attributes, $value, $tooltip ) {
		return \Rocketgenius\Gravity_Forms\Settings\Fields\Checkbox::render_input( $choice, $attributes, $value, $tooltip );
	}
	...
	public function choices_have_icon( $choices = array() ) {
		...
		return \Rocketgenius\Gravity_Forms\Settings\Fields\Base::has_icons( $choices );
	...
	public function get_select_options( $choices, $selected_value ) {
		return \Rocketgenius\Gravity_Forms\Settings\Fields\Select::get_options( $choices, $selected_value );
	}
	...
	public function get_default_field_select_field( $field ) {
		...
		if ( ! is_a( $field, 'Rocketgenius\Gravity_Forms\Settings\Field\Field_Select' ) ) {
			$field['type'] = 'field_select';
			$field         = \Rocketgenius\Gravity_Forms\Settings\Fields::create( $field, $this->get_settings_renderer() );
		}
	...
	public function get_form_fields_as_choices( $form, $args = array() ) {
		 ...
		 *
		 * @var \Rocketgenius\Gravity_Forms\Settings\Fields\Field_Select|WP_Error $field
		 */
		$field = \Rocketgenius\Gravity_Forms\Settings\Fields::create(
			array(
				'type' => 'field_select',
	...
	public function get_field_attributes( $field, $default = array() ) {
		...
		if ( ! $field instanceof Rocketgenius\Gravity_Forms\Settings\Fields\Base ) {
	...
	public function get_choice_attributes( $choice, $field_attributes, $default_choice_attributes = array() ) {
		...
		return \Rocketgenius\Gravity_Forms\Settings\Fields\Base::get_choice_attributes( $choice, $field_attributes, $default_choice_attributes );
	 ...
	 * @since Unknown
	 *
	 * @param \Rocketgenius\Gravity_Forms\Settings\Fields\Base|array $field         Field object.
	...
	public function set_field_error( &$field, $error_message = '' ) {
		...
		if ( is_a( $field, 'Rocketgenius\Gravity_Forms\Settings\Fields\Base' ) ) {
	 ...
	 * @since Unknown
	 *
	 * @param \Rocketgenius\Gravity_Forms\Settings\Fields\Base|array|boolean $field - Optional. The field meta. When specified, errors for this field will be returned
	 *
	...
	public function get_field_errors( $field = false ) {
		...
		} else if ( is_a( $field, 'Rocketgenius\Gravity_Forms\Settings\Fields\Base' ) ) {
	...
	public function maybe_get_tooltip( $item ) {
		...
		return Rocketgenius\Gravity_Forms\Settings::maybe_get_tooltip( $item );
	 ...
	 * @since Unknown
	 *
	 * @param array|\Rocketgenius\Gravity_Forms\Settings\Fields\Base $field - The field meta to be checked
	 *
	...
	public function field_failed_validation( $field ) {
		...
		$field_error = is_a( $field, 'Rocketgenius\Gravity_Forms\Settings\Fields\Base' ) ? $field->get_error() : null;
	 ...
	 * @param array|Rocketgenius\Gravity_Forms\Settings\Fields\Base[] $fields   Field(s) to add.
	 ...
	 *
	 * @return \Rocketgenius\Gravity_Forms\Settings\Fields\Base|array|bool
	 */
