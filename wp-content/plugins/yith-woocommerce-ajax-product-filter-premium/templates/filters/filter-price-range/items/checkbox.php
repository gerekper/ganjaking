<?php
/**
 * Filters Price Range Item - Checkbox
 *
...
<li class="filter-item checkbox <?php echo $filter->is_range_active( $range ) ? 'active' : ''; ?> <?php echo esc_attr( $additional_classes ); ?>">
	<label for="<?php echo esc_attr( $item_id ); ?>" data-range-min="<?php echo esc_attr( $range['min'] ); ?>" data-range-max="<?php echo $range['unlimited'] ? '' : esc_attr( $range['max'] ); ?>">
		<input type="checkbox" id="<?php echo esc_attr( $item_id ); ?>" name="<?php echo esc_attr( $item_name ); ?>" value="<?php echo esc_attr( $formatted_range ); ?>" <?php checked( $filter->is_range_active( $range ) ); ?> />
