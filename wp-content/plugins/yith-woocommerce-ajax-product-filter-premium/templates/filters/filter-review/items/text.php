...
<li class="filter-item text <?php echo $filter->is_review_rate_active( $rate['rate'] ) ? 'active' : ''; ?> <?php echo esc_attr( $additional_classes ); ?>">
	<a href="<?php echo esc_url( $filter->get_filter_url( $rate['rate'] ) ); ?>" <?php yith_wcan_add_rel_nofollow_to_url( true, true ); ?> role="button" data-rating="<?php echo esc_attr( $rate['rate'] ); ?>" class="term-label" >
