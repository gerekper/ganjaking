...
export const getOverviewChartData = async ({ queryKey }) => {
    ...
    let res;
    // apiFetch.use( (options, next) => {console.log(options, next); return next(options);} );
    if (dateRange == -1) {
        if (postID) {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/overview?post_id=${postID}`
            });
        } else {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/overview`
            });
        }
        ...
        if (postID) {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/overview?start_date=${start_date}&end_date=${end_date}&post_id=${postID}`
            });
        } else {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/overview?start_date=${start_date}&end_date=${end_date}`
            });
        }
    }
    return res ?? [];
...
export const getFeedbackChartData = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        //res = await axios.get(`${betterdocs.rest_url}betterdocs/v1/feedbacks`);
        res = await apiFetch({
            path: `betterdocs/v1/feedbacks`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        ...
        res = await apiFetch({
            path: `betterdocs/v1/feedbacks?start_date=${start_date}&end_date=${end_date}`
        });
    }
...
export const getSearchChartData = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/search/date`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/search/date?start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const getTotalDataCount = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        if (postID) {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/totalCount?post_id=${postID}`
            });
        } else {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/totalCount`
            });
        }
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
        ...
        if (postID) {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/totalCount?start_date=${start_date}&end_date=${end_date}&post_id=${postID}`
            });
        } else {
            res = await apiFetch({
                path: `betterdocs/v1/feedbacks/totalCount?start_date=${start_date}&end_date=${end_date}`
            });
        }
    }
    return res ?? [];
...
export const getLeadingDocsData = async ({ queryKey }) => {
    const [_, perPage, page, dateRange] = queryKey;
    let res;
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/leading_docs?per_page=${perPage}&page_now=${page}`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/leading_docs?per_page=${perPage}&page_now=${page}&start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const getLeadingCategoryData = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/leading_category?per_page=${perPage}&page_now=${page}`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/leading_category?per_page=${perPage}&page_now=${page}&start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const getLeadingKBData = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/leading_kb?per_page=${perPage}&page_now=${page}`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/leading_kb?per_page=${perPage}&page_now=${page}&start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const getMostHelpfulData = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/feedbacks/docs?orderby=most_helpful&per_page=${perPage}&page_now=${page}`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/feedbacks/docs?orderby=most_helpful&per_page=${perPage}&page_now=${page}&start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const getLeastHelpfulData = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/feedbacks/docs?orderby=least_helpful&per_page=${perPage}&page_now=${page}`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/feedbacks/docs?orderby=least_helpful&per_page=${perPage}&page_now=${page}&start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const getPopularSearch = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/search/all?per_page=${perPage}&page_now=${page}`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/search/all?per_page=${perPage}&page_now=${page}&start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const getNullSearch = async ({ queryKey }) => {
    ...
    if (dateRange == -1) {
        res = await apiFetch({
            path: `betterdocs/v1/search/not_found?per_page=${perPage}&page_now=${page}`
        });
    } else if (dateRange && Object.entries(dateRange).length) {
        let start_date = moment(dateRange?.start).format("YYYY-MM-DD"),
            end_date = moment(dateRange?.end).format("YYYY-MM-DD");
        res = await apiFetch({
            path: `betterdocs/v1/search/not_found?per_page=${perPage}&page_now=${page}&start_date=${start_date}&end_date=${end_date}`
        });
    }
    return res ?? [];
};
...
export const comparisonFactor = (cur, prev, dateDifference) => {
    ...
    return (
        <p className="btd-chart-comparison">
            ...
            <span>{`vs Previous ${dateDifference != 0 ? `${dateDifference} Days` : "Day"
                }`}</span>
        </p>
    );
};
...
export const Tooltip = (props) => {
    return (
        <span className={`btd-tooltip ${props.className ? props.className : ""}`}>
            <button
                className={`btd-tooltip-button ${props.buttonClassName ? props.buttonClassName : ""
                    }`}
            >
                {props.buttonContent}
            </button>
            <span
                className={`btd-tooltip-content ${props.tooltipClassName ? props.tooltipClassName : ""
                    }`}
            >
...
export const formatDataForChart = (data, filterState) => {
    ...
    if (filterState && data && data?.length) {
        dataForChart.labels = data
            .map((obj) =>
                moment(obj.created_at || obj.search_date || obj.date)
                    .locale("en")
                    .format("MMM D YYYY")
            )
            .reverse();
