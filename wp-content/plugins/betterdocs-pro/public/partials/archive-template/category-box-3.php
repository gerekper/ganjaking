...
echo '<div class="betterdocs-wraper betterdocs-main-wraper">';
	...
	echo '<div class="betterdocs-archive-wrap betterdocs-archive-category-box betterdocs-archive-category-box-2 betterdocs-archive-main">';
		...
		if ( is_tax( 'knowledge_base' ) && BetterDocs_Multiple_Kb::$enable == 1 ) {
			echo do_shortcode( '[betterdocs_category_box_2 multiple_knowledge_base="true" terms_orderby="'.esc_html($terms_orderby).'" title_tag="'.BetterDocs_Helper::html_tag($output['betterdocs_category_title_tag']).'"]' );
		} else {
			echo do_shortcode( '[betterdocs_category_box_2 terms_orderby="'.esc_html($terms_orderby).'" title_tag="'.BetterDocs_Helper::html_tag($output['betterdocs_category_title_tag']).'"]' );
		}
	echo '</div>
</div>';
