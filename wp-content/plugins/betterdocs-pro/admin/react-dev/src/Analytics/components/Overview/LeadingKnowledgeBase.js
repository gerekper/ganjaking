...
const LeadingKnowledgeBase = () => {
  const [leadingKB, setLeadingKB] = useState([]);
  ...
  useEffect(() => {
    setIsLoading(true);
    getLeadingKBData(perPage, page)
      .then((data) => setLeadingKB(data))
  ...
  return (
    <>
      {!isLoading ? (
        <div>
          {leadingKB &&
          leadingKB?.doc_category &&
          leadingKB?.doc_category?.length ? (
            <>
              <div className="BtdDataTableWrapper">
                <table className="BtdDataTable">
                  ...
                  <tbody>
                    {leadingKB.doc_category.map((data) => (
                      <tr key={Math.random()}>
                        <td>
                          <p
              ...
              {leadingKB?.pagination?.total_page > 1 ? (
                <div className="btd-pagination-wrapper">
                  <Pagination
                    defaultCurrent={1}
                    pageSize={perPage}
                    onChange={handlePageChange}
                    total={leadingKB?.pagination?.total_page * perPage}
                    prevIcon={"Prev"}
                    nextIcon={"Next"}
                    locale={localInfo}
                    current={page}
