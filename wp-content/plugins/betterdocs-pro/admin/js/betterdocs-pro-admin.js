(function( $ ) {
	...
	$( document ).ready( function() {
		...
		var ia_color_settings = [
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-conversation-container, .betterdocs-footer-wrapper, .betterdocs-launcher, .betterdocs-ask-wrapper .betterdocs-ask-submit',
					...
					{
						'selector' : '.betterdocs-header-wrapper .betterdocs-header .inner-container.betterdocs-active-answer .toggle:first-of-type svg, .betterdocs-header-wrapper .betterdocs-header .inner-container.betterdocs-active-ask .toggle:last-of-type svg',
						'property' : 'fill',
					}
				]
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-header-wrapper .betterdocs-header .inner-container, .betterdocs-footer-wrapper .betterdocs-footer-emo > div',
						'property' : 'background-color',
					}
				]
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-header-wrapper .betterdocs-sub-header.betterdocs-ans-header > h3, .betterdocs-header-wrapper .betterdocs-sub-header.betterdocs-ask-header > h3',
						'property' : 'color',
					}
				]
			},
			...
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-header-wrapper .betterdocs-sub-header.betterdocs-ans-header > p, .betterdocs-header-wrapper .betterdocs-sub-header.betterdocs-ask-header > p',
						'property' : 'color',
					}
				]
			},
			...
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-search-bar .betterdocs-search-wrap .betterdocs-search-container input',
						'property' : 'color',
					}
				]
			},
			...
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-messages-container .betterdocs-card-link',
						'property' : 'background-color',
					}
				]
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-messages-container .betterdocs-card-link .betterdocs-card-title-wrapper .betterdocs-card-title',
						'property' : 'color',
					}
				]
			},
			...
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-messages-container .betterdocs-card-link .betterdocs-card-body-wrapper .betterdocs-card-body',
						'property' : 'color',
					}
				]
			},
			...
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-footer-wrapper .betterdocs-footer-label p',
						'property' : 'font-size',
					}
				]
			},
			...
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-footer-wrapper .betterdocs-footer-emo > div',
						'property' : 'width',
						'multiple' : 2,
					},
					{
			...
			},
			{
				...
				'settings' : [
					...
					{
						'selector' : '.betterdocs-ask-wrapper .betterdocs-ask-submit',
						'property' : 'background-color',
					}
				]
			},
			{
				...
				'settings' : [
					{
						'selector' : '.betterdocs-ask-wrapper input:not([type="submit"]), .betterdocs-ask-wrapper textarea, .betterdocs-ask-wrapper .betterdocs-attach-button',
						'property' : 'color',
					},
					{
		...
		ia_color_settings.map(function( item, i ){
			...
			$( item.id ).on( event, function( e ){
				var value = e.currentTarget.value, isMultiplied = true;
				item.settings.map(function( single, j ){

					var multiple = single.multiple != undefined ? single.multiple : 1;
	...
	copyCross.on('success', function(e) {
		...
		// Switch to Dark Mode
		$('#betterdocs-mode-toggle').prop('checked', docs_cat_ordering_data.dark_mode );
		...
		// Go To Sorting UI
		$('body.post-type-docs .wp-heading-inline').after( '<a href="admin.php?page=betterdocs-admin" class="page-title-action">'+ docs_cat_ordering_data.menu_title +'</a>' );
		...
		docs_post_list.each(function( i, single_doc_list ){		
			...
			single_doc_list.sortable({
				...
				start: function( event, ui ) {
					...
					$( '.betterdocs-post-list-placeholder' ).css( 'height', item.css( 'height' ) );
				},
				receive : function( event, ui ){
					...
					if( list_term_id != undefined ) { 
						...
						const data = {
							...
							'doc_cat_order_nonce': docs_cat_ordering_data.doc_cat_order_nonce
						};
				...
				update: function( event, ui ) {
					...
					single_doc_list.find( 'li.ui-sortable-handle' ).each( function() {
						const ele = $( this );
		...
		const base_index = parseInt( docs_cat_ordering_data.paged ) > 0 ? ( parseInt( docs_cat_ordering_data.paged ) - 1 ) * parseInt( $( '#' + docs_cat_ordering_data.per_page_id ).val() ) : 0;
			...
			if ( ! tax_table.find( 'tr:first-child' ).hasClass( 'no-items' ) ) {
				...
				tax_table.sortable({
					...
					start: function( event, ui ) {
						...
						$( '.betterdocs-drag-drop-cat-tax-placeholder' ).css( 'height', item.css( 'height' ) );
					},
					// Update callback.
					update: function( event, ui ) {
						const item = $( ui.item[0] );
