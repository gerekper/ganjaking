...
trait ProviderRedirectTrait
{
     ...
     */
    protected function followRequestRedirects(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request)
    {
        $response = null;
        $attempts = 0;
        while ($attempts < $this->redirectLimit) {
            ...
            if ($this->isRedirect($response)) {
                $redirectUrl = new \WPMailSMTP\Vendor\GuzzleHttp\Psr7\Uri($response->getHeader('Location')[0]);
     ...
     * @return boolean
     */
    protected function isRedirect(\WPMailSMTP\Vendor\Psr\Http\Message\ResponseInterface $response)
    {
