...
class TokenProvider
{
    ...
    public static function cache(callable $provider, \WPMailSMTP\Vendor\Aws\CacheInterface $cache, $cacheKey = null)
    {
        ...
        return function () use($provider, $cache, $cacheKey) {
            ...
            if (\is_array($found) && isset($found['token'])) {
                if (isset($found['token']) && $found['token'] instanceof \WPMailSMTP\Vendor\Aws\Token\TokenInterface) {
    ...
    {
        return new \WPMailSMTP\Vendor\Aws\Token\SsoTokenProvider($profileName, $filename, $config);
    }
}
