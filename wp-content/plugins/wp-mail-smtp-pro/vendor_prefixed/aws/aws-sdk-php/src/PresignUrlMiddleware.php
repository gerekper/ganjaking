...
class PresignUrlMiddleware
{
    ...
    public function __construct(array $options, callable $endpointProvider, \WPMailSMTP\Vendor\Aws\AwsClientInterface $client, callable $nextHandler)
    {
    ...
    public static function wrap(\WPMailSMTP\Vendor\Aws\AwsClientInterface $client, callable $endpointProvider, array $options = [])
    {
    ...
    private function createPresignedUrl(\WPMailSMTP\Vendor\Aws\AwsClientInterface $client, \WPMailSMTP\Vendor\Aws\CommandInterface $cmd)
    {
        ...
        return (string) $signer->presign(\WPMailSMTP\Vendor\Aws\Signature\SignatureV4::convertPostToGet($request), $client->getCredentials()->wait(), '+1 hour')->getUri();
    }
}
