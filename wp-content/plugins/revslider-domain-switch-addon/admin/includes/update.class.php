...
class rs_domain_switch_update extends RevSliderFunctions {
	private $plugin_url		 = 'https://codecanyon.net/item/slider-revolution-responsive-wordpress-plugin/2751380';
	private $remote_url_info = 'addons/revslider-domain-switch-addon/revslider-domain-switch-addon.php';
	private $plugin_slug	 = 'revslider-domain-switch-addon';
	private $plugin_path	 = 'revslider-domain-switch-addon/revslider-domain-switch-addon.php';
	private $version;
	private $plugins;
	private $option;
	...
	protected function _check_updates($force_check = false){
		...
		$last_check = get_option('rs_domain-switch-update-check');

		if($last_check == false){ //first time called
			$last_check = time();
			update_option('rs_domain-switch-update-check', $last_check);
		}
		...
		if(time() - $last_check > 60 * 60 * 24 * 30 || $force_check == true){
			...
			if(isset($data->basic)){
				update_option('rs_domain-switch-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic	 = $data->basic;
				$this->data->full	 = $data->full;
				
				update_option('rs_domain-switch-latest-version', $data->full->version);
			}
		}
	...
	public function _retrieve_update_info(){
		...
		$rattr = array(
			...
			'version'	=> urlencode(RS_DOMAIN_SWITCH_VERSION)
		);
