...
class Revslider_Domain_Switch_Addon_Verify {
	
	private $notice = false;
	private $min_rev_slider = '6.0';
	private $addon_title = 'Domain Switch';
	private $text_domain = 'revslider-domain-switch-addon';
	private $notice_slug = 'revslider_domain_switch_addon';
	
	public function __construct() {
		if(!class_exists('RevSliderFront')) {
			$this->notice = 'add_notice_plugin';
		}
		else if(!version_compare(RS_REVISION, $this->min_rev_slider, '>=')) {
	...
	public function enqueue_notice_script() {
		wp_enqueue_script($this->text_domain . '-notice', RS_DOMAIN_SWITCH_PLUGIN_URL . 'admin/assets/js/dismiss-admin-notice.js', array(), RS_DOMAIN_SWITCH_VERSION, true);
	}
