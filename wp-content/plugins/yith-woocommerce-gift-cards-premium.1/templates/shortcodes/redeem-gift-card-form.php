...
if ( current_user_can('administrator') || current_user_can('manage_woocommerce') || apply_filters( 'ywgc_redeem_shortcode_access_condition', false ) ){
    ...
    if ( isset( $_POST["ywgc-check-code"] ) && isset( $_POST["ywgc-used-amount"] ) ){
        ...
        if ( ! is_object( $gift_card ) || !$gift_card->exists() ){
            echo '<div style="font-weight: bolder">' . __( "The code added is not associated to any existing gift card.", 'yith-woocommerce-gift-cards' ) . '</div>';
            echo '<br>';
        }
        else{
            if ( is_object( $gift_card ) && $gift_card->ID != 0 && $gift_card->has_sufficient_credit($used_amount) ){
                ...
                echo '<div style="font-weight: bolder">' . __( "The gift card has been redeemed successfully.", 'yith-woocommerce-gift-cards' ) . '</div><br>';
                echo '<div style="font-weight: bolder">' . __( "New gift card balance: ", 'yith-woocommerce-gift-cards' ) . wc_price( $gift_card->get_balance() ) . '</div>';
                echo '<br>';
            }

            else{
                echo '<div style="font-weight: bolder">' . __( "The gift card balance is not enough to cover this order amount.", 'yith-woocommerce-gift-cards' ) . '</div><br>';
                echo '<div style="font-weight: bolder">' . __( "Gift card balance: ", 'yith-woocommerce-gift-cards' ) . wc_price( $gift_card->get_balance() ) . '</div>';
                echo '<br>';
            }
        }
    }
