...
<table cellspacing="0" class="ywgc-table-template">
    ...
    <tr>
        <td class="ywgc-card-product-name" style="float: left">
            ...
            $product_name_text =  is_object( $product ) && $product instanceof WC_Product_Gift_Card && $object->product_as_present != 1 ? $product->get_name() : __( "Gift card", 'yith-woocommerce-gift-cards' );

            echo apply_filters( 'yith_wcgc_template_product_name_text', $product_name_text . ' ' . __( "on", 'yith-woocommerce-gift-cards' )  . ' ' . get_bloginfo( 'name' ) , $object, $context, $product_id ); ?>
        </td>
    ...
    <?php if ( $context == 'pdf' ): ?>
        <tr>
            <td style="text-align: left;"><b><?php echo __( "From: ", 'yith-woocommerce-gift-cards' )?></b><?php echo $object->sender_name ?></td>
        </tr>

        <tr>
            <td style="text-align: left;"><b><?php echo __( "To: ", 'yith-woocommerce-gift-cards' )?></b><?php echo $object->recipient_name ?></td>
        </tr>
    ...
    <?php do_action( 'yith_wcgc_template_after_message', $object, $context ); ?>
        ...
        <tr>
            <td>
                <span class="ywgc-card-code-title"><?php echo apply_filters('ywgc_preview_code_title', __( "Gift card code:", 'yith-woocommerce-gift-cards' ) ); ?></span>
                <br>
                <br>
    ...
    <?php if ( get_option( 'ywgc_display_description_template', 'no' ) == "yes" ) :?>
        ...
        <tr>
            <td colspan="2" class="ywgc-expiration-message" style="text-align: center"><?php echo get_option( 'ywgc_description_template_email_text', __( "To use this gift card, you can either enter the code in the gift card field on the cart page or click on the following link to automatically get the discount.", 'yith-woocommerce-gift-cards' ) ); ?></td>
        </tr>
        <?php endif; ?>

        <?php if ( $context == "pdf" ) :?>
            <tr>
                <td colspan="2" class="ywgc-expiration-message" style="text-align: center"><?php echo get_option( 'ywgc_description_template_text_pdf', __( "You can automatically apply the gift card in our shop by reading the QR code with your phone.", 'yith-woocommerce-gift-cards' ) ); ?></td>
            </tr>
        <?php endif; ?>
    <?php endif; ?>
