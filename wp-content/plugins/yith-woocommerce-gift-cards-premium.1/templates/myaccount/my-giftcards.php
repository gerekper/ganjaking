...
$gift_card_columns = apply_filters( 'yith_ywgc_my_gift_cards_columns',
	array(
		...
		'direct_link'  => __( 'Auto Apply', 'yith-woocommerce-gift-cards' ),
	) );
...
if ( isset( $_POST["ywgc-link-code"] ) ){
    ...
    if ( ! is_object( $gift_card ) || $gift_card->ID == 0 ){
        echo '<div class="yith-add-new-gc-my-account-notice-message" style="font-weight: bolder">' . __( "The code added is not associated to any existing gift card.", 'yith-woocommerce-gift-cards' ) . '</div>';
    }
    else{
        if ( is_object( $gift_card ) && $gift_card->ID != 0 ){
            $user = wp_get_current_user();
            $gift_card->register_user($user->ID);
            echo '<div class="yith-add-new-gc-my-account-notice-message" style="font-weight: bolder">' . sprintf(__( "The gift card code %s is now linked to your account.", 'yith-woocommerce-gift-cards' ), $gift_card->get_code()) . '</div>';
        }
    }
}
...
<div class="gift-card-panel-title-container">
    <h2 style="float: left"><?php echo apply_filters( 'yith_ywgc_my_account_my_giftcards', __( 'My Gift Cards', 'yith-woocommerce-gift-cards' ) ); ?></h2>
    <button class="yith-add-new-gc-my-account-button" style="float: right"><?php echo apply_filters( 'yith_ywgc_my_account_add_new_text', __( 'Add new', 'yith-woocommerce-gift-cards' ) ); ?></button>
</div>
