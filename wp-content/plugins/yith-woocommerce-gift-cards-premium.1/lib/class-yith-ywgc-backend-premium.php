...
//            add_action( 'add_tag_form', array( $this, 'ywgc_edit_design_category_form' ) );
        ...
        public function manage_bcc( $gift_card ) {
            ...
            if ( get_option ( "ywgc_blind_carbon_copy_to_buyer", 'no' ) == "yes" ) {
        ...
        public function show_as_string_order_item_meta_key($display_key){
            if( strpos($display_key,'ywgc') !== false){
                ...
                elseif( $display_key == '_ywgc_product_as_present' ){
                    $display_key = __('Product as a present','yith-woocommerce-gift-card');
                }
                ...
                elseif( $display_key == '_ywgc_is_digital' ){
                    $display_key = __('Digital','yith-woocommerce-gift-card');
                }
                elseif( $display_key == '_ywgc_sender_name' ){
                    $display_key = __('Sender\'s name','yith-woocommerce-gift-card');
                }
                elseif( $display_key == '_ywgc_recipient_name' ){
                    $display_key = __('Recipient\'s name','yith-woocommerce-gift-card');
                }
                ...
                elseif( $display_key == '_ywgc_delivery_date' ){
                    $display_key = __('Delivery date','yith-woocommerce-gift-card');
                }
                elseif( $display_key == '_ywgc_postdated' ){
                    $display_key = __('Postdated','yith-woocommerce-gift-card');
                }
        ...
        public function yith_ywgc_transform_smart_coupons_buttons(){
            ...
            if ( class_exists( 'WC_Smart_Coupons' ) ) {
                ?>
                <h2><?php _e( 'WooCommerce Smart Coupons integration', 'yith-woocommerce-gift-card' );?></h2>
                <tr id="ywgc_ajax_zone_transform_smart_coupons">
                    <th>
                        <label><?php _e( 'Transfer WooCommerce Smart Coupons to YITH Gift Cards ', 'yith-woocommerce-gift-card' );?></label>
                    </th>
                    <td>
                        ...
                        <span class="description"><?php _e( 'Transfer "Store Credit / Gift Certificate" coupons. This action cannot be undone', 'yith-woocommerce-gift-card' ); ?></span>
                    </td>
                </tr>
                <?php
            }
            ...
            if ( class_exists('WC_Aelia_CurrencySwitcher') ) {
                ...
                $aux = array(
                    ...
                    'aelia_currency_switchers_tab_title'    => array(
                        'type'  => 'title',
                        'name' => __( 'Aelia Currency Switcher integration', 'yith-woocommerce-gift-cards' ),
                        'desc'  => '',
                        'id'    => 'yith_aelia_currency_switchers_tab'
                    ),
                    'enable_aelia_option' => array(
                        'name'    => __( 'Enable Aelia integration', 'yith-woocommerce-gift-cards' ),
                        'type'    => 'checkbox',
            ...
            if ( $woocommerce_wpml ) {
                ...
                $aux = array(
                    ...
                    'wpml_currency_switchers_tab_title' => array(
                        'type' => 'title',
                        'name' => __('WPML Currency Switcher integration', 'yith-woocommerce-gift-cards'),
                        'desc' => '',
                        'id' => 'yith_wpml_currency_switchers_tab'
                    ),
                    'enable_wpml_option' => array(
                        'name' => __('Enable WPML integration', 'yith-woocommerce-gift-cards'),
                        'type' => 'checkbox',
        ...
        public function ywgc_edit_design_category_form() {
            global $current_screen;
                ...
                if ( $current_screen->id == 'edit-giftcard-category' ){
                    ?>
                    <div>
                        <h2><?php echo __('Manage the gift card images through the WordPress Media Library', 'yith-woocommerce-gift-cards');?></h2>
                        <br>
                        <button id="ywgc-media-upload-button" class="button" style="padding: 3px 15px;"><span class="dashicons dashicons-admin-media"></span><?php echo ' ' .  __('Manage media', 'yith-woocommerce-gift-cards'); ?></button>
                        <p><?php echo __('Upload/manage images in the WordPress Media Library and include them in the existing gift card categories.', 'yith-woocommerce-gift-cards');  ?></p>
                    </div>
                    <?php
                }
        }
