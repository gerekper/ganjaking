...
//            add_action ( 'woocommerce_order_item_meta_start', array( $this, 'edit_gift_card'), 10, 3 );
        ...
        public function yiyh_wc_gift_card_woocommerce_after_shop_loop_item_call_back() {
            ...
            $disable_gift_this_product_from_product = get_post_meta( $product->get_id(), '_yith_wcgc_disable_gift_this_product', true ) == 'yes';

            if ( $product && apply_filters('yith_ywgc_give_product_as_present', true, $product ) && ( $product->is_in_stock() && $product->get_type() != 'gift-card' ) && ( get_option( 'ywgc_permit_its_a_present_shop_page' ) == 'yes' ) && ( get_option( 'ywgc_permit_its_a_present' ) == 'yes' ) && $disable_gift_this_product_from_product != true  ) {
        ...
        public function show_manual_amount_area( $product ) {
            ...
            <input id="ywgc-manual-amount" name="ywgc-manual-amount" class="ywgc-manual-amount" type="text"
                   placeholder="<?php echo apply_filters('yith_wcgc_manual_amount_option_text',__ ( "Enter amount", 'yith-woocommerce-gift-cards' )) ?>">
            ...
            if ( is_product () || is_cart () || is_checkout () ||  is_account_page() || apply_filters ( 'yith_ywgc_do_eneuque_frontend_scripts', false ) ) {
                ...
                if ( is_product() ) {
                    ...
                    if (is_numeric($manual_minimal_amount))
                        $manual_minimal_amount_error = __("The minimum amount is", 'yith-woocommerce-gift-cards') . " " . $manual_minimal_amount;
                    else
                ...
                wp_localize_script ( 'ywgc-frontend-script',
                    ...
                    array(
                        ...
                        'invalid_image_size'           => __ ( "The size fo the uploaded file exceeds the maximum allowed", 'yith-woocommerce-gift-cards' ) . " (" .  get_option ( 'ywgc_custom_image_max_size', 1 ) . " MB)",
                        ...
                        'multiple_recipient'           => __ ( "<b>Note</b>: You added more than one recipient, so <i class='ywgc-darkred-text'>you will buy %number_gift_cards% gift cards</i> and each recipient will receive a different gift card.", 'yith-woocommerce-gift-cards' ),
                        ...
                        'name'                         => __( "ENTER ADDITIONAL RECIPIENT'S NAME", 'yith-woocommerce-gift-cards' ),
                        'email'                        => __( "ENTER ADDITIONAL RECIPIENT'S EMAIL ADDRESS", 'yith-woocommerce-gift-cards' ),
                        'label_name'                   => apply_filters('ywgc_recipient_name_label',__( "Name: ", 'yith-woocommerce-gift-cards' )),
                        'label_email'                  => apply_filters('ywgc_recipient_name_label',__( "E-mail: ", 'yith-woocommerce-gift-cards' )),
                        'notice_target'                => apply_filters ( 'yith_ywgc_gift_card_notice_target', 'div.ywgc_enter_code' ),
                        'add_gift_text'                => apply_filters ( 'yith_gift_card_layout_add_gift_button_text', __ ( "Buy gift card", 'yith-woocommerce-gift-cards' ) ),
                ...
                if ( ! $gc->is_pre_printed () ):
                    ...
                    <div>
                        ...
                        if ( $gc->delivery_send_date ){
                            $status_class = "sent";
                            $message      = sprintf ( __ ( "Sent on %s", 'yith-woocommerce-gift-cards' ), $gc->get_formatted_date( $gc->delivery_send_date ) );
                        ...
                        <div>
                            <span><?php echo sprintf ( __ ( "Recipient: %s", 'yith-woocommerce-gift-cards' ), $gc->recipient ); ?></span>
                        </div>
                        <div>
                            <?php if( $gc->delivery_date != '' ): ?>
                                <span><?php echo sprintf ( __ ( "Delivery date: %s", 'yith-woocommerce-gift-cards' ), $gc->get_formatted_date( $gc->delivery_date ) ); ?></span>
                                <br>
                            <?php endif; ?>
        ...
        function yith_ywgc_rename_coupon_field_on_cart( $translated_text, $text, $text_domain ) {
            ...
            if ( 'Apply coupon' === $text ) {
                $translated_text = get_option( 'ywgc_apply_coupon_button_text_button' , __( 'Apply coupon', 'yith-woocommerce-gift-cards' ) );
            }
            return $translated_text;
        }
        ...
        function yith_ywgc_rename_coupon_label( $text ) {
            ...
            $text_option =  get_option( 'ywgc_apply_coupon_label_text' , __( 'Have a coupon?', 'yith-woocommerce-gift-cards' ) );
            ...
            if ( get_option( 'ywgc_apply_gc_code_on_gc_product', 'no' )  == 'yes' ){
                foreach ( $items as $cart_item_key => $values ) {
                    ...
                    if ( $product->get_type() == 'gift-card' ){
                        wc_add_notice( __( 'It is not possible to add a gift card code when the cart contains a gift card product', 'yith-woocommerce-gift-cards'), 'error' );
