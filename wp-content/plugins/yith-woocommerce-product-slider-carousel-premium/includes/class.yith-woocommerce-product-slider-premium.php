...
if ( ! class_exists( 'YITH_WooCommerce_Product_Slider_Premium' ) ) {
	...
	/**
	 * YITH_WooCommerce_Product_Slider_Premium
	 */
	class YITH_WooCommerce_Product_Slider_Premium extends  YITH_WooCommerce_Product_Slider {
		...
		public function __construct() {
			...
			add_action( 'wp_enqueue_scripts', array( $this, 'add_dynamic_layout_style' ), 30 );
		}
		...
		/**
		 * Add_animation_premium
		 *
		 * @param  mixed $animations animations.
		 * @return array
		 */
		...
		public function add_free_slider_in_table() {
			...
			if ( 'yes' === get_option( '_ywcps_first_install', 'yes' ) ) {
				...
				$id = $wpdb->get_var( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key= '_ywcps_free_slider_id' LIMIT 1" ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery
		...
		/**
		 * Include_admin_premium_style_script
		 *
		 * @return void
		 */
		...
		public function update_options_to_new_version() {
			...
			if ( version_compare( $db_option, '1.0.0', '<' ) ) {
				...
				$all_slider_count = count( $all_slider );
				while ( $all_slider_count > 0 ) {
		...
		/**
		 * Add_dynamic_layout_style
		 *
		 * @return void
		 */
		public function add_dynamic_layout_style() {
			$css = ywcps_get_layout_style();
			wp_add_inline_style( 'yith-product-slider-style', $css );
		}
