...
$enable_mousewhell = 'yes' === $enable_mousewhell ? 'true' : 'false';
$js_args           = array(
	'yit_theme'         => defined( 'YIT' ) ? 'true' : 'false',
	'enable_mousewheel' => $enable_mousewhell,
	'margin_item'       => apply_filters( 'yith_product_slider_margin', 20, $id ),
	'loop_or_rewind'    => apply_filters( 'yith_product_slider_margin_loop_or_rewind', 'rewind' ),
);
...
if ( ! wp_script_is( 'jquery.mousewheel' ) ) {
	wp_enqueue_script( 'jquery.mousewheel', YWCPS_ASSETS_URL . 'js/jquery.mousewheel.min.js', array(), false, true ); // phpcs:ignore
}
...
if ( ! empty( $product_type ) ) {
	...
	switch ( $product_type ) {
		...
		case 'on_sale':
			...
			break;
		case 'best_seller':
			$query_args['meta_key'] = 'total_sales'; //phpcs:ignore slow query ok.
		...
		case 'featured':
			if ( version_compare( WC()->version, '3.0.0', '>=' ) ) {
				...
				$query_args['meta_query']   = array(); //phpcs:ignore slow query ok.
				$query_args['meta_query'][] = array(
					'key'   => '_featured',
					'value' => 'yes',
				);
		...
		case 'custom_select_tag':
			...
			break;
		case 'top_rated':
			$query_args['meta_key'] = '_wc_average_rating'; //phpcs:ignore slow query ok.
			$query_args['orderby']  = 'meta_value_num';
			$query_args['order']    = 'DESC';
			break;
	}
	...
	$query_args['tax_query']             = $tax_query; //phpcs:ignore slow query ok.
	$query_args['tax_query']['relation'] = 'AND';

	$order_uppercase = strtoupper( $order );
	switch ( $order_by ) {


		case 'date':
			if ( ! isset( $query_args['orderby'] ) ) {
				if ( apply_filters( 'ywcps_use_modified_date', false ) ) {
					$query_args['orderby'] = 'modified';
				} else {
					$query_args['orderby'] = 'date';
				}

				$query_args['order'] = $order_uppercase;
			}
			break;

		case 'price':
			$query_args['meta_key'] = '_price'; //phpcs:ignore slow query ok.
			$query_args['orderby']  = 'meta_value_num';
			$query_args['order']    = $order_uppercase;
