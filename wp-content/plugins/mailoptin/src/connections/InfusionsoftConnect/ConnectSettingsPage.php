...
class ConnectSettingsPage extends AbstractInfusionsoftConnect
{
    ...
    /**
     * Build the settings metabox for infusionsoft
     *
     * @param array $arg
     *
     * @return array
    ...
    public function connection_settings($arg)
    {
        ...
        if (self::is_connected()) {
            ...
            $description            = sprintf(__('Only re-authorize if you want to connect another Keap/Infusionsoft account.', 'mailoptin'));
            $disconnect_integration = sprintf(
                ...
                wp_nonce_url(
                    add_query_arg('mo-integration-disconnect', 'infusionsoft', MAILOPTIN_CONNECTIONS_SETTINGS_PAGE),
        ...
        $settingsArg[] = array(
            ...
            'infusionsoft_auth'            => array(
                ...
                'data'        => sprintf(
                    ...
                    $this->get_oauth_url('infusionsoft'),
                    $button_color,
                    $button_text,
                    $disconnect_integration
                ),
    ...
    public function remove_access_token_persistence($sanitized_data, $option_name)
    {
        ...
        if ($option_name == MAILOPTIN_CONNECTIONS_DB_OPTION_NAME) {
            unset($sanitized_data['infusionsoft_access_token']);
            unset($sanitized_data['infusionsoft_refresh_token']);
            unset($sanitized_data['infusionsoft_expires_at']);
        }
    ...
    {
        if ( ! isset($_GET['mo-integration-disconnect']) || $_GET['mo-integration-disconnect'] != 'infusionsoft' || ! check_admin_referer('mo_disconnect_integration')) return;

        $old_data = get_option($option_name, []);
        unset($old_data['infusionsoft_access_token']);
        unset($old_data['infusionsoft_refresh_token']);
        unset($old_data['infusionsoft_expires_at']);
    ...
    {
        if ( ! empty($_GET['mo-save-oauth-provider']) && $_GET['mo-save-oauth-provider'] == 'infusionsoft' && ! empty($_GET['access_token'])) {
    ...
    {
        //Not a infusionsoft connection section
        if (MAILOPTIN_CONNECTIONS_DB_OPTION_NAME !== $option || ! isset($args['infusionsoft_auth'])) {
            return;
        }

        echo self::get_optin_error_log_link('infusionsoft');
