...
class ConnectSettingsPage extends AbstractCleverReachConnect
{
    ...
    /**
     * Build the settings metabox for cleverreach
     *
     * @param array $arg
     *
     * @return array
    ...
    public function connection_settings($arg)
    {
        ...
        if (self::is_connected()) {
            ...
            $description            = sprintf(__('Only re-authorize if you want to connect another CleverReach account.', 'mailoptin'));
            $disconnect_integration = sprintf(
                ...
                wp_nonce_url(
                    add_query_arg('mo-integration-disconnect', 'cleverreach', MAILOPTIN_CONNECTIONS_SETTINGS_PAGE),
        ...
        $settingsArg[] = array(
            ...
            'cleverreach_auth'            => array(
                ...
                'data'        => sprintf(
                    ...
                    $this->get_oauth_url('cleverreach'),
                    $button_color,
                    $button_text,
                    $disconnect_integration
                ),
    ...
    public function remove_access_token_persistence($sanitized_data, $option_name)
    {
        ...
        if ($option_name == MAILOPTIN_CONNECTIONS_DB_OPTION_NAME) {
            unset($sanitized_data['cleverreach_access_token']);
            unset($sanitized_data['cleverreach_refresh_token']);
            unset($sanitized_data['cleverreach_expires_at']);
        }
    ...
    {
        if ( ! isset($_GET['mo-integration-disconnect']) || $_GET['mo-integration-disconnect'] != 'cleverreach' || ! check_admin_referer('mo_disconnect_integration')) return;

        $old_data = get_option($option_name, []);
        unset($old_data['cleverreach_access_token']);
        unset($old_data['cleverreach_refresh_token']);
        unset($old_data['cleverreach_expires_at']);
    ...
    {
        if ( ! empty($_GET['mo-save-oauth-provider']) && $_GET['mo-save-oauth-provider'] == 'cleverreach' && ! empty($_GET['access_token'])) {

            check_admin_referer('mo_save_oauth_credentials', 'moconnect_nonce');

            $old_data = get_option($option_name, []);
            $new_data = array_map('rawurldecode', [
                'cleverreach_access_token'  => $_GET['access_token'],
                'cleverreach_refresh_token' => $_GET['refresh_token'],
                'cleverreach_expires_at'    => $this->oauth_expires_at_transform($_GET['expires_at'])
            ]);
    ...
    {
        //Not a cleverreach connection section
        if (MAILOPTIN_CONNECTIONS_DB_OPTION_NAME !== $option || ! isset($args['cleverreach_auth'])) {
            return;
        }

        echo self::get_optin_error_log_link('cleverreach');
