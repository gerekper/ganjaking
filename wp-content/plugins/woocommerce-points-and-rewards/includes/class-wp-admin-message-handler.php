...
if ( ! class_exists( 'WP_Admin_Message_Handler' ) ) :
	...
	class WP_Admin_Message_Handler {
		...
		public function load_messages() {
			...
			if ( isset( $_GET[ self::MESSAGE_ID_GET_NAME ] ) && $this->get_message_id() === wc_clean( wp_unslash( $_GET[ self::MESSAGE_ID_GET_NAME ] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended

				$message_id_name = wc_clean( wp_unslash( $_GET[ self::MESSAGE_ID_GET_NAME ] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$memo            = get_transient( self::MESSAGE_TRANSIENT_PREFIX . $message_id_name );
				...
				$this->clear_messages( $message_id_name );
			}
		}
		...
		public function error_count() {
			return sizeof( $this->errors ); // phpcs:ignore Generic.PHP.ForbiddenFunctions.FoundWithAlternative
		}
		...
		public function message_count() {
			return sizeof( $this->messages ); // phpcs:ignore Generic.PHP.ForbiddenFunctions.FoundWithAlternative
		}
		...
		public function show_messages() {
			if ( $this->error_count() > 0 ) {
				echo wp_kses_post( '<div id="wp-admin-message-handler-error" class="error"><ul><li><strong>' . implode( '</strong></li><li><strong>', $this->get_errors() ) . '</strong></li></ul></div>' );
			}

			if ( $this->message_count() > 0 ) {
				echo wp_kses_post( '<div id="wp-admin-message-handler-message"  class="updated"><ul><li><strong>' . implode( '</strong></li><li><strong>', $this->get_messages() ) . '</strong></li></ul></div>' );
			}
		}
		...
		public function redirect( $location, $status ) {
			...
			if ( $this->set_messages() ) {
				$location = esc_url( add_query_arg( self::MESSAGE_ID_GET_NAME, $this->get_message_id(), $location ) );
			}
