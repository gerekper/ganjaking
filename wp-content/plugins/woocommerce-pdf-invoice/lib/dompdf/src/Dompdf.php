...
use DOMDocument;
use DOMNode;
use WooCommercePDFInvoice\Adapter\CPDF;
use DOMXPath;
use WooCommercePDFInvoice\Frame\Factory;
use WooCommercePDFInvoice\Frame\FrameTree;
use WooCommercePDFInvoice_HTML5_Tokenizer;
use WooCommercePDFInvoice_HTML5_TreeBuilder;
use WooCommercePDFInvoice\Image\Cache;
use WooCommercePDFInvoice\Renderer\ListBullet;
use WooCommercePDFInvoice\Css\Stylesheet;
 ...
 * the {@link WooCommercePDFInvoice\Adapter\CPDF} class provides a simple, stateless interface to
 * PDF generation.)  PDFLib support has now also been added, via the {@link
 * WooCommercePDFInvoice\Adapter\PDFLib}.
 *
 *
 * @package dompdf
 */
class Dompdf
{
    ...
    */
    ...
    public function loadHtml($str, $encoding = 'UTF-8')
    {
        ...
        set_error_handler(array("\\WooCommercePDFInvoice\\Helpers", "record_warnings"));
        ...
        if ($this->options->isHtml5ParserEnabled() && class_exists("WooCommercePDFInvoice_HTML5_Tokenizer")) {
            $tokenizer = new WooCommercePDFInvoice_HTML5_Tokenizer($str);
            $tokenizer->parse();
            $doc = $tokenizer->save();
            ...
            $quirksmode = ($tokenizer->getTree()->getQuirksMode() > WooCommercePDFInvoice_HTML5_TreeBuilder::NO_QUIRKS);
        } else {
            ...
            foreach ($this->tree->get_frames() as $frame) {
                ...
                if ($frame->get_node()->nodeName == "dompdf_generated") {
                    ...
                    $decoded_string = preg_replace_callback("/\\\\([0-9a-fA-F]{0,6})/",
                        function ($matches) { return \WooCommercePDFInvoice\Helpers::unichr(hexdec($matches[1])); },
                        $style->content);
     ...
     *
     * @param string $size 'letter', 'legal', 'A4', etc. {@link WooCommercePDFInvoice\Adapter\CPDF::$PAPER_SIZES}
    ...
    /**
     * Return the underlying Canvas instance (e.g. WooCommercePDFInvoice\Adapter\CPDF, WooCommercePDFInvoice\Adapter\GD)
     *
     * @return Canvas
     */
