...
if ( ! class_exists( 'WC_Sale_Flash_Pro' ) ) {
	...
	class WC_Sale_Flash_Pro {
		...
		public function __construct() {
			...
			$this->settings = array(
				array(
					'name'    => esc_html__( 'Sale flash type', 'wc_sale_flash' ),
					'desc'    => esc_html__( 'How sale flashes are displayed. Can be overridden by editing products.', 'wc_sale_flash' ),
					'id'      => 'woocommerce_sale_flash_type',
					'css'     => 'min-width:175px;',
					'type'    => 'select',
					'options' => array(
						'percent' => esc_html__( 'Percent e.g. 25% off!', 'wc_sale_flash' ),
						'amount'  => esc_html__( 'Amount e.g. &pound;25 off!', 'wc_sale_flash' ),
					),
				),
				array(
					'name' => esc_html__( 'Sale flash price', 'wc_sale_flash' ),
					'desc' => esc_html__( 'Show original price?', 'wc_sale_flash' ),
					'id'   => 'woocommerce_sale_flash_original_price',
					'type' => 'checkbox',
					'std'  => 'no',
				),
				array(
					'name' => esc_html__( 'Enable for variations', 'wc_sale_flash' ),
					'desc' => esc_html__( 'Show sale flashes for variable products e.g. Up to 40% off!', 'wc_sale_flash' ),
		...
		public function sale_flash( $html, $post, $product ) {
			...
			if ( $product->has_child() ) {
				...
				if ( 'percent' === $sale_flash_type ) {
					...
					$html = '<span class="onsale">' . sprintf( esc_html__( 'up to %s%% off!', 'wc_sale_flash' ), $discount ) . '</span>';

				} elseif ( 'amount' === $sale_flash_type && $discount > 0 ) {
					/* translators: 1: discount price */
					$html = '<span class="onsale">' . sprintf( esc_html__( 'up to %s off!', 'wc_sale_flash' ), wc_price( $discount ) ) . '</span>';
				}
				...
				if ( 'percent' === $sale_flash_type ) {
					...
					/* translators: 1: discount price */
					$html = '<span class="onsale">' . sprintf( esc_html__( '%s%% off!', 'wc_sale_flash' ), $discount ) . '</span>';
					...
					if ( $discount > 0 ) {
						/* translators: 1: discount price */
						$html = '<span class="onsale">' . sprintf( esc_html__( '%s off!', 'wc_sale_flash' ), wc_price( $discount ) ) . '</span>';
					}
				}
			}
		...
		public function write_panel() {
			woocommerce_wp_select(
				array(
					'id'          => 'woocommerce_sale_flash_type',
					'label'       => esc_html__( 'Sale flash type', 'wc_sale_flash' ),
					'description' => esc_html__( 'Choose a sale flash style for this product.', 'wc_sale_flash' ),
					'options'     => array(
						''        => esc_html__( 'Default', 'wc_sale_flash' ),
						'percent' => esc_html__( 'Percent', 'wc_sale_flash' ),
						'amount'  => esc_html__( 'Amount', 'wc_sale_flash' ),
					),
				)
			);
			woocommerce_wp_checkbox(
				array(
					...
					'label'       => esc_html__( 'Hide sale flash', 'wc_sale_flash' ),
					'description' => esc_html__( 'Hide the sale flash for this product.', 'wc_sale_flash' ),
				)
			);
		}
