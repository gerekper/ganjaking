...
class AJAX {
	...
	public function start_applying_costs_to_previous_orders() {
		...
		try {
			...
			if ( isset( $_POST['previous_orders'] ) && in_array( $_POST['previous_orders'], array( 'orders-without-costs', 'all-orders' ), true ) ) {
				$which_orders = $_POST['previous_orders']; // maybe 'all-orders'
			} else {
				$which_orders = 'orders-without-costs'; // default safe handling
			}

			// create a background job
			$job = wc_cog()->get_previous_orders_handler_instance()->create_job( array(
				'object_ids' => $this->get_order_ids( $which_orders ),
				'orders'     => $which_orders, /** @see Previous_Orders_Handler::process_item() */
			) );
