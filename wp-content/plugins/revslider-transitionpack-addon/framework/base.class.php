...
/*ST*/		'burnover' => "`uniform vec4 resolution; uniform float progress; uniform sampler2D src1; uniform sampler2D src2; uniform sampler2D displacement; uniform int useTexture; varying vec2 vUv; void main() { float p = progress; vec2 uv = (vUv - vec2(0.5)) * resolution.zw + vec2(0.5); vec3 col = vec3(0.); vec3 heightmap = TEXTURE2D(displacement, uv).rrr; vec3 background = TEXTURE2D(src1, uv).rgb; vec3 foreground = TEXTURE2D(src2, uv).rgb; float t = p * 1.2; vec3 erosion = smoothstep(t - .2, t, heightmap); vec3 border = smoothstep(0., .1, erosion) - smoothstep(.1, 1., erosion); col = (1. - erosion) * foreground + erosion * background; vec3 leadcol = vec3(1., .5, .1); vec3 trailcol = vec3(0.2, .4, 1.); vec3 fire = mix(leadcol, trailcol, smoothstep(0.8, 1., border)) * 2.; col += border * fire; gl_FragColor = vec4(col, 1.0); }`",
