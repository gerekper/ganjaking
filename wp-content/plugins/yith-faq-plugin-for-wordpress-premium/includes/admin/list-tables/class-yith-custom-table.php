	...
	class YITH_Custom_Table extends WP_List_Table {
		...
		public function prepare_items() {
			...
			if ( false === $total_items ) {
				$total_items = $wpdb->get_var( "SELECT COUNT(*) FROM $count_table $count_where" ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			...
			$paged     = isset( $getted['paged'] ) ? intval( $getted['paged'] ) : 0;
			$paged     = ( $paged > $total_pages ) ? $total_pages : $paged;
			$paged     = ( $paged > 0 ) ? $select_limit * ( $paged - 1 ) : 0;
			...
			$this->items = $wpdb->get_results( $sql, ARRAY_A ); //phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL.NotPrepared
		...
		protected function get_table_classes() {
			...
			if ( isset( $this->options['custom_css_classes'] ) ) {
				return $this->options['custom_css_classes'];
			} else {
