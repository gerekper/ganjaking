...
jQuery(document).ready(function($){
	...
	// GET attendee list
		...
		// get individual attendee information
			$('.evors_lightbox').on('click','em.evorsadmin_rsvp',function(){
				OBJ = $(this);
				var data_arg = {
				...
				$.ajax({
					beforeSend: function(){ $('.evors_lightbox .ajde_popup_text').addClass('loading'); },					
					url:evors_admin_ajax_script.ajaxurl,
					data: data_arg,type: 'POST',dataType:'json',					
					success:function(data){
						//alert(data);
						if(data.status=='0'){
							$('.evors_attendee_info_lightbox').find('.ajde_popup_text').html(data.content);
							$('.evors_attendee_info_lightbox').addClass('show');
						}else{
							// $('.evors_lightbox').find('.ajde_popup_text').html('Could not load attendee list');
						}
					},complete:function(){ $('.evors_lightbox .ajde_popup_text').removeClass('loading');}
				});	
			});
