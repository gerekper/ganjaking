			...
			foreach ( $package['contents'] as $item_id => $values ) {
				...
				if ( $values['quantity'] > 0 ) {
					if ( $values['data']->needs_shipping() ) {
						...
						if ( $rule ) {
							$item_shipping_cost += (float) $rule->rule_item_cost * $values['quantity'];
							$item_shipping_cost += (float) $rule->rule_cost;
						} elseif ( $this->cost === '0' || $this->cost > 0 ) {
							// Use default
							$item_shipping_cost += (float) $this->cost * $values['quantity'];
						} else {
						...
						// Fee
						$item_shipping_cost += (float) $this->get_fee( $this->fee, $item_shipping_cost ) * $values['quantity'];

						$shipping_cost += $item_shipping_cost;

						if ( get_option( 'woocommerce_calc_taxes' ) === 'yes' && 'taxable' === $this->tax_status && 'none' !== $product_tax_status ) {
