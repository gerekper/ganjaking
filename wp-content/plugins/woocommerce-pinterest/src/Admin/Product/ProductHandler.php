...
class ProductHandler {
	...
	public function init() {
		...
		// Variations
		add_action('woocommerce_variation_options', array($this, 'addPinTitleOption'), 10, 3);
		add_action('woocommerce_variation_options', array($this, 'addPinDescriptionOption'), 10, 3);
		add_action('woocommerce_product_after_variable_attributes', array($this, 'renderPinTitleField'), 10, 3);
    ...
    $sanitizedHasTitleFields = filter_input(INPUT_POST, 'variable_is_pin_title', FILTER_VALIDATE_BOOLEAN, FILTER_FORCE_ARRAY);

    $hasTitle = ! empty($sanitizedHasTitleFields[$i]);

    $this->pinService->setVariationHasPinTitle($variationId, $hasTitle);

    $sanitizedPinTitleTemplateFields = filter_input(INPUT_POST, 'pin_title_template', FILTER_SANITIZE_STRING, FILTER_FORCE_ARRAY);
    $sanitizedPinTitleTemplate       = isset($sanitizedPinTitleTemplateFields[$i]) ?  sanitize_text_field($sanitizedPinTitleTemplateFields[$i]) : '';

    $this->pinService->updatePinTitleTemplate($variationId, $sanitizedPinTitleTemplate);
  ...
  /**
   * Render variation pin title field
   *
   * @param int $loop
   * @param array $variation_data
   * @param WP_Post $variation
   */
  public function renderPinTitleField( $loop, $variation_data, $variation ) {
    $this->fileManager->includeTemplate('admin/woocommerce/variation-pin-title.php', array(
      'hidden' => ! $this->pinService->isVariationHasPinTitle($variation->ID),
      'loop' => $loop,
      'pinTitle' => $this->pinService->getProductPinTitleTemplate($variation->ID),
   ...
   * @param WP_Post $variation
   */
  public function addPinTitleOption( $loop, $variation_data, $variation ) {
    $this->fileManager->includeTemplate('admin/woocommerce/variation-title-option.php', array(
      'loop' => $loop,
      'isVariableHasPinTitle' => $this->pinService->isVariationHasPinTitle($variation->ID)
    ));
  }
    ...
    $dbCarousel = $this->pinService->carouselIsPinned($post->ID);
    $dbCarousel = ! empty( $dbCarousel );
		...
		try {
			...
			$title                = $this->pinService->getProductPinTitleTemplate($post->ID);
    ...
    $pinTitle = filter_input(INPUT_POST, 'woocommerce_pinterest_pin_title_template', FILTER_SANITIZE_STRING);
    $pinTitle = $pinTitle ? $pinTitle : '';
    $pinTitle = sanitize_text_field($pinTitle);
    ...
    $carousel = filter_input(INPUT_POST, 'woocommerce_pinterest_images_carousel', FILTER_SANITIZE_STRING, FILTER_FORCE_ARRAY);
    $createCarousel = ! empty( $carousel );

		$this->pinService->synchronize($post_id, $selectedAttachments, $createCarousel);

		$this->pinService->updatePinDescriptionTemplate($post_id, $pinDescription);
		$this->pinService->updatePinTitleTemplate($post_id, $pinTitle);
	}
