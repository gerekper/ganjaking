...
class AuthHandler {
	...
	/**
	 * Base request url for v1 API requests
	 *
	 * @var string
	 */
	private $v1BaseUrl = 'https://connect.woocommerce.com/login/pinterestv1?';
	...
	public function handlePinterestAuthResponse() {
		$token           = filter_input(INPUT_GET, 'pinterestv1_access_token', FILTER_SANITIZE_STRING);
		$tokenApiVersion = 'v1';
		if (! $token) {
		...
		if ($token && $nonce && get_transient($transientName) === $nonce) {
			update_option('v1' === $tokenApiVersion ? ApiState::V1_TOKEN_OPTION_NAME : ApiState::V3_TOKEN_OPTION_NAME, $token);
	...
	public function connectPinterest() {
		...
		$allowedApiVersions = array('v1', 'v3');

		if (! in_array($apiVersion, $allowedApiVersions, true)) {
			wp_die('unknown API version');
		}

		if ('v1' !== $apiVersion && ! $this->apiState->isConnected('v1')) {
			$notifier = ServiceContainer::getInstance()->getNotifier();
			$notifier->flash(__('You must be connected to the basic level API first.', 'woocommerce-pinterest'), AdminNotifier::ERROR);
		...
		$baseUrl = 'v1' === $apiVersion ? $this->v1BaseUrl : $this->v3BaseUrl;
