...
class Reports_Controller {
    ...
    public function overdue_tasks_csv( WP_REST_Request $request ) {
        ...
        foreach ( $results['data'] as $key => $result ) {
            ...
            foreach ( $result['task_lists']['data'] as $key => $list_results ) {
                ...
                fputcsv( $output, ['',''] );
                fputcsv( $output, [__('Tasks', 'pm-pro'), __('Date Between','pm-pro'), __('Created At','pm-pro'), __('Created By','pm-pro')] );
                ...
                foreach ( $list_results['tasks']['data'] as $key => $task ) {
                    ...
                    fputcsv( $output, [$task['title'], $date_between, $created_at, $creator] );
                }
    ...
    public function milestone_tasks_CSV( WP_REST_Request $request ) {
        ...
        $results    = $this->milestone_tasks( $request );
        $total      = $results['meta']['total_result'];
        $user_id    = intval( $request->get_param('user_id') );
        $project_id = intval( $request->get_param('project_id') );
        ...
        $date       = 'Date between ' . $start_date . ' to ' . $due_date;
        $milestone  = $results['meta']['milestone']['title'];
        ...
        fputcsv( $output, [$all_project, $milestone, $total] );
        fputcsv( $output, ['',''] );
        ...
        foreach ( $results['data'] as $key => $result ) {
            ...
            foreach ( $result['task_lists']['data'] as $key => $list_results ) {
                ...
                fputcsv( $output, [__( 'Task List title: ', 'pm-pro'  ) . $list_results['title']] );
                fputcsv( $output, ['',''] );
                fputcsv( $output, [__('Tasks', 'pm-pro'), __('Assigned User','pm-pro'), __('Date Between','pm-pro'), __('Tracked Time','pm-pro'), __('Completed At','pm-pro'), __('Status','pm-pro')] );

                foreach ( $list_results['tasks']['data'] as $key => $task ) {

                    $users = '';
                    foreach( $task['assignees']['data'] as $user ) {
                    ...
                    $created_at   = $task['created_at']['date'];
                    $creator      = $task['creator']['data']['display_name'];
                    $completed_at = $task['completed_at']['date'] ? $task['completed_at']['date'] : '--';
                    $status       = $task['status'];
                    ...
                    fputcsv( $output, [$task['title'], $users, $date_between, $trackd_time, $completed_at, $status] );
                }
    ...
    public function unassigned_CSV( WP_REST_Request $request ) {
        ...
        foreach ( $results['data'] as $key => $result ) {
            ...
            foreach ( $result['task_lists']['data'] as $key => $list_results ) {
                ...
                fputcsv( $output, [__( 'Task List title: ', 'pm-pro'  ) . $list_results['title']] );
                fputcsv( $output, [__('Tasks', 'pm-pro'), __('Date Between','pm-pro'), __('Created At','pm-pro'), __('Status','pm-pro'), __('Created By','pm-pro')] );
                ...
                foreach ( $list_results['tasks']['data'] as $key => $task ) {
                    ...
                    fputcsv( $output, [$task['title'], $date_between, $created_at, $status, $creator] );
                }
