<?php
/**
 * Class YITH_WCBK_Global_Price_Rule_Data_Store
 * Data store for global price rule.
 *
 ...
 * @since 5.0.0
 */
class YITH_WCBK_Global_Price_Rule_Data_Store extends YITH_WCBK_Custom_Table_Data_Store {
	...
	protected $keys_to_props = array(
		...
		'base_price_operator' => 'base_price_operator',
		'base_price'          => 'base_price',
		'change_base_fee'     => 'change_base_fee',
		'base_fee_operator'   => 'base_fee_operator',
		'base_fee'            => 'base_fee',
		'priority'            => 'priority',
	 ...
	 * Create
	 *
	 * @param YITH_WCBK_Global_Price_Rule $rule The rule.
	 */
	public function create( &$rule ) {
		global $wpdb;
		$data = $this->get_data_to_update( $rule, true );

		$result = $wpdb->insert( $wpdb->yith_wcbk_global_price_rules, $data );
		...
		if ( $id ) {
			...
			do_action( 'yith_wcbk_global_price_rule_created', $rule );
	...
	public function read( &$rule ) {
		...
		$props = $wpdb->get_row(
			$wpdb->prepare( "SELECT * from {$wpdb->yith_wcbk_global_price_rules} where id=%d", $rule->get_id() ),
			ARRAY_A
		);
		...
		do_action( 'yith_wcbk_global_price_rule_read', $rule );
	}
	...
	public function update( &$rule ) {
		...
		if ( $data ) {
			$result = $wpdb->update( $wpdb->yith_wcbk_global_price_rules, $data, array( 'id' => $rule->get_id() ) );
		...
		if ( $updated ) {
			$rule->apply_changes();
			do_action( 'yith_wcbk_global_price_rule_updated', $rule );
	...
	public function delete( &$rule, $args = array() ) {
		...
		if ( $id ) {
			$object = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->yith_wcbk_global_price_rules} WHERE id = %d", $id ) );

			if ( $object ) {
				do_action( 'yith_wcbk_global_price_rule_before_delete', $rule );

				$result = $wpdb->delete( $wpdb->yith_wcbk_global_price_rules, array( 'id' => $id ) );

				$this->delete_associations( $rule );

				$rule->set_id( 0 );
				do_action( 'yith_wcbk_global_price_rule_deleted', $id );
	...
	protected function get_data_to_update( $rule, $force = false ) {
		...
		$serializable_props = array( 'conditions' );

		$data = array();
		foreach ( $props_to_update as $key => $prop ) {
			$value = $rule->{"get_$prop"}( 'edit' );

			if ( in_array( $prop, array( 'enabled', 'exclude_products', 'change_base_fee', 'change_base_price' ), true ) ) {
				$value = wc_string_to_bool( $value );
			}

			if ( in_array( $prop, array( 'base_price', 'base_fee' ), true ) ) {
	...
	protected function read_excluded_product_ids( $rule ) {
		...
		return array_map(
			...
			$wpdb->get_col(
				$wpdb->prepare(
					"SELECT object_id FROM $wpdb->yith_wcbk_global_price_rules_associations WHERE rule_id = %d",
					$rule->get_id()
				)
			)
		);
			...
			switch ( $prop ) {
				case 'excluded_product_ids':
					...
					foreach ( $to_add as $object_id ) {
						$updated = true;
						$wpdb->insert(
							$wpdb->yith_wcbk_global_price_rules_associations,
							array(
								'rule_id'   => $rule->get_id(),
								'object_id' => $object_id,
							)
					...
					if ( $to_remove ) {
						...
						$wpdb->query(
							$wpdb->prepare(
								"DELETE FROM $wpdb->yith_wcbk_global_price_rules_associations WHERE rule_id = %d AND object_id IN ($to_remove_ids)", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
								$rule->get_id()
							)
						);
					}
	...
	protected function delete_associations( $rule ) {
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM $wpdb->yith_wcbk_global_price_rules_associations WHERE rule_id = %d",
				$rule->get_id()
			)
		);
	}
	 ...
	 * @param array $args Arguments.
	 *
	 * @return YITH_WCBK_Global_Price_Rule[]|int[]|int
	 */
	public function query( $args = array() ) {
		...
		$table              = $wpdb->yith_wcbk_global_price_rules;
		$associations_table = $wpdb->yith_wcbk_global_price_rules_associations;
		$alias              = 'global_price_rules';
		...
		if ( 'ids' === $args['return'] ) {
			...
			$items = $wpdb->get_col( $query ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
			$items = array_filter( array_map( 'yith_wcbk_get_global_price_rule', $items ) );
		}
	...
	public static function get_greatest_priority() {
		...
		$max = $wpdb->get_var( "SELECT MAX(priority) FROM {$wpdb->yith_wcbk_global_price_rules}" );
	...
	public static function update_priorities( $values ) {
		...
		foreach ( $values as $value ) {
			...
			if ( ! ! $id && false !== $priority ) {
				...
				$wpdb->update( $wpdb->yith_wcbk_global_price_rules, array( 'priority' => $priority ), array( 'id' => $id ) );
