...
class WC_Local_Pickup_Plus_Integrations {
	...
	public function __construct() {
		...
		if ( $this->is_avatax_active() ) {
			$this->avatax = wc_local_pickup_plus()->load_class( '/includes/integrations/AvaTax.php', \SkyVerge\WooCommerce\Local_Pickup_Plus\Integrations\AvaTax::class );
		}

		if ( $this->is_composite_products_active() ) {
			$this->composite_products = wc_local_pickup_plus()->load_class( '/includes/integrations/woocommerce-composite-products/class-wc-local-pickup-plus-integration-composite-products.php', 'WC_Local_Pickup_Plus_Integration_Composite_Products' );
		}

		// WooCommerce Customer Order CSV Export
		if ( $this->is_csv_export_active() ) {
			$this->csv_export = wc_local_pickup_plus()->load_class( '/includes/integrations/Customer_Order_Coupon_Export.php', \SkyVerge\WooCommerce\Local_Pickup_Plus\Integrations\Customer_Order_Coupon_Export::class );
		}
		...
		if ( $this->is_mix_and_match_products_active() ) {
			$this->mix_and_match_products = wc_local_pickup_plus()->load_class( '/includes/integrations/woocommerce-mix-and-match-products/class-wc-local-pickup-plus-integration-mix-and-match-products.php', 'WC_Local_Pickup_Plus_Integration_Mix_And_Match_Products' );
		}
		...
		if ( $this->is_product_bundles_active() ) {
			$this->product_bundles = wc_local_pickup_plus()->load_class( '/includes/integrations/woocommerce-product-bundles/class-wc-local-pickup-plus-integration-product-bundles.php', 'WC_Local_Pickup_Plus_Integration_Product_Bundles' );
		}
