...
class WC_Local_Pickup_Plus_Orders {
	...
	public function get_orders_by_appointment_range( $start = 0, $end = 0 ) {
		...
		$item_results   = $wpdb->get_results( "
			SELECT order_item_id
			FROM {$order_itemmeta}
			WHERE ( order_item_id IN ( SELECT order_item_id FROM {$order_itemmeta} WHERE meta_key = '_pickup_appointment_start' AND meta_value >= {$start} )
				AND order_item_id IN ( SELECT order_item_id FROM {$order_itemmeta} WHERE meta_key = '_pickup_appointment_end' AND meta_value <= {$end} ) )
				OR ( meta_key = '_pickup_date' AND UNIX_TIMESTAMP( STR_TO_DATE( meta_value, '%Y-%m-%d' ) ) BETWEEN {$start} AND {$end} )
		", ARRAY_N );
			...
			if ( is_admin() ) {
				...
				if ( ! empty ( $packages ) ) {
					...
					foreach ( $packages as $key => $package ) {
						...
						$shipping_method = wc_local_pickup_plus()->get_packages_instance()->get_chosen_shipping_method_for_package( $key, $package );
