...
class WC_Local_Pickup_Plus_Packages {
	...
	public function handle_packages( $packages ) {
		...
		if ( ! empty( $packages ) && $pickup_data && $local_pickup_plus->is_available() ) {
	 ...
	 *
	 * This methods is a backwards compatible implementation of a WooCommerce core function introduced in 3.2.0.
	 * @see wc_get_chosen_shipping_method_for_package()
	 * Should be deprecated when 3.2.0 is the minimum supported version.
	 *
	 * @since 2.7.5
	 *
	...
	/**
	 * Determines if the shipping methods have changed since the last request.
	 *
	 * Do not open to public, this is a backwards replacement of a core method added in WooCommerce 3.2.0 that is needed internally to support yet another replacement method.
	 * @see \WC_Local_Pickup_Plus_Packages::get_chosen_shipping_method_for_package()
	 * @see wc_get_chosen_shipping_method_for_package()
	 * @see wc_shipping_methods_have_changed()
	 *
	 * @since 2.7.5
	 *
	 * @param int|string $key package ID
	 ...
	 * @return bool
	 */
	private function shipping_methods_have_changed( $key, $package ) {

		if ( function_exists( 'wc_shipping_methods_have_changed' ) ) {

			$shipping_methods_have_changed = wc_shipping_methods_have_changed( $key, $package );
			...
			$shipping_methods_have_changed = $new_rates !== $prev_rates;
		}

		return $shipping_methods_have_changed;
	}


	/**
	 * Gets the default shipping method of a package.
	 *
	 * Do not open to public, this is a backwards replacement of a core method added in WooCommerce 3.2.0 that is needed internally to support yet another replacement method.
	 * @see \WC_Local_Pickup_Plus_Packages::get_chosen_shipping_method_for_package()
	 * @see wc_get_chosen_shipping_method_for_package()
	 ...
	 * @since 2.7.5
	 *
	 * @param int $key package ID
	 * @param array $package package data
	 * @param string $chosen_method chosen method ID
	 * @return string
	 */
	private function get_default_shipping_method_for_package( $key, $package, $chosen_method ) {

		if ( function_exists( 'wc_get_default_shipping_method_for_package' ) ) {

			$default_shipping_method_for_package = wc_get_default_shipping_method_for_package( $key, $package, $chosen_method );
			...
			$default_shipping_method_for_package = apply_filters( 'woocommerce_shipping_chosen_method', $default, $package['rates'], $chosen_method );
		}

		return $default_shipping_method_for_package;
	}
