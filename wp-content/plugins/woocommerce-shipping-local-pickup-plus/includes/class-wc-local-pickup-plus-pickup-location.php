...
class WC_Local_Pickup_Plus_Pickup_Location {
	 ...
	 *
	 * TODO remove this deprecated method by version 3.0.0 or by November 2020 {FN 2019-11-21}
	 *
	 * @param int $start_time optional local start time (defaults to current local timestamp)
	 * @return int local timestamp of the first available pickup time
	 */
	public function get_first_available_pickup_time( $start_time = null ) {

		wc_deprecated_function( __METHOD__, '2.7.0', 'wc_local_pickup_plus_get_pickup_location()->get_appointments()->get_first_available_pickup_time()' );

		try {
			return $this->get_appointments()->get_first_available_pickup_time( new \DateTime( empty( $start_time ) ? 'now' : date( 'Y-m-d H:i:s', strtotime( 'now', (int) $start_time ) ) ) )->getTimestamp();
		} catch ( \Exception $e ) {
