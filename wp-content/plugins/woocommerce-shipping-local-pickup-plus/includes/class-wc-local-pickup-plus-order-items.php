...
class WC_Local_Pickup_Plus_Order_Items {
	...
	/**
	 * Gets the scheduled pickup date for an order shipping item.
	 *
	 * TODO remove this deprecated method by version 3.0.0 or November 2020 {FN 2019-11-29}
	 *
	 * @since 2.0.0
	 * @deprecated 2.7.0
	 *
	...
	public function get_order_item_pickup_date( $order_item, $format = 'Y-m-d' ) {
		...
		wc_deprecated_function( __METHOD__, '2.7.0', 'Appointment::get_start()->format( $format ) or Appointment::get_start()->getTimestamp()' );

		try {

			$appointment = new Appointment( $order_item );

			if ( 'timestamp' === $format ) {
				$pickup_date = $appointment->get_start()->setTime( 0, 0, 0 )->getTimestamp();
			} else {
				$pickup_date = $appointment->get_start()->setTime( 0, 0, 0 )->format( $format );
			}
	...
	/**
	 * Sets the pickup date for a pickup order item.
	 *
	 * TODO remove this deprecated method by version 3.0.0 or November 2020 {FN 2019-11-29}
	 *
	 * @since 2.0.0
	...
	/**
	 * Gets the minimum pickup time set for an order item.
	 *
	 * TODO remove this deprecated method by version 3.0.0 or November 2020, whichever comes first {FN 2019-11-29}
	 *
	 * @since 2.3.5
	 * @deprecated 2.7.0
	 *
	...
	public function get_order_item_pickup_minimum_hours( $order_item ) {
		...
		try {
			$hours = max( 0, (int) wc_get_order_item_meta( $order_item instanceof \WC_Order_Item ? $order_item->get_id() : $order_item, '_pickup_minimum_hours' ) );
		} catch ( \Exception $e ) {
			$hours = 0;
		}
	 ...
	 *
	 * TODO: remove this method by version 3.0.0 or November 2020 {WV 2019-11-22}
	 *
	 * @since 2.3.5
	 * @deprecated 2.7.0
	 *
