...
class Checkout {
	...
	public function add_template_variables_to_packages( $packages ) {
		...
		foreach ( $packages as $key => $package ) {
			...
			$chosen_shipping_method = wc_local_pickup_plus()->get_packages_instance()->get_chosen_shipping_method_for_package( $key, $package );
	...
	public function add_package_group_row_start( $template_name, $template_path, $located, $args ) {
		...
		if ( 'cart/cart-shipping.php' === $template_name && ! empty( $args['package'] ) ) {
			...
			if ( ! empty( $package['first_pickup_package'] ) ) {
				...
				echo '<tr><th>' . esc_html( wc_local_pickup_plus_shipping_method()->get_method_title() ) . '</th><td>';
				echo $this->add_package_wrapper_start();
			}

			if ( ! empty( $package['first_shipping_package'] ) ) {

				echo '<tr><th>' . esc_html__( 'Shipping', 'woocommerce-shipping-local-pickup-plus' ) . '</th><td>';
				echo $this->add_package_wrapper_start();
			}
		}
	}
	...
	/**
	 * Adds a toggle to enable pickup when no shipping methods are available at checkout.
	 *
	 * If the no shipping methods available text string is being output, while Local Pickup Plus is available, it is safe to assume pickup should be always a possibility if at least one item in cart can be picked up.
	 *
	 * TODO remove this method by version 3.0.0 or November 2020 {DM 2019-11-15}
	 *
	 * @internal
	 *
	 * @since 2.3.17
	 ...
	 *
	 * @param string $no_shipping_available_text text displayed to customer when no shipping methods are available at checkout (may contain HTML)
	 * @return string HTML
	 */
	public function add_pickup_toggle_when_no_shipping_available( $no_shipping_available_text ) {
