...
class Frontend {
	...
	public function __construct() {
		...
		if ( $local_pickup_plus && $local_pickup_plus->is_available() ) {
			...
			require_once( $plugin_path . '/includes/frontend/Data_Store/Pickup_Data.php' );
			require_once( $plugin_path . '/includes/frontend/Data_Store/Cart_Item_Pickup_Data.php' );
			require_once( $plugin_path . '/includes/frontend/Data_Store/Package_Pickup_Data.php' );

			// load field objects
			require_once( $plugin_path . '/includes/frontend/Fields/Field.php' );
			require_once( $plugin_path . '/includes/frontend/Fields/Pickup_Location_Field.php' );
			require_once( $plugin_path . '/includes/frontend/Fields/Cart_Item_Pickup_Location_Field.php' );
			require_once( $plugin_path . '/includes/frontend/Fields/Package_Pickup_Location_Field.php' );
			require_once( $plugin_path . '/includes/frontend/Fields/Cart_Item_Handling_Toggle.php' );
			require_once( $plugin_path . '/includes/frontend/Fields/Package_Pickup_Appointment_Field.php' );
			require_once( $plugin_path . '/includes/frontend/Fields/Package_Pickup_Items_Field.php' );

			// load handlers
			$this->cart     = wc_local_pickup_plus()->load_class( '/includes/frontend/Cart.php', 'SkyVerge\WooCommerce\Local_Pickup_Plus\Cart' );
			$this->checkout = wc_local_pickup_plus()->load_class( '/includes/frontend/Checkout.php', 'SkyVerge\WooCommerce\Local_Pickup_Plus\Checkout' );
	...
	public function force_save_session_after_updating_shipping_method( $template_name ) {
		...
		if ( 'cart/cart-shipping.php' === $template_name && is_ajax() && 'update_shipping_method' === Framework\SV_WC_Helper::get_requested_value( 'wc-ajax' ) ) {
