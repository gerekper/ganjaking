<?php
/**
 * WooCommerce Order Delivery requirements
 *
 * @package WC_OD
 * @since   2.0.0
 */
	...
	protected static function check_requirements() {
		if ( ! self::is_php_compatible() ) {
			self::$errors[] = sprintf(
				...
				_x( '<strong>WooCommerce Order Delivery</strong> requires PHP %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-order-delivery' ),
			...
			self::$errors[] = sprintf(
				...
				_x( '<strong>WooCommerce Order Delivery</strong> requires WordPress %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-order-delivery' ),
				self::MINIMUM_WP_VERSION,
				get_bloginfo( 'version' )
			);
		} elseif ( ! is_woocommerce_active() ) {
			self::$errors[] = _x( '<strong>WooCommerce Order Delivery</strong> requires WooCommerce to be activated to work.', 'admin notice', 'woocommerce-order-delivery' );
		} elseif ( ! self::is_wc_compatible() ) {
			self::$errors[] = sprintf(
				/* translators: 1: Minimum WooCommerce version 2: Current WooCommerce version */
				_x( '<strong>WooCommerce Order Delivery</strong> requires WooCommerce %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-order-delivery' ),
