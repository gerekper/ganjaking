		...
		* * * * * * * * * */
		...
		private function _hooks() {
			...
			if ( ! isset( $_GET['customize_changeset_uuid'] ) && $cap_lost ) {
				add_action( 'lostpassword_post', array( $this, 'loginpress_recaptcha_lostpassword_auth' ) );
			}
      ...
      $cap_type   = isset( $this->loginpress_settings['recaptcha_type'] ) ? $this->loginpress_settings['recaptcha_type'] : 'v2-robot';
					...
					<script type="text/javascript">
						...
						var onloadV2Callback = function() {
							grecaptcha.render('wp-submit', {
								'sitekey' : '<?php echo $this->loginpress_settings['site_key']; ?>',
								'callback' : onSubmit
							});
						};
					</script>
			...
			if ( 'v3' == $cap_type ) {
				...
				if ( ! empty( $cap_site ) && ! empty( $cap_secret ) ) :
					...
					<script src="https://www.google.com/recaptcha/api.js?render=<?php echo $cap_site; ?>"></script>
				  <script>
							grecaptcha.ready(function() {
							grecaptcha.execute('<?php echo $cap_site; ?>', {action: 'loginpage'}).then(function(token) {
