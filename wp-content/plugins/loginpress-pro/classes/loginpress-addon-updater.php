...
class LoginPress_AddOn_Updater {
  ...
  function __construct( $_addon_id, $_plugin_file, $_version ) {
    ...
    // var_dump($_addon_id);
    // var_dump(plugin_basename( $_plugin_file ));
    // var_dump($_version);

    $this->api_url    = 'https://wpbrigade.com/';
    //$this->api_url    = 'http://localhost/wpedd/';
  ...
  public function show_update_notification( $file, $plugin ) {
    ...
    if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $update_cache->response[ $this->name ]->new_version, '<' ) ) {
      ...
      if ( empty( $version_info->download_link ) ) {
        printf(
          __( 'There is a new version of %1$s availablee. <a target="_blank" class="thickbox" href="%2$s">View version %3$s details</a>.', 'loginpress-pro' ),
        ...
        printf(
          __( 'There is a new version of %1$s availablee. <a target="_blank" class="thickbox" href="%2$s">View version %3$s details</a> or <a href="%4$s">update now</a>.', 'loginpress-pro' ),
          esc_html( $version_info->name ),
          esc_url( $changelog_link ),
          esc_html( $version_info->new_version ),
  ...
  private function api_request( $_action, $_data ) {
    ...
    //$data['license'] = 'd7d4172f9397810ed44b8b9b595998d6'; //get_option( 'loginpress_pro_license_key' );
  ...
  public function show_changelog() {
    if( empty( $_REQUEST['loginpress_action'] ) || 'view_plugin_changelog' != $_REQUEST['loginpress_action'] ) {
        return;
    }
