...
if( !class_exists( 'YITH_WC_Pending_Survey_Email_Table' ) ){

    class YITH_WC_Pending_Survey_Email_Table extends  WP_List_Table{
        ...
        public function __construct()
        {
            ...
            parent::__construct( array(
                'singular' => _x('email', 'yith-woocommerce-pending-order-survey'),     //singular name of the listed records
                'plural' => _x('emails', 'yith-woocommerce-pending-order-survey'),    //plural name of the listed records
                'ajax' => false        //does this table support ajax?
            ));

            $this->post_type = YITH_Pending_Email_Type()->post_type_name;
        ...
        public function get_columns()
        {
           $columns = array(
             ...
             'send_after'   => __( 'Send After', 'yith-woocommerce-pending-order-survey' ),
              'email_status' => __('Status', 'yith-woocommerce-pending-order-survey'),
            ...
            switch( $column_name ){
                ...
                case 'post_title':
                    ...
                    $actions            = array(
                        ...
                        'delete'    => '<a href="'.$this->get_delete_post_link('', $item->ID ).'">'.__( 'Delete', 'yith-woocommerce-pending-order-survey' ) . '</a>'
                    );
                ...
                case 'send_after':
                    ...
                    $after = sprintf('%s minutes', get_post_meta( $item->ID, '_ywcpos_send_after', true ) );
                ...
                case 'email_status':
                    ...
                    $enable = get_post_meta(  $item->ID,'_ywcpos_enable_email', true );
                    if ($enable)
                        echo '<mark class="show tips" data-tip="enabled">enabled</mark>';
                    else
                        echo '<mark class="hide tips" data-tip="disabled">disabled</mark>';
        ...
        public function get_bulk_actions() {
            ...
            if( isset( $_REQUEST['ywcpos_email_ids'] ) ){

                $pending_surveys = $_REQUEST['ywcpos_email_ids'];
        ...
        public function column_cb($item) {
            return sprintf(
                '<input type="checkbox" name="ywcpos_email_ids[]" value="%s" />',  $item->ID
            );
        }
