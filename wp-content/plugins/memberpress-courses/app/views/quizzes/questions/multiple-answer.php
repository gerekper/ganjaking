...
<?php
  ...
  $classes = ['mpcs-quiz-question', 'mpcs-quiz-question-multiple-answer'];
      ...
      <?php if(isset($question->options) && is_array($question->options)) : ?>
        <?php foreach($question->options as $index => $option) : ?>
          <?php
            ...
            if($viewing_attempt && $show_results && $has_answer && is_array($answer->answer) && in_array($option, $answer->answer, true)) {
          ...
          <div class="<?php echo esc_attr(join(' ', $option_classes)); ?>">
            <input type="checkbox" id="mpcs-quiz-question-field-<?php echo esc_attr($question->id); ?>-<?php echo esc_attr($index + 1); ?>" class="mpcs-quiz-question-field mpcs-quiz-question-field-multiple-answer" data-question-id="<?php echo esc_attr($question->id); ?>" name="mpcs_quiz_question_<?php echo esc_attr($question->id); ?>[]" value="<?php echo esc_attr($option); ?>"<?php echo $has_answer ? checked(is_array($answer->answer) && in_array($option, $answer->answer, true), true, false) : ''; ?><?php echo $viewing_attempt ? ' disabled' : ''; ?>>
            <label for="mpcs-quiz-question-field-<?php echo esc_attr($question->id); ?>-<?php echo esc_attr($index + 1); ?>">
              <i class="mpcs-checkbox-checked"></i>
              <i class="mpcs-checkbox-unchecked"></i>
            </label>
            ...
            <?php foreach($question->options as $index => $option) : ?>
              <?php if($question->is_option_correct($option)) : ?>
                <div class="mpcs-quiz-question-option">
                  <input type="checkbox" class="mpcs-quiz-question-field mpcs-quiz-question-field-multiple-answer" checked disabled>
                  <label>
                    <i class="mpcs-checkbox-checked"></i>
                  </label>
