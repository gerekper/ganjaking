=== YITH WooCommerce Terms & Conditions Popup ===

Contributors: yithemes
Tags: terms, conditions, privacy, popup, woocommerce, checkout, themes, yit, e-commerce, shop
Requires at least: 5.3
Tested up to: 5.6
Stable tag: 1.3.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Documentation: https://yithemes.com/docs-plugins/yith-woocommerce-terms-conditions-popup/

== Changelog ==

= 1.3.1 - Released on 21 February 2021 =

* New: support for WooCommerce 5.0
* Update: plugin framework

= 1.3.0 - Released on 12 January 2021 =

* New: support for WooCommerce 4.9
* Update: plugin framework

= 1.2.17 - Released on 11 December 2020 =

* New: support for WooCommerce 4.8
* Update: plugin framework

= 1.2.16 - Released on 10 November 2020 =

* New: support for WordPress 5.6
* New: support for WooCommerce 4.7
* New: possibility to update plugin via WP-CLI
* Update: plugin framework
* Dev: removed deprecated .ready method from scripts

= 1.2.15 - Released on 16 October 2020 =

* New: support for WooCommerce 4.6
* Update: plugin framework

= 1.2.14 - Released on 18 September 2020 =

* New: support for WooCommerce 4.5
* Update: plugin framework


= 1.2.13 - Released on 19 August 2020 =

* New: support for WordPress 5.5
* New: support for WooCommerce 4.4
* Update: plugin framework

= 1.2.12 - Released on 11 June 2020 =

* New: support for WooCommerce 4.2
* Update: plugin framework

= 1.2.11 - Released on 12 May 2020 =

* New: support for WooCommerce 4.1
* Update: plugin framework
* Tweak: moved terms checkbox inside label tag, to improve appearance and usability
* Tweak: use more accurate selector in js, to avoid possible interference with other elements of the page

= 1.2.10 - Released on 18 March 2020 =

* New: support for WordPress 5.4
* New: support for WooCommerce 4.0
* Update: plugin framework
* Tweak: trigger change after changing checkbox status
* Tweak: code refactoring

= 1.2.9 – Released on 24 December 2019 =

* New: support for WooCommerce 3.9
* Update: plugin framework
* Update: Italian language
* Update: Spanish language
* Fix: restored correct behaviour for "Hide Checkboxes" option

= 1.2.8 - Released on 07 November 2019 =

* New: support for WordPress 5.3
* New: support for WooCommerce 3.8
* New: option to change position of Terms and Conditions and Privacy Policy checkboxes on checkout page
* Tweak: add Terms and Conditions and Privacy Policy labels to error message on checkout page
* Update: Plugin framework
* Fix: do not trigger update_checkout event when closing popup, to avoid losing checkbox status

= 1.2.7 - Released on 29 May 2019 =

* New: Support to WordPress 5.2
* Update: Plugin Framework
* Dev: New filter 'yith_wctc_terms_page_id' to show a different page as terms.

= 1.2.6 - Released on 11 April 2019 =

* New: WooCommerce 3.6 support
* Tweak: added trigger update checkout when close the popup
* Tweak add the errors using the class WP_Error
* Update: internal plugin framework
* Update: Spanish language

= 1.2.5 - Released on 11 February 2019 =

* New: WooCommerce 3.5.4 support
* Update: internal plugin framework
* Update: dutch translation

= 1.2.4 - Released on 19 December 2018 =

* New: WordPress 5.0 support
* New: WooCommerce 3.5.2 support
* Update: internal plugin framework
* Update: italian language

= 1.2.3 - Released on 24 October 2018 =

* New: WooCommerce 3.5 support
* Tweak: updated plugin framework
* Update: Italian language

= 1.2.2 - Released on 16 October 2018 =

* New: WooCommerce 3.5-rc support
* New: WordPress 4.9.8 support
* Tweak: updated plugin framework
* Update: Spanish language
* Update: Italian language

= 1.2.1 - Released on 28 May 2018 =

* New: WooCommerce 3.4 compatibility
* New: WordPress 4.9.6 compatibility
* New: updated plugin framework to latest version
* New: GDPR compliance

= 1.2.0 - Released on 05 February 2018 =

* New: added WooCommerce 3.3.x support
* New: added WordPress 4.9.2 support
* New: updated plugin-fw
* New: added dutch translation
* New: added support to VC shortcodes into T&C popup content
* Tweak: fixed "Force to read" option when text is not scrollable
* Tweak: switched to Perfect Scrollbar to improve Browser/OS support
* Tweak: added CSS code to avoid strange appearance problem when closing popup

= 1.1.0 - Released on 04 April 2017 =

* New: Added WordPress 4.7.3 compatibility
* New: Added WooCommerce 3.0.0 RC2 compatibility
* New: Added italian - ITALY translation
* Tweak: using is_checkout() instead of checking page directly
* Tweak: to avoid problems with browsers, now only 95% of total scroll is required
* Fix: popup transitions

= 1.0.5 - Released on 13 June 2016 =

* Added: WooCommerce 2.6-RC1 support
* Added: spanish translation

= 1.0.4 - Released on 09 February 2016 =

* Added: WooCommerce 2.5.x compatibility
* Added: WordPress 4.4.x compatibility
* Tweak: Performance improved with new plugin core 2.0
* Tweak: Added filter the_content to "Terms & Conditions" and "Privacy" popup content
* Tweak: Added do_shortcode to "Terms & Conditions" and "Privacy" popup content
* Updated: jQuery ScrollBar to version 0.2.10
* Fixed: checkbox no more required on checkout (since WC 2.4)

= 1.0.3 - Released on 13 August 2015 =

* Added: Compatibility with WC 2.4.2
* Tweak: Updated internal plugin-fw
* Fixed: WPML compatibility

= 1.0.2 - Released on 04 May 2015 =

* Added: WP 4.2.1 support
* Fixed: "Plugin Documentation" appearing on all plugins
* Fixed: Wpml page id issue

= 1.0.1 - Released on 20 April 2015 =

* Added: link specific class
* Tweak: changed include_once in include for templates
* Tweak: changed js to use classes, instead of ids
* Tweak: changed background-color in background for a much stronger condition
* Fixed: popup close button style

= 1.0.0 - Released on 11 March 2015 =

* Initial release