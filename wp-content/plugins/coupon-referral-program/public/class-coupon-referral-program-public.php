...
class Coupon_Referral_Program_Public {
	...
	function mwb_cpr_create_coupons($mwb_cpr_code,$mwb_cpr_coupon_amount,$creation_id,$mwb_cpr_discount_type,$expirydate,$coupon_description,$customer_email=''){
	...
	function get_social_sharing_html($user_id) {
		...
		$whtsapp = '<a target="_blank" href=https://web.whatsapp.com/send?text='.$referral_link.' class="mwb_crp_whatsapp_button"><img src ="'.COUPON_REFERRAL_PROGRAM_DIR_URL.'public/images/watsap.png"></a>';
		...
		if( get_option('mwb_crp_share_whtsapp','no') == 'yes' ){
	...
	public function save_utilize_coupon_aomunt( $order,$order_id ) {
		...
		if ( isset( $utilize_coupon_amount_updated ) && $utilize_coupon_amount_updated == 'set' ) {
			return;
		}
	...
	public function mwb_crp_send_referal_link_mail(){
		...
		$emal_id = isset( $_POST['email'] ) ? $_POST['email']:'';
		$user_id = get_current_user_id();
	...
	public function mwb_crp_generate_referral_coupon_callback($coupon_code,$user_id){
		$woo_ver = WC()->version;
		$amount = get_option('mwb_crp_referral_user_coupon_amount',1);//set amount
		$discount_type = get_option('mwb_crp_user_referral_coupon_type','mwb_cpr_referral_user_coupon_percent');
		if( $discount_type == 'mwb_cpr_referral_user_coupon_fixed' ) {
			$discount_type = 'fixed_cart'; 
		}
		else {
			$discount_type = 'percent';
				...
				<div class="mwb_cpr_logged_wrapper">
					...
					<div class="mwb_cpr_refrral_code_copy">
					    ...
					    <p id="mwb_cpr_referal_code_copy">
							<code id="mwb_cpr_referal_copyy_code" class="mwb_crp_referl_code"><?php echo $referral_code; ?></code>
	...
	public function get_referral_code_on_popup( $user_id ){
		if ($this->check_share_vai_referal_code()) {
			...
			?>
			  <h6><?php _e('Referral Code: ','coupon-referral-program');?></h6>
		      <div class="mwb_cpr_refrral_code_copy">
		        <p id="mwb_cpr_copy">
		            <code id="mwb_cpr_copyy_code" style="background-color: <?php echo Coupon_Referral_Program_Admin::get_selected_color();?>">
		              <?php echo $referal_code; ?>
		            </code>
		            <span class="mwb_cpr_copy_btn_wrap">
