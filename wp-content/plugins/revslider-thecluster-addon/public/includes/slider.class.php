...
class RsTheClusterSliderFront extends RevSliderFunctions {
	...
	private function isEnabled($slider){
		...
		foreach($slides as $slide){
			...
			foreach($layers as $layer){
				if($this->get_val($layer, 'subtype', false) === 'thecluster'){
					$enabled = true;
					break;
				}
			}
	...
	public function add_addon_files($html, $export){
		...
		$_jsPathMin = file_exists(RS_THECLUSTER_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
		if(!$export->usepcl){
			$export->zip->addFile(RS_THECLUSTER_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
		}else{
			$export->pclzip->add(RS_THECLUSTER_PLUGIN_PATH.'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_THECLUSTER_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
		}
		
		$html = str_replace(RS_THECLUSTER_PLUGIN_PATH.'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin .'.js', $export->path_js .'revolution.addon.' . $this->pluginTitle . $_jsPathMin .'.js', $html);
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['thecluster'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
		$return['toload']['threejs'] = '<script'. $addition .' src="'. RS_PLUGIN_URL . 'public/assets/js/libs/three.min.js"></script>';
		$return['toload']['theclustergpu'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/GPUComputationRenderer.js"></script>';
