				...
				<form method="post" class="woocommerce-ResetPassword lost_reset_password">
					...
					<?php if ( version_compare( $porto_woo_version, '2.6', '>=' ) ) : ?>
						...
						<p class="woocommerce-form-row form-row clearfix">
							...
							<button type="submit" class="woocommerce-Button button btn-lg pt-right<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" value="<?php esc_attr_e( 'Reset password', 'woocommerce' ); ?>"><?php esc_html_e( 'Reset password', 'woocommerce' ); ?></button>
						</p>
