...
class WC_Dynamic_Pricing_FrontEnd_UX {
	...
	public function on_display_cart_item_price_html( $html, $cart_item, $cart_item_key ) {
		if ( $this->is_cart_item_discounted( $cart_item ) ) {
			...
			if ( function_exists( 'get_product' ) ) {
				...
				$price_adjusted = get_option( 'woocommerce_tax_display_cart' ) == 'excl' ? wc_get_price_excluding_tax( $_product, array(
					'price' => $price_to_calculate,
					'qty'   => 1
				) ) : wc_get_price_including_tax( $_product, array( 'price' => $price_to_calculate, 'qty' => 1 ) );
	...
	public function is_cart_item_discounted( $cart_item ): bool {
		return isset( $cart_item['discounts'] )
		       && isset($cart_item['discounts']['price_adjusted'])
		       && floatval($cart_item['discounts']['price_adjusted']) < floatval($cart_item['discounts']['display_price']);
