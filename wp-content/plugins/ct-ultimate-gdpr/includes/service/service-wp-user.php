...
class CT_Ultimate_GDPR_Service_WP_User extends CT_Ultimate_GDPR_Service_Abstract
{
    ...
    {
        if (!is_user_logged_in() && ct_ultimate_gdpr_should_age_block_features()) {
            $page_id = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('age_verification_page', '', CT_Ultimate_GDPR_Controller_Age::ID);
            wp_enqueue_script('ct-ultimate-gdpr-service-wp-user', ct_ultimate_gdpr_url('assets/js/service-wp-user.js'), array('jquery'));
            wp_localize_script('ct-ultimate-gdpr-service-wp-user', 'ct_ultimate_gdpr_wp_user', array(
                'link' => ct_ultimate_gdpr_render_template(ct_ultimate_gdpr_locate_template('service/service-wp-user-link', false), false, array(
    ...
    public function registration_errors_filter($errors)
    {
        ...
        if (
            !ct_ultimate_gdpr_is_doing_cli()
            && $inject
            && !ct_ultimate_gdpr_get_value('ct-ultimate-gdpr-consent-field', $_REQUEST)
            && !ct_ultimate_gdpr_should_age_unblock_features()
        ) {
    ...
    public function signup_add_form_fields()
    {
        ...
        if ($inject && !ct_ultimate_gdpr_should_age_unblock_features()) {
