 ...
 * @return int
 */
function ct_ultimate_gdpr_date_to_age($date)
{
    try {
        ...
        $age = ct_ultimate_gdpr_get_default_user_age();
    }
 ...
 * @return string
 */
function ct_ultimate_gdpr_get_user_date_of_birth()
{
    $cookie = ct_ultimate_gdpr_get_user_age_data_array();
    $date   = is_array($cookie) && isset($cookie['date']) ? $cookie['date'] : '';
    return $date;
}
 ...
 * @return array
 */
function ct_ultimate_gdpr_get_user_age_data_array()
{
    $value  = ct_ultimate_gdpr_get_encoded_cookie(CT_Ultimate_GDPR_Controller_Age::ID);
    $data = $value ? json_decode(stripslashes($value), true) : array();

    if (!$data){
        $data = CT_Ultimate_GDPR::instance()->get_controller_by_id(CT_Ultimate_GDPR_Controller_Age::ID)->get_user_meta_data_array();
    }
 ...
 * @return int
 */
function ct_ultimate_gdpr_get_user_age()
{
    $date = ct_ultimate_gdpr_get_user_date_of_birth();
    if ($date) {
        $age = ct_ultimate_gdpr_date_to_age($date);
    }

    return isset($age) ? $age : ct_ultimate_gdpr_get_default_user_age();
}
 ...
 * @return int
 */
function ct_ultimate_gdpr_get_guard_age()
{
    $cookie = ct_ultimate_gdpr_get_user_age_data_array();
    $date   = is_array($cookie) && isset($cookie['guard_date']) ? $cookie['guard_date'] : '';
    $age    = $date ? ct_ultimate_gdpr_date_to_age($date) : 0;
 ...
 * @return int
 */
function ct_ultimate_gdpr_get_guard_name()
{
    $cookie = ct_ultimate_gdpr_get_user_age_data_array();
    $name   = is_array($cookie) && isset($cookie['guard_name']) ? $cookie['guard_name'] : '';

    return $name;
}

/**
 * If conditions met, users of age above 16yo should have all features unblocked
 *
 * @return bool
 */
function ct_ultimate_gdpr_should_age_unblock_features()
{
    $ageLimit   = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('age_limit_to_sell', '16', CT_Ultimate_GDPR_Controller_Age::ID);
    if (ct_ultimate_gdpr_do_age_restriction_apply() && (ct_ultimate_gdpr_get_user_age() >= $ageLimit || (ct_ultimate_gdpr_get_guard_age() >= $ageLimit && ct_ultimate_gdpr_get_guard_name()))) {
        return true;
    }
 ...
 * @return bool
 */
function ct_ultimate_gdpr_do_age_restriction_apply()
{
    $ageEnabled = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('age_enabled', false, CT_Ultimate_GDPR_Controller_Age::ID);
    $isCa       = CT_Ultimate_GDPR::instance()->get_controller_by_id(CT_Ultimate_GDPR_Controller_Age::ID)->is_user_from_ca();
    return $ageEnabled && $isCa;
}

/**
 * If conditions met, users of age below 13yo should have all features blocked
 *
 * @return bool
 */
function ct_ultimate_gdpr_should_age_block_features()
{
    $ageLimitToEnter = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('age_limit_to_enter', '13', CT_Ultimate_GDPR_Controller_Age::ID);
    $ageLimitToSell  = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('age_limit_to_sell', '16', CT_Ultimate_GDPR_Controller_Age::ID);
    if (ct_ultimate_gdpr_do_age_restriction_apply() && ct_ultimate_gdpr_get_user_age() < $ageLimitToEnter && ct_ultimate_gdpr_get_guard_age() < $ageLimitToSell) {
        return true;
    }
 ...
 * @return bool
 */
function ct_ultimate_gdpr_is_user_underage()
{
    $ageLimitToEnter = CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('age_limit_to_enter', '13', CT_Ultimate_GDPR_Controller_Age::ID);
    if (ct_ultimate_gdpr_do_age_restriction_apply() && ct_ultimate_gdpr_get_user_age() < $ageLimitToEnter) {
        return true;
    }
...
/**
 * Get default user age
 *
 * @return mixed|void
 */
function ct_ultimate_gdpr_get_default_user_age()
{
    return apply_filters(
        'ct_ultimate_gdpr_get_default_user_age',
        CT_Ultimate_GDPR::instance()->get_admin_controller()->get_option_value('age_assume_default', '13', CT_Ultimate_GDPR_Controller_Age::ID)
    );
}


/**
 *  Create WP NONCE field for export import GDPR dashboard
 * @param $action
 * @param $nonce_field
 * @return mixed
 */
function ct_ultimate_gdpr_nonce_field( $action, $nonce_field) {

    if( $action && $nonce_field ) {

        return wp_nonce_field( $action,$nonce_field );
