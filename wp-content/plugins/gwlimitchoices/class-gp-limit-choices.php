...
class GP_Limit_Choices extends GWPerk {
            ...
            gperk.addEnableLimitsCheckbox = function() {
                ...
                if(jQuery('#field_choice_limits_enabled').length < 1) {
                    var checkbox = '<div style="float:right;margin-right:10px;"> \
                        ...
                        <label class="inline gfield_value_label" for="field_choice_limits_enabled"><?php _e('enable limits', 'gp-limit-choices'); ?></label> \
                    </div>';
    ...
    public function display_choiceless_message($input, $field, $value, $lead_id, $form_id) {
        ...
        $message = sprintf( '<p class="choiceless"><p>', __( 'There are no options available for this field.', 'gp-limit-choices' ) );
    ...
    public function get_selected_values( $field ) {
	    ...
	    } else if( $this->is_gview_edit_view() ) {
		    $entry = $this->get_gview_entry();
		    $values = GFFormsModel::get_lead_field_value( $entry, $field );
	    } else {
    ...
    public static function get_choice_count( $value, $field, $form_id ) {
        ...
        } else if( version_compare( GFForms::$version, '2.0', '>=' ) ) {
    ...
    public static function get_choice_counts( $form_id, $field ) {
		 ...
		 */
		$counts = gf_apply_filters( array( 'gplc_choice_counts', $form_id, $field->id ), $counts, $form, $field );
