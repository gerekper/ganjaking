...
function woocomposer_loop_style01( $atts, $element ) {
	...
	$ult_dls_01_settings['img_animate'] = 'wcmp-img-' . $ult_dls_01_settings['img_animate'];
	if ( '' !== $ult_dls_01_settings['sale_price'] ) {
		$sale_price_size = 'font-size:' . $ult_dls_01_settings['sale_price'] . 'px;';
	}
	if ( '' !== $ult_dls_01_settings['border_style'] ) {
		$border .= 'border:' . $ult_dls_01_settings['border_size'] . 'px ' . $ult_dls_01_settings['border_style'] . ' ' . $ult_dls_01_settings['border_color'] . ';';
		$border .= 'border-radius:' . $ult_dls_01_settings['border_radius'] . 'px;';
	}
	if ( '' !== $ult_dls_01_settings['color_product_desc_bg'] ) {
		$desc_style .= 'background:' . $ult_dls_01_settings['color_product_desc_bg'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_product_desc'] ) {
		$desc_style .= 'color:' . $ult_dls_01_settings['color_product_desc'] . ';';
	}
	$columns      = 3;
	$display_type = $ult_dls_01_settings['disp_type'];
	if ( '' !== $ult_dls_01_settings['color_heading'] ) {
		$heading_style = 'color:' . $ult_dls_01_settings['color_heading'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['size_title'] ) {
		$heading_style .= 'font-size:' . $ult_dls_01_settings['size_title'] . 'px;';
	}
	if ( '' !== $ult_dls_01_settings['color_categories'] ) {
		$cat_style = 'color:' . $ult_dls_01_settings['color_categories'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['size_cat'] ) {
		$cat_style .= 'font-size:' . $ult_dls_01_settings['size_cat'] . 'px;';
	}
	if ( '' !== $ult_dls_01_settings['color_price'] ) {
		$price_style = 'color:' . $ult_dls_01_settings['color_price'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['size_price'] ) {
		$price_style .= 'font-size:' . $ult_dls_01_settings['size_price'] . 'px;';
	}
	if ( '' !== $ult_dls_01_settings['color_rating'] ) {
		$rating_style .= 'color:' . $ult_dls_01_settings['color_rating'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_rating_bg'] ) {
		$rating_style .= 'background:' . $ult_dls_01_settings['color_rating_bg'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_quick_bg'] ) {
		$view_bg_style = 'background:' . $ult_dls_01_settings['color_quick_bg'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_quick'] ) {
		$view_style = 'color:' . $ult_dls_01_settings['color_quick'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_cart_bg'] ) {
		$cart_bg_style = 'background:' . $ult_dls_01_settings['color_cart_bg'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_cart'] ) {
		$cart_style = 'color:' . $ult_dls_01_settings['color_cart'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_on_sale_bg'] ) {
		$label_style = 'background:' . $ult_dls_01_settings['color_on_sale_bg'] . ';';
	}
	if ( '' !== $ult_dls_01_settings['color_on_sale'] ) {
		$label_style .= 'color:' . $ult_dls_01_settings['color_on_sale'] . ';';
	}
...
if ( 'top_rated_products' == $display_type ) {
	...
	if ( $query->have_posts() ) :
		while ( $query->have_posts() ) :
			...
			$uid        = uniqid();
			$output    .= '<div id="product-' . $uid . '" style="' . $style . '" class="' . $class . ' wpb_column column_container wooproduct" data-animation="animated ' . $ult_dls_01_settings['product_animation'] . '">';
			if ( 'carousel' == $element ) {
			...
			$output     .= "\n" . '<div class="wcmp-product woocommerce wcmp-' . $ult_dls_01_settings['product_style'] . ' ' . $ult_dls_01_settings['img_animate'] . '" style="' . $border . ' ' . $desc_style . '">';
			...
			if ( '' !== $on_sale ) {
				$output .= "\n" . '<div class="wcmp-onsale ' . $ult_dls_01_settings['on_sale_alignment'] . ' ' . $ult_dls_01_settings['on_sale_style'] . '"><span class="onsale" style="' . $label_style . ' ' . $sale_price_size . '">' . $on_sale . '</span></div>';
			}
			...
			if ( '' !== $ult_dls_01_settings['text_align'] ) {
				$text_align_style = 'text-align:' . $ult_dls_01_settings['text_align'] . ';';
			}
