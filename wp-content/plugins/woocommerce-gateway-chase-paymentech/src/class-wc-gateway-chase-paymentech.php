...
class WC_Gateway_Chase_Paymentech extends Framework\SV_WC_Payment_Gateway_Direct {
	...
	public function process_payment( $order_id ) {
		...
		if ( $this->is_certification_mode() ) {
			...
			// save the test details in the session to persist the settings with future tests
			WC()->session->set( 'wc_chase_paymentech_certification_test_details', $test_details );

			// save the test details in order meta so they can be accessible when processing IPN
			$this->update_order_meta( $order, 'wc_chase_paymentech_certification_test_details', $test_details );
	...
	public function process_ipn() {
		...
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-ipn-response.php' );
	...
	public function handle_transaction_error() {
		...
		if ( $this->is_certification_mode() ) {
			...
			// get the current test's details from the session; default to test details saved in order meta
			$test_details = (array) WC()->session->get( 'wc_chase_paymentech_certification_test_details', $this->get_order_meta( $order, 'wc_chase_paymentech_certification_test_details' ) );
			...
			// save the test details in order meta as well
			$this->update_order_meta( $order, 'wc_chase_paymentech_certification_test_details', $test_details );
		}
	...
	public function get_api() {
		...
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api-request.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api-response.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api-capture-response.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api-refund-response.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api-void-response.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api-new-order-response.php' );
		require_once( $this->get_plugin()->get_plugin_path() . '/src/api/class-wc-orbital-gateway-api-profile-delete-response.php' );
