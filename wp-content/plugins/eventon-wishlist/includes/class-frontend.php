	...
	// Show add to wish list element		
		public function eventtop_content($OT, $object, $EVENT){
			...
			if(is_singular( 'ajde_events' )){
				...
				if( !EVO()->cal->check_yn('evowi_on_sin_pg','evcal_1')) return;
			...
			$data = "data-ei='{$EVENT->ID}' data-ri='{$EVENT->ri}'";
			$evOpt = evo_get_options('1');
			$event_ri = $EVENT->ri;		

			if( $this->fnc->is_event_wishlisted($EVENT->ID, $event_ri, $this->user_wishlist) ){
				$OT .= "<span class='evowi wishlisted' $data>
					<span class='evowi_wi_area'>
						<i class='fa ".get_eventON_icon('evcal_evowi_001', 'fa-heart',$evOpt )."'></i>
						<em>".$this->fnc->get_wishlist_count($EVENT->ID, $event_ri)."</em>
					</span>".evo_lang('In your wishlist')."</span>";
			}else{
				$OT .= "<span class='evowi notlisted' $data>
					<span class='evowi_wi_area'>
