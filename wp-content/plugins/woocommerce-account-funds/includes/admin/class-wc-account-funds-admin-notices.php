...
/**
 * WC_Account_Funds_Admin_Notices Class.
 */
class WC_Account_Funds_Admin_Notices {
	...
	public static function load_dismiss_notices() {
		...
		foreach ( $notices as $notice ) {
			if ( 0 !== strpos( $notice, 'wc_account_funds_' ) ) {
				continue;
			}
	...
	public static function add_notices() {
		...
		if ( ! wp_style_is( 'woocommerce-activation' ) ) {
			...
			$screen_id       = wc_account_funds_get_current_screen_id();
	...
	protected static function get_notice_content_from_file( $notice ) {
		$file = str_replace( array( 'wc_account_funds_', '_' ), array( '', '-' ), $notice );
...
WC_Account_Funds_Admin_Notices::init();
