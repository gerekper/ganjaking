...
class MpActiveCampaign {
  ...
  public function maybe_admin_notice() {
    if(defined('MEPR_VERSION') && version_compare(MEPR_VERSION, '1.7.3', '<')) {
      ...
      $message = __('Your ActiveCampaign integration with MemberPress may be broken. Please update MemberPress to version 1.7.3 or newer to fix this issue.', 'memberpress-activecampaigntags', 'memberpress-activecampaign');
...
<!--        <br/>
  ...
  public function maybe_add_subscriber($txn) {
    ...
    $user     = $txn->user();
    $enabled  = (bool)get_post_meta($txn->product_id, '_mepractivecampaign_list_override', true);
    $list_id  = get_post_meta($txn->product_id, '_mepractivecampaign_list_override_id', true);
