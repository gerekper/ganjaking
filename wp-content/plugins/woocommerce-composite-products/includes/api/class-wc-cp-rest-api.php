		...
		if ( 'create' === $action && ! empty( $posted_item_data[ 'composite_configuration' ] ) ) {
			...
			if ( $composite && $composite->is_type( 'composite' ) ) {
				...
				try {
					...
					$message = sprintf( __( 'The submitted composite configuration could not be added to this order: %s', 'woocommerce-composite-products' ), $e->getMessage() );
