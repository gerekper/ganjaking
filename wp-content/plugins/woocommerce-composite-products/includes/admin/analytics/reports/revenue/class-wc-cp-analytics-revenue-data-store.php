...
class WC_CP_Analytics_Revenue_Data_Store extends WC_CP_Analytics_Data_Store {
	...
	protected function assign_report_columns() {
		...
		$this->report_columns = array(
			'product_id'            => 'composite_id as product_id',
			'items_sold'            => "SUM( CASE WHEN product_id = composite_id THEN {$table_name}.product_qty END ) as items_sold",
			'composited_items_sold' => "SUM( CASE WHEN product_gross_revenue >= 0 AND product_id <> composite_id THEN {$table_name}.product_qty END ) as composited_items_sold",
	...
	public static function init() {
		// @see WC_CP_Admin_Analytics_Sync
	}
	...
	protected function add_sql_query_params( $query_args ) {
		...
		if ( $included_products ) {
			...
			// Hint: We can "play" with the composite id, as long as we are at the subquery level only.
			$this->subquery->add_sql_clause( 'where', "AND {$order_product_lookup_table}.composite_id IN ({$included_products})" );

		} else {
			$this->add_from_sql_params( $query_args, 'inner', "{$order_product_lookup_table}.composite_id" );
		}

		// Hint: We could add a composited item id clause here, but we can't SUM the items_sold in a single query. We may need to revisit and and a UNION helper in such cases.
		...
		foreach ( $products_data as $key => $product_data ) {
			...
			if ( $query_args[ 'extended_info' ] ) {
				...
				if ( ! $product ) {
					...
					$products_data[ $key ][ 'extended_info' ][ 'name' ] = $product_names[ $product_id ] ? sprintf( __( '%s (Deleted)', 'woocommerce-composite-products' ), $product_names[ $product_id ] ) : __( '(Deleted)', 'woocommerce-composite-products' );
					continue;
				}

				$extended_attributes = apply_filters( 'woocommerce_rest_reports_composites_extended_attributes', $this->extended_attributes, $product_data );
	...
	protected function initialize_queries() {
		...
		$this->subquery->add_sql_clause( 'select', 'composite_id' );
		$this->subquery->add_sql_clause( 'from', self::get_db_table_name() );
		$this->subquery->add_sql_clause( 'group_by', 'composite_id' );
	}
}
