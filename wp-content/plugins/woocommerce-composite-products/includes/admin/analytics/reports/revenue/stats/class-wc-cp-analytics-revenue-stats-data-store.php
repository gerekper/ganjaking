...
class WC_CP_Analytics_Revenue_Stats_Data_Store extends WC_CP_Analytics_Revenue_Data_Store {
	...
	protected function assign_report_columns() {
		...
		$this->report_columns = array(
			...
			'products_count'        => 'COUNT(DISTINCT composite_id) as products_count'
		);
	}
	...
	protected function update_sql_query_params( $query_args ) {
		...
		if ( $included_products ) {
			$products_where_clause .= " AND {$order_product_lookup_table}.composite_id IN ({$included_products})";
		}
	...
	public function get_data( $query_args ) {
		...
		if ( false === $data ) {
			...
			if ( null === $totals ) {
				return new \WP_Error( 'woocommerce_analytics_composites_stats_result_failed', __( 'Sorry, fetching revenue data failed.', 'woocommerce-composite-products' ) );
			}
