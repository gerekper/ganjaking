			...
			try {
					...
					if ( 'variable' === $composited_product_type ) {
						...
						if ( ! $all_set ) {
							...
							if ( $missing_attributes ) {
								/* translators: %1$s: Missing attributes, %2$s: Component title. */
				...
				if ( is_wp_error( $matching_scenarios ) ) {
					...
					if ( in_array( $error_code, array( 'woocommerce_composite_configuration_selection_required', 'woocommerce_composite_configuration_selection_invalid' ) ) ) {
						...
						if ( 'cart' === $context ) {
							/* translators: %1$s: Composite title. */
				...
				foreach ( $validation_data as $component_id => $component_validation_data ) {
					...
					if ( $qty < $qty_min && 'yes' !== $sold_individually ) {
						...
						/* translators: %1$s: Product title, %2$d: Quantity. */
	...
	*/
	...
	public function update_quantity_in_cart( $cart_item_key, $quantity = 0 ) {
		...
		if ( ! empty( WC()->cart->cart_contents[ $cart_item_key ] ) ) {
			...
			if ( wc_cp_is_composite_container_cart_item( $cart_item ) ) {
				...
				$composite_children = wc_cp_get_composited_cart_items( $cart_item );
			...
			} elseif ( $parent = wc_cp_get_composited_cart_item_container( $cart_item ) ) {
	...
	public function update_cart_validation( $passed, $cart_item_key, $cart_item, $quantity ) {
		...
		if ( $parent = wc_cp_get_composited_cart_item_container( $cart_item ) ) {
			...
			if ( $quantity < $min_quantity ) {
				/* translators: %1$s: Product title, %2$d: Quantity. */
				wc_add_notice( sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be lower than %2$d.', 'woocommerce-composite-products' ), $cart_item[ 'data' ]->get_title(), $min_quantity ), 'error' );
				return false;

			} elseif ( $max_quantity && $quantity > $max_quantity ) {
				/* translators: %1$s: Product title, %2$d: Quantity. */
				wc_add_notice( sprintf( __( 'The quantity of &quot;%1$s&quot; cannot be higher than %2$d.', 'woocommerce-composite-products' ), $cart_item[ 'data' ]->get_title(), $max_quantity ), 'error' );
				return false;

			} elseif ( $quantity % $parent_quantity != 0 ) {
				/* translators: %1$s: Product title, %2$d: Quantity. */
				wc_add_notice( sprintf( __( 'The quantity of &quot;%1$s&quot; must be entered in multiples of %2$d.', 'woocommerce-composite-products' ), $cart_item[ 'data' ]->get_title(), $parent_quantity ), 'error' );
				return false;
	...
	public function update_cart_action_remove_item() {
		...
		if ( wc_cp_is_composited_cart_item( $cart_item ) ) {
			...
			$composite_container_item = wc_cp_get_composited_cart_item_container( $cart_item );
			...
			if ( ! $component->is_optional() && 0 !== $component->get_quantity( 'min' ) ) {
	...
	public function cart_item_remove( $cart_item_key, $cart ) {
		...
		if ( wc_cp_is_composite_container_cart_item( $cart->removed_cart_contents[ $cart_item_key ] ) ) {
			...
			$composited_item_cart_keys = wc_cp_get_composited_cart_items( $cart->removed_cart_contents[ $cart_item_key ], $cart->cart_contents, true );

			foreach ( $composited_item_cart_keys as $composited_item_cart_key ) {

				$remove                                                   = $cart->cart_contents[ $composited_item_cart_key ];
				$cart->removed_cart_contents[ $composited_item_cart_key ] = $remove;
			...
			if ( empty( $composite_data ) || ! isset( $composite_data[ $component_id ] ) ) {
				return;
			}

			// Unset product related keys.
			if ( 'yes' === $composite_data[ $component_id ][ 'optional' ] ) {

				$composite_data[ $component_id ][ 'product_id' ] = '';
				unset( $composite_data[ $component_id ][ 'variation_id' ] );
				unset( $composite_data[ $component_id ][ 'attributes' ] );
	...
	public function cart_item_restore( $cart_item_key, $cart ) {
		...
		if ( wc_cp_is_composite_container_cart_item( $cart->cart_contents[ $cart_item_key ] ) ) {
			...
			$composited_item_cart_keys = wc_cp_get_composited_cart_items( $cart->cart_contents[ $cart_item_key ], $cart->removed_cart_contents, true );

			foreach ( $composited_item_cart_keys as $composited_item_cart_key ) {

				$restore_item = $cart->removed_cart_contents[ $composited_item_cart_key ];
				...
				$cart->cart_contents[ $composited_item_cart_key ] = $restore_item;

				/** WC core action. */
				do_action( 'woocommerce_restore_cart_item', $composited_item_cart_key, $cart );

				unset( $cart->removed_cart_contents[ $composited_item_cart_key ] );
				 ...
				 * @param  WC_Cart $cart
				 */
				do_action( 'woocommerce_cart_item_restored', $composited_item_cart_key, $cart );
			}
		}
	}
