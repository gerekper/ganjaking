...
class WC_Dropshipping_CSV_Import {
	...
	*/
	...
	public function woo_dropship_export_taxonomy_id( $value, $product ) {
		...
		$terms = get_terms( array( 'object_ids' => $product->get_ID(), 'taxonomy' => 'dropship_supplier' ) );
				...
				if( !empty( $bigArr ) ) {
					foreach ( $bigArr as $arr ) {
						...
						if ( $term !== 0 && $term !== null ) {		
							...
							$term_meta = array('account_number'=>$arr['account_number'],'order_email_addresses'=>$arr['email'],'csv_delimiter'=>'','csv_column_indicator'=>'','csv_column_sku'=>'');
		...
		if( ! empty( $productSku ) ) {
			...
			if ( ( $pId > 0 ) && isset ( $row[$dropship_supplier_slug_key] ) && ! empty( $row[$dropship_supplier_slug_key] ) ) {				
				...
				if ($dropship_supplier) {
					...
					$updateTerm = wp_update_term( $dropship_supplier->term_id, 'dropship_supplier', array(
									'name' => $dropship_supplier_name,
									'slug' => $dropship_supplier_slug,
									'description' => $dropship_supplier_description
								) );
					
					$term_meta = array('account_number'=>$dropship_supplier_account_number,'order_email_addresses'=>$dropship_supplier_email,'csv_delimiter'=>'','csv_column_indicator'=>'','csv_column_sku'=>'');
