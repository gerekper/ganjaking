    ...
    public function get_per_week_profit(){
          ...
          private function get_total_orders_supplierr_by_order_id($orders, $date){
            ...
            if(!empty($orders)){
                ...
                foreach($orders as $order_id )
                 ...
                 $order_with_supp =  ["post_date"=>$date,"total"=> count($order_with_supp)];
            }  
             else
                $order_with_supp =  ["post_date"=>$date,"total"=>0];
        ...
        private function get_supplierr_by_order_id($orders, $date){
            ...
            if(!empty($orders)){
		    	...
		    	$order_with_supp = (object) ["post_date"=>$date,"total"=> round($total_profit)];
                
            }
            else
                $order_with_supp = (object) ["post_date"=>$date,"total"=>0];
