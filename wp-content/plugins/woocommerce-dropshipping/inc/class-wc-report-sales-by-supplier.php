		 ...
		 if ( $the_query->have_posts() ) {
			 ...
			 while ( $the_query->have_posts()) {
				...
				if (!empty($order_shipping_address_2)) {
					...
					$fullShipAddress = $order_shipping_first_name . ' ' . $order_shipping_last_name .', '. $order_shipping_address_1 . ', ' . $order_shipping_address_2 . ', ' . $order_shipping_city . ', ' . $order_shipping_state . ', ' . $order_shipping_postcode . ', ' . $order_shipping_country;

				} else {

					$fullShipAddress = $order_shipping_first_name . ' ' . $order_shipping_last_name .', '. $order_shipping_address_1 . ', ' . $order_shipping_city . ', ' . $order_shipping_state . ', ' . $order_shipping_postcode . ', ' . $order_shipping_country;
				...
				$htmlTable .='<td>' . $fullShipAddress . '</td>';
		...
		$wp_session['end_date'] = $end_date;
		 ...
		 if ( $the_query->have_posts() ) {
			 ...
			 while ( $the_query->have_posts()) {
				...
				$lineData = array('#'. get_the_ID() .' '.$order_billing_first_name.' '.$order_billing_last_name, $order_date_created, $order_status, $fullBillAddress, $fullShipAddress, $order_billing_email, $order_billing_phone, $currency . $order_total_final );
