...
if ( ! class_exists( 'YITH_ARS_On_Hold_User_Email' ) ) {
    ...
    class YITH_ARS_On_Hold_User_Email extends WC_Email {
        ...
        public function __construct() {
            ...
            $this->title         = esc_html_x( 'YITH Advanced Refund System: On hold request - email for user', 'Email descriptive title',
                'yith-advanced-refund-system-for-woocommerce' );
            $this->description   = esc_html__( 'The user will receive an email when the request is on hold.', 'yith-advanced-refund-system-for-woocommerce' );
            $this->heading       = esc_html__( 'Your refund request is on hold', 'yith-advanced-refund-system-for-woocommerce' );
            $this->subject       = esc_html__( 'Your refund request is on hold', 'yith-advanced-refund-system-for-woocommerce' );
            $this->email_body    = esc_html__( 'Hi {customer_name}! Your request concerning order {order_number} is currently on hold.',
                'yith-advanced-refund-system-for-woocommerce' );
        ...
        public function trigger( $request_id ) {
            ...
            $this->email_body = $this->get_option( 'email_body', esc_html__( 'Hi {customer_name}! Your request {request_number} for order {order_number} is currently on hold.', 'yith-advanced-refund-system-for-woocommerce' ) );
