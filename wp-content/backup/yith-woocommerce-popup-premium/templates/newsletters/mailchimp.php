	...
	<fieldset>
		<ul class="group">
			<li>
				<?php
				if ( $type_label == 'label' ) {
					echo '<label for="yit_mailchimp_newsletter_form_email">' . wp_kses_post( $email_label ) . '</label>'; }
				?>
			...
			<li class="ypop-submit">
				...
				<button type="submit" class="btn submit-field popup-mailchimp-subscription-ajax-submit"><?php echo wp_kses_post( $current_icon ) . ' ' . wp_kses_post( $submit_label ); ?></button>
			</li>
		</ul>
	</fieldset>
</form>
...
<?php
wp_enqueue_script( 'yitpop-mailchimp-ajax-send-form', YITH_YPOP_ASSETS_URL . '/js/mailchimp-ajax-subscribe.js', array( 'jquery' ), '', true );
wp_localize_script(
	'yitpop-mailchimp-ajax-send-form',
