 ...
 *
 * @class   YITH_WCGPF_Google_Product_Feed_List_Table
...
class YITH_WCGPF_Google_Product_Feed_List_Table extends WP_List_Table {


    /**
     * YITH_WCGPF_Google_Product_Feed_List_Table constructor.
     *
     * @param array $args
     */
    public function __construct( $args = array() ) {
        parent::__construct( array(
            'singular' => esc_html__( 'Google product feed table', 'yith-google-product-feed-for-woocommerce' ),
            'plural'   => esc_html__( 'Google product feed table', 'yith-google-product-feed-for-woocommerce' ),
            'ajax'     => false
        ) );
    }
    ...
    function get_columns() {
        ...
        $columns = array(
            ...
            'type_file'  =>esc_html__('Feed type','yith-google-product-feed-for-woocommerce'),
            'feed_url'  =>esc_html__('Feed URL','yith-google-product-feed-for-woocommerce'),
            'actions' =>esc_html__('Actions','yith-google-product-feed-for-woocommerce'),
        );
    ...
    function column_default( $item, $column_name ) {
        ...
        switch( $column_name ) {
            ...
            case 'file_name':
                return (isset($item->post_title)) ? '<span>'.$item->post_title.'</span>':'';
                break;

            case 'merchant':
                return ( isset($feed['merchant'])) ? '<span>'.ucfirst( $feed['merchant']).'</span>' : '<span>Google</span>' ;
                break;

            case 'type_file':
                return  (isset($feed['feed_type'])) ? '<span>'.strtoupper($feed['feed_type']).'</span>' : '<span>XML</span>' ;;
                break;

            case 'feed_url':
                 return ( isset($feed['feed_url']) && $feed['feed_url'] ) ? '<a target="_blank" href="'.apply_filters('yith_wcgpf_feed_url_merchant',$feed['feed_url']).'">'.apply_filters('yith_wcgpf_feed_url_merchant',$feed['feed_url']).'</a>':'';
                break;

            case 'actions':
                    return ( isset($feed['feed_file']) ) ? '<div><input data-feed="'.$item->ID.'" data-type="'.$feed['feed_type'].'" id=yith-wcgpf-view-'.$item->ID.' class="button yih-wcgpf-view yith-wcgpf-download-feed-file" value="'. esc_html__('Re-generate feed file','yith-google-product-feed-for-woocommerce') . '"/></div>' : '<div><input data-feed="'.$item->ID.'" data-type="'.$feed['feed_type'].'" id=yith-wcgpf-view-'.$item->ID.' class="button yih-wcgpf-view yith-wcgpf-download-feed-file" value="'. esc_html__('Generate feed file','yith-google-product-feed-for-woocommerce') . '"/></div>';
                break;
    ...
    function prepare_items() {
        ...
        $perpage = apply_filters( 'yith_wcgpf_feed_per_page', 15 );
