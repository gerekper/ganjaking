...
if ( ! class_exists( 'YITH_Delivery_Date_Order_Manager' ) ) {
	...
	class YITH_Delivery_Date_Order_Manager {
		...
		public function save_order_meta( $post_id ) {
			...
			if ( 'shop_order' == $post_type && isset( $_POST['ywcdd_order_shipped'] ) ) {
		...
		public function include_scripts() {
			...
			if ( $current_screen->id == 'shop_order' ) {
				...
				wp_register_script( 'delivery_date_order_metabox', YITH_DELIVERY_DATE_ASSETS_URL . 'js/' . yit_load_js_file( 'yith_deliverydate_order_metaboxes.js' ), array(
					'jquery',
		...
		",
			...
			?>
			<label for="filter-by-<?php echo $type;?>" class="screen-reader-text"><?php _e( 'Filter by date' ); ?></label>
