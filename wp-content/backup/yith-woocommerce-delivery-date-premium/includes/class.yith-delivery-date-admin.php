...
if ( ! class_exists( 'YITH_Delivery_Date_Admin' ) ) {
	...
	class YITH_Delivery_Date_Admin {
		...
		public function __construct() {
			...
			add_filter( 'yith_plugin_fw_toggle_element_title_ywcdd_holidays_option', array(
				$this,
				'show_holiday_toggle_element_title'
			), 10, 3 );
			add_filter( 'yith_plugin_fw_toggle_element_subtitle_ywcdd_holidays_option', array(
				$this,
				'show_holiday_toggle_element_subtitle'
			), 10, 3 );

			add_filter( 'yith_plugin_fw_toggle_element_title_yith_new_shipping_day_prod', array(
				$this,
				'show_processing_product_toggle_element_title'
			), 10, 3 );

			add_filter( 'yith_plugin_fw_toggle_element_title_yith_new_shipping_day_cat', array(
				$this,
		...
		public function add_holidays() {
			...
			if ( isset( $_REQUEST['action'] ) && 'add_holidays' === $_REQUEST['action'] && isset( $_REQUEST['ywcdd_holidays_option'] ) ) {
				...
				wp_send_json( array(  'title' => $title, 'subtitle' => $subtitle ) );
			}
		...
		public function show_admin_notices() {
			...
			if ( isset( $_GET['page'] ) && 'yith_delivery_date_panel' == $_GET['page'] ) {
				...
				if ( $tot_carrier_post == 0 ) {
					...
					$message      = sprintf( '%s <a href="%s" class="page-title-action" style="top:0;font-size:11px;">%s</a>', __( 'In order to use the plugin, it is essential to create at least a Carrier', 'yith-woocommerce-delivery-date' ),
						$new_post_url, __( 'Add new Carrier', 'yith-woocommerce-delivery-date' ) );
