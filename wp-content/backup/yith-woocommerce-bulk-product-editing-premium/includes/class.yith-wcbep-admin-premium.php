    ...
    class YITH_WCBEP_Admin_Premium extends YITH_WCBEP_Admin {
        ...
        public function bulk_edit_products() {
            ...
            if ( isset( $_POST[ 'keys' ] ) && isset( $_POST[ 'row' ] ) && isset( $_POST[ 'edited' ] ) ) {
                ...
                $low_stock_amount_index = array_search( 'low_stock_amount', $keys );
                ...
                $low_stock_amount = $row[ $low_stock_amount_index ];
                ...
                if ( $product ) {
                    ...
                    // EDIT STOCK QUANTITY
                    if ( !$product->is_type( 'grouped' ) && isset( $low_stock_amount ) )
                            ...
                            if ( !empty( $file_urls[ $i ] ) ) {
                                ...
                                if ( in_array( $file_is, array( 'absolute', 'relative' ) ) ) {
                                    ...
                                    if ( !empty( $extension ) && !in_array( $file_type[ 'type' ], $allowed_file_types ) ) {
                                        echo sprintf( __( 'The downloadable file %s cannot be used as it does not have an allowed file type. Allowed types include: %s', 'yith-woocommerce-bulk-product-editing' ), '<code>' . basename( $file_url ) . '</code>', '<code>' . implode( ', ', array_keys( $allowed_file_types ) ) . '</code>' );
                                        continue;
                                    }
                                }
                                ...
                                if ( 'relative' === $file_is && !apply_filters( 'woocommerce_downloadable_file_exists', file_exists( $file_url ), $file_url ) ) {
                                    echo sprintf( __( 'The downloadable file %s cannot be used as it does not exist on the server.', 'yith-woocommerce-bulk-product-editing' ), '<code>' . $file_url . '</code>' );
                                    continue;
                                }
