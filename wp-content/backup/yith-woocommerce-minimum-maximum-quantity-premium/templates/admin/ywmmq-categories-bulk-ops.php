	...
	class YWMMQ_Categories_Bulk_Ops {
		...
		public function output() {
			...
			$table->options = array(
				...
				'count_table'      => '( SELECT COUNT(*) FROM ' . $wpdb->prefix . 'terms a INNER JOIN ' . $wpdb->prefix . 'term_taxonomy b ON a.term_id = b.term_id INNER JOIN ' . $wpdb->prefix . 'termmeta c ON c.term_id = a.term_id WHERE b.taxonomy = "product_cat" AND ( c.meta_key = "_ywmmq_category_exclusion" OR c.meta_key = "_ywmmq_category_quantity_limit_override" OR c.meta_key = "_ywmmq_category_value_limit_override") AND c.meta_value = "yes" GROUP BY a.term_id ) AS count_table',
				'count_where'      => '',
				'key_column'       => 'ID',
				'view_columns'     => array(
				...
				'custom_columns'   => array(
					...
					'column_override_qty' => function ( $item ) {
						...
						if ( $item['override_qty'] == 'yes' ) {
							$class  = 'show';
							$tip    = __( 'Yes', 'yith-woocommerce-minimum-maximum-quantity' );
							$min    = get_term_meta( $item['ID'], '_ywmmq_category_minimum_quantity', true );
							$max    = get_term_meta( $item['ID'], '_ywmmq_category_maximum_quantity', true );
							$step   = get_term_meta( $item['ID'], '_ywmmq_category_step_quantity', true );
					...
					'column_override_val' => function ( $item ) {
						...
						if ( $item['override_val'] == 'yes' ) {
							$class  = 'show';
							$tip    = __( 'Yes', 'yith-woocommerce-minimum-maximum-quantity' );
							$min    = get_term_meta( $item['ID'], '_ywmmq_category_minimum_value', true );
							$max    = get_term_meta( $item['ID'], '_ywmmq_category_maximum_value', true );
						...
						'function_delete' => function () {
							...
							if ( ! empty( $ids ) ) {
								$wpdb->query( "UPDATE {$wpdb->prefix}termmeta
                                           SET meta_value='no'
                                           WHERE ( meta_key = '_ywmmq_category_exclusion' OR meta_key = '_ywmmq_category_quantity_limit_override' OR meta_key = '_ywmmq_category_value_limit_override') AND term_id IN ( $ids )"
								);
							}
						}
					)
			...
			if ( ! empty( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], basename( __FILE__ ) ) ) {
				...
				if ( $item_valid !== true ) {
					...
					$step_qty     = isset( $_POST['_ywmmq_category_step_quantity'] ) ? $_POST['_ywmmq_category_step_quantity'] : 1;
					...
					foreach ( $category_ids as $category_id ) {
						...
						update_term_meta( $category_id, '_ywmmq_category_exclusion', $exclusion );
						update_term_meta( $category_id, '_ywmmq_category_quantity_limit_override', $override_qty );
						update_term_meta( $category_id, '_ywmmq_category_minimum_quantity', $minimum_qty );
						update_term_meta( $category_id, '_ywmmq_category_maximum_quantity', $maximum_qty );
						update_term_meta( $category_id, '_ywmmq_category_step_quantity', $step_qty );
						update_term_meta( $category_id, '_ywmmq_category_value_limit_override', $override_val );
						update_term_meta( $category_id, '_ywmmq_category_minimum_value', $minimum_val );
						update_term_meta( $category_id, '_ywmmq_category_maximum_value', $maximum_val );
			...
			if ( 'delete' === $table->current_action() ) {
				...
				$message = sprintf( _n( '%s category removed successfully', '%s categories removed successfully', $items, 'yith-woocommerce-minimum-maximum-quantity' ), $items );
			}
			...
			if ( isset( $_GET['id'] ) && ! empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
				...
				$item = array(
					'ID'           => $_GET['id'],
					'excluded'     => get_term_meta( $_GET['id'], '_ywmmq_category_exclusion', true ),
					'override_qty' => get_term_meta( $_GET['id'], '_ywmmq_category_quantity_limit_override', true ),
					'minimum_qty'  => get_term_meta( $_GET['id'], '_ywmmq_category_minimum_quantity', true ),
					'maximum_qty'  => get_term_meta( $_GET['id'], '_ywmmq_category_maximum_quantity', true ),
					'step_qty'     => get_term_meta( $_GET['id'], '_ywmmq_category_step_quantity', true ),
					'override_val' => get_term_meta( $_GET['id'], '_ywmmq_category_value_limit_override', true ),
					'minimum_val'  => get_term_meta( $_GET['id'], '_ywmmq_category_minimum_value', true ),
					'maximum_val'  => get_term_meta( $_GET['id'], '_ywmmq_category_maximum_value', true )
				);
				...
				<?php if ( ! empty( $_GET['action'] ) && ( 'insert' == $_GET['action'] || 'edit' == $_GET['action'] ) ) : ?>
					...
					<form id="form" method="POST" action="<?php echo $list_url; ?>">
						...
						<table class="form-table">
							...
							<tr valign="top">
								...
								<td class="forminp">
									...
									$select_args = array(
										...
										'id'               => 'category_ids',
										'name'             => 'category_ids',
										'data-placeholder' => __( 'Search for a category&hellip;', 'yith-woocommerce-minimum-maximum-quantity' ),
							...
							<tr valign="top">
								<th scope="row" class="titledesc">
									<label for="_ywmmq_category_step_quantity"><?php _e( 'Category quantity groups of', 'yith-woocommerce-minimum-maximum-quantity' ) ?></label>
								</th>
								<td class="forminp forminp-number">
									<input
										id="_ywmmq_category_step_quantity"
										name="_ywmmq_category_step_quantity"
										type="number"
		...
		public function add_options() {
			...
			if ( 'yith-plugins_page_yith-wc-min-max-qty' == get_current_screen()->id && ( isset( $_GET['tab'] ) && $_GET['tab'] == 'bulk' ) && ( isset( $_GET['section'] ) && $_GET['section'] == 'categories' ) && ( ! isset( $_GET['action'] ) || ( $_GET['action'] != 'edit' && $_GET['action'] != 'insert' ) ) ) {
