		...
		<div id="<?php echo esc_attr( $id ); ?>-container" data-index="<?php echo esc_attr( $index ); ?>"
			...
			<div class="option">
				<div class="conversion-role">
					<select name="<?php echo esc_attr( $current_name ) . '[role]'; ?>" id="<?php echo esc_attr( $current_id ) . '[role]'; ?>">
						<?php foreach ( $roles as $key => $role ) : ?>
							<option value="<?php echo esc_attr( $key ); ?>" <?php selected( $single_role, $key, 1 ); ?>><?php echo esc_html( translate_user_role( $role['name'] ) ); ?></option>
						<?php endforeach; ?>
					</select>
				</div>
				<div class="conversion-currencies">
					...
					foreach ( $currencies as $current_currency ) :
						...
						?>
						 <p>
						<input type="number" name="<?php echo esc_attr( $current_name ); ?>[points]" step="1" min="0"
