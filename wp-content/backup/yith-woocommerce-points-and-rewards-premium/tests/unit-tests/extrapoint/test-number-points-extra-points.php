<?php
/**
 * Class YWPAR_Tests_Number_Points_Extra_Points
 *
 * @package YITH
 */
 ...
 */
class YWPAR_Tests_Number_Points_Extra_Points extends YWPAR_Unit_Test_Case_With_Store {
	function test_number_point_1(){
		$ywpar_exp = array(
			'list' => array(
				array(
					'number' => 10,
		...
		YITH_WC_Points_Rewards()->set_option('number_of_points_exp', $ywpar_exp );
		YITH_WC_Points_Rewards()->set_option( 'enable_number_of_points_exp', 'yes');

		$product = parent::create_and_store_points_product( array('_price'=> 40 ));
		//qty = 4
		...
		$this->assertEquals( $total_point, 26 );
		...
		$total_point = get_user_meta( $user->ID, '_ywpar_user_total_points', true );
		$this->assertEquals( $total_point, 76 );

	}

	function _test_number_point_2(){
		$ywpar_exp = array(
			'list' => array(
				array(
					'number' => 50,
